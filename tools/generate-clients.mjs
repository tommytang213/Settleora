...
const webOutputPath = resolveGeneratedOutputPath(
  process.env.SETTLEORA_CLIENT_WEB_OUTPUT_PATH ?? "packages/client-web/src/generated",
  {
    name: "web",
...
const dartOutputPath = resolveGeneratedOutputPath(
  process.env.SETTLEORA_CLIENT_DART_OUTPUT_PATH ?? "packages/client-dart/lib/generated",
  {
    name: "Dart",
...
await writeGeneratedFile(webOutputPath, "models.ts", generateTypeScriptModels(schemas));
await writeGeneratedFile(webOutputPath, "client.ts", generateTypeScriptClient(operations));
await writeGeneratedFile(webOutputPath, "index.ts", generateTypeScriptIndex());

await writeGeneratedFile(dartOutputPath, "models.dart", generateDartModels(schemas));
await writeGeneratedFile(dartOutputPath, "client.dart", generateDartClient(operations));
await writeGeneratedFile(dartOutputPath, "settleora_api.dart", generateDartIndex());
...
function generatedEntryPath(directory, entryName) {
...
async function writeGeneratedFile(directory, fileName, contents) {
  await writeFile(generatedEntryPath(directory, fileName), contents, "utf8");
}
...
async function cleanGeneratedDirectory(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    await rm(generatedEntryPath(directory, entry.name), { force: true, recursive: true });
  }
}
