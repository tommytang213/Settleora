...
test('native OCR acceptance is exact-head, device-backed, and retains only bounded evidence', () => {
  ...
  const appDelegateSha = createHash('sha256').update(read('apps/mobile/ios/Runner/AppDelegate.swift')).digest('hex');
  assert.ok(iosRunner.includes(appDelegateSha));
  assert.ok(iosRunner.includes('settleoraNetworkInterposerLoaded() == 1'));
  assert.ok(iosRunner.includes('ios_simulator_app_delegate_probe=present'));
