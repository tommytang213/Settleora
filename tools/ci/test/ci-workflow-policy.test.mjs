...
test('native OCR acceptance is exact-head, device-backed, and retains only bounded evidence', () => {
  ...
  assert.ok(iosRunner.includes("printf '\\nENABLE_DEBUG_DYLIB = NO\\nOTHER_LDFLAGS = $(inherited) %s -Wl,-u,_settleora_require_network_interposer -L%s %s -Wl,-needed-lSettleoraOcrNetworkDeny\\nLIBRARY_SEARCH_PATHS = $(inherited) %s\\n'"));
  ...
  for (const stage of ['build_simulator_link_anchor', 'verify_simulator_link_anchor_object',
    'verify_simulator_link_anchor_symbol', 'verify_simulator_linker_resolution',
    'verify_debug_link_config', 'verify_network_link_path',
  ...
  assert.ok(iosRunner.includes('ios_runner_anchor_in_link_invocation=present'));
  assert.ok(iosRunner.includes('ios_runner_anchor_root_in_link_invocation=present'));
  assert.ok(iosRunner.includes('ios_simulator_rooted_anchor_setting=missing'));
  assert.ok(iosRunner.includes('ios_simulator_built_rooted_anchor_setting=missing'));
  assert.ok(iosRunner.includes('ios_runner_forced_symbol_in_link_invocation=absent'));
  ...
  assert.ok(iosRunner.includes("grep -Fx 'ios_runner_anchor_in_link_invocation=present'"));
  assert.ok(iosRunner.includes("grep -Fx 'ios_runner_anchor_root_in_link_invocation=present'"));
  assert.ok(iosRunner.includes("grep -Fx 'ios_runner_forced_symbol_in_link_invocation=absent'"));
...
test('iOS linker diagnostic binds the needed-library flag to the exact Runner invocation', () => {
  ...
  assert.match(diagnose(searched, '0'), /ios_runner_link_argument_order=absent/);
  const rooted = searched.replace('-Wl,-u,_settleora_network_interposer_loaded', '-Wl,-u,_settleora_require_network_interposer');
  assert.match(diagnose(rooted, '0'), /ios_runner_anchor_root_in_link_invocation=present/);
  assert.match(diagnose(rooted, '0'), /ios_runner_forced_symbol_in_link_invocation=absent/);
  assert.match(diagnose(rooted, '0'), /ios_runner_link_argument_order=present/);
  assert.match(diagnose(rooted.replace('_settleora_require_network_interposer', '_settleora_require_network_interposer.bak'), '0'), /ios_runner_anchor_root_in_link_invocation=absent/);
  assert.match(diagnose(rooted.replace('_settleora_require_network_interposer', '_settleora_require_network_interposer.bak'), '0'), /ios_runner_link_argument_order=absent/);
  const neededWithoutAnchor = `${header}    /Applications/Xcode/usr/bin/clang -L/tmp ${dylib} -Wl,-needed-lSettleoraOcrNetworkDeny -o /tmp/Runner.app/Runner\n`;
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_link_argument_order=absent/);
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_anchor_in_link_invocation=absent/);
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_forced_symbol_in_link_invocation=absent/);
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_link_invocation_count=1/);
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_link_architecture=absent/);
  assert.match(diagnose(neededWithoutAnchor, '0'), /ios_runner_dead_strip_dylibs=absent/);
  ...
  assert.match(diagnose(forcedWithoutAnchor, '0'), /ios_runner_forced_symbol_in_link_invocation=present/);
  const anchorWithoutForced = neededWithoutAnchor.replace(' -L/tmp ', ` ${anchor} -L/tmp `);
  assert.match(diagnose(anchorWithoutForced, '0'), /ios_runner_anchor_in_link_invocation=present/);
  assert.match(diagnose(anchorWithoutForced, '0'), /ios_runner_forced_symbol_in_link_invocation=absent/);
  assert.match(diagnose(rooted.replace(' -L/tmp ', ' -L/tmp/elsewhere '), '0'), /ios_runner_link_argument_order=absent/);
  assert.match(diagnose(rooted.replace(` -L/tmp ${dylib} `, ` ${dylib} -L/tmp `), '0'), /ios_runner_link_argument_order=absent/);
  const disguisedOrder = `${header}    /Applications/Xcode/usr/bin/clang ${anchor} -Wl,-u,_settleora_require_network_interposer -L/tmp -Xlinker -weak_library -Xlinker ${dylib} -Wl,-needed-lSettleoraOcrNetworkDeny ${dylib} -o /tmp/Runner.app/Runner\n`;
  ...
  const separateSearchOperand = `${header}    /Applications/Xcode/usr/bin/clang ${anchor} -Wl,-u,_settleora_require_network_interposer -L/tmp -L ${dylib} -Wl,-needed-lSettleoraOcrNetworkDeny ${dylib} -o /tmp/Runner.app/Runner\n`;
