...
test("canonical wrapper fails closed around projection, locks, package inspection, and signing", () => {
  ...
  assert.match(resourceInventoryLoop,
    /Frameworks\/GoogleDataTransport\.framework\/GoogleDataTransport_Privacy\.bundle\/\*\|Frameworks\/GTMSessionFetcher\.framework\/GTMSessionFetcher_Core_Privacy\.bundle\/PrivacyInfo\.xcprivacy\|Frameworks\/MLKitTextRecognition\.framework\/LatinOCRResources\.bundle\/\*/);
  assert.match(resourceInventoryLoop,
  ...
  const matchesNestedBundle = (relativePath) => {
    const result = spawnSync('bash', ['-c', 'case "$1" in Frameworks/GoogleDataTransport.framework/GoogleDataTransport_Privacy.bundle/*|Frameworks/GTMSessionFetcher.framework/GTMSessionFetcher_Core_Privacy.bundle/PrivacyInfo.xcprivacy|Frameworks/MLKitTextRecognition.framework/LatinOCRResources.bundle/*|Frameworks/image_picker_ios.framework/image_picker_ios_privacy.bundle/PrivacyInfo.xcprivacy) printf reviewed ;; *) printf reject ;; esac', '_', relativePath], { encoding: 'utf8' });
  ...
  assert.equal(matchesNestedBundle('Frameworks/GTMSessionFetcher.framework/GTMSessionFetcher_Core_Privacy.bundle/Info.plist'), 'reject');
