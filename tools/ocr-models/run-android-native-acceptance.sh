...
echo "os_runtime=$(sanitize "android-api$(timeout 5 "$adb" -s emulator-5554 shell getprop ro.build.version.sdk | tr -d '\r')-$(timeout 5 "$adb" -s emulator-5554 shell getprop ro.build.id | tr -d '\r')-$(timeout 5 "$adb" -s emulator-5554 shell getprop ro.build.version.incremental | tr -d '\r')")" >> "$GITHUB_OUTPUT"
