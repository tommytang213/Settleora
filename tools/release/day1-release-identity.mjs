...
function collectAndroid(repoRoot, input, source) {
  ...
  const expectedRuntimeGradleMutablePaths = ['.tmp', 'caches/CACHEDIR.TAG', 'caches/build-cache-1', 'caches/gc.properties', 'caches/journal-1', 'caches/keyrings', 'caches/modules-2', 'android', 'daemon', 'kotlin-profile', 'native', 'notifications', 'workers', `wrapper/${guardedWrapperLock}`];
  if (provenance.toolchainMutationGuard.algorithm !== 'linux-inotify-authenticated-runner-v3' || provenance.toolchainMutationGuard.queueOverflowFailsClosed !== true
    ...
    || canonicalJson(provenance.toolchainMutationGuard.sealedGradleExecutableCachePaths) !== canonicalJson([`caches/${guardedGradleVersion ?? ''}`, 'caches/jars-9', 'caches/transforms-4'])
