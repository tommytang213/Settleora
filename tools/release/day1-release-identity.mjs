...
function collectAndroid(repoRoot, input, source) {
  ...
  const expectedRuntimeGradleMutablePaths = ['.tmp', 'caches/CACHEDIR.TAG', 'caches/build-cache-1', `caches/${guardedGradleVersion ?? ''}/file-changes`, `caches/${guardedGradleVersion ?? ''}/fileContent`, `caches/${guardedGradleVersion ?? ''}/fileHashes`, `caches/${guardedGradleVersion ?? ''}/gc.properties`, `caches/${guardedGradleVersion ?? ''}/javaCompile`, `caches/${guardedGradleVersion ?? ''}/jvms`, `caches/${guardedGradleVersion ?? ''}/md-rule`, `caches/${guardedGradleVersion ?? ''}/md-supplier`, `caches/${guardedGradleVersion ?? ''}/dependencies-accessors/gc.properties`, `caches/${guardedGradleVersion ?? ''}/generated-gradle-jars/generated-gradle-jars.lock`, `caches/${guardedGradleVersion ?? ''}/groovy-dsl/gc.properties`, `caches/${guardedGradleVersion ?? ''}/kotlin-dsl/gc.properties`, `caches/${guardedGradleVersion ?? ''}/transforms/gc.properties`, 'caches/jars-9/jars-9.lock', 'caches/gc.properties', 'caches/journal-1', 'caches/keyrings', 'caches/modules-2', 'android', 'daemon', 'kotlin-profile', 'notifications', 'workers', ...guardedNativeLocks.map((entry) => `native/${entry}`), `wrapper/${guardedWrapperLock}`];
  ...
  if (provenance.toolchainMutationGuard.algorithm !== 'linux-inotify-authenticated-runner-v3' || provenance.toolchainMutationGuard.queueOverflowFailsClosed !== true
    ...
    || guardedNativeLocks.length < 1 || guardedNativeLocks.length > 16
    || guardedNativeLocks.some((entry) => !/^(?:jansi\/[0-9.]+\/[A-Za-z0-9._-]+\/libjansi\.so|[0-9a-f]{64}\/[A-Za-z0-9._-]+\/libnative-platform(?:-curses)?\.so|[0-9.]+\/[A-Za-z0-9._-]+\/libgradle-fileevents\.so)\.lock$/u.test(entry))
    || canonicalJson([...guardedNativeLocks].sort()) !== canonicalJson(guardedNativeLocks)
    ...
    || !Array.isArray(provenance.toolchainMutationGuard.preStabilizationKotlinDslAccessorBases)
    || provenance.toolchainMutationGuard.preStabilizationKotlinDslAccessorBases.length < 1
    || provenance.toolchainMutationGuard.preStabilizationKotlinDslAccessorBases.some((entry) => !provenance.toolchainMutationGuard.gradleKotlinDslTransientBases.includes(entry))
    || canonicalJson([...provenance.toolchainMutationGuard.preStabilizationKotlinDslAccessorBases].sort()) !== canonicalJson(provenance.toolchainMutationGuard.preStabilizationKotlinDslAccessorBases)
    || !Array.isArray(provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases)
    || provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases.length < 1
    || provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases.some((entry) => {
      const match = /^caches\/([0-9.]+)\/kotlin-dsl\/scripts\/[0-9a-f]{32}$/u.exec(entry);
      return !match || match[1] !== guardedGradleVersion;
    })
    || canonicalJson([...provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases].sort()) !== canonicalJson(provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases)
    || !Array.isArray(provenance.toolchainMutationGuard.preStabilizationKotlinDslScriptBases)
    || provenance.toolchainMutationGuard.preStabilizationKotlinDslScriptBases.length < 1
    || provenance.toolchainMutationGuard.preStabilizationKotlinDslScriptBases.some((entry) => !provenance.toolchainMutationGuard.gradleKotlinDslScriptTransientBases.includes(entry))
