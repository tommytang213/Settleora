...
test('rejects source, API revision, API digest and floating-tag mismatches', (t) => {
  ...
  const provenance = { runDetails: { builder: { id: `${publication.url}/attempts/1` } }, buildDefinition: { resolvedDependencies: f.input.apiImage.buildMaterials.map((material) => ({ uri: material.uri, digest: { sha256: material.digest.slice(7) } })), externalParameters: { request: { root: { configSource: { path: 'Dockerfile' }, request: { args: { 'vcs:revision': f.commit, 'vcs:source': 'https://github.com/tommytang213/Settleora' } } } } } } };
