...
import {
  ...
  parseTrustedSshCommand, renderTrustedSshFixtures, trustedSshPackageContract, trustedSshPaths,
  validateEffectiveSshdOutput, validateNativeStaticExecutable, validateTrustedSshFixtures,
...
`);
  ...
  execFileSync("/usr/bin/gcc", [...compilerFlags,
  ...
  assert.doesNotMatch(result.stdout, /BASH_ENV|NODE_OPTIONS|LD_PRELOAD|PYTHON|RUBY|PERL|SHELLOPTS/u);
...
test("fixed no-argument root gate independently authenticates the exact package and operation", () => withFixture((fixture) => {
  ...
  const result = runTrustedSshRootGate({
    cwd: path.join(packageRoot, key), handoffRoot: packageRoot, uid: 0, euid: 0, expectedPackageUid: process.getuid(),
...
function buildNative(root, { expectedUid = 0 } = {}) {
  ...
  execFileSync("/usr/bin/gcc", [...compilerFlags, path.join(sourceRoot, "native/settleora-trusted-ssh-entry.c"), "-o", entry]);
  ...
  execFileSync("/usr/bin/gcc", [...compilerFlags, path.join(sourceRoot, "native/settleora-trusted-ssh-root-gate.c"), "-o", rootGate]);
...
function createPackage(root) {
  ...
  const manifest = {
    ...
    members: [{ byteCount: entrypoint.length, mode: "0550", path: "remote-entrypoint.sh", sha256: sha256(entrypoint) }],
