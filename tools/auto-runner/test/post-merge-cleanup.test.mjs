...
test("normal merged-task path creates and persists exact recovery lineage only after target tree acceptance and hygiene", () => {
  ...
  assert.equal(result.ok, true); assert.equal(result.ownership.reviewedHeadSha, source); assert.equal(result.ownership.mergeSha, merge); assert.equal(result.ownership.acceptance.targetHeadSha, target); assert.match(result.ownership.executionLineage, /^recovery-run-947:/);
