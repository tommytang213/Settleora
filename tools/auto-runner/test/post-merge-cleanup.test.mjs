...
test("normal merged-task path creates and persists exact recovery lineage only after target tree acceptance and hygiene", () => {
  ...
  const context = { taskKey: "20260723-1007", runId: "context-fallback-must-not-win", sessionLifecycle: { sessions: { current: "recovery-run-947:session-2" } }, issue: { number: 947 }, branchName: "feature/auto-947-cleanup", pr: { headRefName: "feature/auto-947-cleanup", baseRefName: "main", url: "https://github.com/owner/repo/pull/949" }, baseSha: base, expectedOriginMainSha: s("f"), recoveryState: { taskKey: "20260723-1007", run: { runId: "recovery-run-947" }, issue: { number: 947 }, branch: { name: "feature/auto-947-cleanup", baseSha: base, currentHeadSha: source }, mutationMarkers: { branch_ownership_created: { [`feature/auto-947-cleanup:${base}`]: { target: "feature/auto-947-cleanup", correlation: base } } } }, validation: { passed: true, headSha: source, baseSha: s("f") }, externalReview: { status: "pass" }, review: { verdict: { verdict: "approve" } } };
  const effect = { runner, mergeSha: merge, hygiene: { status: "merged", closeDecision: { close: true }, closure: { status: "updated" }, comment: { status: "updated" }, labelCleanup: { status: "skipped", reason: "no_transient_labels" }, parentProgress: { status: "updated" }, project: { status: "not_updated", reason: "project_status_mapping_not_configured" }, ledger: { status: "created" } }, sourceHeadSha: source, repositorySlug: "owner/repo", prNumber: 949, cleanupBranchSafety: { ok: true, protected: false, defaultBranch: "main" } };
  const result = preparePostMergeCleanupOwnership({ logsRoot, repoRoot: "/repo", runnerRunId: "config-fallback-must-not-win" }, context, effect);
  assert.equal(result.ok, true); assert.equal(result.ownership.reviewedHeadSha, source); assert.equal(result.ownership.mergeSha, merge); assert.equal(result.ownership.acceptance.targetHeadSha, target); assert.equal(result.ownership.executionLineage, "recovery-run-947:recovery-run-947:session-2"); assert.equal(result.ownership.correlations.session, "recovery-run-947:session-2");
  const noLineage = preparePostMergeCleanupOwnership({ logsRoot, repoRoot: "/repo" }, { ...context, runId: null, recoveryState: { ...context.recoveryState, run: null } }, effect);
  assert.equal(noLineage.ok, false); assert.equal(noLineage.reasonCode, "cleanup_execution_lineage_unproven");
});
