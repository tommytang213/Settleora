...
test("Route B terminal summary fields require positive canonical values", () => {
  const positiveCases = [
    ["absent optional terminal fields", {}],
    ["positive outcome", { outcome: "completed" }],
    ["positive result", { result: "success" }],
    ["positive status", { status: "succeeded" }],
    ["positive terminal state", { terminalState: "completed" }],
    ["positive terminal outcome", { terminalOutcome: "completed" }],
    ["all positive fields", {
      outcome: "completed",
      result: "success",
      status: "succeeded",
  ...
  for (const [label, summaryOverrides] of positiveCases) {
    const config = tempConfig();
    try {
      const planned = fixtureOutageState();
      const child = exactMergedChild(config, planned, {
        runId: `supervised-20260715T010000Z-0000000007${String(positiveCases.indexOf(positiveCases.find((item) => item[0] === label))).padStart(2, "0")}`,
      }, { summaryOverrides });
      const result = runOutageResubmissionController({
        config,
        source: source(),
      ...
      assert.equal(result.outcome, "recovered", label);
      assert.equal(result.reasonCode, "confirmed_child_recovered", label);
    } finally {
      config.cleanup();
    }
  }
  ...
  const rejectedValues = [
    ...
    "missing-required",
    "child-said-ok",
    "completed token=super-secret-token",
  ];
  const fields = ["outcome", "result", "status", "terminalState", "terminalOutcome"];
  for (const field of fields) {
    for (const value of rejectedValues) {
      ...
      try {
        const planned = fixtureOutageState();
        const child = exactMergedChild(config, planned, {
          runId: `supervised-20260715T010000Z-${String(fields.indexOf(field) + 800).padStart(12, "0")}`,
        }, { summaryOverrides: { [field]: value } });
        const result = runOutageResubmissionController({
          config,
          source: source(),
          outageState: planned,
        ...
        assert.equal(result.outcome, "blocked", `${field}:${value}`);
        assert.equal(result.reasonCode, "child_completed_without_exact_recovery_proof", `${field}:${value}`);
        assert.equal(result.events.some((item) => item.reasonCode === "confirmed_child_recovered"), false, `${field}:${value}`);
        assert.equal(result.counts.githubMutationCalls, 0, `${field}:${value}`);
        assert.equal(result.counts.systemdCalls, 0, `${field}:${value}`);
        assert.equal(JSON.stringify(result).includes("super-secret-token"), false, `${field}:${value}`);
      } finally {
        config.cleanup();
      }
    }
  ...
  try {
    const planned = fixtureOutageState();
    const child = exactMergedChild(config, planned, {
      runId: "supervised-20260715T010000Z-000000000899",
    }, {
      summaryOverrides: {
        status: "completed",
        terminalOutcome: "max_runtime_reached",
      },
    });
    const first = runOutageResubmissionController({
      config,
      source: source(),
      outageState: planned,
      recoveryState: incompleteRecoveryState(),
    ...
    const second = runOutageResubmissionController({
      config,
      source: source(),
      outageState: first.outageState,
      recoveryState: incompleteRecoveryState(),
      existingChildren: [child],
      dryRun: false,
      now,
    ...
    assert.equal(first.reasonCode, "child_completed_without_exact_recovery_proof");
    assert.equal(second.outcome, "noop");
    assert.equal(second.reasonCode, "terminal_outage_marker_preserved");
    assert.equal(first.counts.systemdCalls + second.counts.systemdCalls, 0);
    assert.equal(first.counts.githubMutationCalls + second.counts.githubMutationCalls, 0);
  } finally {
    config.cleanup();
  }
});
