...
test("repository templates keep loopback defaults and avoid deployment secrets in ExecStart", () => {
  ...
  assert.equal(
    ...
    "/workspace/auto-runner/.runtime.node-exec-boundary --mode health --node /usr/bin/node --home %h -- /workspace/auto-runner/.runtime.launcher.mjs --runtime-root /workspace/auto-runner/runtime --entry settleora-auto-runner-health-service.mjs -- --host 127.0.0.1 --port 8787 --config /workspace/auto-runner/config/settleora.json",
  );
  assert.equal(
    getLast(notifier, "Service", "ExecStart"),
    "/workspace/auto-runner/.runtime.node-exec-boundary --mode notifier --node /usr/bin/node --home %h -- /workspace/auto-runner/.runtime.launcher.mjs --runtime-root /workspace/auto-runner/runtime --entry settleora-auto-runner-terminal-notifier.mjs -- --config /workspace/auto-runner/config/settleora.json",
  );
...
function assertExecStartIsSafe(unit, name) {
  ...
  assert.match(execStart, /^\/workspace\/auto-runner\/\.runtime\.node-exec-boundary --mode (?:health|notifier) --node \/usr\/bin\/node --home %h -- \/workspace\/auto-runner\/\.runtime\.launcher\.mjs --runtime-root \/workspace\/auto-runner\/runtime --entry settleora-auto-runner-(?:health-service|terminal-notifier)\.mjs -- (?:--host 127\.0\.0\.1 --port 8787 )?--config \/workspace\/auto-runner\/config\/settleora\.json$/);
