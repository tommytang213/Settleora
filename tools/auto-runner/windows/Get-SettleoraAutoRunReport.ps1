...
$remoteArgs = @("cd", $remoteRepo, "&&", "node", "tools/auto-runner/settleora-auto-runnerctl.mjs", "report") + $selector + @("--json")
$json = & ssh.exe $SshTarget -- $remoteArgs
if ($LASTEXITCODE -ne 0) { throw "Remote report lookup failed with exit code $LASTEXITCODE." }
