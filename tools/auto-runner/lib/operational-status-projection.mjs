...
function reconcileConflicts(repository, github, local) {
  ...
  if (github?.issue?.manualGate) reasons.push("github_issue_manual_gate");
  if (github?.issue?.dangerGate) reasons.push("github_issue_danger_gate");
  ...
  if (local?.active === true && local?.task?.headSha != null && local.task.headSha !== "" && !validSha(local.task.headSha)) reasons.push("active_task_head_identity_invalid");
