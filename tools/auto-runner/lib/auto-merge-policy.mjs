...
export function preparePostMergeCleanupOwnership(config, context, { runner, mergeSha, hygiene, sourceHeadSha, repositorySlug, prNumber, cleanupBranchSafety }) {
  ...
  if (!executionRunId) return { ok: false, eligible: true, reasonCode: "cleanup_execution_lineage_unproven" };
  const lifecycleSessionId = context.sessionLifecycle?.sessions?.current || null;
  let ownership;
  try {
    ownership = createCleanupOwnershipRecord({ repository: repositorySlug, rootTaskKey, executionLineage: `${executionRunId}:${lifecycleSessionId || "session"}`, issueNumber: context.issue?.number, branchName, branchKind: branchName.split("/")[0], baseBranch: targetBranch, baseSha: context.baseSha || context.expectedOriginMainSha, reviewedHeadSha: sourceHeadSha, prNumber, prUrl: context.pr?.url || `https://github.com/${repositorySlug}/pull/${prNumber}`, mergeSha, targetBranch, acceptance: { passed: true, targetHeadSha, evidenceDigest }, correlations: { recovery: context.recoveryState?.taskKey || null, session: lifecycleSessionId, bundle: context.featureBundle?.bundleId || null, stack: context.stackId || null }, worktree: cleanupWorktreeIdentity(config, runner, repositorySlug, branchName, sourceHeadSha, recovery) });
