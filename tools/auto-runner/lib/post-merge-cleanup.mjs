...
function processOwnsPath(candidate) { const lsof = spawnSync("lsof", ["-t", "+D", candidate], { encoding: "utf8", windowsHide: true }); if (lsof.error || ![0, 1].includes(lsof.status)) return true; if (lsof.status === 0 && String(lsof.stdout || "").trim()) return true; try { for (const entry of readdirSync("/proc")) { if (!/^[1-9][0-9]*$/.test(entry)) continue; for (const link of [`/proc/${entry}/cwd`, `/proc/${entry}/root`, `/proc/${entry}/exe`]) { try { if (insidePath(readlinkSync(link), candidate)) return true; } catch { /* lsof provided the complete path-use result */ } } try { for (const fd of readdirSync(`/proc/${entry}/fd`)) { try { if (insidePath(readlinkSync(`/proc/${entry}/fd/${fd}`), candidate)) return true; } catch { /* fd may close after lsof */ } } } catch { /* lsof provided the complete path-use result */ } } } catch { return true; } return false; }
