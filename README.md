...
- `apps/mobile/` existing Flutter mobile app with first-launch local/server configuration, secure-storage-backed app/session state boundaries, and a starter receipt OCR review queue/detail/edit foundation backed by the mobile repository seam. Server-mode receipt review calls stay session-gated; mobile sign-in, refresh, logout, session/device list, and local-mode expense storage are still future work.
