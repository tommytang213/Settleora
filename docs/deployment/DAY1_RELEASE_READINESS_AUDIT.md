...
R02 completion addendum: [Issue #1205](https://github.com/tommytang213/Settleora/issues/1205) is implemented by [PR #1206](https://github.com/tommytang213/Settleora/pull/1206) plus corrective convergence [PR #1207](https://github.com/tommytang213/Settleora/pull/1207). Final reviewed source `4df10f7d9ae5478288327cc24ad68ad78116110d`, tree `537684248fbc909a4939ac2522d5e049ba9c2a58`, normally merged as `01bf000ecfb718fa5708202892d77178fc9a7fb4`. Web-affecting PRs now produce source-bound, deterministic `dist` checksum/provenance and a retained GitHub artifact without deployment or serving. Completed R03 subsequently consumed this primitive in an exact final-main web build; #373 still owns product completeness and R08 remains blocked on it.

R01 completion addendum: [Issue #1209](https://github.com/tommytang213/Settleora/issues/1209) is implemented by [PR #1212](https://github.com/tommytang213/Settleora/pull/1212), reviewed source `2d7ceac310433870aef9bdf4ed93f84feec9dac3`, tree `fa21fe8238e6753b969dc6107f5b5c838edd1014`, normal merge `6ee8250b7e7c1a1bdb425daaec50f32d14dabaca`. Exact-source debug APK, minified release APK and minified release AAB builds passed with the bundled Latin ML Kit model and no optional-script dependency expansion. This is compile/package correctness only: `com.example.mobile`, debug release signing, Play and physical-device acceptance remain R07/#975 gaps. Completed R03 subsequently rebuilt and bound the release APK/AAB from final main; autonomous queue activation remains disabled.

R03 completion addendum: [Issue #1214](https://github.com/tommytang213/Settleora/issues/1214) is implemented by [PR #1215](https://github.com/tommytang213/Settleora/pull/1215) with corrective convergence in [PR #1228](https://github.com/tommytang213/Settleora/pull/1228), [PR #1229](https://github.com/tommytang213/Settleora/pull/1229), and [PR #1230](https://github.com/tommytang213/Settleora/pull/1230). Final main `13c8eaaedf83a29194befcde6c37c428da3300e6`, tree `00395fb3e8843989f40d26c5152dde2f5bedfbf4`, produced and independently revalidated external candidate `day1-13c8eaaedf83` with identity digest `624e846e...`. The manifest binds the normal exact-SHA GHCR publication, selected Linux/amd64 API and dependency digests, 39 repository-source-only migrations, exact-source R02 web output, and freshly rebuilt APK/AAB evidence. This is non-production provenance only: it did not deploy, promote, publish a release, apply migrations, change secrets, or prove database/schema/file rollback safety. R04 subsequently consumed the immutable identity evidence; #946 remains later-day and no successor was started.

R04 completion addendum: [Issue #1232](https://github.com/tommytang213/Settleora/issues/1232) is implemented by [PR #1233](https://github.com/tommytang213/Settleora/pull/1233), reviewed source `31d58e06ccbb31fda079bf532aae8cb696606fea`, tree `d2122b86b22884c609048c02975fc9077831c832`, normal merge `3b0a78bc9883b643e889d28d5b43adcdbd47bd19`. The repository now contains an unpublished TrueNAS 25.10.x Docker Apps skeleton that semantically consumes the retained R03 identity, selects immutable Linux/amd64 runtime digests, preserves R11/R12, renders only five private services, and publishes only exact-RFC1918 HTTPS. Exact-head and merged-source direct/official rendering, negatives, determinism, CI/scanners, and independent reviews passed. No catalog publication, live TrueNAS action, deployment, real secret/certificate, migration/database operation, backup/restore, or public/admin exposure occurred; R05/#975 retain all live/manual acceptance.
...
Admin web has no application package. An unpublished TrueNAS catalog skeleton and
deterministic official render path now exist, but catalog publication, live install/upgrade,
backup-before-upgrade evidence, and tested restore/rollback paths do not. The LAN
...
[#1195](https://github.com/tommytang213/Settleora/issues/1195) and merged
[PR #1196](https://github.com/tommytang213/Settleora/pull/1196) add a
...
availability/install remain external/manual; current TrueNAS acceptance is now
blocked on live/manual evidence rather than the R04 repository skeleton, and production/staging exposure is unavailable.

With the #1195 R12, #1198 R10, #1201 mobile-portability, #1205 R02, #1209 R01, #1214 R03, and #1232 R04 repository slices, the 69 evaluated
capabilities comprise 30 `implemented`, 15 `partial`, 7 `documentation-only`, 5 `externally-gated`, 5
...
| B04 | Unpublished TrueNAS catalog package skeleton | `implemented` | `infra/truenas-catalog/settleora/`; `tools/truenas-catalog/` | R04 provides metadata, form, immutable-image materialization, first-class migration gating, exact-host private HTTPS, private dependencies, and deterministic direct/official render validation. This classification is repository package implementation only; the separate source-build environment and historical #483 live evidence are not current-head install proof | Completed R04; R05 retains live acceptance, and the source-build environment remains partial in the environment matrix below |
| B05 | Catalog-materialized immutable image package | `implemented` | `tools/truenas-catalog/render.mjs`; `infra/truenas-catalog/settleora/` | The R04 materializer requires semantic R03 identity plus a detached expected digest and selects immutable Linux/amd64 API/PostgreSQL/RabbitMQ/Caddy identities. This capability is distinct from the legacy floating-default image Compose environment, which remains partial in the environment matrix below | Completed R03/R04; publication and live selection remain manual |
...
| C10 | Current live self-host acceptance | `blocked` | issue #483 comments | Historical accepted proof is limited to TrueNAS SCALE `25.10.4`, custom app `1.0.0`, 21 migrations/0 pending, API/dependencies ready; it predates this baseline and the completed R04 skeleton, omitted bootstrap/sign-in, and cannot supply current physical-mobile smoke | R05 owns current acceptance; #975 only consumes the evidence |
| D01 | Safe promotion order | `partial` | migration runner, LAN Compose, install/upgrade plan | Intended order: backup/quiesce -> exact image -> private dependencies -> migration gate -> API -> readiness -> compatible-client smoke. R03 supplies identity evidence, but only Compose start ordering is implemented | R05 |
...
| D03 | Client compatibility gate | `partial` | versioned `/api/v1`, OpenAPI, generated-client validation; R03 identity manifest | Contract drift is tested and R03 binds source, mobile/web outputs, migrations, and artifact digests, but no candidate acceptance gate proves live client/server compatibility | R05/R06/R07/R08 acceptance owners |
...
| G01 | Exact source identity | `implemented` | Git commit/tree, OCI revision label; `tools/release/` | PR/check evidence is SHA-bound; completed R03 also requires a clean exact-source checkout and binds source/tree across the candidate | #1214 |
| G02 | API image digest/tag discoverability | `implemented` | GHCR workflow/run logs; external R03 candidate | Completed R03 binds the exact `sha-<source>` tag, immutable index and selected-platform digests, matching OCI revision, BuildKit provenance, and successful normal publication run. Floating `main` is rejected as authority | #1214; publication is not promotion/deployment |
| G03 | Mobile version/build identity | `partial` | `pubspec.yaml`; Codemagic vars; R03 candidate | R03 deterministically records static `1.0.0+1`; Codemagic overrides build number. No accepted cross-platform release/version policy or current signed build record | R06/R07 |
| G04 | Release notes/changelog | `partial` | mobile bundled What's New; R03 bounded candidate notes | R03 binds explicit bounded candidate notes by digest and rejects inferred commit-message claims, but no broader product release-note authority or release epoch exists | Product release owner; broad epoch automation is #946 later-day |
...
| G07 | Incident/recovery and retention | `documentation-only` | rollback/backup docs; R03 evidence retention | R03 records bounded evidence retention and an immutable prior API artifact with an explicit rollback-safety caveat. No product deployment incident drill or restore/rollback rehearsal exists | R05 |
| G08 | Production artifact promotion | `blocked` | no current production deployment workflow | R03 supplies a non-production cross-artifact identity candidate, not a complete promotable product candidate. A supported deployment path and product acceptance remain prerequisites, and promotion stays an explicit production/manual gate | R05/R08 before R09; immutable UAT-to-production automation is #946 later-day |
...
| H05 | Current TrueNAS install/upgrade/rollback | `blocked` | historical #483 install/update subset | No current-baseline upgrade, backup/restore, rollback, or auth/mobile smoke; completed R04 adds the repository package/render path but is not live host/device evidence | R05 owns acceptance; #975 consumes it |
| H06 | Staging/production deployment | `unavailable` | no supported environment/deployment path or record | R03 identity evidence is complete, but R05/R08 and other product owners must still produce a complete accepted candidate and supported deployment handoff; deployment becomes externally gated only after that preparation exists | R09 after prerequisites |
...
| Android release APK/AAB | Flutter/Gradle | Final-main R03 APK: 96,920,419 bytes, `sha256:56df6f82...`; AAB: 77,132,938 bytes, `sha256:f2d7a171...`; `1.0.0+1`, R8 enabled | Build outputs remain ignored; immutable identities are retained inside the external R03 candidate | Exact-source compile/package identity is implemented; `com.example.mobile`, debug signing and store readiness remain incomplete |
| R03 cross-artifact candidate | `tools/release/` generator/validator | `day1-13c8eaaedf83`; identity digest `sha256:624e846e...`; manifest SHA-256 `c63bbd9b...` | `/workspace/logs/settleora-release-candidates/day1-13c8eaaedf83/`, retained through R04/Day 1 acceptance; deletion is manual | Reproducible non-production provenance evidence; not deployment, promotion, an applied-migration claim, or proof of rollback safety |
...
| Version identity | `1.0.0+1`; Codemagic name `1.0.0` and provider build number | R03 records exact final-main `1.0.0+1`; no accepted signed candidate | R06/R07 |
...
### #1205 R02 additive user-web CI build/package lane

- Initial [PR #1206](https://github.com/tommytang213/Settleora/pull/1206)
  merged source `be5516cc...` normally as `497b8c40...`; the orchestrator had
  printed but failed to assert the unresolved-thread count before that merge.
  Same-task [PR #1207](https://github.com/tommytang213/Settleora/pull/1207)
  resolved every review finding, passed the complete gates on source
  `4df10f7d9ae5478288327cc24ad68ad78116110d` (tree `53768424...`), and
  normally merged as `01bf000ecfb718fa5708202892d77178fc9a7fb4`.
- `run_web_user_validation` selects `apps/web-user/**`,
  `packages/client-web/**`, and its workflow/classifier/helper/test control
  surface. Unknown proof selects web plus every existing expensive lane;
  docs-only changes intentionally skip them. `Validate scaffold` remains the
  sole stable required context and rejects missing or incorrectly skipped web
  results.
- The exact-source-head job runs Node 22, web `npm ci`, lint, 139 tests and the
  production build. It stages only `dist/` and an external canonical manifest
  containing source/tree, lock identity, bounded tool versions, sorted file
  sizes/SHA-256 values, and the deterministic tree digest. Tracked inputs are
  byte/mode checked against HEAD with raw path handling; symlinks, special
  files, source maps, unsafe paths and likely sensitive/host material fail
  closed.
- Final artifact `user-web-dist-4df10f7d9ae5478288327cc24ad68ad78116110d`
  (ID `10190883387`, 14-day retention) contains three files totaling 426,988
  bytes plus the external manifest. Downloaded bytes independently regenerated
  the same manifest and tree digest `6bf6cd648121edfc0633cc8bb15ce7a3980881a9b7b2feb2572b24af86da00be`.
  Full API/Docker, mobile, iOS, CodeQL, Semgrep, Trivy and aggregate checks and
  final Gemini/local/GitHub Codex reviews passed with zero unresolved threads.
- No web was served, deployed, exposed or declared product-complete. #373 stays
  open and R08 retains product/auth/privacy and serving gates. Completed R03
  consumed only this bounded checksum/provenance primitive.
...
| R02 — **completed by Issue #1205 / PRs #1206 and #1207:** additive user-web CI build/package lane | #1185 classifier/aggregate now runs exact-source `apps/web-user` clean install/lint/test/build and publishes staged `dist` plus canonical provenance for web-affecting PRs. No deploy, auth completion, public exposure or redesign | CI/workflow gate completed | workflow, classifier, manifest helper and focused policy/tests | final source `4df10f7d...`; 139 tests, repeated deterministic build, downloaded artifact match, full CI/scanners and strong reviews | Exact web-affecting proof is complete. Completed R03 consumed the checksum primitive; #373 and R08 retain product/serving work |
| R03 — **completed by Issue #1214 / PRs #1215, #1228, #1229 and #1230:** Day 1 release identity manifest | Repository-owned schema/generator/validator binds source SHA/tree, exact API publication, PostgreSQL/RabbitMQ/Caddy index and selected-platform digests, repository-source-only migrations, R02 web identity, exact-source Android APK/AAB, version/build, bounded notes, rollback artifact caveat, and external retention. No release epochs, promotion or deployment | Artifact publication/promotion remains manual | `tools/release/`, schema, fixtures and focused tests; candidate outside Git | final source `13c8eaae...`; final-main candidate identity `624e846e...`; repeated exact-source builds, mismatch refusals, full CI/scanners, Gemini/local/GitHub Codex | Completed with independently validated `day1-13c8eaaedf83`; R04 subsequently consumed the immutable identity evidence. `com.example.mobile`, debug signing, live rollback safety and #946 remain outside R03 |
| R04 — **completed by Issue #1232 / PR #1233:** TrueNAS catalog package skeleton and render validation | Unpublished metadata/form/topology/dataset/secret-input/migration-gated package consumes R03 and preserves R11/R12. Non-goals remain publish, deploy, real secrets, public/admin exposure | Repository Docker/Compose/deployment review completed; live operations remain manual | PR #1233 source `31d58e06ccbb31fda079bf532aae8cb696606fea`, tree `d2122b86b22884c609048c02975fc9077831c832`, normal merge `3b0a78bc9883b643e889d28d5b43adcdbd47bd19` | 16/16 catalog tests, pinned official render/negatives, retained-R03 byte-identical proof, full CI/scanners, Gemini/local/GitHub Codex, zero unresolved threads | Repository skeleton complete and unpublished; R05/#975 retain live TrueNAS, DNS/TLS, secret, backup/restore, rollback, and physical-client acceptance |
| R05 — **manual/external evidence recommendation:** current TrueNAS acceptance | After completed R03/R04/R11/R12, collect current TrueNAS install/upgrade, pre-upgrade backup, bounded non-destructive operator restore evidence using the existing Day 1 runbook, rollback limits, health/auth/mobile smoke. Non-goals: destructive production restore, hidden env change, or product-integrated restore automation | Host, secret, database/storage, migration, deployment manual gates | external operator lane; no unattended repo mutation | exact version/digest, sanitized migration/health/readiness/smoke, consistency-set and recovery evidence; human + strong deployment review | Close only with current-candidate TrueNAS evidence and separately approved operator restore/rollback evidence. Product acceptance readiness remains a prerequisite; #380 remains the umbrella and #975 consumes the result. Automated integrity verification, disposable restore environments, freshness tracking, scheduling, and admin UX remain Day 2 under #1077 |
...
and deployment boundaries are incomplete; non-production release identity is implemented while live retention/acceptance remains partial;
