...
| B05 | TrueNAS/LAN image package | `partial` | `infra/docker-compose.truenas-lan.image.yml` | Config validates; defaults to floating `ghcr.io/tommytang213/settleora-api:main`, while operators may set an exact SHA tag/digest. Completed R03 proves an external exact-source candidate identity but does not rewrite deployment selection | R04; catalog pinning/immutable selection remains unenforced |
...
| User web | Vite plus R02 canonical manifest | Final-main R03 build identity: three files/426,988 bytes, tree digest `sha256:6bf6cd64...`, bound to source/tree and lock digest | Web-affecting PR artifacts retain staged output/manifest for 14 days; the external R03 candidate retains the final-main canonical manifest, not the built `dist` files | Exact-source build/provenance implemented; final-main package retention, serving, product completeness and protected-push packaging remain incomplete |
