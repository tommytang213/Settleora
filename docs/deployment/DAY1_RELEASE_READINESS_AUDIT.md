...
| F01 | Automatic Linux mobile validation and macOS simulator compile | `implemented` | A04/A05 | PR #1193 exact head `c38bcbf3...` passed both the shared Linux release gate and reusable hosted-macOS simulator build on Flutter 3.44.8; neither is signing/upload/install | Completed #1185/#1192; retain both distinct lanes |
...
- The corrected exact head passed 1,051 local non-visual Flutter tests, hosted
  macOS simulator and Linux mobile jobs, full validation, the stable aggregate
  and all scanners/reviews. Repository checks prove Codemagic remains
  manual-only and preserve its signing/upload configuration.
