...
| 7 | User-web local backup/restore planning and package contract gates before package runtime or restore mutation | `docs/user-web-local-backup-package-contract-plan-461` | Package readiness, manifest, generation/download, verification, encryption/key handling, file/privacy, retention, audit, and browser safety gate; see [User web local backup package contract plan](USER_WEB_LOCAL_BACKUP_PACKAGE_CONTRACT_PLAN.md). Restore preview and restore confirmation remain later gates in [User web local backup and restore plan](USER_WEB_LOCAL_BACKUP_RESTORE_PLAN.md). |
