...
| 7 | User-web local backup/restore planning and package contract gates before package runtime or restore mutation | `docs/user-web-local-backup-package-contract-plan-461`, `docs/user-web-local-backup-package-session-plan-461`, then `docs/user-web-local-backup-package-generation-download-plan-461` | Package readiness, manifest, session lifecycle, generation/download eligibility, short-lived download action, verification, encryption/key handling, file/privacy, retention, audit, and browser safety gates; see [User web local backup package contract plan](USER_WEB_LOCAL_BACKUP_PACKAGE_CONTRACT_PLAN.md), [User web local backup package manifest and session plan](USER_WEB_LOCAL_BACKUP_PACKAGE_SESSION_PLAN.md), and [User web local backup package generation and download plan](USER_WEB_LOCAL_BACKUP_PACKAGE_GENERATION_DOWNLOAD_PLAN.md). Restore preview and restore confirmation remain later gates in [User web local backup and restore plan](USER_WEB_LOCAL_BACKUP_RESTORE_PLAN.md). |
