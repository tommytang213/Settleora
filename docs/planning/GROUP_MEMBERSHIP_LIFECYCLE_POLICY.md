...
| `MEM-001` | Add enables member operations and currently exposes earlier group bills and schedules, including recurring and sync lifecycle mutations | Add has no current remove reversal identity | Active membership; add checks actor ownership and target account/profile before mutation, but omits target `DisabledAtUtc` and does not atomically bind actor authority to save; the composite primary key plus caught/rechecked `DbUpdateException` atomically prevents two same-pair inserts from both committing | Disabled targets are accepted; group, future, recurring, and import active-member loaders omit target accounts. Bill/report/reconciliation, future/recurring visibility, and sync archive/restore have no membership-start boundary; recurring and sync mutations use broad group visibility, and in-flight sync lifecycle writes do not recheck membership at commit. An owner demoted/removed after authorization can still commit. Missing account parity, pre-membership-history and schedule read/mutation policy, atomic actor predicates, denied/race coverage, idempotent retry result, and denial audit. |
...
| `MEM-001/002` | `MEM-CHOICE-006/007/011/014/015` | Implemented current fact, mechanics/audit partial; add and group/future/recurring selection loaders are account-unaware; add grants pre-membership bill/schedule visibility plus report/reconciliation/recurring/sync lifecycle mutation, and sync lifecycle writes can commit after actor removal; same-pair duplicate add is atomically constrained and mapped to conflict, while membership writes still lack atomic actor, target-prior-state, and owner-floor predicates and same-role update repeats a success write/audit; accepted notifications are unimplemented | Existing group-member, bill, and sync runtime; #722/#724 follow-up; notification owner | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC`, conditional `G-PRODUCT/G-SCHEMA/G-CONTRACT/G-CLIENT`; disabled-at add and eligibility paths; pre-membership bill/schedule/report/reconciliation/recurring/sync archive-restore coverage; sync concurrent-removal commit denial; target-account promotion; duplicate-add conflict/retry; same-role repeat; stale-owner-at-commit; simultaneous same-target role/update and two-owner demotion; viable-owner and notification tests. |
...
| `HIST-*` | `MEM-CHOICE-001/006/007/009/011/015` | Persistence implemented; access policy incomplete in both directions; new members get pre-membership bill/schedule visibility and reconciliation/recurring/sync lifecycle mutations, sync lifecycle writes can commit after actor removal, settlement/revision former-member gaps exist, import GET/discard omit post-removal membership and archived-group checks, removal can strand pending bill, ordinary/future draft, and active settlement work, and ordinary or stored-reference creation/submission can race/use removal | #718 financial owner plus #722 authorization/sync split; #724 retention; notification/sync owners | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC/G-DEST` for changed behavior; immutable references; pre-membership bill/schedule reads and reconciliation/recurring/sync archive-restore mutations; sync concurrent-removal commit denial; revision-wide and settlement account-state tests; concurrent removal versus ordinary group/future/recurring create/generate; removed creator and archived-group import-session denial; pending-participant, ordinary/future-draft, and active-settlement resolution; stored-target revalidation/notification tests. |
...
| `MEM-CHOICE-006` | Server group membership plus bill/revision/settlement/schedule/sync authorization | Which pre-membership and post-participation bill and schedule reads/mutations remain authorized; which sync lifecycle actors and commit-time membership predicate are required; should one former creator/owner/participant/payer block every revision or settlement action; and when must target account availability block a counterparty workflow? | Revision operations require active membership for creator, owner, every participant and payer. Settlement paths use active membership but generally omit target account availability. In the opposite direction, current group membership exposes earlier bills through list/read/export/report, reconciliation mutation, and sync archive/restore without ordinary lifecycle owner checks; sync authorization/load/apply/save are separate so removal can race an in-flight mutation. Current membership also exposes earlier future bills/recurring templates, and recurring mutations lack an owner/editor predicate. #718 `FIN-CHOICE-011/058..061` owns financial actor decisions. | Preserve records and current gates/exposure only as implemented facts; do not infer access from history, treat initial membership as accepted blanket or commit-time historical/schedule/sync mutation authority, or treat account-unaware eligibility as accepted policy. Route changes to focused authorization/money/sync owners. | #961 synthesis → focused group/financial/sync authz owners with #718 `FIN-CHOICE-011/058..061`; `G-AUTHZ/G-PRIV/G-MONEY/G-SYNC`. |
...
| `MEM-CHOICE-011` | Server group/membership mutation mechanics | What operation identity, expected version, and atomic actor-authority, target-prior-state, and owner-floor predicates govern retries/races, including group rename, stale-owner writes, same-role repeats, same-target conflicts, and simultaneous owner demotion/removal? | Current entities have timestamps but no concurrency token/idempotency contract. The membership composite primary key plus caught/rechecked write conflict already prevents two duplicate adds from both committing. Rename and other membership authorization, target load, owner count, and write remain separate, allowing stale actors, concurrent-name or role last-writer state, repeated same-role success audits, duplicate removal success audits, and zero owners. | Credit duplicate-add conflict safety, but do not treat uncertain retries or same-role writes as proven replay or initial authorization as commit authority. Future writes must atomically prove actor, target prior state, and owner floor or fail closed; rename must have bounded audit/result evidence. | #722 API/schema owner; `G-AUTHZ`, conditional `G-SCHEMA/G-CONTRACT`. |
...
| `MEM-CHOICE-015` | Server group membership, temporary-participant creation authority, import sessions, and financial dependency-safe removal | Who may create a temporary participant; when may non-active/account-unavailable/temporary people be included; what atomic membership predicate or target revalidation is mandatory at ordinary create/submit, future/recurring, and CSV saves; must import GET/discard revalidate membership and group state; and must removal be blocked or use an explicit resolution/transfer path when a pending participant, ordinary/future-draft creator/owner, or active settlement party would be stranded? | Group/future/recurring loaders omit target accounts and run separately from persistence. Ordinary submission resets/notifies stored participants without revalidation; CSV confirmation and future posting can likewise use removed targets. Import GET/discard recheck neither membership nor group state. Removal can block a pending participant's response, leave ordinary/future drafts without their required creator/owner identity, and make active settlement reads/mutations fail for the remaining counterparty when debtor, creditor, or requester loses membership. No bill/settlement dependency check, transfer/resolution operation, or placeholder actor contract exists. | Authorize no temporary actor, removal exception, transfer, or financial rewrite. Preserve current gaps only as implemented facts. Until focused owners decide, later removal implementation must fail closed rather than invent a dependency resolution; current runtime remains unchanged by this policy. | Group/bill/settlement product-money-authz/notification owners after #961 with #718 financial authority; `G-PRODUCT/G-AUTHZ/G-MONEY/G-PRIV`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
