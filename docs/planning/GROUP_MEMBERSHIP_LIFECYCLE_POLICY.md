...
- Current group bill create validates every submitted participant and payer
  ...
  excluded from the ordinary synchronous create path. That active-member query
  does not check target `AuthAccount` status, deletion, or `DisabledAtUtc`, so
  an account-unavailable but active membership remains eligible. Group CSV
  import confirmation has another exception: it rechecks only that the
  confirming actor is active, then materializes stored candidate participants
  and payers without revalidating all references, so a member removed after
  preflight can still enter newly created draft bills.
...
| `MEM-001` | Registered-user membership — **Add member** | Absent → active `member|owner`; active owner requests, API accepts an account with active status and no account/profile deletion, but does not check `DisabledAtUtc` | Eligible for explicit future group-bill participant/payer selection; eligibility remains account-unaware after add, and current create paths do not automatically select every active member | Current group/member access and role-bounded mutation eligibility; a status-active but disabled target can currently be added and later selected | Accepted `group.membership_added` in-app baseline for the affected recipient is unimplemented; exact recipient/policy handling remains notification-owned | **Implemented current fact with disabled/account-unavailable eligibility gaps; active eligibility is not automatic default selection.** `AddMemberAsync`; group-bill and CSV-import `LoadActiveGroupMemberIdsAsync`; `AllSubmittedMembersAreActive`; OpenAPI/tests. |
...
| `MEM-007` | Active membership — **Remove member** | Active → removed tombstone; active owner/API; sequential last-owner removal is blocked by an account-unaware count, but the count-then-write is not concurrency-safe | Excluded from all future active-member defaults/current group bill create | Current group/member and most group bill/settlement paths fail closed; historical links remain stored | Accepted `group.membership_removed` in-app baseline for the affected recipient is unimplemented; historical/security eligibility remains separately unresolved | **Implemented current fact with viable-owner-floor and zero-owner race gaps.** `RemoveMemberAsync`; `CountActiveOwnersAsync`; authorization and endpoint tests; audited success. |
...
| `MEM-001` | Add enables member operations | Add has no current remove reversal identity | Active membership; current add revalidates active group, non-deleted target profile and `AuthAccount` with active status, actor owner role, absence of any active/removed duplicate row, and target role; it omits `AuthAccount.DisabledAtUtc` | A status-active but disabled target is currently accepted, and both later group-bill active-member loaders omit all target-account state. Missing account-availability parity at add and future selection, denied-case coverage, version/idempotency, and denial audit. |
| `MEM-002` | Role change alters owner-only mutations prospectively | Role update is reversible subject to the current last-owner guard | Active membership; current update revalidates active group/profile/membership, actor owner role, target role, and owner count, but does **not** require an active target `AuthAccount` | `LoadActiveMembershipAsync` omits target-account state; `ResolveActiveAuthAccountIdAsync` may return null without blocking the save, and `CountActiveOwnersAsync` may count that membership. The separate count-then-write also permits concurrent owners to both demote after each observes two owners. Target-account viability, atomic viable-owner-floor revalidation, version/idempotency, and denial audit are missing. |
...
| `MEM-007` | Removed blocks current group/member and active-member-dependent bill/settlement operations; row remains | Current transition has no restore; logical removal is not physical deletion | None in current runtime. Any future target requires all group/account/role/duplicate/owner/dependency/version checks | Current last-owner guard counts active owner memberships without verifying an available `AuthAccount`; its separate count-then-write also lets concurrent owners both remove after each observes two owners. Missing atomic account-aware viable-owner-floor denial coverage, reason, version, idempotency, full dependency impact, denial/conflict audit, and restore rule. |
...
| `MEM-001/002` | Current composite ID, role/status, create/update time; no reason/version/idempotency. Sequential duplicate add conflicts; repeated same-role update is accepted/audited again. Role demotion uses a non-atomic owner count followed by an update, so two concurrent owners can both pass and leave zero owners. | Membership history retained; no hard delete; purge unresolved. | Current success audit is bounded. Add denial/conflict/replay, same-state semantics, and owner-floor race evidence need focused coverage. |
...
| `MEM-007` | Current status and `UpdatedAtUtc`; no removed-at/actor/reason/version/idempotency field on membership. One sequential accepted call writes status/audit; repeat sees no active row and fails closed. The non-atomic owner count and later update permit a two-owner concurrent removal race to zero owners. | Removed tombstone and dependencies retained; hard delete ineligible; purge unresolved/separate. | Current `group_member.removed` success audit stores bounded group and target profile IDs plus status. Add blocked/conflict/replay/concurrent-owner evidence later; never log the request body, raw email/login/contact identifier, or unrelated private identity data. |
...
| `MEM-001/002` | `MEM-CHOICE-007/011/014/015` | Implemented current fact, mechanics/audit partial; add and later bill selection are account-availability unaware, role update lacks target-account viability and atomic owner-floor enforcement, and accepted membership notifications are unimplemented | Existing group-member and bill runtime; #722/#724 follow-up; notification owner for required in-app baselines | `G-AUTHZ/G-PRIV/G-RET`, conditional `G-PRODUCT/G-SCHEMA/G-CONTRACT/G-CLIENT`; disabled-at add denial, account-unavailable bill-selection policy, target-account-disabled/deleted promotion denial, simultaneous two-owner demotion denial, viable-owner floor, exact membership-notification recipient/reauthorization/redaction, duplicate, hostile payload, replay, stale-role tests. |
...
| `MEM-007` | `MEM-CHOICE-005/006/007/009/011/014` | Implemented removal with account-unaware and concurrency-unsafe owner-floor gaps; restoration absent; accepted removal notification unimplemented | Existing runtime then #722/#724; notification owner for required in-app baseline | Current fact needs no new gate; changed semantics require `G-PRODUCT/G-AUTHZ/G-PRIV/G-RET`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; viable account-aware last-owner denial including disabled/deleted account, simultaneous two-owner removal denial, dependency/history, replay, and exact notification recipient/reauthorization/redaction evidence. |
...
| `HIST-*` | `MEM-CHOICE-006/007/009/015` | Persistence implemented; post-change access policy incomplete; current candidate/request queries require active creator plus every participant and payer; CSV confirmation can materialize stale participant/payer candidates | #718 financial owner plus #722 authorization split; #724 retention; notification owner | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-DEST` for changed runtime/disposition; immutable-reference, record-specific access, unrelated-former-participant versus still-active-party settlement eligibility, all-participant gate, confirmation-time revalidation after participant/payer removal or account unavailability, and redaction tests. |
...
| `MEM-CHOICE-011` | Server group/membership mutation mechanics | What operation identity, expected version, and atomic owner-floor predicate govern create/status/reactivation retries and races, including simultaneous demotion/removal by two owners? | Current entities expose timestamps but no membership/group concurrency token or idempotency contract; owner checks count first and write later, so each of two owners can observe two and both leave zero. | Do not treat uncertain retries as safe replay; future writes and any owner-floor-sensitive change must fail closed or use an atomic accepted predicate until defined. | #722 API/schema owner; `G-AUTHZ`, conditional `G-SCHEMA/G-CONTRACT`. |
...
| `MEM-CHOICE-015` | Server group membership, temporary-participant creation authority, and future bill/context selection | Which currently authorized bill creator/editor, active group member, or group/context-manager role may create a temporary participant; when may default-excluded, left, removed, account-unavailable, or temporary participants be included; and what confirmation-time revalidation is mandatory for stored import candidates? | Ordinary group create accepts submitted active registered memberships without target-account availability checks. CSV import confirmation rechecks only the actor, then uses stored candidate participants/payers, so a membership removed after preflight can enter a draft. No placeholder actor/creation contract exists, and prose leaves special/manual flows incomplete. | Authorize no temporary-participant creation actor or new special inclusion path. Preserve the two current runtime facts without treating either as accepted policy: ordinary create remains account-unaware, and CSV confirmation may use a stale candidate; route both gaps to the focused owner before changing behavior. | Group/bill product-money-authz owners after #961; `G-PRODUCT/G-AUTHZ/G-MONEY`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
...
| `GroupMemberManagementEndpoints` and tests | `LoadActiveProfileWithAccountAsync`; `LoadActiveMembershipAsync`; `CountActiveOwnersAsync`; add/update/remove | Current mutations and tombstone; add omits `DisabledAtUtc`; update/removal owner viability is account-unaware; non-atomic owner count then write permits a concurrent zero-owner race |
