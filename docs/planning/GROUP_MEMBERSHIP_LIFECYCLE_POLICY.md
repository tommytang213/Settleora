...
| bill participant acceptance/rejection and revision handlers; bill confirmation rules | group access gates actor response/revision while retained participant status contributes to bill confirmation | Removing a `pending_acceptance` participant can deny that actor's response/revision path while leaving the retained row pending and the bill unable to confirm |
