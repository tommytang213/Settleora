...
- Those ordinary active-member checks are also separate reads before the
  ...
  therefore commit already unusable by a revoked actor or target. Commit-time atomic membership
  eligibility or an equivalent serialization rule and focused actor/target
  race coverage remain `MEM-CHOICE-015`.
...
- Group future-bill and recurring-template visibility also begins with current
  ...
  set without an owner/editor predicate. These implemented exposures and
  mutation powers are not accepted policy; `MEM-CHOICE-006` routes the history
  boundary while #718 `FIN-CHOICE-011/058..061` retains the financial actor
  decisions.
...
| `HIST-*` | `MEM-CHOICE-001/006/007/009/011/015` | Persistence implemented; access policy incomplete in both directions; new members get pre-membership bill/schedule/change-feed visibility and reconciliation/recurring/sync lifecycle mutations; participant acknowledgement, reconciliation, and sync lifecycle writes can commit after actor removal; settlement/revision former-member gaps exist; CSV entry and settlement-request creation can race removal; import GET/discard omit post-removal membership and archived-group checks; removal can strand pending bill, ordinary/future draft, and active settlement work; and ordinary or stored-reference creation/submission can race/use removal | #718 financial owner plus #722 authorization/sync split; #724 retention; notification/sync owners | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC/G-DEST` for changed behavior; immutable references; pre-membership bill/schedule/change-feed reads and reconciliation/recurring/sync archive-restore mutations; participant acknowledgement and reconciliation/sync concurrent-removal commit denial; revision-wide and settlement account-state tests; concurrent removal versus ordinary group/future/recurring, CSV direct/session, and settlement direct/basket creation; removed creator and archived-group import-session denial; pending-participant, ordinary/future-draft, and active-settlement resolution; stored-target revalidation/notification tests. |
...
| `MEM-CHOICE-011` | Server group/membership and membership-gated mutation mechanics | What operation identity, expected version, and atomic actor-authority, target-prior-state, and owner-floor predicates govern retries/races, including group rename, participant acknowledgement and reconciliation/sync stale-actor writes, same-role repeats, same-target conflicts, and simultaneous owner demotion/removal? | Current entities have timestamps but no concurrency token/idempotency contract. The membership composite primary key plus caught/rechecked write conflict already prevents two duplicate adds from both committing. Rename, membership, participant acknowledgement, reconciliation, and sync lifecycle authorization/load/write remain separate, allowing stale actors, concurrent-name or role last-writer state, repeated same-role success audits, duplicate removal success audits, and zero owners. | Credit duplicate-add conflict safety, but do not treat uncertain retries or same-role writes as proven replay or initial authorization as commit authority. Future writes must atomically prove applicable actor, target prior state, and owner floor or fail closed; rename and record mutations must have bounded audit/result evidence. | #722 API/schema/authz and focused bill/sync owners; `G-AUTHZ`, conditional `G-SCHEMA/G-CONTRACT`. |
...
| `MEM-CHOICE-015` | Server group membership, temporary-participant creation authority, import sessions, settlement creation, and financial dependency-safe removal | Who may create a temporary participant; when may non-active/account-unavailable/temporary people be included; what atomic actor/target membership predicate or revalidation is mandatory at ordinary create/submit/participant accept-reject, future/recurring, CSV direct/session-create/confirm, and direct/basket settlement-request saves; must import GET/discard revalidate membership and group state; and must removal be blocked or use an explicit resolution/transfer path when a pending participant, ordinary/future-draft creator/owner, or active settlement party would be stranded? | Group/future/recurring loaders omit target accounts and run separately from persistence. Participant accept/reject authorizes and loads before a separate acknowledgement/status/audit/notification save; ordinary submission resets/notifies stored participants without revalidation; CSV direct import and session creation snapshot actor/targets before planning, CSV confirmation and future posting can use removed targets, and direct/basket settlement creation expands eligible actors/parties before saving requests, lines, audits, and applicable notifications. Import GET/discard recheck neither membership nor group state. Removal can block a pending participant's response, leave ordinary/future drafts without their required creator/owner identity, and make active settlement reads/mutations fail for the remaining counterparty when debtor, creditor, or requester loses membership. No bill/settlement dependency check, transfer/resolution operation, or placeholder actor contract exists. | Authorize no temporary actor, removal exception, transfer, or financial rewrite. Preserve current gaps only as implemented facts. Until focused owners decide, later removal implementation must fail closed rather than invent a dependency resolution; current runtime remains unchanged by this policy. | Group/import/sync/bill/settlement product-money-authz/notification owners after #961 with #718 financial authority; `G-PRODUCT/G-AUTHZ/G-MONEY/G-PRIV/G-SYNC`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
...
| `FutureBillEndpoints.VisibleFutureBills`; update/cancel/post handlers | current active group membership exposes group drafts; update/cancel require creator or bill owner and post requires creator | Newly added members can read pre-membership drafts; removing a draft creator/owner can strand a draft because remaining members lack the required identity to progress or cancel it |
...
| `RecurringBillEndpoints.VisibleTemplates`; update/pause/resume/archive/generate handlers | active group membership supplies template visibility and mutation eligibility without membership-start or owner/editor predicate | Newly added members can read and mutate pre-membership schedules; actor-policy decisions remain #718 `FIN-CHOICE-011/058..061` |
