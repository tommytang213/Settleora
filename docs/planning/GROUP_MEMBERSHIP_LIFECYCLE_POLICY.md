...
- The sync `bill_archive` and `bill_restore` path is broader still:
  ...
  This is another implemented authority mismatch under `MEM-CHOICE-006`, not
  accepted sync or historical-access policy.
...
| `HIST-*` | `MEM-CHOICE-001/006/007/009/015` | Persistence implemented; access policy incomplete in both directions; new members get pre-membership bill/schedule visibility and reconciliation/recurring/sync lifecycle mutations, settlement/revision former-member gaps exist, import GET/discard omit post-removal membership and archived-group checks, removal can strand pending bill, ordinary/future draft, and active settlement work, and ordinary or stored-reference creation/submission can race/use removal | #718 financial owner plus #722 authorization split; #724 retention; notification/sync owners | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC/G-DEST` for changed behavior; immutable references; pre-membership bill/schedule reads and reconciliation/recurring/sync archive-restore mutations; revision-wide and settlement account-state tests; concurrent removal versus ordinary group/future/recurring create/generate; removed creator and archived-group import-session denial; pending-participant, ordinary/future-draft, and active-settlement removal resolution; stored-target revalidation/notification tests. |
...
| `MEM-CHOICE-006` | Server group membership plus bill/revision/settlement/schedule/sync authorization | Which pre-membership and post-participation bill and schedule reads/mutations remain authorized; should one former creator/owner/participant/payer block every revision or settlement action; and when must target account availability block a counterparty workflow? | Revision operations require active membership for creator, owner, every participant and payer. Settlement paths use active membership but generally omit target account availability. In the opposite direction, current group membership exposes earlier bills through list/read/export/report, reconciliation mutation, and sync archive/restore without ordinary lifecycle owner checks; it exposes earlier future bills/recurring templates through list/read, and recurring update/pause/resume/archive/generation lacks an owner/editor predicate. #718 `FIN-CHOICE-011/058..061` owns financial actor decisions. | Preserve records and current gates/exposure only as implemented facts; do not infer access from history, treat current membership as accepted blanket historical/schedule/sync mutation authority, or treat account-unaware eligibility as accepted policy. Route changes to focused authorization/money/sync owners. | #961 synthesis → focused group/financial/sync authz owners with #718 `FIN-CHOICE-011/058..061`; `G-AUTHZ/G-PRIV/G-MONEY/G-SYNC`. |
...
| `SyncOperationService.ProcessOperationAsync`; `ExpenseBillLifecycleService.LoadVisibleAsync`; `ApplyLoadedAsync` | sync bill archive/restore uses current group visibility, then mutates without the creator/owner restriction of the ordinary lifecycle endpoint | A newly added active member can archive or restore a pre-membership group bill through sync |
