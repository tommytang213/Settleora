...
- Last-active-owner demotion and removal return conflict in a sequential call,
  ...
  both demote/remove, leaving zero; `MEM-CHOICE-011/014` owns that race. A
  removed actor's later group/member request normally fails, removed rows are
  omitted from member lists, and no restore/reactivation endpoint exists.
  `ListMembersAsync` has a query-time exception: it authorizes first, then its
  roster query filters by group and target-member state without binding the
  requesting actor's membership. Removal between those operations can still
  disclose member names, roles, statuses, and timestamps. This is an
  implemented privacy/authz race, not accepted former-member access;
  `MEM-CHOICE-011` owns the mechanical correction.
...
- Ordinary group bill, future-bill, and recurring-template create/update/draft
  ...
  so an account-unavailable active membership remains eligible. Stored-reference
  exceptions also exist: ordinary group-bill submission rechecks only the
  creator before resetting stored non-creator participants to
  `pending_acceptance` and notifying them; CSV confirmation rechecks only the
  actor before materializing stored candidates; and future-bill posting
  rechecks only the posting actor before materializing stored
  participants/payers and sending pending-participant notifications. Removal
  after draft creation, import preflight, or future-bill creation can therefore
  enter new activity without target revalidation.
- Those ordinary active-member checks are also separate reads before the
  eventual bill/template/draft save. The same applies when CSV direct import or
  import-session creation snapshots the actor and active targets before parsing
  and planning, and when direct or basket settlement-request creation expands
  eligible bills/parties before persistence. A concurrent membership removal
  can commit between validation and persistence, after which profile foreign
  keys do not enforce membership. New drafts, persisted private import review
  data, or a settlement request/lines and its applicable audit/notification can
  therefore commit already unusable by a revoked actor or target. Commit-time
  atomic membership eligibility or an equivalent serialization rule and
  focused actor/target race coverage remain `MEM-CHOICE-015`.
- Group CSV import-session GET and discard have a different revocation gap:
  ...
  profile IDs. Neither path rechecks current group membership **or current
  group state**, so a removed creator—or the creator of a session whose group
  later has `DeletedAtUtc` set—can still read persisted review data or discard
  the session. `MEM-CHOICE-001/015` own those access/mutation exceptions as
...
- Conversely, group bill list/read/export, group monthly reports, and group
  ...
  exposures, not accepted historical-access or mutation decisions. The
  reconciliation authorization, bill load, mutation, audit, and save are also
  separate, so an actor removed after authorization can still commit the
  status/note and success audit. The read paths have a distinct revocation
  race: `CanAccessGroupAsync` completes before the group-ID bill, export, or
  report query, and those queries do not bind the actor's membership. An owner
  can remove the actor in between and the actor can still receive financial
  data. This is an implemented query-time authority gap, not accepted
  post-participation access. Pre-membership history, query-time read authority,
  and commit-time mutation authority remain `MEM-CHOICE-006/011`.
- The sync `bill_archive` and `bill_restore` path is broader still:
  `ExpenseBillLifecycleService.LoadVisibleAsync` uses current active group
  access, and `ApplyLoadedAsync` then mutates the bill without the
  creator/owner restriction used by the ordinary lifecycle endpoint. A newly
  added member can therefore archive or restore a pre-membership group bill.
  Authorization, tracked load, apply, and save also lack a commit-time
  membership predicate, so an actor removed after `LoadVisibleAsync` can still
  commit the in-flight archive/restore. These are implemented authority and
  stale-actor gaps under `MEM-CHOICE-006/011`, not accepted sync or
  historical-access policy.
- The sync change feed separately uses only current active membership to admit
  retained expense-bill `SyncResourceVersion` rows for a currently active group
  after `sinceVersion`, without a
  membership-start, bill-participant, or bill-archive predicate. A newly added
  member requesting from version zero can therefore receive pre-membership and
  archived-bill resource IDs, versions, change kinds, and group IDs. This is an
  implemented metadata exposure under `MEM-CHOICE-006`, not accepted history
  access policy.
- Group future-bill and recurring-template visibility also begins with current
  active group membership and has no membership-start boundary. A newly added
  active member can therefore list/read pre-membership schedules. Recurring
  update, pause, resume, archive, and draft generation reuse that broad visible
  set without an owner/editor predicate. Future-bill update/cancel/post and
  those recurring mutations can also commit after the acting member is removed
  because visibility/load and save are separate. Occurrence generation has a
  recipient exception too: when a removed template owner is absent from the
  stored payload, another active member can generate and the notification
  writer still targets that removed owner with new group-activity metadata and
  an inaccessible bill action URL. These implemented exposure, stale-actor,
  and recipient gaps are not accepted policy; `MEM-CHOICE-006/007/011/015`
  route them while #718 `FIN-CHOICE-011/058..061` retains the financial actor
  decisions.
- Removing a member can strand active financial work. A removed
  `pending_acceptance` bill participant loses the group accept/reject and
  revision paths while the retained participant row can keep the bill pending.
  Acceptance and rejection have a separate stale-actor race: group
  authorization and bill load precede `SaveWorkflowAsync`, so an owner can
  remove the participant after those checks and the removed participant can
  still commit the acknowledgement, bill-status change, audit, and
  notifications. That is an implemented commit-time authority gap under
  `MEM-CHOICE-011/015`, not accepted acknowledgement policy.
  ...
  commit-time authority coverage for every membership-gated record write, not
  only membership, acknowledgement, reconciliation, or sync endpoints.
  A removed creator of an ordinary group draft loses submit authority, and a
  removed creator/owner of a group future-bill draft loses visibility and
  creator-required post/cancel/update authority; remaining active members lack
  the required identity to progress those drafts. Removing the debtor,
  creditor, or requester of an active group settlement can also make its read
  and mutation paths fail for the remaining active counterparty because those
  queries require all three parties to remain active members. Removal currently
  has no bill/settlement dependency check or transfer/resolution operation.
  `MEM-CHOICE-015` keeps the safe future requirement explicit without selecting
  block, transfer, or another money-resolution rule.
...
| `MEM-002` | Active membership role — **Change role** | Active role `member ↔ owner`, or same-role request → same role with refreshed timestamp and another success audit; active owner/API, sequential last-owner guard; no atomic actor-authority or same-target prior-state predicate | Remains eligible; role does not itself rewrite participant defaults | Future owner authority changes after API acceptance; concurrent writes can last-write-win and an in-flight stale owner can still commit | Accepted `group.membership_role_changed` in-app baseline for the affected recipient is unimplemented; current same-role request still writes the success audit | **Implemented current fact with target-account, stale-actor, same-role replay, and same-target concurrency gaps.** `UpdateMemberAsync`; tests; audited success. |
...
| `HIST-001` | Bill participant, payer, item split/assignment, revision identity — **not independently removable** | Membership change leaves accepted references unchanged; ordinary submission and stored CSV/future-bill candidates can use removed targets without revalidation | No automatic future selection follows from historical linkage | Read/mutation only through bill-specific current authorization; group revisions require every creator/owner/participant/payer active | Notify only when event-specific authorization independently permits it; ordinary submission and future posting currently notify stored pending participants without target-membership revalidation | **Implemented persistence plus documented invariant and current coupling gaps.** Expense/workflow/revision/future/import sources; `MEM-CHOICE-006/015`. |
...
| `MEM-001` | Add enables member operations and currently exposes earlier group bills and schedules, including sync change metadata plus recurring and sync lifecycle mutations | Add has no current remove reversal identity | Active membership; add checks actor ownership and target account/profile before mutation, but omits target `DisabledAtUtc` and does not atomically bind actor authority to save; the composite primary key plus caught/rechecked `DbUpdateException` atomically prevents two same-pair inserts from both committing | Disabled targets are accepted; group, future, recurring, and import active-member loaders omit target accounts. The member-roster query plus bill/report/reconciliation, future/recurring visibility, sync change feed, and sync archive/restore have incomplete query-time or membership-start boundaries; bill list/read/export/report and the member roster do not bind the requesting actor's membership after authorization, while recurring and sync mutations use broad group visibility. Listed acknowledgement, revision, reconciliation, OCR apply, schedule, settlement-payment, and sync lifecycle writes do not recheck membership atomically at commit. An actor removed after roster/bill-read authorization can still receive roster or financial data, and an owner demoted/removed after mutation authorization can still commit. Missing account parity, pre-membership-history/change-feed and schedule read/mutation policy, query-time read predicates, cross-family atomic actor predicates, denied/race coverage, idempotent retry result, and denial audit. |
| `MEM-002` | Role change alters owner-only mutations prospectively; same-role requests still refresh `UpdatedAtUtc` and emit another success audit | Role update is reversible subject to the current sequential guard; same-role replay is a new write, not a proven replay result | Active membership; update checks actor ownership, target membership/role, and owner count, but omits target account state and does not bind actor authority or target prior state to save | Owner count is account-unaware and count-then-write; two owners can both demote. Concurrent same-target changes last-write-win with separate success audits, same-role sequential requests repeat writes/audits, and an owner revoked after authorization can commit. Atomic target-state/actor/viable-owner predicates, version/idempotency, and denial audit are missing. |
...
| `MEM-007` | Removed blocks current group/member and active-member-dependent operations; stored import/future candidates and in-flight participant acknowledgements remain exceptions; pending bill acceptance, ordinary/future drafts, and active settlements can be stranded; row remains | Current transition has no restore; logical removal is not physical deletion | None in current runtime. Any future target requires all group/account/role/duplicate/owner/dependency/version checks | Owner guard is account-unaware/non-atomic; concurrent owners can remove to zero. Removal has no pending-participant, ordinary/future draft creator/owner, or active-settlement party dependency check, transfer, or resolution path. Participant accept/reject can commit after removal because authority/load and save are separate. Concurrent same-target removes can both emit success audit, and a stale-authority owner can commit. Missing atomic actor/target/viable-owner predicates, reason, version/idempotency, dependency resolution, denial/conflict audit, and restore rule. |
...
| `HIST-001/002` | Membership transition cannot mutate historical money/identity; record-owner workflows remain separate | Not a membership reversal target | Not applicable; future membership re-entry never changes these rows | #718 financial graph; #719 files/links; #721 account/identity; notifications. Missing policy covers newly added members' pre-membership bill/schedule/change-feed visibility plus reconciliation/recurring/sync lifecycle mutation; bill list/read/export/report completion after actor removal; acknowledgement, revision, reconciliation, OCR apply, scheduled, settlement-payment, and sync commits after actor removal; recurring removed-owner notification; revision-wide all-member gates; account-unaware settlement counterparties; CSV and settlement-create races; stranded work; and stored-reference revalidation. |
...
| `MEM-001/002` | Current composite ID, role/status, create/update time; no reason/version/idempotency/concurrency token. The composite primary key plus caught/rechecked `DbUpdateException` prevents concurrent duplicate adds from both committing, but actor authorization remains separate from save. Same-target role writes last-write-win; same-role sequential requests and simultaneous operations can emit repeated success audits; stale owners and two-owner demotions can commit. | Membership history retained; no hard delete; purge unresolved. | Current success audit is bounded. Add conflict/retry-result, stale-actor, same-target conflict, same-role repeat, and owner-floor race evidence need focused coverage. |
...
| `MEM-001/002` | `MEM-CHOICE-006/007/011/014/015` | Implemented current fact, mechanics/audit partial; add and group/future/recurring selection loaders are account-unaware; add grants pre-membership bill/schedule/change-feed visibility plus report/reconciliation/recurring/sync lifecycle mutation; the member roster and group bill list/read/export/report can return after concurrent removal because their data queries do not bind actor membership; listed membership-gated financial writes lack atomic commit-time actor membership; same-pair duplicate add is atomically constrained and mapped to conflict, while membership writes still lack atomic actor, target-prior-state, and owner-floor predicates and same-role update repeats a success write/audit; accepted notifications are unimplemented | Existing group-member, bill/OCR/settlement/schedule, notification, and sync runtime; #722/#724 follow-up | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC`, conditional `G-PRODUCT/G-SCHEMA/G-CONTRACT/G-CLIENT`; disabled-at add and eligibility paths; pre-membership bill/schedule/report/change-feed/reconciliation/recurring/sync coverage; query-time membership predicates plus concurrent-removal denial for the member roster and group bill list/read/export/report; concurrent-removal commit denial for every membership-gated write including acknowledgement, revision, reconciliation, OCR apply, future/recurring mutations, settlement payment, and sync; target-account promotion; duplicate/replay/owner-floor/notification tests. |
...
| `MEM-007` | `MEM-CHOICE-005/006/007/009/011/014/015` | Implemented removal with account-unaware and concurrency-unsafe actor/target/owner predicates; stored-reference exceptions, in-flight participant acknowledgements, and stranded pending-participant, ordinary/future-draft creator, and active-settlement dependencies remain; restoration absent; accepted removal notification unimplemented | Existing runtime then #722/#724; #718 financial owners; notification owner | Current fact needs no gate; changed semantics require `G-PRODUCT/G-AUTHZ/G-PRIV/G-RET/G-MONEY`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; account-aware owner floor; stale-owner-at-commit; simultaneous same-target and two-owner removal; duplicate audit/replay; participant accept/reject commit denial after removal; ordinary submission, CSV confirmation, and future posting after removal; pending-acceptance, ordinary/future-draft, and active-settlement dependency resolution; history and notification evidence. |
...
| `HIST-*` | `MEM-CHOICE-001/006/007/009/011/015` | Persistence implemented; access policy incomplete in both directions; new members get pre-membership bill/schedule/change-feed visibility and broad mutations; group bill list/read/export/report can complete after actor removal; membership-gated acknowledgement, revision, reconciliation, OCR apply, schedule, settlement-payment, and sync writes can commit after actor removal; recurring generation can notify a removed owner; settlement/revision former-member gaps exist; CSV and settlement creation can race removal; import GET/discard omit reauthorization; removal can strand work; and stored references can re-enter activity after removal | #718 financial owner plus #722 authorization/sync split; #724 retention; OCR/notification/sync owners | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-SYNC/G-DEST` for changed behavior; immutable references; pre-membership reads/mutations; query-time membership predicates and concurrent-removal denial for bill reads/exports/reports; commit-time actor membership for every membership-gated write with named cross-family race tests; revision-wide and settlement account-state tests; CSV/settlement creation races; removed-owner recurring-recipient reauthorization; import denial; stranded-work resolution; stored-target revalidation/notification tests. |
...
| --- | --- | --- | --- | --- | --- |
| `MEM-CHOICE-001` | Server group root, memberships, and record-specific reads | Does group archival use `DeletedAtUtc`, a separate archive state, or another overlay, and exactly what remains readable/mutable—including import-session GET/discard? | A nullable column exists, but no endpoint, audit, wording, or accepted lifecycle semantics exist. Most current group paths filter deleted groups, while import-session GET/discard never revalidate group state and would remain available to the bound creator after `DeletedAtUtc`. | Do not expose archive/reactivate or treat the import exception as accepted semantics. Preserve current behavior only as fact; any later archive implementation must explicitly close or approve every record-specific exception. | #961 → #722 group/schema/API plus import/bill authz owner; #723 UI; `G-PRODUCT/G-AUTHZ/G-PRIV`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
...
| `MEM-CHOICE-006` | Server group membership plus bill/revision/settlement/schedule/sync authorization | Which pre-membership and post-participation bill, schedule, and sync change-feed reads/mutations remain authorized; which group bill list/read/export/report queries require query-time membership predicates when removal races an in-flight read; which reconciliation/sync lifecycle actors and commit-time membership predicates are required; should one former creator/owner/participant/payer block every revision or settlement action; and when must target account availability block a counterparty workflow? | Revision operations require active membership for creator, owner, every participant and payer. Settlement paths use active membership but generally omit target account availability. In the opposite direction, current group membership exposes earlier bills through list/read/export/report and reconciliation mutation, plus pre-membership and archived-bill change metadata and sync archive/restore without ordinary lifecycle owner checks. Bill list/read/export/report authorize before querying, and their group-ID queries do not bind the actor membership, so concurrent removal can still allow financial data to return. Reconciliation and sync lifecycle authorization/load/mutation/audit/save are separate so removal can race an in-flight mutation. Current membership also exposes earlier future bills/recurring templates, and recurring mutations lack an owner/editor predicate. #718 `FIN-CHOICE-011/058..061` owns financial actor decisions. | Preserve records and current gates/exposure only as implemented facts; do not infer access from history, treat an earlier authorization check as query-time authority after removal, treat current membership as accepted blanket or commit-time historical/schedule/sync mutation authority, or treat account-unaware eligibility as accepted policy. Future read changes must bind current actor membership in the data query (or an equivalently atomic read-authority mechanism), fail closed when removal wins, and prove that race without deciding which historical records a former member should otherwise access. Route changes to focused authorization/money/sync owners. | #961 synthesis → focused group/financial/sync authz owners with #718 `FIN-CHOICE-011/058..061`; `G-AUTHZ/G-PRIV/G-MONEY/G-SYNC`. |
| `MEM-CHOICE-007` | Server invitation-, membership-, and record-specific notification eligibility | Beyond accepted invite create/accept and membership add/role-change/removal affected-recipient in-app baselines, which decline/cancel/expire, historical, settlement, scheduled, policy, and security events may notify invited, default-excluded, left, removed, or archived-group people? | The taxonomy decides those baselines, but no producer exists and other recipient/state eligibility is unspecified. Current recurring occurrence generation can unconditionally notify a removed template owner when that owner is absent from the payload and another active member generates, exposing new group-activity metadata and an inaccessible action URL. Notification remains distinct from invitation action, access, and defaulting. | Preserve accepted baselines and the current recurring exception only as implemented fact; do not accept removed-owner eligibility by implication. Produce no new runtime notification until focused recipient reauthorization, preference/mute, block/unfriend, and redaction gates pass; leave other cases unresolved. | Notification owner with invitation/group/schedule/authz owners; `G-PRODUCT/G-AUTHZ/G-PRIV`. |
...
| `MEM-CHOICE-011` | Server group/membership and membership-gated read/mutation mechanics | What query-time actor predicate, operation identity, expected version, and atomic actor-authority, target-prior-state, and owner-floor predicates govern reads, retries, and races, including the member roster plus every membership-gated write such as group rename/member changes, participant acknowledgement, revisions, reconciliation, OCR apply, future/recurring mutations, settlement payment claims, sync lifecycle, same-target conflicts, and simultaneous owner demotion/removal? | Current entities have timestamps but no shared concurrency/idempotency contract. The membership composite primary key plus caught/rechecked write conflict already prevents two duplicate adds from both committing. Member-roster authorization and query are separate, allowing removal to race disclosure. The named authorization/visibility/load and save steps are likewise separate, allowing revoked actors to commit tracked record/audit/notification changes; membership writes also permit concurrent-name/role last-writer state, repeated success audits, and zero owners. | Credit duplicate-add conflict safety, but do not treat uncertain retries, an earlier authorization check, or tracked loads as current authority. Every future membership-gated read must bind applicable actor membership in its data query or an equivalently atomic read-authority mechanism and fail closed when removal wins; every future membership-gated write must atomically prove applicable actor/target membership and prior state or fail closed. Owner-floor writes need the viable-owner predicate, and record mutations need bounded audit/result evidence. | #722 API/schema/authz with group-member/privacy and focused bill/OCR/schedule/settlement/sync owners; `G-AUTHZ/G-PRIV/G-MONEY`, conditional `G-SCHEMA/G-CONTRACT`. |
...
| `MEM-CHOICE-015` | Server group membership, temporary-participant creation authority, imports, scheduled/OCR/revision/settlement activity, and dependency-safe removal | Who may create a temporary participant; when may non-active/account-unavailable/temporary people be included; what atomic actor/target membership predicate or revalidation is mandatory for every membership-gated record save—including create/submit/accept-reject/revision/OCR apply, future/recurring, CSV direct/session, settlement request/payment, and sync—and notification recipient; must import reads/discard reauthorize; and when must removal block or use an explicit resolution/transfer path? | The named loaders and authorization queries run separately from persistence. Current stored-reference exceptions, CSV and settlement creation races, acknowledgement/revision/OCR/schedule/payment/sync stale-actor writes, removed-owner recurring notification, and import read/discard gaps are source-proven. Removal can strand pending participants, ordinary/future drafts, and active settlements. No common membership-qualified save, complete dependency check, transfer/resolution operation, or placeholder actor contract exists. | Authorize no temporary actor, removal exception, transfer, notification eligibility, or financial rewrite. Preserve current gaps only as implemented facts. Until focused owners decide, later removal implementation must fail closed rather than invent dependency resolution; current runtime remains unchanged by this policy. | Group/import/sync/bill/OCR/schedule/settlement product-money-authz/notification owners after #961 with #718 financial authority; `G-PRODUCT/G-AUTHZ/G-MONEY/G-PRIV/G-SYNC`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
...
| Archived group | No new group-wide defaults | Group/member/financial history preserved | Most current deleted-group paths fail closed; import-session GET/discard remain exceptions; future archive reads unresolved | New activity generally blocked; import-session discard remains an exception; historical/security unresolved | Group-only reactivation; never cascade members |
...
| `GroupMemberManagementEndpoints.ListMembersAsync`; member read tests | `CanAccessGroupAsync` followed by a roster query filtered by group and target-member state without a requesting-actor membership predicate | Removal after authorization but before the query can still disclose active member names, roles, statuses, and timestamps; focused concurrent-removal read denial is missing |
| `GroupMemberManagementEndpoints` mutation handlers and tests | `LoadActiveProfileWithAccountAsync`; `LoadActiveMembershipAsync`; `CountActiveOwnersAsync`; add/update/remove | Current mutations and tombstone; add omits `DisabledAtUtc`; actor authorization, target load/absence, owner count, and write are separate, so stale-owner, same-target last-writer/duplicate-audit, and zero-owner races remain possible |
...
| `ExpenseBillWorkflowEndpoints.SubmitGroupBillAsync`; `LoadGroupBillForCreatorAsync`; `SubmitBillAsync` | creator group access without stored-participant membership revalidation, then status reset and pending-participant notification | A participant removed after draft creation can be returned to `pending_acceptance` activity and notified on ordinary group-bill submission, then cannot answer through the group route |
| `GroupBillEndpoints.ListGroupBillsAsync`; `GetGroupBillAsync`; `ExpenseBillSearchQueries.VisibleGroupBillsIncludingArchived`; `BuildGroupExportResponseAsync` and group export endpoints | current `CanAccessGroupAsync` followed by group-ID bill query without participant, membership-start, or actor-membership predicate | A newly added active member can immediately list/read/export pre-membership group bills; removal after authorization but before the query can still allow those financial rows to return |
| `MonthlyReportEndpoints.GetMonthlyReportAsync`; `VisibleBillsQuery`; reporting tests | current group access followed by unarchived group-ID query without participant, membership-start, or actor-membership predicate | A newly added active member's group monthly report includes pre-membership group bills; removal after authorization but before the query can still allow report data to return |
| `ExpenseBillReconciliationEndpoints.UpdateGroupBillReconciliationAsync`; reconciliation tests | current group access followed by unarchived group-ID bill load, mutation, success audit, and save without participant, membership-start, or commit-time actor-membership predicate | A newly added active member can mutate reconciliation status/note on pre-membership group bills; an actor removed after authorization can still commit the write and audit |
| `SyncOperationService.ProcessOperationAsync`; `ExpenseBillLifecycleService.LoadVisibleAsync`; `ApplyLoadedAsync` | sync bill archive/restore uses current group visibility, then mutates without the creator/owner restriction of the ordinary lifecycle endpoint or a commit-time membership recheck | A newly added active member can archive/restore a pre-membership group bill; a member removed after visibility load can still commit the in-flight sync mutation |
| `SyncOperationService.ListChangesAsync`; `VisibleResourceVersionsQuery` | current active membership admits retained group resource versions without membership-start, participant, or bill-archive predicate | A newly added member requesting from version zero can receive pre-membership and archived-bill IDs, versions, change kinds, and group IDs |
...
| `FutureBillEndpoints.VisibleFutureBills`; update/cancel/post handlers | current active group membership exposes and loads group drafts before separate tracked mutations/save | Newly added members can read pre-membership drafts; removing a draft creator/owner can strand it, while removal after visibility load can still allow that actor's in-flight update/cancel/post commit |
...
| `RecurringBillEndpoints.VisibleTemplates`; update/pause/resume/archive/generate handlers | active group membership supplies template visibility and tracked mutation eligibility before separate save, without membership-start, owner/editor, or commit-time membership predicate | Newly added members can read/mutate pre-membership schedules, and a member removed after load can still commit an in-flight scheduled mutation; actor-policy decisions remain #718 `FIN-CHOICE-011/058..061` |
| `RecurringBillEndpoints.GenerateDraftAsync`; `InAppNotificationEvents.WriteRecurringDraftGeneratedNotificationAsync` | generation revalidates acting member/payload profiles but unconditionally targets the stored template owner | An active member can generate when a removed owner is absent from the payload, sending that removed owner new group-activity metadata and an inaccessible bill action URL |
| `BillCsvImportEndpoints.CreateGroupBillImportSessionAsync`; `ImportGroupBillsCsvAsync`; `ConfirmBillImportSessionAsync`; `CandidateJson`; import tests | session creation and direct import snapshot actor/active targets before CSV parsing/planning and save; confirmation actor check precedes stored-candidate deserialization | Concurrent actor/target removal can race session/direct persistence, leaving stale private review/candidate data or committing drafts; confirmation does not revalidate every stored participant/payer |
| `BillCsvImportEndpoints.GetBillImportSessionAsync`; `DiscardBillImportSessionAsync`; `LoadActorSessionAsync` | stored account/session/profile match without group-membership or current-group-state predicate | A removed group-session creator, or the creator of a session for a group later marked deleted/archived, can still read review data or discard the import session |
| `ExpenseBillWorkflowEndpoints.AcceptGroupBillParticipantAsync`; `RejectGroupBillParticipantAsync`; `SaveWorkflowAsync`; revision handlers; bill confirmation rules | group access and bill load gate actor response/revision before separate acknowledgement/status/audit/notification save | Removing a `pending_acceptance` participant can deny later response/revision while leaving the retained row pending and the bill unable to confirm; removal after authorization/load can still allow the in-flight accept/reject commit |
| ordinary group-bill submission and active group settlement request/payment handlers | creator-only draft submission; settlement visibility/mutations require debtor, creditor, and requester active membership | Removing an ordinary-draft creator strands submission; removing any required active settlement party can block the remaining counterparty's workflow |
| `ExpenseBillRevisionEndpoints.LoadVisibleBillAsync`; create/revise/submit/withdraw/approve/reject/payer-confirm/apply handlers; `SaveAndRespondAsync` | active group-membership predicates over creator, owner, every participant, and every payer precede tracked revision/bill/audit/notification writes | One former referenced person can block every otherwise eligible actor; removal after load can instead allow the in-flight revision mutation to commit |
| `ReceiptOcrReviewEndpoints.LoadVisibleBillContextAsync`; `ApplyReceiptOcrReviewAsync`; `LoadTrackedBillForApplyQuery` | membership-gated visible context precedes tracked bill reload/apply/save without a membership predicate on the reload | A removed group-bill creator/owner can still commit in-flight OCR-derived items, recalculated bill/payer state, and audit after initial authorization |
...
| `SettlementRequestCreateEndpoints.CreateGroupBillSettlementRequestAsync`; `SettlementBasketCreateEndpoints.CreateSettlementBasketAsync`; `SettlementBasketExpansionService` | direct query or basket expansion checks actor/counterparty membership before separate request/line/audit and applicable notification persistence | Concurrent removal of a required actor/party can still allow a newly created settlement workflow that subsequent membership-gated reads/mutations immediately reject |
| `SettlementPaymentClaimEndpoints.CreateSettlementPaymentAsync`; `SettlementPaymentClaimQuery` | debtor/required-party group membership and group access checks precede separate payment/allocation/residual/request-status/audit/notification persistence | Concurrent removal after the checks can allow an in-flight payment claim to commit even though later membership-gated settlement access rejects the workflow |
