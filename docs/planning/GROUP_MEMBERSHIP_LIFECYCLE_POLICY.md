...
- Conversely, group bill list/read/export, group monthly reports, and group
  ...
  status/note and success audit. Pre-membership history and commit-time
  authority remain `MEM-CHOICE-006/011`.
...
| `MEM-001` | Add enables member operations and currently exposes earlier group bills and schedules, including sync change metadata plus recurring and sync lifecycle mutations | Add has no current remove reversal identity | Active membership; add checks actor ownership and target account/profile before mutation, but omits target `DisabledAtUtc` and does not atomically bind actor authority to save; the composite primary key plus caught/rechecked `DbUpdateException` atomically prevents two same-pair inserts from both committing | Disabled targets are accepted; group, future, recurring, and import active-member loaders omit target accounts. Bill/report/reconciliation, future/recurring visibility, sync change feed, and sync archive/restore have no membership-start boundary; recurring and sync mutations use broad group visibility. Listed acknowledgement, revision, reconciliation, OCR apply, schedule, settlement-payment, and sync lifecycle writes do not recheck membership atomically at commit. An owner demoted/removed after authorization can still commit. Missing account parity, pre-membership-history/change-feed and schedule read/mutation policy, cross-family atomic actor predicates, denied/race coverage, idempotent retry result, and denial audit. |
...
| `MEM-CHOICE-006` | Server group membership plus bill/revision/settlement/schedule/sync authorization | Which pre-membership and post-participation bill, schedule, and sync change-feed reads/mutations remain authorized; which reconciliation/sync lifecycle actors and commit-time membership predicates are required; should one former creator/owner/participant/payer block every revision or settlement action; and when must target account availability block a counterparty workflow? | Revision operations require active membership for creator, owner, every participant and payer. Settlement paths use active membership but generally omit target account availability. In the opposite direction, current group membership exposes earlier bills through list/read/export/report and reconciliation mutation, plus pre-membership and archived-bill change metadata and sync archive/restore without ordinary lifecycle owner checks. Reconciliation and sync lifecycle authorization/load/mutation/audit/save are separate so removal can race an in-flight mutation. Current membership also exposes earlier future bills/recurring templates, and recurring mutations lack an owner/editor predicate. #718 `FIN-CHOICE-011/058..061` owns financial actor decisions. | Preserve records and current gates/exposure only as implemented facts; do not infer access from history, treat current membership as accepted blanket or commit-time historical/schedule/sync mutation authority, or treat account-unaware eligibility as accepted policy. Route changes to focused authorization/money/sync owners. | #961 synthesis → focused group/financial/sync authz owners with #718 `FIN-CHOICE-011/058..061`; `G-AUTHZ/G-PRIV/G-MONEY/G-SYNC`. |
