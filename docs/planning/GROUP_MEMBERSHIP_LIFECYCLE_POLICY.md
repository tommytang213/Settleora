...
- Removing a member can strand active financial work. A removed
  ...
  A removed creator/owner of a group future-bill draft loses visibility and
  creator-required post/cancel/update authority, while other active members
  lack the required identity to progress or cancel it. Removal currently has
  no dependency check or transfer/resolution operation. `MEM-CHOICE-015` keeps
  the safe future requirement explicit without selecting block, transfer, or
  another money-resolution rule.
...
| `MEM-001` | Add enables member operations and currently exposes earlier group bills and schedules, including recurring mutations | Add has no current remove reversal identity | Active membership; add checks actor ownership and target account/profile before mutation, but omits target `DisabledAtUtc` and does not atomically bind actor authority or absence of a duplicate to save | Disabled targets are accepted; group, future, recurring, and import active-member loaders omit target accounts. Group bill/report/reconciliation and future/recurring visibility have no membership-start boundary; recurring mutations reuse broad visibility. An owner demoted/removed after authorization can still commit. Missing account parity, pre-membership-history and schedule read/mutation policy, atomic actor/duplicate predicate, denied/race coverage, version/idempotency, and denial audit. |
...
| `MEM-001/002` | `MEM-CHOICE-006/007/011/014/015` | Implemented current fact, mechanics/audit partial; add and every group/future/recurring selection loader are account-unaware; add grants current group-wide pre-membership bill/schedule visibility, bill export/report/reconciliation mutation, and recurring-template mutation; membership writes lack atomic actor, target-state, and owner-floor predicates; accepted notifications are unimplemented | Existing group-member and bill runtime; #722/#724 follow-up; notification owner | `G-AUTHZ/G-PRIV/G-RET/G-MONEY`, conditional `G-PRODUCT/G-SCHEMA/G-CONTRACT/G-CLIENT`; disabled-at add and each group/future/recurring/import eligibility path; pre-membership bill and schedule visibility plus bill-report/reconciliation and recurring-mutation allowed/denied coverage; target-account promotion; stale-owner-at-commit; simultaneous same-target role/update and two-owner demotion; viable-owner; notification; duplicate/replay/stale-role tests. |
...
| `HIST-*` | `MEM-CHOICE-001/006/007/009/015` | Persistence implemented; access policy incomplete in both directions; new members get pre-membership bill/schedule visibility and some mutations, settlement/revision former-member gaps exist, import GET/discard omit post-removal membership and archived-group checks, removal can strand pending bill/future-draft work, and ordinary or stored-reference creation/submission can race/use removal | #718 financial owner plus #722 authorization split; #724 retention; notification owner | `G-AUTHZ/G-PRIV/G-RET/G-MONEY/G-DEST` for changed behavior; immutable references; pre-membership bill/schedule read and mutation; revision-wide and settlement account-state tests; concurrent removal versus ordinary group/future/recurring create/generate; removed creator and archived-group import-session GET/discard denial; pending-participant and future-draft creator removal resolution; ordinary submit/CSV confirmation/future-post target revalidation and notification; notification/redaction tests. |
...
| `MEM-CHOICE-006` | Server group membership plus bill/revision/settlement/schedule authorization | Which pre-membership and post-participation bill and schedule reads/mutations remain authorized; should one former creator/owner/participant/payer block every revision or settlement action; and when must target account availability block a counterparty workflow? | Revision operations require active membership for creator, owner, every participant and payer. Settlement paths use active membership but generally omit target account availability. In the opposite direction, current group membership exposes earlier bills through list/read/export/report and reconciliation mutation, and exposes earlier future bills/recurring templates through list/read; recurring update/pause/resume/archive/generation also lack an owner/editor predicate. #718 `FIN-CHOICE-011/058..061` owns those financial actor decisions. | Preserve records and current gates/exposure only as implemented facts; do not infer access from history, treat current membership as accepted blanket historical/schedule authority, or treat account-unaware eligibility as accepted policy. Route changes to focused authorization/money owners. | #961 synthesis → focused group/financial authz owners with #718 `FIN-CHOICE-011/058..061`; `G-AUTHZ/G-PRIV/G-MONEY`. |
...
| `MEM-CHOICE-015` | Server group membership, temporary-participant creation authority, import sessions, and financial dependency-safe removal | Who may create a temporary participant; when may non-active/account-unavailable/temporary people be included; what atomic membership predicate or target revalidation is mandatory at ordinary create/submit, future/recurring, and CSV saves; must import GET/discard revalidate membership and group state; and must removal be blocked or use an explicit resolution/transfer path when a pending participant or future-draft creator/owner would be stranded? | Group/future/recurring loaders omit target accounts and run separately from persistence. Ordinary group-bill submission resets and notifies stored non-creator participants without membership revalidation; CSV confirmation and future posting can likewise materialize stored targets after removal. Import GET/discard recheck neither membership nor group state. Removing a pending bill participant blocks that actor's response and can prevent confirmation/revision; removing a future-draft creator/owner can leave no actor with the identity required to progress or cancel it. No removal dependency check, transfer/resolution operation, or placeholder actor contract exists. | Authorize no temporary actor, removal exception, transfer, or financial rewrite. Preserve current gaps only as implemented facts. Until focused owners decide, later removal implementation must fail closed rather than invent a dependency resolution; current runtime remains unchanged by this policy. | Group/bill product-money-authz/notification owners after #961 with #718 financial authority; `G-PRODUCT/G-AUTHZ/G-MONEY/G-PRIV`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`. |
