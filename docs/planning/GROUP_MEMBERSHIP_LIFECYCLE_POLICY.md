...
| `MEM-CHOICE-011` | Server group/membership and membership-gated mutation mechanics | What operation identity, expected version, and atomic actor-authority, target-prior-state, and owner-floor predicates govern retries/races for every membership-gated write, including group rename/member changes, participant acknowledgement, revisions, reconciliation, OCR apply, future/recurring mutations, settlement payment claims, sync lifecycle, same-target conflicts, and simultaneous owner demotion/removal? | Current entities have timestamps but no shared concurrency/idempotency contract. The membership composite primary key plus caught/rechecked write conflict already prevents two duplicate adds from both committing. The named authorization/visibility/load and save steps remain separate, allowing revoked actors to commit tracked record/audit/notification changes; membership writes also permit concurrent-name/role last-writer state, repeated success audits, and zero owners. | Credit duplicate-add conflict safety, but do not treat uncertain retries, initial authorization, or tracked loads as commit authority. Every future membership-gated write must atomically prove applicable actor/target membership and prior state or fail closed; owner-floor writes need the viable-owner predicate, and record mutations need bounded audit/result evidence. | #722 API/schema/authz with focused bill/OCR/schedule/settlement/sync owners; `G-AUTHZ/G-MONEY`, conditional `G-SCHEMA/G-CONTRACT`. |
