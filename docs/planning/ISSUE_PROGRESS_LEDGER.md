...
- High-priority Day 1 owner recommendations are a focused `mobile-build-config` English catalog/native-label/permission-resource prerequisite followed by focused mobile catalog/bootstrap/key retention, auth-reviewed copy, What's New and contextual-help slices; non-runnable `docs-planning` parents keep all remaining mobile/web ordinary and shared-control copy, formatting, plural/collation, expansion, visible-enum/percentage and manually gated money call sites owned through bounded runtime children. Separate `web-user-ui` catalog/bootstrap/document-language, notification-key, password-reset and other auth/session slices follow #963/#973/#965 reconciliation; #964 remains open through admin catalog, formatting and expansion child ownership. Focused server push, generic SMTP notification-email and auth-email locale seams remain separate; #973 owns dynamic in-app-summary reconciliation. Non-runnable parents also own top-level/per-field validation-problem compatibility/adoption and non-problem server display fields. #409 remains the durable audit/registry reference after closure but owns no runtime mutation; none of its unnumbered recommendations is runnable until separately reconciled into a numbered focused issue.
