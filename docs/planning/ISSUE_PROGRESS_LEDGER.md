...
- This task changes only the policy and ledger. It makes no runtime,
  ...
  production, secret/configuration, destructive, product requirement,
  architecture direction, or Day 1 scope change. R05/#1235, #975, #946,
  #722 execution, #723 design, successor child execution, and autonomous queue
...
- `infra/truenas-catalog/settleora/` is an unpublished TrueNAS 25.10.x Docker Apps skeleton with current metadata/questions/Jinja/Compose layout and a pinned official library/validator authority. `tools/truenas-catalog/` semantically consumes a retained `settleora.day1-release-identity.v1` manifest only with its detached expected identity digest, validates exact R03 source/tree and index-to-selected-Linux/amd64 provenance, materializes immutable API/PostgreSQL/RabbitMQ/Caddy runtime identities, and records a public install plan without private paths or secret values.
- The rendered contract contains only ingress, migrate, API, PostgreSQL, and RabbitMQ. Only exact-RFC1918 HTTPS is host-published; API HTTP, PostgreSQL, AMQP/management, migration, and storage stay private. The package preserves R11 stable RabbitMQ persistence identity, R12 external-trust private HTTPS, immutable dataset/credential/passkey identities, UID/GID `999:999` API storage and data-protection-key preflight, safe migration modes, API-on-migration-success gating, and explicit backup/rollback limitations. Web/admin/OCR worker, public mode, automatic certificate management, arbitrary environment injection, and destructive migration options are excluded.
- Exact source passed doctors, 16/16 catalog tests, pinned official metadata/questions/Jinja/Compose rendering and adversarial post-render negatives, 38 Node + 27 Python release-identity tests, all three Compose validators, private-LAN HTTPS, R11 source/image recreate and persistence regression, docs/scaffold, 136/136 CI policy tests, scope/sensitive-material checks, and clean-tree proof. Fresh Gemini `strong_independent` and local Codex reviews returned zero findings. All 16 classifier-selected GitHub checks passed, including API/web/mobile/iOS, CodeQL, Semgrep, and Trivy; fresh GitHub Codex found no major issue. The fully paginated final inventory covered 20 issue comments, 66 reviews, 102 inline comments, and 52 threads, with zero unresolved actionable threads asserted before merge.
- Merged-main validation repeated the 16/16 package suite, pinned official renderer/negative matrix, and 38/38 + 27/27 release identity suites. Two real offline renders independently consumed retained R03 candidate `day1-13c8eaaedf83` from separate application source `13c8eaaedf83a29194befcde6c37c428da3300e6` / tree `00395fb3e8843989f40d26c5152dde2f5bedfbf4`; their complete packet trees were byte-identical. Both bind package source to merge `3b0a78bc9883b643e889d28d5b43adcdbd47bd19` / tree `d2122b86b22884c609048c02975fc9077831c832`, packet digest `ed1a5a07...`, package digest `dec4ea6d...`, and official semantic Compose digest `76f4003a...`, with published/deployed/host-mutated/migration-applied all false.
- No catalog publication, live TrueNAS install/host mutation, deployment, real secret/certificate change, DNS/TLS/router/VPN/tunnel action, database/migration application, backup/restore execution, image mutation, public/admin exposure, branch deletion, store action, #946 activation, or R05/#975 execution occurred. This retained same-task docs-only hygiene branch reconciles the ledger/readiness record before #1232 closes. #380 remains open, R05/#975 remain manual/live, #946 remains deferred, queue activation is disabled, and no successor was started.

### Issue #1214 — R03 reproducible Day 1 release identity manifest (2026-09-14 HKT)

- Logical task key `20260911-1943` started from main `a56433edaac8cfe4f712708e5685690b119ae05c`, tree `746a78d8db12c120a3d3a5dbfffed54c1c028554`. Retained implementation branch `release/r03-day1-identity-manifest-20260911-1943` produced [PR #1215](https://github.com/tommytang213/Settleora/pull/1215), reviewed source `558b42a16498eaffbb047312fbdb4185740122be`, normally merged as `13ba9f5d58191784a77daf87a8fff4eef6c83eef`. Corrective [PR #1228](https://github.com/tommytang213/Settleora/pull/1228), [PR #1229](https://github.com/tommytang213/Settleora/pull/1229), and [PR #1230](https://github.com/tommytang213/Settleora/pull/1230) converged live BuildKit provenance parsing, sealed evidence-descriptor inheritance, and stable fail-closed Android dependency provenance. Final correction source `0660baeb99fe8ee0b3ef1968cf8f1659158313cf` normally merged as final main `13c8eaaedf83a29194befcde6c37c428da3300e6`, tree `00395fb3e8843989f40d26c5152dde2f5bedfbf4`.
- `settleora.day1-release-identity.v1` and `tools/release/` separate deterministic manifest construction/validation from bounded Git, registry, migration, web, and Android collection. Identity uses SHA-256 over canonical JSON after excluding `generatedAt` and `identityDigest`. Inputs reject symlinks, path escape, special files, credential-like material, dirty/wrong source, mutable API authority, registry index/platform/revision mismatch, migration-set drift, stale web source, and APK/AAB identity drift.
- Normal main workflow run [34768309210](https://github.com/tommytang213/Settleora/actions/runs/34768309210) published `ghcr.io/tommytang213/settleora-api:sha-13c8eaaedf83a29194befcde6c37c428da3300e6`, index `sha256:f23a8001...`, Linux/amd64 manifest `sha256:13ecfac4...`, with matching OCI revision and verified BuildKit materials. Read-only resolution bound PostgreSQL `16-alpine`, RabbitMQ `3.13-management-alpine`, and Caddy `2.11.4-alpine` to their immutable indexes and selected Linux/amd64 manifests from `infra/docker-compose.truenas-lan.image.yml`; Compose defaults were not changed.
- External candidate `/workspace/logs/settleora-release-candidates/day1-13c8eaaedf83/` binds 39 deterministically ordered repository migration identities with set digest `1d4adafb...` and explicitly calls them source-only/not applied. Its exact-source R02 web entry contains 3 files / 426,988 bytes with canonical tree `6bf6cd64...`. Fresh minified Android outputs are APK 96,920,419 bytes / `56df6f82...` and AAB 77,132,938 bytes / `f2d7a171...`, version `1.0.0+1`, R8 mapping `922d7f4f...`, application ID `com.example.mobile`, and debug signing recorded honestly as non-store-ready.
- Independent validation rebuilt web and Android from final main and reproduced identity digest `624e846e691573defa856d8976c9c997b6720039f81782e3c823a00794a9b63a`; changing only `generatedAt` reproduced the same digest. Focused source/API/dependency/migration/web/APK/AAB mismatch and registry index/platform/revision tests all failed closed. The manifest file SHA-256 is `c63bbd9b...`; bounded release notes are digest-bound, and prior API rollback artifact metadata explicitly says availability does not prove database/schema/file rollback safety. Retention is through R04 and Day 1 acceptance; deletion remains a separate manual action.
- Full implementation and correction convergence passed root/API/Compose/API-image, web, 1,059/1,059 mobile tests, Android APK/AAB builds, 38 Node release tests, 27 Python policy tests, docs/scaffold, exact scope and sensitive-material guards. Fresh Gemini `strong_independent`, local Codex and GitHub Codex reviews converged; PR #1230 passed all 22 checks/scanners with one prior actionable thread corrected and final fully paginated inventory explicitly asserting zero unresolved threads.
- No deployment, promotion, production/store/public exposure, manual registry mutation or image publication, registry configuration change, environment/secret/auth/session/security runtime, storage/privacy, money/settlement/bill, schema/migration source/application, OpenAPI/generated-client, signing identity, Compose selection, release epoch, or #946 change occurred. The existing main-push workflow performed its normal automatic publication, which this task only observed. This retained docs-only hygiene branch reconciles the ledger/readiness record before #1214 closes. #380 remains open; R04 may consume the evidence, #946 remains deferred, and autonomous queue activation is disabled, so no successor was started.

### Issue #1209 — Android OCR release APK/AAB R8 dependency closure (2026-09-11 HKT)

- Logical task `20260911-1836` started from main `2e1cb0a7f357743029a98fe4c686f82acc9ce4a9`, tree `e90042e0980b05e5d69ba5522dee2d3b5cf16cc2`. Retained implementation branch `fix/1209-android-ocr-r8-20260911-1836` produced [PR #1212](https://github.com/tommytang213/Settleora/pull/1212), reviewed source `2d7ceac310433870aef9bdf4ed93f84feec9dac3`, tree `fa21fe8238e6753b969dc6107f5b5c838edd1014`, normally merged as `6ee8250b7e7c1a1bdb425daaec50f32d14dabaca`.
- The exact two-file implementation wires the optimized default release rules plus `apps/mobile/android/app/proguard-rules.pro`. Its eight exact `-dontwarn` entries match only the Chinese, Devanagari, Japanese, and Korean option/Builder types referenced by the plugin's compile-only switch arms. There is no wildcard, blanket suppression, package update, dependency/lockfile churn, recognizer-family addition, provider/parser change, or #959 mutation. Current Settleora source continues to select only `TextRecognitionScript.latin`.
- Before the fix, exact-main `flutter build apk --release` failed at `:app:minifyReleaseWithR8`. After it, doctor and `validate:mobile` passed with 1,059 tests; clean debug APK, minified release APK, and minified release AAB builds passed. Artifact evidence: debug APK 193,739,763 bytes / SHA-256 `c4fdaba07e1d7c82deba813b31d2f26a5c798483e8e76b042af7416a289a93c4`; release APK 96,920,419 bytes / `1354cc0881fe6e14daf5287e8102b209e8b0e9306f48a1239c6675ed1fd8db8d`; release AAB 77,132,675 bytes / `26cf964589a38e645337a4f0efa8e17639930b99952452d491c9feae00cf1444`.
- Before/after debug and release runtime graphs are semantically unchanged: `com.google.mlkit:text-recognition:16.0.1` and `text-recognition-bundled-common:17.0.0` remain resolved, with no optional-script recognizer artifact. APK/AAB inspection found the Latin `Latn_ctc` model and ML Kit OCR pipeline native libraries. The fix therefore adds no dependency/model bytes and does not alter bundled/offline Latin behavior. Static archive proof is not physical-device acceptance, and a future non-Latin selection must add its matching dependency and revisit the exact suppressions.
- Fresh exact-head Gemini `strong_independent` and local Codex Android/Gradle/R8 reviews passed with no blocking findings. All classifier-selected GitHub lanes passed: full API/generated/Compose/API-image, Linux mobile, hosted-macOS iOS simulator, stable aggregate, CodeQL, Semgrep and Trivy; user-web correctly skipped because routing was untouched. A fresh GitHub Codex review found no major issue. Fully paginated review inventory was 0 total / 0 unresolved / 0 actionable unresolved, explicitly asserted before merge; branch code-scanning alerts were zero.
- This is compile/package correctness, not a store release. Android still uses `com.example.mobile` and debug signing; R07 retains application identity, production signing, Play and later device/store acceptance. #975 retains physical-device OCR acceptance. #357/#359/#380 remain open. R03 may consume the completed R01 Android and R02 web evidence, but autonomous queue activation is disabled and no successor was started.
- Pre-hygiene checkpoint: #1209 remains open only until retained branch `docs/1209-android-ocr-r8-hygiene-20260911-1836` merges this ledger/readiness/OCR-audit reconciliation. No API/OpenAPI/generated-client, auth/session/security, storage/file/privacy, money/settlement/bill, schema/migration, OCR provider/parser, iOS, application identity, signing, store, deployment/CI, environment/secret, public exposure, production, or #959 source/issue/branch/lifecycle state changed.

### Issue #970 — Day 1 OCR completeness audit and R01 ownership reconciliation (2026-09-11 HKT)

- Logical task `20260911-1711` starts from main `d2472faaf855f326e9928a02e7efa89ae7415430`, tree `5b784c51f78b071b1b22550e3029365d27c1aba4`, on retained branch `docs/970-day1-ocr-completeness-audit-20260911-1711`. The exact tracked scope is `docs/qa/DAY1_OCR_COMPLETENESS_AUDIT.md` and this ledger.
- The audit inventories 25 distinct OCR capabilities: 6 `implemented`, 12 `partial`, 4 `blocked`, 2 `externally-gated`, 1 `documentation-only`, and 0 `unavailable`. It credits current camera/gallery/file intake, in-memory JPEG/thumbnail artifact processing, ML Kit provider bootstrap, parser/preview, personal/group editable review, saved-review API handoff, queue/discovery, duplicate guidance, explicit draft apply, and non-draft server revision routing without claiming the complete workflow or final acceptance. It records four normalization bypasses: ML Kit reads the inherited original path, failed/unsupported normalization can leave original bytes uploadable, existing-bill attachment upload forwards picked bytes directly, and supporting-to-receipt draft purpose change only relabels the attachment. It classifies offline behavior as partial: warm authenticated-shell extraction remains reachable after connectivity loss, while Local Mode and offline/server-unavailable cold bootstrap cannot reach real OCR and durable local review/queue behavior is absent.
- Exact-main bounded evidence reproduced `flutter build apk --release` failure after 57.8 seconds at `:app:minifyReleaseWithR8`. ML Kit Chinese, Devanagari, Japanese, and Korean recognizer option classes are absent from `releaseRuntimeClasspath`; the plugin bridge references them while Settleora currently requests only the Latin recognizer. Debug APK evidence remains valid, but no release APK/AAB is accepted. The audit does not choose or implement a dependency/R8 fix.
- Live duplicate search found no focused R01 owner. Closed #437 is planning-only; #358 owns intake/normalization; #438 owns failure/recovery UX; #740/#959 own parser quality, and #959 explicitly excludes native build inputs. New Issue [#1209](https://github.com/tommytang213/Settleora/issues/1209), **R01: fix Android OCR release APK/AAB R8 dependency closure**, is the sole R01 owner. It carries an independent `M = 2 MD` estimate, no `auto-ready` label, a narrow Android/pubspec-if-proven envelope, full debug/release APK/AAB/R8/package-model/size/offline/Linux/iOS review gates, and an explicit statement that debug signing plus `com.example.mobile` leave store readiness to R07.
- #959 remains open and untouched. Current main lacks its exact HK fixture completion; preserved candidate `92b60cec46114c11a47184687509d30da6f5df10` remains outside main on its existing branch. This task does not resume or mutate its claim, charge, counters, lifecycle, recovery, labels, branch/worktree, runtime, or issue state. #740 remains broader than #959 because the shipped provider selects the Latin script: parser hardening alone cannot prove Chinese image recognition, and any added native model/configuration needs a separate size/offline/build decision.
- Dependency chain: merge/close #970 -> R01/#1209 -> R03 may consume completed R02 web evidence plus a valid Android release entry -> R07 separately handles application identity/signing/store readiness. Docs-only audit #975 may reconcile final evidence only after #358/#401/#438/#740/#959/#442/#966, #967 and every focused runtime child it creates/adopts, #971 and every focused runtime child it creates/adopts, and #357's server-complement child/disposition. #975 then creates or adopts focused acceptance owners; those owners bind exact final artifacts to physical-device/platform/UI/privacy evidence, while explicitly authorized humans retain sign-off. #975 itself neither performs runtime acceptance nor grants sign-off. #959 is independent. #357 owns the required server complement until it approves a focused child. Docs-only #967 owns reconciliation/splitting of broader Day 1 OCR correction semantics, while its approved children own runtime closure. Docs-only #971 owns reconciliation and creation/adoption of focused Local Mode/offline OCR reachability and sync implementation children; those approved children, not #971 itself, own runtime closure. #357/#359/#380 remain open.
- The audit changes no mobile/runtime/provider/parser/native dependency, API/OpenAPI/generated-client, auth/session/security, storage/file/privacy behavior, money/settlement/bill authority, schema/migration, worker, deployment/CI/environment, signing, identity, secret, store, production, auto-runner, or #959 state. Autonomous queue activation remains disabled, so no successor is started.

### Issue #1205 — R02 additive user-web CI build/package lane (2026-09-11 HKT)

- Logical task key `20260911-1253`; starting main `8d6ffbbe24d4b6b88240ea38d77a875566557fae`. Retained implementation branch `ci/r02-user-web-build-package-20260911-1253` produced initial [PR #1206](https://github.com/tommytang213/Settleora/pull/1206), whose source `be5516cc292a2da527400ea9ec73efcb31e82800` (tree `03ee522a0be844cef9ef6b1905a490aede30dfa7`) normally merged as `497b8c40572e62c68c0830c728a17977ebfdf722`. A pre-merge orchestration assertion failed to stop on the printed unresolved-thread count, so same-task corrective [PR #1207](https://github.com/tommytang213/Settleora/pull/1207) completed review convergence from exact head `4df10f7d9ae5478288327cc24ad68ad78116110d` (tree `537684248fbc909a4939ac2522d5e049ba9c2a58`) and normally merged as `01bf000ecfb718fa5708202892d77178fc9a7fb4`. No finding was dismissed or left unresolved.
- The existing #1185 classifier now emits `run_web_user_validation`. Changes under `apps/web-user/**`, `packages/client-web/**`, or the workflow/classifier/manifest/policy-test control surface select the web lane; ambiguous evidence selects all expensive lanes. Ordinary docs-only changes remain lightweight. The stable required context remains exactly `Validate scaffold`, which requires a selected web lane to succeed and accepts only an intentional skip when it is not selected.
- The source-head-bound web job uses Node 22 and `apps/web-user/package-lock.json`, then runs `npm ci`, lint/typecheck, all tests, and the production Vite build. It rejects empty output and stages only `dist/` plus an external `settleora.user-web-dist-manifest.v1` manifest. The manifest records source commit/tree, lockfile SHA/version, bounded tool versions, sorted relative paths, byte sizes, per-file SHA-256 values, and one canonical tree SHA-256; raw tracked Git paths, modes and bytes are verified against HEAD, while symlinks, special files, source maps, path escapes, credential-like material and host-local paths fail closed.
- Exact final-head local evidence passed root/web clean installs, zero-vulnerability web audit, TypeScript lint, 14 Vitest files / 139 tests, Vite 8.1.0 with 33 transformed modules, scaffold validation and 91/91 CI/helper tests. Two clean builds produced identical three-file, 426,988-byte manifests with tree digest `6bf6cd648121edfc0633cc8bb15ce7a3980881a9b7b2feb2572b24af86da00be`.
- PR #1207 artifact `user-web-dist-4df10f7d9ae5478288327cc24ad68ad78116110d` (ID `10190883387`, 14-day retention) contained exactly the three static files plus the external manifest. Independent download and regeneration matched every byte and manifest field. Classifier, web, full API/generated/Compose/API-image, mobile, iOS simulator, stable aggregate, five CodeQL analyzers, Semgrep CE/OSS and Trivy checks passed; final Gemini strong-independent, local Codex and GitHub Codex reviews passed, with zero unresolved threads.
- This is inspectable CI/package evidence, not a deployment, serving package, product-completeness claim, or public release. No dependency, application runtime/UI, API/OpenAPI/generated-client, auth/session/authz, storage/privacy, money/settlement/bill, schema/migration, mobile/iOS/Codemagic, environment/secret, hosting, DNS/TLS, ruleset or production behavior changed. #373 and #380 remain open; R08 still waits for #373 plus this build primitive. Live reconciliation confirms open #970 must adopt or split R01's Android release-build gap before R03 can satisfy its declared cross-platform close rule; neither was started because autonomous queue activation is disabled. This retained docs-only hygiene branch supplies the required intentional-skip proof before #1205 closes.

### Issue #1201 — Mobile test-environment portability (2026-09-11 HKT)

- Logical task key `20260911-1124`; starting main `883870de484919432911378664c764822db9679d`. Retained implementation branch `ci/mobile-test-environment-portability-20260911-1124` produced [PR #1202](https://github.com/tommytang213/Settleora/pull/1202), reviewed source `f5e2802a9decd4eb26185d3c6cd55fd6d5d5d0e0` (tree `483f095291e2c39051586b408dd4127d4d0a2e58`), normally merged as `c6bcab3bf42cfe61b88e55131676cc1fe05de65c`.
- The shared test-font boundary now prefers Codemagic's supported `FLUTTER_ROOT`, falls back by walking ancestors of the active Flutter/Dart test runtime, canonicalizes the SDK and both material-font files, requires the exact Roboto and Material Icons artifacts to exist inside that SDK, and fails with bounded diagnostics. GitHub no longer manufactures `/opt/flutter`; hosted macOS runs a real font-load proof before its unchanged simulator build.
- Every current runnable visual writer now uses the shared `SETTLEORA_VISUAL_OUTPUT_ROOT` boundary. Its normal default is ignored `apps/mobile/build/settleora-visual-qa`; Codemagic binds the override and artifact glob to `$CM_BUILD_DIR/apps/mobile/build/settleora-visual-qa`, while DevBox operators may explicitly choose `/workspace/logs`. Historical evidence records were not rewritten.
- Release selection is not weakened: baseline 63 normal/mixed, 31 dedicated visual and 2 mixed visual-tagged files became 64/31/2 only by adding the resolver regression file. The shared release script is unchanged, every pre-existing normal file remains selected, dedicated visual files remain excluded from release validity, and Codemagic's separate visual lane selects 31 dedicated plus exactly 2 mixed tagged files without duplicate execution.
- Flutter 3.44.8 dependency resolution and analysis passed; resolver coverage passed 8/8, a representative ordinary helper consumer passed 4/4, and both the shared release gate and root mobile validation passed 1,059/1,059 tests. One dedicated visual capture produced 2 PNGs and one mixed tagged file produced 22 PNGs under an explicit task-owned root; all 24 hashes matched the corresponding pre-fix evidence exactly.
- Fresh exact-head Gemini `strong_independent`, local Codex, and GitHub Codex reviews passed with zero findings. GitHub synthetic merge `f7d73d30e551f3be359800d0257ac67094d95a48` (tree `483f095291e2c39051586b408dd4127d4d0a2e58`, identical to reviewed source) passed the classifier, Linux mobile, full API/generated/Compose/API-Docker validation, stable aggregate, Semgrep CE and Trivy repository jobs; the hosted-macOS font proof/iOS simulator job and five CodeQL analyzers checked source head `f5e2802a...`. Their CodeQL, Semgrep OSS and Trivy provider summaries also passed. Fully paginated review submissions and threads had zero actionable or unresolved entries.
- No Codemagic cloud build, signing, upload, App Store Connect, TestFlight, store, deployment or production action ran or is inferred. Repository portability is merged; maintainers still manually run `mobile-ios-validation` and, when hosted screenshots are desired, `mobile-ios-visual-evidence`. The signed internal workflow remains a separate R06/#975 action. #380 and #975 remain open, and R02 returns as the next dependency-safe repository recommendation after this broken validation gate. Final report: `/workspace/logs/settleora-codex-report-20260911-1124-mobile-test-environment-portability.md`.
- No app runtime, API/OpenAPI/generated-client, auth/session/security, storage/privacy, money/settlement/payment/OCR/sync, schema/migration, Flutter version, signing/publishing, secret, exposure, visual baseline or production behavior changed. This retained hygiene branch reconciles the ledger/readiness record before #1201 closes.

### Issue #1198 — R10 user-web dependency-alert remediation (2026-09-11 HKT)

- Logical task key `20260911-1028`; starting main `389f495062de9f4c3cb4da7e1ec4c951cb6f26f2`. Retained implementation branch `security/r10-web-user-dependency-alerts-20260911-1028` produced [PR #1199](https://github.com/tommytang213/Settleora/pull/1199), reviewed source `439cbe7f33edf82f4b7e752c08c32056799fc58d` (tree `8a8b751b0115e2ceac02d54afb9951e7d6cd201c`), normally merged as `5fc712695657be51baf54548ecefc1892497fdb7`.
- The only implementation file is `apps/web-user/package-lock.json`; `package.json` is byte-identical to the starting main. Browserslist moved `4.28.4 -> 4.28.7`, baseline-browser-mapping `2.10.40 -> 2.11.0`, and Vitest plus its seven exact-coupled `@vitest/*` nodes `4.1.9 -> 4.1.11`. Browserslist's three newly required data nodes took their minimum satisfying versions, and Nano ID moved `3.3.16 -> 3.3.18` for the independent same-graph npm-audit advisory. No package node was added or removed.
- Clean user-web install audited 144 packages with zero vulnerabilities after the baseline reported three moderate/two high vulnerable nodes. Exact `npm explain`/`npm ls` paths remained plugin-react/Babel to Browserslist/baseline, direct Vitest to `@vitest/mocker`, and Vite/PostCSS to Nano ID. Registry integrity matched every changed node; the pinned resolver repeat preserved exact lock and diff hashes.
- User-web TypeScript lint passed, Vitest 4.1.11 passed 14 files / 139 tests, and Vite 8.1.0 built 33 modules into three files. Browserslist/baseline remain build-host tooling; Vitest/mocker remain test/tool-host risk, with no browser-mode or public mocker plugin configured. No affected marker enters `dist`, and the candidate's three artifact hashes are byte-identical to a build from the vulnerable starting lock graph, proving none of these package updates changes or ships in the browser artifact.
- Fresh exact-head Gemini `strong_independent` and local Codex mechanics/security/build reviews passed with no material findings. GitHub Codex reviewed `439cbe7f...` with no major issues. All 15 check contexts converged: five CodeQL analyzers plus aggregate, Semgrep CE/OSS, Trivy repository/summary, classifier, full API/generated/Compose/API-Docker validation, and stable scaffold aggregate succeeded; mobile/iOS correctly skipped. Review submissions, inline comments, and fully paginated threads had zero actionable/unresolved entries; branch-scoped code and repository secret-scanning alert counts were zero.
- Dependabot #32 (`GHSA-73wf-gq98-2v4g`), #34 (`GHSA-w5vr-8v7q-w6rv`), and #35/#36 (`GHSA-82fw-gwwq-j7x9`) closed automatically as `fixed` at `2026-09-11T02:55:20Z`/`02:55:21Z` after main ingested the patched graph. Every dismissal reason/comment/timestamp remains null; no suppression, waiver, ignore or reclassification was used.
- GitHub automation closed #1198 at `2026-09-11T02:55:16Z`, one second after PR #1199 merged and four to five seconds before Dependabot recorded the targeted alerts as fixed. The immediate post-merge readback reconciled #32/#34-#36 successfully, so the issue was not reopened; retained branch `docs/1198-r10-web-user-dependency-hygiene-20260911-1028` completes the ledger/readiness reconciliation. #380 and #975 remain open. R10 is complete; R02 additive user-web CI build/package evidence is the next dependency-safe recommendation, but autonomous queue activation remains disabled.
- No application source/config, React/Vite/TypeScript/jsdom, runtime/API/OpenAPI/generated-client, auth/session/authz, storage/privacy, money/settlement/bill, schema/migration, Docker/CI/deployment/serving/exposure, environment/secret, mobile/admin, release or production action changed. Final report: `/workspace/logs/settleora-codex-report-20260911-1028-r10-web-user-dependency-alert-triage.md`.

### Issue #1195 — R12 fail-closed private LAN bind and mobile-safe HTTPS (2026-09-11 HKT)

- Logical task key `20260911-0332`; starting main `b2bc717fee47d67887606343889fa4f7d9ef19b7`. Retained implementation branch `infra/r12-private-lan-mobile-safe-https-20260911-0332` produced [PR #1196](https://github.com/tommytang213/Settleora/pull/1196), reviewed source `849c5695fc5e90eddf788777cab588ab9e90c784` (tree `91e414b4d58b8640124e34bb35f0c168cc0d4ca5`), normally merged as `43a828b9e5172069a00e80928069e815ae0dec9e`.
- Both source-build and image LAN Compose variants now fail closed unless the operator supplies one canonical RFC1918 IPv4 interface, one exact certificate hostname and HTTPS port, and readable non-empty external certificate/key files with no other-user key permissions. Wildcard, IPv6, loopback, link-local, public, malformed, ambiguous-octet and documentation-only values are rejected. Only Caddy's exact `${SETTLEORA_API_BIND_ADDRESS}:${SETTLEORA_API_HTTPS_PORT}` ingress is published; API HTTP is un-published, Caddy cannot reach the backend network, and PostgreSQL/RabbitMQ/migrate remain internal-only.
- The pinned `caddy:2.11.4-alpine` ingress uses exact-host routing, external read-only TLS mounts with host-path autocreation disabled, automatic HTTPS/admin/access logging disabled, redacted error logging, a read-only filesystem, bounded tmpfs, UID/GID `1000:1000`, `cap_drop: ALL`, and `no-new-privileges`. Android declares network access and iOS declares the local-network purpose; mobile URL and platform TLS verification behavior was not weakened.
- The supported physical-device trust contract is an operator-owned registered certificate hostname whose publicly/system-trusted certificate chain covers that exact name and whose private DNS answer resolves only to the selected RFC1918 address. No public listener, public A/AAAA service record, tunnel, router forward, ACME/account automation, arbitrary self-signed trust, private-key material, or certificate bypass is included. Real DNS, certificate/key provisioning, TrueNAS activation and physical-device acceptance remain R05/#975 manual evidence.
- Exact final-head local validation passed shell syntax, both Compose renders, the deterministic bind/TLS/key-mode refusal matrix, unprivileged zero-capability disposable HTTPS `/health/ready` proxy smoke, failure-log redaction, docs, 19-path scaffold, 70/70 CI policy tests, 1,520/1,520 API tests, Flutter 3.44.8 analysis and 1,051/1,051 mobile tests, and the full R11 RabbitMQ continuity/adoption/refusal suite for both Compose variants. The R11 suite had transient connection refusals while intentionally run beside the resource-heavy API/mobile suites, then passed cleanly in isolation; no live dataset was used.
- Fresh final-head Gemini `strong_independent` and local Codex reviews passed with zero findings. GitHub Advanced Security initially opened four Semgrep review threads for shell-global `IFS` mutation; final head `849c5695` replaced it with bounded `awk` parsing, the complete validation/review loop passed again, all four threads became outdated and were resolved, and GitHub Codex reported no major issues on the exact final head. All 15 checks passed: classifier/API/mobile/aggregate, Semgrep CE/OSS and Trivy repository/nested checks validated synthetic merge `20f4ad654d802e3ef741f2ae3b22edca7305e654`; hosted-macOS iOS and the five CodeQL analyzers plus aggregate were bound to source head `849c5695fc5e90eddf788777cab588ab9e90c784`.
- API `AllowedHosts`, CORS and forwarded-header middleware remain intentionally unchanged: the exact-host Caddy route is the only client ingress, API HTTP is not published, the native mobile client does not require browser CORS, and current API code consumes no forwarded request metadata. No auth/session, storage/privacy, money/settlement/bill calculation, schema/migration, OpenAPI/generated-client, catalog, CI/deployment activation, real environment, secret, certificate, signing/store, public or admin behavior changed.
- Pre-hygiene checkpoint: #1195 remains open only until retained branch `docs/1195-r12-private-lan-https-hygiene-20260911-0332` merges this ledger/readiness reconciliation. #380 remains open; R12 repository implementation is complete, R05/#975 remain live/manual, and R10 user-web dependency-alert triage is the first dependency-safe next logical task.
- Final report: `/workspace/logs/settleora-codex-report-20260911-0332-r12-private-lan-mobile-safe-https.md`.

### Issue #1192 — Flutter revision correctness and release-validation parity (2026-09-11 HKT)

- Logical task key `20260911-0204`; starting main `9658c6a9d400e6ab5b37ec6a5e0e94006e08d506`. Implementation branch `ci/mobile-flutter-codemagic-validation-parity-20260911-0204` is retained. [PR #1193](https://github.com/tommytang213/Settleora/pull/1193) merged corrective reviewed source `c38bcbf30980ac76af46e2f0cd5fb50b2ee6104e` (tree `59b1a6ecfb7f90ff92f380533444c3d3989096c1`) normally as `082e48e6e836e1648b416bc3fd65123aba1af9e1`.
- Personal and group bill revision creation now awaits `createBillRevision` inside each existing fresh-capability `try`, so asynchronously completed errors reach `_createRevisionFailureFrom` while successful return values, capability rechecks and typed failure behavior remain unchanged. Focused widget tests prove both success paths and both asynchronous mapped-failure paths.
- `apps/mobile/tool/validate-release.sh` is the single repository-owned normal release gate used by root `validate:mobile`, Codemagic validation and Codemagic internal TestFlight. It runs dependency resolution, fatal analysis and the non-visual test set; dedicated visual capture/evidence filenames remain separate, mixed files retain their ordinary cases, and `--exclude-tags visual` filters only visual-tagged cases. The script is Bash 3.2/BSD compatible and fails on an empty selection.
- GitHub Linux mobile, GitHub-hosted macOS iOS simulator and every Codemagic mobile workflow now use exact Flutter `3.44.8`. Codemagic remains manual-only; the separate visual workflow and the internal workflow's App Store Connect integration, distribution signing, bundle ID, build name/number, export plist, IPA/archive artifacts, upload auth, `submit_to_testflight: false`, absent beta groups and `submit_to_app_store: false` are preserved.
- Exact-head local validation passed npm install with zero audit findings, YAML parsing, diff/shell checks, 19-path scaffold, docs, 70/70 CI policy tests, four focused revision tests and Flutter 3.44.8 / Dart 3.12.2 dependency resolution, clean analyzer, 63 selected normal/mixed files, 31 excluded visual-filename files and **1,051/1,051** non-visual tests. Visual workflow selection was proved statically; no visual run is claimed.
- Fresh Gemini `strong_independent` and local Codex reviews passed the final head with no remaining findings. Earlier review convergence corrected GNU-only `sort -z` and a GitHub Codex finding that initially dropped ordinary cases from mixed files. GitHub validated synthetic PR merge `9b80cfe41260f41f6c680bcdcf77518cc324157d` for Linux mobile, full API/generated/Compose/Docker validation and the stable `Validate scaffold` aggregate, while the hosted-macOS simulator job explicitly checked source head `c38bcbf30980ac76af46e2f0cd5fb50b2ee6104e`; all 15 checks including CodeQL, Semgrep CE/OSS and Trivy passed. One review thread is resolved and branch-scoped code-scanning alerts are zero.
- Pre-hygiene checkpoint: #1192 remains open until this same-task ledger/readiness reconciliation merges from retained branch `docs/1192-mobile-validation-parity-hygiene-20260911-0204`. #380 remains open. No Codemagic cloud build, Apple signing operation, App Store Connect upload/processing, tester distribution, TestFlight install, store submission, deployment or production action was run or inferred; those external/manual gates remain owned by R06/#975.
- No backend/API/OpenAPI/generated-client, auth/session/security runtime, storage/privacy, money/settlement/bill-calculation authority, schema/migration, dependency/lockfile, Docker/deployment/environment, secret, signing identity or store-distribution setting changed. Final report: `/workspace/logs/settleora-codex-report-20260911-0204-mobile-flutter-codemagic-validation-parity.md`.

### Issue #1189 — Stable RabbitMQ persistence identity (2026-09-11 HKT)

- Logical task key `20260910-2353`; starting main `2c8b3c7d3d0dcd6c223484735e55b7f532723782`. Implementation branch `infra/1189-rabbitmq-stable-persistence-20260910-2353` is retained. [PR #1190](https://github.com/tommytang213/Settleora/pull/1190) merged reviewed source `231cc64a3f481be556f09ff213807a48c704f878` (tree `2199949a64b9ca9bd9678f3c8be005f6ab6454db`) normally as `861d5ef1c9462e04eca778a1375abce2aa786fd3`.
- Exact implementation paths: `infra/docker-compose.truenas-lan.yml`, `infra/docker-compose.truenas-lan.image.yml`, `infra/env/.env.truenas-lan.example`, new `infra/rabbitmq/settleora-entrypoint.sh`, new `infra/tests/validate-rabbitmq-persistence.sh`, and `docs/deployment/TRUENAS_LAN_DOCKER_TESTING.md`.
- Both LAN variants now derive explicit container hostname and `RABBITMQ_NODENAME` from the single required `SETTLEORA_RABBITMQ_NODE_HOSTNAME`; the clean-install example is `settleora-rabbitmq`, producing `rabbit@settleora-rabbitmq`. There is no default. Missing configuration fails Compose interpolation; the entrypoint refuses hostname divergence, a wrong persisted identity (exit 66), or multiple/ambiguous database-shaped identities (exit 65) before the official broker entrypoint starts. API messaging remains `rabbitmq:5672` with the existing vhost, credentials, image `rabbitmq:3.13-management-alpine`, healthcheck, mounts, ports and private topology.
- Baseline disposable proof reproduced the audit risk in each exact pre-change variant: container recreation changed the generated hostname/nodename, created a second node directory under the retained Mnesia base, and left the original durable marker invisible while broker health remained green. No live or operator dataset was read.
- Exact-head candidate proof passed both variants: two remove/recreate cycles retained the same node, durable marker queue, one ready persistent message and verified non-sensitive marker payload with healthy broker state. Per-variant pre-change fixtures discovered the old node by direct container execution while the new env variable was absent, refused a wrong identity without creating a database, and non-destructively reopened the marker after explicitly adopting the prior hostname. A database-shaped suffix identity and the ambiguous sibling pair `rabbit@queue`/`rabbit@queue-plugins-expand` respectively proved exits 66/65 with no hidden/new node start.
- Local exact-head validation passed `npm run doctor:docker`, all three Compose validators, the focused continuity validator, `npm run validate:docs`, `npm run validate:scaffold`, 66/66 CI policy tests, OpenAPI/client validation, 1,520/1,520 API tests, and the API Docker build. The unrelated active M15 scope guard correctly rejects this explicitly authorized deployment lane; live `.devcommand/repository-policy.json` authorizes the six paths under `docker-compose-ci-deployment`.
- Fresh Gemini `strong_independent` and local Codex passed final head with zero findings after three earlier review findings were corrected: suffix classification, legacy discovery without new-env interpolation, and ambiguous database-shaped suffix siblings. GitHub Codex reviewed `231cc64a` with no major issues. PR #1190 finished 13 successful checks, 2 intentional mobile/iOS skips, 0 failures, 0 review threads, and 0 branch-scoped code/secret scanning alerts; CodeQL, Semgrep CE/OSS and Trivy all passed.
- Post-merge proof found source head ancestral and all six reviewed blobs byte-identical on fresh main. Same-task hygiene branch `docs/1189-rabbitmq-persistence-hygiene-20260910-2353` reconciles this ledger and the completed #974 audit; its PR/head/normal merge are recorded in the final issue comment and report because a commit cannot truthfully contain its own future identity.
- #1189 closes only after hygiene merges and final state reconciliation. #380 remains open. R11 is complete; R12 is now the first dependency-safe repository recommendation, while R04/R05 still depend on R03/R12 and live/manual acceptance. #974 and #483 remain closed; #959 remains open and untouched.
- No live TrueNAS/production action, destructive migration/reset, credential/password/cookie change or exposure, RabbitMQ upgrade, clustering/federation/HA, API/worker/application messaging, PostgreSQL, schema, OpenAPI/generated-client, R12 network/TLS, deployment, CI, release/signing, mobile/web/admin, secret, or #959 change was made. Any live identity adoption, node rename, reset, cookie operation, or incompatible dataset migration remains a separately approved manual gate.
- Final report: `/workspace/logs/settleora-codex-report-20260910-2353-issue-1189-rabbitmq-stable-persistence-identity.md`.

### Issue #1181 — Mobile contextual-help English-catalog extraction (2026-09-10 HKT)

- Logical task `20260910-1552` completes `L10N-D1-022` from starting main `0a54ae4d1908ce64b7f5e53cabad853039eb34fc` as two lane-correct implementation slices. Catalog [PR #1182](https://github.com/tommytang213/Settleora/pull/1182) merged reviewed head `fa2a72b02ca7f219fd7f91347eb723e61354563b` (tree `d90f8112d6ca111b161b1914660d58aec5f31591`) normally as `2d5b1a81c608fb71f9f5c737a6444f3adfc14109`; application [PR #1183](https://github.com/tommytang213/Settleora/pull/1183) merged reviewed head `0352692ab9f96a6529c8d623f213578adb56f03e` (tree `1c2c34cd037b9a556221fc87b25ff845efb0c6c8`) normally as `7621fff3249b22cf4e5c109b23b89a3000762908`. Retained branches are `build/1181-mobile-contextual-help-catalog-20260910-1552` and `feature/1181-mobile-contextual-help-catalog-consumption-20260910-1552`; each started from fresh main in merge order.
- The `mobile-build-config` slice changed only `apps/mobile/l10n/app_en.arb`, adding exactly 70 systematic English getters across the ten stable topics `first-launch`, `dashboard`, `bills`, `ocr-review`, `groups`, `settlements`, `recurring`, `reports-search`, `backup-restore` and `settings-security`: one sheet title, heading, description, three ordered guidance points and tooltip/accessibility entry label per topic. Generated Dart remained ignored/untracked/uncommitted, no translation was added and generic `Close help` remains outside the catalog.
- The `mobile-application` slice changed exactly `apps/mobile/lib/help/contextual_help.dart`, `apps/mobile/test/contextual_help_candidate_a_test.dart`, new `apps/mobile/test/contextual_help_localization_test.dart`, and Candidate A/B/C visual capture tests. The production registry now owns only stable topic/version metadata; one explicit generated-`AppLocalizations` switch resolves display/semantic content. All ten `keyName` values and content version `2026-09-08.1` remain exact, all 70 fields and 30 topic-local points mechanically match the frozen starting source, every entry label is catalog-backed, and no duplicate production English registry, raw key/getter rendering, localized identity, network/provider, persistence, analytics or repository dependency was introduced.
- #1093 behavior remains complete: the exact launcher/topic mapping, setup/pre-auth availability, shared bottom sheet/guidance/button composition, dismiss/reopen, 48dp/action and heading semantics, focus restoration, non-mutating behavior, no seen-state, and OCR detail help suppression during conflicting deletion remain unchanged. High-consequence bills/OCR/groups/settlements/backup/settings wording is byte-for-byte preserved, including server authority, authorization, financial truth, payment, OCR provisionality, restore-disabled, backup-exclusion and security-policy boundaries. #1171 bootstrap/notification localization and #1176 What's New remain separate and green.
- Flutter 3.44.8 / Dart 3.12.2 doctor, pub get, gen-l10n, generated-getter/count/copy proofs, analyzer, 53 focused contextual/visual/bootstrap/notification/What's New tests, direct full Flutter **1,109/1,109**, `validate:mobile` with **1,109/1,109**, and 19-path scaffold validation passed on exact application head. Catalog head separately passed generation/copy proofs, analyzer and **1,103/1,103** full tests. Exact diff/scope guards passed both slices.
- Thirty-nine canonical production-widget PNGs under `/workspace/logs/issue-1181-contextual-help-localization/visual-evidence/candidate-{a,b,c}/` cover every established Candidate A/B/C surface plus required 390px/1× and 320px/2× Bills/OCR/Groups/Settlements/Settings states, reachable Close and visible focus-after-close. Original-resolution inspection found exact localized English, no raw keys, clipping or overflow. The SHA-256 of the sorted 39-line per-artifact SHA-256 inventory is `8075658728704a7778a7bdaea71a41fc8387788637bcaaaed213df2a9ed6b022`; full paths and hashes are retained in the final report.
- Fresh Gemini `strong_independent`, independent local Codex and GitHub Codex passed both exact source heads with zero actionable findings. PR #1182 had 11 successful checks/scanners, 0 reviews, 0 threads and 0 branch alerts; PR #1183 had 11 successful checks/scanners, 0 reviews, 0 threads and 0 branch alerts. Both PRs were CLEAN/mergeable at the expected-head normal merge gate, and their reviewed blobs/heads are ancestral and byte-identical on main.
- `L10N-D1-022` is ready only for this bounded English resource extraction; overall M61/mobile localization remains partial. Remaining boundaries include `002`/`003`/`004`/`011`/`018`/`023`/`026`, #973/`019`, actual translation/Traditional Chinese and #975 native/device acceptance. #1181 closes only after this retained `docs/1181-mobile-contextual-help-localization-hygiene-20260910-1552` same-task hygiene PR merges and final issue/umbrella reconciliation completes; the exact hygiene head/merge is recorded in the issue completion evidence and final report because a commit cannot self-record its own identity. #372 remains open for fresh dependency/priority admission.
- #1093/#409/#1168/#1171/#1176 remain closed/completed; #301 receives no update because shared component behavior did not change. #959 remains open and untouched at `updatedAt` `2026-07-29T14:39:41Z`. No API/OpenAPI/generated-client, server/provider, auth/session/security behavior, storage/file/privacy, money/settlement/bill calculation, OCR parser/apply, sync/import/restore, schema/migration, native resource, permission, signing/release/deployment/CI/config/secret, language preference, translated content, Figma/reference or #959 change was made.

### Issue #1176 — Mobile What's New English-catalog extraction (2026-09-10 HKT)

- Logical task `20260910-1334` completes `L10N-D1-021` from starting main `7fdeb195ab0f872377783c3241290bd19732beab` as two lane-correct implementation slices. Catalog [PR #1177](https://github.com/tommytang213/Settleora/pull/1177) merged reviewed head `e414682f4cc8d5cd2591cec6fbc36cf5f904121a` (tree `40b3d4c6a217d88326321545d4025e13510d5fd0`) normally as `806b4d679aac320114a14fcf134d270ca6ee0e7c`; application [PR #1178](https://github.com/tommytang213/Settleora/pull/1178) merged corrective reviewed head `3f36bb42d05997aaf27fda8d200cd5047481430a` (tree `5100273d3c615f5e75a002024f008e966c6dbc17`) normally as `334f07263ce3eb48f9f2068755f8a85acdc15b6d`. Retained branches are `build/1176-mobile-whats-new-catalog-20260910-1334` and `feature/1176-mobile-whats-new-catalog-consumption-20260910-1334`; each started from fresh main in merge order.
- The `mobile-build-config` slice changed only `apps/mobile/l10n/app_en.arb`, adding exactly five current-release messages: `What's New in Settleora 1.0`; `A quick look at what you can do in this self-hosted mobile build.`; `Choose device-only Local Mode or connect to your self-hosted Settleora server.`; `Work with bills, groups, settlements, recurring bills, reports, and notifications after signing in.`; and `Review receipt details before applying them to a draft bill.` Getter names are `whatsNewCurrentHeading`, `whatsNewCurrentDescription`, `whatsNewCurrentPointDeviceMode`, `whatsNewCurrentPointFeatures` and `whatsNewCurrentPointReceiptReview`; generated Dart remained ignored/untracked/uncommitted.
- The `mobile-application` slice changed exactly `apps/mobile/lib/app/version_notes.dart`, `apps/mobile/test/version_notes_test.dart` and `apps/mobile/test/ui/version_notes_visual_capture_test.dart`. A private typed content-source marker distinguishes the built-in current catalog payload from caller-owned static bundles; presentation resolves catalog getters through the existing root `BuildContext`, while injected notes—including changed release keys—retain caller-provided heading, description and points. Localization-resolution failure returns `false` before sheet completion, so startup remains usable, no raw getter renders and seen persistence is not written.
- Release identity remains exactly `currentBundledVersionNotesKey = '1.0.0+1'`; validation/max-length semantics and storage key `settleora.presentation.whats_new.seen_release_key.v1` are unchanged, and only the trimmed release key is persisted. Tests retain null/invalid/unusable payload handling, unseen automatic setup, seen suppression, ordinary dismissal/write-once, delayed read, read/write fail-open, process guard, manual Local Mode and Settings reopen, configured-server pre-sign-in, manual/auto race, focus restoration, shared sheet/guidance/button composition and zero repository creation. Wrapper `Version notes` and action `Close What's New` remain outside the catalog.
- Flutter 3.44.8 / Dart 3.12.2 doctor, pub get, gen-l10n and five-getter proof, analyzer, 121 focused What's New/bootstrap/notification/contextual-help tests, and full Flutter **1,103/1,103** passed on the corrective head; the full suite passed again through `validate:mobile`, and the 19-path scaffold validation passed. Catalog validation additionally passed analyzer, **1,100/1,100** full tests, debug APK, Gradle dependency/assembleDebug and web build. Exact diff/scope guards passed for both slices.
- Twelve original-resolution production-widget artifacts under `/workspace/logs/issue-1176-mobile-whats-new/visual-evidence/` cover automatic setup, configured-server sign-in, Local Mode, authenticated Settings/manual reopen, current catalog at 320 logical pixels/DPR2 and retained expanded injected content at 320px/2× text scale. Required visible copy, all three points and Close are readable/reachable with no raw identifier, overflow or clipping. SHA-256 by artifact: settings entry `347d0304adb2c2e7bfd2f83fb3bf9a3115059609f58ae2ebe0e8064c13f80c77`; settings open `0c18d0852e57f6e933d158fb917bc01dc5af28ecca2c6b3e4fc8cbad9cd27b75`; catalog top `ecbd627a982929efcf8a7ee59ce0ed61f889f52c45916a25adadc36c813fa4b3`; catalog close-reachable `ecbd627a982929efcf8a7ee59ce0ed61f889f52c45916a25adadc36c813fa4b3`; catalog manual `ecbd627a982929efcf8a7ee59ce0ed61f889f52c45916a25adadc36c813fa4b3`; Local dismissed `5d3dcc70f6014e32326ce653ba4247a0529cb082b4a6778a60e82d7b8f4a8546`; Local unseen `26e590d1a9366a86e40ebd10505f8e0c435742c194c6b13863a3da4723354ecd`; expanded close `432e4c5ded8fa0ce32e75df05aad9591264715688494d0f634c85a40927bc686`; expanded top `28623ff0bf1eb8329cbcd2a228d6a3da6aed3aee0af592bfc2f0f9654619d9ce`; server sign-in `4e8a920f32e642acfdb08dc930a42e947790516ee8ff675e55ff53bb60db1443`; setup dismissed `0e3f1d534713d960a33bbee8e022826fae963cdb466f551259282d7a909491e3`; setup unseen `f1c51335bc582341b974bdf91e4276adc27e654d70801e033deaf857a5db4879`.
- Fresh exact-head Gemini `strong_independent` and local Codex passed both final implementation slices with high confidence and no findings. All 11 GitHub checks/scanners passed each final head and branch-scoped alerts were zero. PR #1177 had 0 total/resolved/unresolved review threads. PR #1178 had one GitHub Codex P2 thread: its first candidate had replaced #1092's expanded 2× text-scale fixture with DPR-only evidence; the corrective head restored that fixture alongside the catalog captures, repeated all validation/reviews, received a fresh no-major-issues GitHub review, and finished 1 total / 1 resolved / 0 unresolved threads.
- `L10N-D1-021` is now ready only for the bounded current English resource extraction; overall mobile localization remains partial. Remaining separate boundaries include `002`/`003`/`004`/`011`/`018`/`022`/`023`/`026`, #973/`019`, Day 2 translations/Traditional Chinese, future release content and #975 native/device acceptance. #1092 remains closed and behavior-complete; #372 stays open for fresh admission; #409/#1168/#1171 remain completed; #301 receives no update because shared component behavior did not change; #959 remains open and untouched at `updatedAt` `2026-07-29T14:39:41Z`.
- No API/OpenAPI/generated-client, server/provider, auth/session/security, storage/file/privacy authority, analytics, money/bill/settlement/payment calculation, schema/migration, OCR, native resource/permission, dependency/lockfile, signing/release/deployment/CI/config, secret, Figma/reference, language preference or translated-resource change was made. Same-task audit/checklist/ledger hygiene is carried by retained branches `docs/1176-mobile-whats-new-localization-hygiene-20260910-1334` and `docs/1176-mobile-whats-new-localization-hygiene-correction-20260910-1334`; their normal-merge PR identities are recorded in final task/issue evidence.

### Issue #1171 — Mobile localization bootstrap and notification-key retention (2026-09-10 HKT)

- Task `20260910-1110` completes `L10N-D1-001` from starting main `93ca1363452dd423907aed7db2b7d43a3d8ffdf8` as one lane-correct logical task. Catalog [PR #1172](https://github.com/tommytang213/Settleora/pull/1172) merged reviewed head `7635d7fa0949b4d2190e08c370ed33dfd145a9d3` (tree `5ac3c913a995ff3476b83ea91bdec84c643647bd`) normally as `c60457f66ba283f142f0f60cf6c6310ca8d5b2b4`; Flutter 3.44.8 importability correction [PR #1173](https://github.com/tommytang213/Settleora/pull/1173) merged reviewed head `540461600f692683a5ea0ef852ba90d34fa24573` (tree `24aff6888abeb8c8823238f7b5c1932b852d686e`) as `2b00a56d27ff7bad40998c0f8a3a59c87537c9f9`; application [PR #1174](https://github.com/tommytang213/Settleora/pull/1174) merged reviewed head `326157f96b310fcea42a96d1448718b13b7c237f` (tree `7fa4b538ca3dc300314b954175789acb80476eec`) as `9b2c57f0ddb5c53b8c22e376aa0b7b0915616032`. All retained branches were based in merge order from fresh main.
- The `mobile-build-config` catalog PR changed only `apps/mobile/l10n/app_en.arb`, adding exact English title/message entries for all 27 stable event pairs currently emitted by live API/domain source. The separate correction changed only `apps/mobile/l10n.yaml`: Flutter 3.44.8 rejected imports escaping `lib` from the former root `.dart_tool`, so generation now lands under ignored `lib/.dart_tool/flutter_gen/gen_l10n` and is importable as `package:mobile/.dart_tool/flutter_gen/gen_l10n/app_localizations.dart`. Generated Dart remains untracked; pubspec/lock/native resources are unchanged.
- The `mobile-application` PR changed exactly five `apps/mobile/lib/**` and five `apps/mobile/test/**` files. `SettleoraMobileApp` installs generated delegates and exactly `en` supported locale behavior before authentication or server connectivity, and resolves visible title `Settleora` through generated `appTitle`. `SettleoraNotificationRow` retains nullable exact `titleKey`/`messageKey`; the common generated-response mapper covers list, mark-read and archive responses without generated-client edits.
- The explicit 27-pair handwritten adapter maps server machine keys to generated getters. Recognized titles retain existing product meaning; bounded `safeSummary` keeps precedence, safe static message fallback is used only without usable summary, and absent/unknown keys return existing event/subject copy without rendering raw identifiers. No names, dates, counts or amounts are parsed or guessed; #973 / `L10N-D1-019` retains typed dynamic arguments.
- Exact application-head validation passed: Flutter 3.44.8 doctor/pub get/gen-l10n/analyze, 175 focused bootstrap/repository/notification/dashboard/What's New/contextual-help/visual tests, full Flutter **1,100/1,100** twice (direct and `validate:mobile`), and 19-path scaffold. Fresh Gemini `strong_independent`, local Codex and GitHub Codex passed with no material findings; all 11 checks/scanners passed, fully paginated review threads were 0/0 unresolved, and branch alerts were 0. Four inspected production-widget PNGs under `/workspace/logs/issue-1171-mobile-localization/visual-evidence/` cover 390px/1x and 320px/2x known/unknown/detail states with no raw key, clipping or overflow; SHA-256 values are `b8b1c7226e0d99028b6d4551e7c95ff119370670b564aff4bddd000d3842a2e2`, `c513d47c8250b4552f3de1c636c5e9e1da20a957858663a840db788f6334be7b`, `37a2a804621bc92fda5ee7a73256ae45267b88aa39dd2dbb6643c6adf89d3a65`, and `484ec44bb8c117e9b5c1bf85c5071bbf076f48fe36ae12986dceba0662d8001a`.
- `L10N-D1-001` and `014` are ready foundations, not overall mobile localization completion. Remaining `002`/`003`/`004`/`011`/`018`/`021`/`022`/`023`/`026` stay separate, actual translations/Traditional Chinese remain Day 2, and #372 stays open for fresh priority/dependency reconciliation. #409/#1168 remain closed; #301 receives no update because shared-component behavior did not change; #959 remains OPEN and untouched at `updatedAt` `2026-07-29T14:39:41Z`.
- No API/OpenAPI/generated-client, server producer/template, auth/session/security, storage/file/privacy, money/settlement/bill calculation, schema/migration, OCR, deployment/CI/config, secret, native-resource/permission, language selector/preference/negotiation, or translated-content change was made. Same-task audit/checklist/ledger hygiene follows only after all implementation merges.

### Issue #1168 — Day 1 mobile localization build/native foundation (2026-09-10 HKT)

- Task `20260910-0945` completed `L10N-D1-014` from starting main `bb152a1aa7baf3edaf863379ddf90a93c4e9c95b`. Implementation [PR #1169](https://github.com/tommytang213/Settleora/pull/1169) merged normally from reviewed head `15a72e9268f8a15a2665be0be9100318ada5b641` (tree `d31df1c7e1afadea7b73732645a7f2d9402a1d07`) as `d176b6a35445b26d21f54e543c6e5a7e07edccff`; the feature branch is retained.
- Exact implementation paths were `apps/mobile/pubspec.yaml`, deterministic `pubspec.lock`, `l10n.yaml`, `l10n/app_en.arb`, Android manifest/`values/strings.xml`, iOS base plist/English `InfoPlist.strings`, and `Runner.xcodeproj/project.pbxproj`. Flutter 3.44.8 generates the minimal English `appTitle` catalog only under ignored `.dart_tool/flutter_gen/gen_l10n`; no generated Dart, `lib/**`, test or application-copy migration was committed.
- Android now resolves the launcher label through `@string/app_name` = `Settleora`. iOS has one English `InfoPlist.strings` variant/resource entry with `CFBundleDisplayName`/`CFBundleName` = `Settleora` and safe base fallbacks. Camera/photo/location descriptions match starting main exactly; SHA-256 values are `83984c86d6ae450dd7179f400d85ac2948800939d9495f3320ec47b44e96d817`, `409113964e991cfbccc0912241be3822c87d99eb15c9b6032b2eb0999b76d1cf`, and `760388ad53d3cdcb622b44255734714458879e4d082c409b986d623a3d138698`.
- Exact-head doctor, pub get, gen-l10n and analyzer passed; 1,093 full Flutter tests passed twice; `validate:mobile`, debug APK, Gradle dependency resolution/assembleDebug, web build, Linux plist/strings/PBX/protected-setting checks and GitHub iOS-simulator build passed. Fresh Gemini `strong_independent`, local Codex and GitHub Codex passed; all 12 checks/scanners passed, 0 review threads and 0 branch alerts.
- Same-task audit/checklist/ledger hygiene is carried by the retained `docs/1168-mobile-localization-build-native-hygiene-20260910-0945` branch and its normal-merge PR recorded in final task/issue evidence. Close #1168 after that merge. Keep #372 open: `L10N-D1-001` is the next application catalog/bootstrap and notification-key prerequisite; `002`/`003`/`004`/`011`/`018`/`021`/`022`/`023`/`026`, Traditional Chinese runtime and #975 device acceptance remain incomplete.
- No permission wording/behavior, signing, bundle identity, release, backend/API/OpenAPI/generated-client, auth/security runtime, money, storage/privacy runtime, OCR behavior, schema, CI/deployment, secret, or architecture/PRD scope change was made. #409 remains closed audit authority and #959 remains open/untouched (`updatedAt` `2026-07-29T14:39:41Z`).

### Issue #1164 — Day 1 user-web English localization foundation (2026-09-10 HKT)

- Task `20260910-0245` completed `L10N-D1-005` from starting main `a8987e682a899040f3d0876f674c79e77aed5f2b`. Implementation [PR #1165](https://github.com/tommytang213/Settleora/pull/1165) merged normally from reviewed head `284a580ee160d4f39f59bf9099398df57de810b9` (tree `ccb951b495dc67008642d2a5e5ccd3e5d94ba795`) as `a0ae6d58e5207c5137703c3ac54db108a6144585`.
- The user web now has one repository-local typed English catalog, supported/resolved locale `en`, deterministic unsupported injectable-locale fallback to `en`, a `LocaleProvider` around `App` exposing locale and typed lookup, deterministic out-of-provider and missing-key developer failures, retained static `<html lang="en">`, and runtime `document.documentElement.lang` synchronization independent of auth/session success.
- The representative migration is exactly ordinary primary/More navigation labels, descriptions and action labels; compact Home/Bills/Groups/Settle/Import/Alerts labels; navigation aria labels; and the More/All functions heading. Visible English, route IDs, primary/More membership, statuses, ordering, hash normalization, callbacks, keyboard/button semantics and layout are unchanged. Auth/session/password-reset state copy, notification message resolution, formatting, money, broad feature bodies and server-authored strings remain outside the catalog.
- Exact validation passed: `npm --prefix apps/web-user ci`; focused localization/shell tests (14/14); TypeScript lint; all user-web tests (139/139); production build; 19-path scaffold validation; diff/scope/forbidden-path guards. Original-resolution production captures passed at desktop and narrow widths: `desktop-1440x1000.png` SHA-256 `ca6e415823877b1c85b9a87b00fab6802553c9cadc385c3eae9a946808363775` and `narrow-390x844.png` SHA-256 `4dd3e463529936ba45db058395a32ff59d945c6a792209d09fe3f611c9887747` under `/workspace/logs/issue-1164-user-web-localization-foundation/visuals/`; no raw keys, clipping or horizontal overflow appeared.
- Fresh Gemini `strong_independent` and local Codex factual/mechanics/accessibility/localization-boundary reviews passed exact head with zero material findings. All 11 GitHub checks/scanners passed; the fresh final GitHub Codex exact-head review found no major issues. Fully paginated inventory: 4 total threads, all four contract-conflicting scope suggestions adjudicated/replied/resolved, 0 unresolved; branch code-scanning alerts 0.
- P17 and `WEB-D1-034` remain **partial**, not complete: `L10N-D1-006/008/010/012/013/016/017/019/020/024/025/027` and the separate server/auth template owner `009/015` remain incomplete with their dependency/manual gates. #373 remains OPEN; #409/#963 remain completed audit authorities; #959 remains untouched.
- Beyond the bounded user-web presentation runtime recorded above, no package/lockfile, dependency/framework, backend/API/domain runtime, OpenAPI/generated-client, auth/session/security behavior, password reset, notification resolver/provider, money/settlement/payment calculation, date/time/number/plural/collation, server presentation, schema/migration, storage/file/privacy, OCR, sync/import/restore, deployment/CI/config, secret, Figma/reference or translated-resource change was made.

### Issue #963 — Day 1 user-web completeness audit (2026-09-09 HKT)

- Task `20260909-1725` reconciles source, tests, generated transport, merged history and live issue ownership in [the canonical Day 1 user-web completeness audit](../qa/DAY1_USER_WEB_COMPLETENESS_AUDIT.md), from starting `origin/main` `c10285dbb9703923f2a34a58fd4d9e4785a1db80`. Exact candidate head/tree, PR/review/check evidence and normal merge SHA are recorded externally after candidate freeze because a commit cannot embed its own identity truthfully.
- The canonical inventory has **44 rows**: `implemented` 2, `partial` 19, `unavailable` 14, `blocked` 9, `superseded` 0. Current runtime credit includes shell/reset completion; bill/group/settlement/profile/payment/notification/report readouts; export; staged import confirmation; backup package/download; restore preview/confirmation metadata; and sync/local-status readout. Protected source paths remain unreachable because the shell supplies no credential; generated methods are not UI or authority.
- A systematic requirements-completeness matrix binds the 25 packets to applicable product fields, read/write and lifecycle states, audit/redaction, authorization, money/file/local-security boundaries, responsive/accessibility/reference evidence and localization readiness. Later children cannot close from generated transport presence or a happy path, and product-safe identifiers remain a reusable invariant through the final all-surface sweep.
- Forty-two gaps are deduplicated into 25 execution packets. Existing-owner reuse additionally routes cross-record lifecycle through #716, quantity/self-claim and revision depth through #350/#967, duplicate receipt policy through #401, privacy/files through #966/#421, and OCR review through #970/#967 while leaving #959 untouched. Other named owners remain authoritative. New-child recommendations for bundled What’s New, contextual help and product-safe identifier/copy cleanup passed live duplicate search; none is runnable merely by appearing in the audit.
- First safe wave is #409 `L10N-D1-005`: a separate `web-user-ui` English catalog/locale-bootstrap/document-language child after #963, excluding auth/session copy and all product runtime. #965/#966/#969/#971/#973/#977 audits may reconcile in parallel. Later auth, API/OpenAPI/domain, privacy/file, money/settlement and sync/restore children retain their gates; composed responsive/accessibility/visual proof is acceptance-only.
- Close recommendation: close #963 only after exact two-file validation/review/check convergence, normal expected-head merge, reviewed-blob/ancestry proof and #373 completed/remaining graph update. Keep #373 open. No runtime, test, API, OpenAPI/generated-client, auth/security, money, schema, storage/privacy, sync/restore, provider, deployment/configuration, secret or Figma change is authorized.

#### Post-merge review reconciliation — same logical task `20260909-1725`

- The original #963 terminal claim after PR #1161 was premature. A fully paginated live inventory found **109** total threads: **100 resolved and 9 current, non-outdated, actionable unresolved**, contradicting the reported 100/100 resolution. The unresolved IDs were `PRRT_kwDOSNHStc6gtKir`, `PRRT_kwDOSNHStc6gtKiw`, `PRRT_kwDOSNHStc6gtKi2`, `PRRT_kwDOSNHStc6guEUP`, `PRRT_kwDOSNHStc6guEUZ`, `PRRT_kwDOSNHStc6guEUg`, `PRRT_kwDOSNHStc6guEUj`, `PRRT_kwDOSNHStc6guEUr`, and `PRRT_kwDOSNHStc6guEUx`.
- Direct final-main adjudication classified all nine as `DOCS_CORRECTION_REQUIRED`. The corrective branch is `docs/963-post-merge-review-reconciliation-20260909-1725`, based on original merge/main `3ee018ff63d2d21342f50d4833995b4c45a563fc`. The corrective PR, reviewed source head/tree and normal merge identity are immutable external evidence to be recorded on #963 and in the same-task report after candidate freeze/merge; a commit cannot embed its own final SHA or merge SHA.
- The correction strengthens P03 quantity-claim lifecycle, P04 viewer-filtered revision snapshots, P09 optional/clearable payment details, P10 complete notification taxonomy disposition, P13 complete export filters/provenance, P14 backup credential/key denylist, P20 required server-announcement categories, P22 explicit not-strict-zero-knowledge consequence copy, and P25 false OCR line removal. Capability/status/gap/packet counts remain **44 / 2 implemented / 19 partial / 14 unavailable / 9 blocked / 42 gaps / 25 packets**.
- No runtime scope is admitted. #963 closes again only after corrective validation and exact-head local/GitHub review convergence, normal corrective merge, zero unresolved actionable threads on both the corrective PR and original PR #1161, corrected #963/#373 evidence, and report reconciliation. #373 stays open and #959 remains untouched.

### Issue #409 — Day 1 localization-readiness audit (2026-09-09 HKT)

- Task `20260909-1312` audits current `origin/main` base `3fa603828c141fcbd9f91fb700c573d0448df3e1` in [the Day 1 localization-readiness control](../qa/DAY1_LOCALIZATION_READINESS_AUDIT.md). The exact candidate head, PR, review/check result and normal merge SHA are immutable external evidence recorded on #409 and in the task report after candidate freeze/merge; a Git commit cannot truthfully embed its own SHA.
- Overall readiness is **partial**. Twenty-seven Day 1 structural findings are recorded: mobile 11, user web 8, admin web 1, server/cross-boundary 7; severity high 21 / medium 6; readiness `missing` 3 / `partial` 22 / `blocked/reference` 2. English-only Day 1 remains allowed. Actual Traditional Chinese resources, language preference and translated notifications/errors remain Day 2.
- High-priority Day 1 owner recommendations are a focused `mobile-build-config` English catalog/native-label/permission-resource prerequisite followed by focused mobile catalog/bootstrap/key retention, What's New and contextual-help slices; non-runnable `docs-planning` parents keep all remaining mobile/web ordinary/shared-control copy, all shell/domain-adapter auth-session copy, formatting, plural/collation, expansion, visible-enum/percentage and manually gated money call sites owned through bounded runtime children. Separate `web-user-ui` catalog/bootstrap/document-language, notification-key and password-reset slices follow #963/#973/#965 reconciliation; every auth-session child remains manual-gated and #964 stays open through admin catalog, formatting and expansion ownership. Focused server push, generic SMTP notification-email and auth-email locale seams remain separate; #973 owns dynamic in-app-summary reconciliation. Non-runnable parents also own top-level/per-field validation-problem compatibility/adoption and non-problem server display fields. #409 remains the durable audit/registry reference after closure but owns no runtime mutation; none of its unnumbered recommendations is runnable until separately reconciled into a numbered focused issue.
- Existing-owner reuse and boundaries: #964 owns admin runtime reconciliation; #963 and #973 must reconcile their respective child creation; #1094 owns the separately blocked server-announcement authority/contract/localization seam; #406 remains the portability implementation-split owner with #971/#966 as required sync/storage inputs; #975 retains final native/platform acceptance. #1092/#1093/#1096/#299 and #295's Day 1 slice are credited complete, #301 retains component adoption, #295 remains open for Day 2 custom layout, #372 remains open, and the preserved #959 OCR parser chain is untouched.
- Close recommendation: close #409 only after this exact two-file candidate passes docs/scaffold/scope/link/path/owner/identifier validation, fresh Gemini and independent local Codex exact-head review, GitHub Codex/check/scanner convergence, normal expected-head merge, reviewed-blob/ancestry proof, zero unresolved threads, #409 completion comment/Project reconciliation and #372 checkpoint. No runtime, API, OpenAPI/generated-client, auth/security, money, schema, storage/privacy, provider, deployment/config, secret, Figma or translation change is authorized by this checkpoint.

### Issue #295 — Day 1 lightweight Home shortcuts (2026-09-09 HKT)

- Day 1 checkpoint: **complete** after this same-task ledger/checklist/evidence hygiene, final reviewed-blob proof and #372 reconciliation. Issue #295 remains **OPEN** because it is still the explicit owner of Day 2 drag/drop/custom-layout work; this completion does not claim or remove that future scope. Its linked Settleora Day 1 Execution Board item was `Needs Figma / Reference` before hygiene; terminal reconciliation moves that Day 1 item to the board's `Merged` option because the approved derivative layout required no new reference and its Day 1 implementation merged, while the open issue retains future Day 2 work.
- Implementation [PR #1158](https://github.com/tommytang213/Settleora/pull/1158), reviewed source `735d7f65e0e42727e62ca6bb7672c155e49c75d7`, tree `1c7098b91eb2a0391c9b41c3fd66cb6eb8feb282`, normal merge `35f33dc0486a79bad94801e181e87265d660c128`, starting main `2b9264cd78d7880201863946b4b9014ae18816f4`. Branch `feature/295-day1-lightweight-home-shortcuts-20260909-0934` is retained.
- Home now presents compact `Quick access` actions in fixed Notifications, Recurring bills, Receipt reviews, Reports order. Notifications plus Recurring bills are the safe default; users may show zero through all four. Zero omits the section cleanly. Each action reuses its existing canonical opener without mutation or authority change.
- `More -> App settings -> Home shortcuts` uses existing SettingsRow/shared-sheet patterns. The isolated device-local presentation key `settleora.presentation.home_shortcuts.v1` stores only version `1` plus stable machine keys: no profile/account/domain identifier, token/session/authz value, analytics or server sync. Unknown/duplicate/malformed/read-failure inputs are safe, stored order cannot override product order, and empty selection is valid. Successful writes update Home; failures retain the last persisted effective selection, show redacted retryable copy and never claim success. Reads/writes/settings opens are coalesced or serialized so initial/default flashes, duplicate routes, stale sheets and false write success cannot occur.
- More retains canonical Notifications, Recurring bills, Receipt reviews and Reports routes regardless of Home selection; hiding Notifications changes no unread/delivery/filter/deep-link/preference state. The locked bottom navigation remains Home / Bills / Groups / Settle / More. Accounts & income and its unavailable fallback remain unchanged under More.
- Exact-head doctor, pub get and analyzer passed; **145 focused tests**, **1,093 full mobile tests**, 19-path scaffold and diff checks passed. Fresh Gemini `strong_independent` and independent local Codex approved exact source and 15 images with high confidence and zero findings. Six earlier GitHub Codex lifecycle findings were corrected with deterministic regressions; its final exact-head review found no major issues. All 11 CI/security checks passed, all seven threads were resolved and open branch-scoped scanning alerts were zero.
- Fifteen production-widget captures cover the required 390px/1x and 320px/2x matrix, fixed/focused order, zero through all four, canonical More, settings/customization, and reachable failure/retry. Paths and hashes are recorded in [Issue #295 evidence](../qa/evidence/issue-295/README.md). Widget tests prove 48dp+ actions, one semantic action, selected state and deterministic keyboard behavior; #975 retains native device/screen-reader acceptance.
- Day 2 drag/drop, arbitrary ordering/actions, custom bottom navigation, layout builders, server-synced preferences and experience modes were not implemented. #412 retains experience modes; #301 receives no update because no reusable shared component changed; #372 remains open. #409/#975/#1122/#959/#970 retain their independent scopes.
- No backend/domain/API/OpenAPI/generated-client, auth/session/authz, storage/file privacy, schema/migration, notification delivery, recurring/report/OCR behavior, money/settlement/payment calculation, analytics, dependency/plugin, deployment/config/secret or shared design-system change. Same logical task `20260909-0934` includes this hygiene.
- Final report path (written only after terminal completion): `/workspace/logs/settleora-codex-report-20260909-0934-issue-295-day1-lightweight-home-shortcuts.md`.

### Issue #299 — Dashboard metric actionability (2026-09-09 HKT)

- Pre-hygiene checkpoint: **OPEN**; close after this same-task ledger/checklist/evidence merge, final reviewed-blob proof, #372 reconciliation and Project-item reconciliation. Manual product authority is Option 3 in issue comment `5588938396`; the earlier blocker ended without source mutation and is superseded only for this exact bounded disposition.
- Implementation [PR #1156](https://github.com/tommytang213/Settleora/pull/1156), reviewed source `32f3f504e965858273208963d75d013c94ad692a`, tree `8e54f36e2227ffb49a226dcbef80b35e288c2e28`, normal merge `72ee4224b1edcdf6d08a2723003b2a770470271b`, starting main `881f0d47a6ef82b2a0ad050c872812958f32dcab`. Branch `feature/299-dashboard-metric-actionability-20260909-0056` is retained.
- The hero review total remains exactly `settlementActionCount + upcomingForecastCount + notificationSummary.attentionCount + notificationSummary.urgentCount`. It is rendered as a static informational summary with no callback, button semantics, keyboard activation, tooltip or destination; no aggregate calculation or data source changed and no cross-domain chooser or partial notification mapping was added.
- Four exact convenience handoffs are actionable: Active bills opens the existing personal Bills view with Active selected; Unread opens Notification Center with Unread selected; You owe opens Settlements with Outgoing selected; You're owed opens Settlements with Incoming selected. Zero counts use the same routes and existing truthful filtered-empty language. Each action has Material ink/focus feedback, a 48dp+ target, one semantic button/tap node, one deterministic activation and a label that includes navigation purpose plus the visible count or unchanged amount/currency/caption.
- Bills, Notifications and Settlements expose small typed presentation-only initial-view enums that default to All and apply only during initial state construction; existing controls can change or clear them. Settlement direction constrains both balance rows and requests. Notification discovery deliberately remains the existing loaded-row client filter over the ordinary 50-row request under the explicit no-backend-query contract. The dashboard Bills handoff sets the backward-compatible `autoFlushPendingOnLoad: false`, preventing queued bill submission merely from navigation; all other callers retain the prior default.
- Exact-head doctor, pub get and analyzer passed; **358 focused tests** and **1,066 full mobile tests** passed; 19-path scaffold and diff checks passed. The active M15 docs/control-only allowlist correctly surfaced all nine explicitly authorized mobile implementation/test paths for human scope review. Fresh Gemini `strong_independent` and independent local Codex approved source plus all 10 original-resolution captures with zero findings. GitHub Codex's accessibility and balance-direction findings were corrected; its server unread-query suggestion contradicted the explicit accepted client-filter/no-backend-query authority and was recorded as such. A fresh exact-head GitHub review found no major issues; all 11 CI/CodeQL/Semgrep/Trivy/scaffold checks passed, all three threads were resolved, and relevant open code-scanning alerts were zero.
- Ten production-widget captures cover 390px/1x Home nonzero/zero, focused active-bills and money actions, Bills/Active, Notifications/Unread, Settlements/Outgoing and Settlements/Incoming, plus 320px/2x Home and Bills/Active. Paths and SHA-256 values are recorded in [Issue #299 evidence](../qa/evidence/issue-299/README.md); no critical clipping/overflow was observed. #975 retains native device/screen-reader acceptance.
- Accounts & income, its More entry and unavailable fallback are unchanged; #295 remains open and exclusively owns broader placement/navigation. No shared component changed, so #301 receives no progress update. #372 stays open for parent reconciliation; #409/#975/#1122/#959/#970 retain their independent scopes.
- No backend/domain/API/OpenAPI/generated-client, auth/session/authz, storage/privacy, schema/migration, dependency/plugin, deployment/config/secret, money/settlement/payment calculation, bill/notification/settlement/payment mutation, OCR, localization runtime or shared design-system change. Same logical task `20260909-0056` includes this ledger/checklist/evidence hygiene.
- Final report path (written only after terminal completion): `/workspace/logs/settleora-codex-report-20260909-0056-issue-299-dashboard-metric-actionability.md`.

### Issue #1096 — Day 1 mobile setup persistence and truthful server feedback (2026-09-08 HKT)

- Pre-hygiene checkpoint: **OPEN**; close after this same-task ledger/checklist/evidence merge, final reviewed-blob proof and #372 reconciliation. No linked Project item is present. Native device, IME and screen-reader acceptance is not claimed; #975 retains that gate.
- Implementation [PR #1154](https://github.com/tommytang213/Settleora/pull/1154), reviewed source `448972a27bd3156843a98f582edfa4c2bd93a9b7`, tree `9a7bc5d0b996414a8fa9b45d49858abab45d31aa`, normal merge `ee26fdfa1d8443c536d4638ec053ff2fc35019c6`, starting main `0e35ff4760d10223894d3f3bdde38aa9c862219d`. Branch `feature/1096-setup-persistence-server-feedback-20260908-2243` is retained.
- Server verification uses only the existing generated `getAuthBootstrapStatus` call for anonymous, read-only `GET /api/v1/auth/bootstrap/status`. A successful typed `BootstrapStatusResponse` proves the candidate base URL exposes the expected Settleora public API; the `bootstrapRequired` value is deliberately ignored and never reaches setup product state. No access token, account inference, mutation, `/health/ready`, certificate bypass or global transport change is involved. Each probe-owned HTTP client is force-closed in `finally`, including timeout paths.
- Server setup now binds `Server not checked`, `Checking server`, `Server verified` and `Server unavailable` to the exact current normalized URL. Syntax alone, saved configuration, cached session and rendering sign-in never verify a URL. An effective URL change invalidates prior verification and stale failures; normalization-equivalent text retains the same identity. Local Mode performs zero probes and retains `Local stays local` behavior.
- Probe failures are safely redacted, preserve the URL/mode, call the save callback zero times and leave Save available for a fresh probe. Persistence callback failures render `Setup not saved` with bounded device-save copy, retain entries and—only for the same exact successfully checked URL—retain verification so retry calls the callback once without another probe. Local persistence failures use the same retryable treatment. The existing application order remains configuration write, server-session clear, edit exit and bootstrap reload; no auth/session semantics or rollback were changed.
- Exact-head doctor, pub get, analyzer (no issues), **153 focused tests**, **1,050 full mobile tests**, 19-path scaffold validation and diff/scope checks passed. The active M15 controller allowlist is docs/control-only, so the issue-authorized mobile paths were correctly surfaced for human review rather than silently admitted. Fresh Gemini `strong_independent` and independent local Codex approved exact head and all 12 original-resolution production-widget captures. GitHub Codex found two bounded P2 issues on the first candidate; exact corrective head `448972a` reset invalid edited URLs and closed per-attempt clients, passed a fresh no-major-issues review, all 11 checks/scanners and both resolved threads.
- Twelve branch-rendered production/shared-widget capture files (11 distinct bitmaps; the unchanged persistence-failure/retry-ready state is recorded under both acceptance names) cover server unverified/checking/verified/unavailable, verified persistence failure, retry ready/focused, Local Mode, and 320px/2× reachability. Paths and SHA-256 values are recorded in [Issue #1096 evidence](../qa/evidence/issue-1096/README.md). Widget coverage proves deterministic keyboard submit, useful non-duplicative status/error semantics, duplicate-save suppression and 48dp+ Save behavior; #975 retains physical-device/native assistive-technology acceptance.
- Completed #1092 What's New and #1093 contextual setup help remain unchanged. #337/#965 retain auth/account onboarding, #971 local workspace/sync/portability, #412 experience modes, #1122 privacy-security, #959 parsing and #970 OCR acceptance. No reusable shared-component behavior changed, so no #301 progress claim is made; #301 and #372 remain open.
- No API runtime, OpenAPI, generated-client, auth/session ordering or policy, storage/file privacy authority, schema/migration, dependency/plugin, deployment/config, money/bill/settlement/payment, OCR or secret changes. Same logical task `20260908-2243` includes this ledger/checklist/evidence hygiene.
- Final report path (written only after terminal completion): `/workspace/logs/settleora-codex-report-20260908-2243-issue-1096-setup-persistence-server-feedback.md`.

### Issue #1093 — Day 1 mobile contextual screen help (2026-09-08 HKT)

- Pre-hygiene checkpoint: **OPEN**; close after this same-task ledger/evidence merge, final reviewed-blob proof and #372 reconciliation. GitHub prematurely closed #1093 at `2026-09-08T12:49:14Z` after intermediate PR #1150 despite its explicit non-closing intent; the issue was reopened during final live-state reconciliation and remains open for this gate. No linked Project item was present. Native device/screen-reader acceptance is not claimed.
- Three bounded implementation candidates merged normally to `main`: [PR #1150](https://github.com/tommytang213/Settleora/pull/1150), reviewed source `c26d5b97f726fb4b2eb52c838f5b25931c4bdd49`, tree `a341febb13bf588d15d1b66cb355597527ce1b23`, merge `ff72022d67928358452115cdcbf44fd8ceedaf3b`; [PR #1151](https://github.com/tommytang213/Settleora/pull/1151), reviewed source `13082ab06e74eb5e77f88eea0946fb20ba4b5cab`, tree `204d1a6f693af5f3b26b07309ab9821050e68a8c`, merge `02cd17283c8bf4bfe595dc6d910acbac041e1fbf`; and [PR #1152](https://github.com/tommytang213/Settleora/pull/1152), reviewed source `99ed53effbcd46670b28c57426e66bfaa485c93f`, tree `46eb5608ea950c785658a19f0f3ab215ee98bcf0`, merge `5322ba97b8fe76fe3ea536993f53a247570bc796`. Starting main was `21863aa4f4878d956b3f570e9f0db4494060c74a`; all source branches are retained.
- [Settleora contextual help](../../apps/mobile/lib/help/contextual_help.dart) owns one deterministic registry and reusable launcher over the existing `showSettleoraBottomSheet`, `SettleoraBottomSheetFrame`, `SettleoraGuidanceContent` and `AppButton` path. Every topic uses copy revision `2026-09-08.1`, launcher key `contextual-help-<topic-key>`, explicit `Close help`, barrier/drag skip, immediate reopen and focus return. Help is user-invoked and has no preference, seen-state, release-key, analytics, repository or domain side effect.

| Stable topic key | Actual mobile entry | Implemented / unavailable-state meaning |
| --- | --- | --- |
| `first-launch` | Setup header before sign-in | Describes Local/Server selection and connection dependency; help does not select or save a mode. |
| `dashboard` | Authenticated Home app bar | Describes loaded overview cards and existing shortcuts; help performs no reload. |
| `bills` | Personal and group Bills app bars | Describes bill records, provisional receipt warnings and currency attachment; no live provider FX/conversion estimate is claimed and unavailable actions stay unavailable. |
| `ocr-review` | Receipt Reviews queue and Receipt Review detail app bars | Describes provisional OCR, non-final preview and guarded draft apply; unsupported actions remain disabled. Detail help is omitted while a mutation is active so an asynchronous route completion cannot target its sheet. |
| `groups` | Groups app bar | Describes server-provided membership/roles and protected changes; temporary participants are explicitly unavailable and client visibility does not grant authorization. |
| `settlements` | Settlements app bar | Describes balances, requests, payment records and statuses without payment advice; permission, confirmation and server validation remain authoritative. |
| `recurring` | Recurring bills app bar | Describes templates, derived forecasts and explicit draft generation; forecasts are not confirmed truth and confirmation remains separate. |
| `reports-search` | Monthly Report app bar | Describes read-only loaded-row search/filtering, currency grouping and refresh; no report or related record mutation. |
| `backup-restore` | App settings → Data safety section trailing action | Describes the actual non-mutating pasted-JSON import preview, excluded sensitive/server/file content and disabled restore apply; a valid preview promises no future acceptance. |
| `settings-security` | App settings app bar | Describes local notification/readout and protected account boundaries; no credentials are exposed, no security choice is prescribed and protected changes stay server-authoritative. |

- Candidate A passed analyzer, 149 focused tests, **1,010 full mobile tests** and 19-path scaffold validation; Candidate B passed analyzer, 10 candidate tests, 255 domain tests, 50 shared-component guardrails, **1,020 full mobile tests** and 19-path scaffold validation; final Candidate C passed analyzer, 8 candidate tests, 159 combined focused tests, **1,028 full mobile tests** and 19-path scaffold validation. Fresh exact-head Gemini `strong_independent` and independent local Codex reviews approved every final source head and inspected copy authority, accessibility and original-resolution production-widget captures. GitHub CodeQL/Semgrep/Trivy/scaffold checks passed on each exact PR source head; the final GitHub Codex correction removed help during an in-flight OCR deletion, its regression passed, the fresh review found no major issues and the sole thread was resolved.
- Twenty-seven branch-rendered production-widget captures cover every entry and open sheet, setup reopened after barrier dismissal, visible keyboard focus returned after close, and 320px/2× long-copy scrolling. Paths and SHA-256 values are recorded in [Issue #1093 evidence](../qa/evidence/issue-1093/README.md); widget tests independently assert the dismissal/reopen/focus mechanics. Shared heading/button semantics, non-duplicative launcher labels and 48dp targets are covered; #975 retains native screen-reader/device acceptance.
- High-consequence copy is descriptive and cautionary: no financial/payment/authorization/security/restore recommendation is made, and OCR/server/domain/storage/restore authority is not redefined. Centralized caller-owned strings are ready for later extraction; #409 retains localization runtime. Mobile has no admin-maintenance destination, so no admin key exists and #964 retains independently approved admin guidance.
- #1092 stays closed and its `1.0.0+1` version-seen behavior is untouched. #1122 remains OPEN/manual privacy-security gated; #959 remains parser-owned; #970 remains OCR completeness-audit-owned; #975 retains native/platform acceptance; #301 and #372 remain open. No reusable shared-component behavior was added, so no #301 progress claim is made.
- No backend/domain/API/OpenAPI/generated-client, auth/session/authz policy, storage/file privacy authority, restore/apply, analytics, money/bill/settlement/payment calculation, OCR parser/provider/apply authority, recurring/report domain behavior, schema/migration, dependency/plugin, CI/deployment/config or secret changes. Same logical task `20260908-1952` includes this ledger/checklist/evidence hygiene.
- Report: `/workspace/logs/settleora-codex-report-20260908-1952-issue-1093-contextual-screen-help.md`.

### Issue #1092 — Mobile bundled What's New and version-seen state (2026-09-08 HKT)

- Pre-hygiene checkpoint: **OPEN**; close after this same-task ledger merge, final reviewed-blob proof and #301/#372/#1093 reconciliation. No linked Project item was present before implementation merge; no native-platform acceptance is claimed.
- Implementation [PR #1148](https://github.com/tommytang213/Settleora/pull/1148), reviewed source `b10c8368f38dbe646c0e9660896fcb4659400aef`, tree `f49cefe46a73fc8c4b94dff0c49d59ffa7aff869`, normal merge `2cc9b138ee67c738de6f549a3703b61bedf38b1c`, starting main `e1976fa413aaad73f517088b6cf035f07c4a93ef`. Branch `feature/1092-mobile-whats-new-version-seen-20260908-1556` retained.
- Mobile `1.0.0+1` now bundles conservative product-facing notes behind the explicit release key `1.0.0+1`. Valid unseen notes present only after the normal setup, signed-out server, local-only or authenticated bootstrap surface renders. Barrier/explicit dismissal and manual-first dismissal record only the trimmed local release key; read/write/presentation failures and missing/invalid/overlong payloads fail open, while a process-lifetime per-release guard prevents immediate repeats and delayed-read races.
- Local Mode retains Connect to Server and adds a small manual reopen action. Authenticated More → App settings contains stable `settings-whats-new`; both launchers reuse the exact shared bottom sheet, guidance content and button path. Settings close returns focus to the actionable row, and manual reopen never unsees the current release. No release-note repository or network call exists.
- Exact-head doctor, pub get, analyzer (no issues), **151 focused tests**, **3 visual tests**, **1,001 full mobile tests** and **19-path scaffold validation** passed. Nine production/shared-widget captures cover setup, signed-out server, local-only, authenticated settings and 320px/2× long-content scroll reachability under `/workspace/logs/settleora-visual-qa/20260908-1556-issue-1092-whats-new/`. Fresh exact-head Gemini `strong_independent` and independent local Codex reviews passed with zero findings. Earlier GitHub Codex rounds produced the bounded fixes; after those corrections, its final no-major-issues comment explicitly recorded reviewed commit `b10c8368f38dbe646c0e9660896fcb4659400aef`. All 11 GitHub checks/scanners passed and all seven review threads were resolved, including the preserved #409 localization boundary.
- #1093 remains separate and OPEN: this task does not implement contextual screen help. #1122 remains OPEN/manual privacy-security gated; #959 remains parser-owned; #970 remains OCR completeness-audit-owned; #975 retains native device/screen-reader/platform acceptance. #301/#372 remain open; the small reusable settings-focus and flexible-sheet seams are credited without claiming either parent complete.
- No backend/domain/API/OpenAPI/generated-client, auth/session/authz, account preference, storage/file privacy authority, analytics, money/bill/settlement/payment calculation, OCR behavior, schema/migration, dependency/plugin, CI/deployment/config or secret changes. Same logical task `20260908-1556` includes this ledger/checklist hygiene.
- Report: `/workspace/logs/settleora-codex-report-20260908-1556-issue-1092-mobile-whats-new-version-seen.md`.
...
- Cancel and Save were equivalent except for a minimal generic semantic-label override and now adopt secondary/primary `AppButton`; no raw action was retained. The initial migration added optional `semanticLabel`; corrective focus review added optional `focusNode` forwarding, both with unchanged defaults. Host Tooltip wrappers replace the migrated controls' helper-owned Tooltips while reusing the exact idle/saving/other-busy messages; the shared control owns one semantic node. Save loading follows only saving; other busy work disables it without false progress.
...
- Implementation [PR #1141](https://github.com/tommytang213/Settleora/pull/1141), reviewed source `c64ecdeddf7ed387e18140b6c684f854380ab75e`, tree `0424c526c768249d419def5e9fbc1d9f1f576123`, normal merge `a5d1a0c9a60d149661796f8fe311e7ba92580b86`, starting main `55315fba5c6f29d21cc3db999d68a1ff134a4616`. Branch `feature/1140-receipt-ocr-review-shared-state-panels-20260908-1103` retained.
- All nine local `_StatePanel` usages and both local `_LoadingPanel` usages were directly equivalent and adopt `SettleoraStatePanel` / `SettleoraLoadingPanel`; both private definitions were removed, no local shell was retained, and no shared API extension was needed. Five compact call sites explicitly preserve `Alignment.centerLeft`. Exact icon/title/message/action contracts are recorded in the [durable equivalence, validation and SHA256 image inventory](../qa/evidence/issue-1140/README.md).
- Queue disconnected/loading/true-empty/no-match/failure branches, loaded-row refresh failure behavior, search/filter/count/no-match logic, deleted-route suppression, follow-up refresh, typed detail open/return state and repository order/counts remain unchanged. Detail loading/failure/no-result/no-lines branches and load/edit/save/delete/preview/apply generations, route/result handling and duplicate-action guards remain unchanged.
- `_FailurePanel` changed only its shell. Failure classification/icon/title/safe display mapping and existing Retry widget/callback remain exact; one semantic Retry activation produces exactly one additional repository list call. Shared loading adds one useful, non-duplicative live-region label.
- Exact-head doctor, pub get, analyzer (no issues), **99 focused tests**, **974 full mobile tests**, scaffold and branch capture validation passed. Fresh Gemini `strong_independent` and independent local Codex reviewed exact source plus all **20** production images; GitHub Codex and all 11 checks/scanners passed exact head with zero review threads and zero open PR-scoped scanning alerts.
- Actual production/shared panels were captured at 390px/1× and 320px/2× across queue disconnected/loading/empty/no-match/failure and detail loading/failure/no-OCR/no-lines/editor-no-lines. Migrated panels have no critical clipping/overflow and Retry remains visible. Unchanged surrounding narrow detail chrome remains separately scoped. This is scoped Flutter panel evidence, not native IME/device/screen-reader/whole-screen acceptance; #975 retains that gate.
- #1122 remains OPEN/manual privacy-security gated and untouched. #959 remains independently owned parser work and untouched. #970 remains independently owned OCR completeness-audit work and was not executed. Historical visual-parity PR #675 remains credited. #301/#372 remain open; remaining #301 candidates include other raw forms/buttons/private-dialog framing, token/summary styling, participant/assignment sharing and other independently inventoried components.
- No backend/domain/API/OpenAPI/generated-client, auth/session/authz, storage/file privacy, OCR parser/provider/extraction/apply authority, money/bill/settlement/payment calculation, schema/migration, CI/deployment/config or secret changes. Same logical task `20260908-1103` includes this ledger/static-evidence hygiene.
- Report: `/workspace/logs/settleora-codex-report-20260908-1103-issue-1140-receipt-ocr-review-shared-state-panels.md`.
...
## Issue #1185 — GitHub automatic PR CI and Codemagic manual release separation

- Logical task key: `20260910-1758`; starting main: `8d0ca548231ebf4e891d8cb33bfc421175bdd6c3`.
- Implementation branch: `ci/1185-github-pr-validation-codemagic-manual-20260910-1758`; source head/tree: `8a12f344aeb700a6fdaf7cb4934a3c28d8a1be15` / `9c02dfb19f192b3e31a4f5f1b0c624a3cfc95cea`; PR #1186 merged normally as `81d5b0deb2cc9f62ef8d7d682d4e99572776610b`.
- Implementation files: `.github/workflows/scaffold-validation.yml`, `.github/workflows/mobile-ios-validation.yml`, `tools/ci/scaffold-validation-changes.mjs`, `tools/ci/test/scaffold-validation-changes.test.mjs`, and `tools/ci/test/ci-workflow-policy.test.mjs`.
- The single fail-closed classifier now emits `docs_only`, `run_full_validation`, `run_mobile_validation`, and `run_ios_validation`. Mobile/iOS routing covers `apps/mobile/**`, consumed `packages/client-dart/**`, gate self-changes, and `codemagic.yaml`; `package.json` and `tools/doctor-validation.mjs` force Linux mobile at minimum. Untrusted proof forces full, mobile, and iOS validation.
- The final job remains exactly `Validate scaffold` and explicitly requires success from every classifier-required lane. Live ruleset 17875790 still requires that GitHub Actions context; no ruleset change was needed or performed.
- Exact implementation PR run 34475116831 passed classifier/scaffold policy, full OpenAPI/client/API/Compose/API-Docker validation, Linux `npm run validate:mobile` with Flutter 3.44.8, the GitHub-hosted macOS iOS simulator build, and the stable aggregate. CodeQL, Semgrep CE/OSS, and Trivy checks passed; GitHub Codex reviewed the exact final head with no issues; all threads resolved; branch-scoped scan alerts were zero.
- `codemagic.yaml` remained byte-identical. It defines no `triggering.events`; GitHub workflows invoke no Codemagic API/webhook. The manual `mobile-ios-testflight-internal` integration, App Store signing, internal-only export option, IPA/archive artifacts, integration-auth upload, `submit_to_testflight: false`, and `submit_to_app_store: false` remain intact under current official Codemagic semantics. This task did not invoke Codemagic, signing, upload, TestFlight/App Store action, or production deployment.
- External limitation: repository webhook metadata is readable and shows an active Codemagic event webhook, but Codemagic dashboard/account and build history were not independently observable. The repository YAML and GitHub workflow evidence prove there is no repository-configured PR/push/merge build route under the current YAML; whether any unrelated external/manual run occurred remains unverified and requires manual dashboard inspection.
- Documentation proof branch: `docs/1185-github-pr-ci-hygiene-20260910-1758`. This docs-only PR must prove classifier/lightweight success, intentional full/mobile/iOS skips, successful final `Validate scaffold`, security checks, and no Codemagic run before #1185 closes. Its final PR/head/merge evidence is recorded in the task report and issue completion comment because a commit cannot truthfully contain its own future SHA or merge commit.
- Close recommendation: close #1185 only after that real docs-only PR is merged. Keep #380 and #974 open for their wider infrastructure/release-readiness scope; keep #382, #383, and #1181 closed; do not mutate #959.
