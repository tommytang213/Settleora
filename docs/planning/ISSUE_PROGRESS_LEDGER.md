...
- Overall readiness is **partial**. Twenty-four Day 1 structural findings are recorded: mobile 10, user web 7, admin web 1, server/cross-boundary 6; severity high 18 / medium 6; readiness `missing` 3 / `partial` 20 / `blocked/reference` 1. English-only Day 1 remains allowed. Actual Traditional Chinese resources, language preference and translated notifications/errors remain Day 2.
- High-priority Day 1 owner recommendations are a focused `mobile-build-config` English catalog/native-label/permission-resource prerequisite followed by focused mobile catalog/bootstrap/key retention, auth-reviewed copy, What's New and contextual-help slices; non-runnable `docs-planning` parents keep all remaining mobile/web ordinary copy, formatting, plural/collation, expansion, visible-enum/percentage and manually gated money call sites owned through bounded runtime children. Separate `web-user-ui` catalog/bootstrap, notification-key and auth-reviewed password-reset slices follow #963/#973/#965 reconciliation; admin catalog bootstrap remains through #964. Focused server push, generic SMTP notification-email and auth-email locale seams remain separate; #973 owns dynamic in-app-summary reconciliation. Non-runnable parents also own top-level/per-field validation-problem compatibility/adoption and non-problem server display fields. #409 remains the durable audit/registry reference after closure but owns no runtime mutation; none of its unnumbered recommendations is runnable until separately reconciled into a numbered focused issue.
