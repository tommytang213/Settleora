...
- Overall readiness is **partial**. Fourteen Day 1 structural findings are recorded: mobile 6, user web 5, admin web 1, server/cross-boundary 2; severity high 11 / medium 3; readiness `missing` 3 / `partial` 10 / `blocked/reference` 1. English-only Day 1 remains allowed. Actual Traditional Chinese resources, language preference and translated notifications/errors remain Day 2.
- High-priority Day 1 owner recommendations are a focused `mobile-build-config` English resource/config prerequisite followed by separate `mobile-application` catalog adoption, ordinary-formatting and manually gated money-display tasks; separate `web-user-ui` catalog/bootstrap, ordinary-formatting, money-display, notification-key and expansion tasks after #963/#973 reconciliation; admin catalog bootstrap through the child split by #964; a manual-gated `auth-session-security` email locale seam; and bounded `api-domain-runtime` adoption of the existing `ProblemDetails.code` field in one low-risk domain. Mobile plural/collation and expansion/direction remain separate focused tasks. #409 owns none of that runtime after this audit.
