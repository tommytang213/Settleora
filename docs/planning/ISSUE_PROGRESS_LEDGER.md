...
- Fresh Gemini `strong_independent` and local Codex reviews passed the final head with no remaining findings. Earlier review convergence corrected GNU-only `sort -z` and a GitHub Codex finding that initially dropped ordinary cases from mixed files. The corrected PR head passed all 15 GitHub checks: full API/generated/Compose/Docker validation, Linux mobile, hosted-macOS simulator, stable `Validate scaffold`, CodeQL, Semgrep CE/OSS and Trivy; one review thread is resolved and branch-scoped code-scanning alerts are zero.
