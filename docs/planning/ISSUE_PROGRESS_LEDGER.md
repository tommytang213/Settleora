...
- Overall readiness is **partial**. Thirteen Day 1 structural findings are recorded: mobile 5, user web 5, admin web 1, server/cross-boundary 2; severity high 10 / medium 3; readiness `missing` 2 / `partial` 10 / `blocked/reference` 1. English-only Day 1 remains allowed. Actual Traditional Chinese resources, language preference and translated notifications/errors remain Day 2.
- High-priority Day 1 owner recommendations are separate mobile English catalog/bootstrap, ordinary-formatting and manually gated money-display tasks; separate user-web catalog/bootstrap, ordinary-formatting, money-display, notification-key and expansion tasks after #963/#973 reconciliation; admin catalog bootstrap inside #964's first admitted shell slice; a manual-gated auth-email locale seam; and bounded adoption of the existing `ProblemDetails.code` field in one low-risk API domain. Mobile plural/collation and expansion/direction remain separate focused tasks. #409 owns none of that runtime after this audit.
