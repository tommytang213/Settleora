...
- The audit inventories 25 distinct OCR capabilities: 6 `implemented`, 11 `partial`, 4 `blocked`, 2 `externally-gated`, 1 `documentation-only`, and 1 `unavailable`. It credits current camera/gallery/file intake, in-memory JPEG/thumbnail artifact processing, ML Kit provider bootstrap, parser/preview, personal/group editable review, saved-review API handoff, queue/discovery, duplicate guidance, explicit draft apply, and non-draft server revision routing without claiming the complete workflow or final acceptance. It records two normalization bypasses: ML Kit reads the inherited original path, and a selection whose normalization fails can remain in the draft for later original-byte upload. It also classifies real OCR as unreachable from Local Mode and an offline/server-unavailable cold bootstrap, even though the provider itself is on-device.
...
- Dependency chain: merge/close #970 -> R01/#1209 -> R03 may consume completed R02 web evidence plus a valid Android release entry -> R07 separately handles application identity/signing/store readiness. Docs-only audit #975 may reconcile final evidence only after #358/#401/#438/#740/#959/#442/#966/#967, #971 and every focused runtime child it creates/adopts, and #357's server-complement child/disposition. #975 then creates or adopts focused acceptance owners; those owners bind exact final artifacts to physical-device/platform/UI/privacy evidence, while explicitly authorized humans retain sign-off. #975 itself neither performs runtime acceptance nor grants sign-off. #959 is independent. #357 owns the required server complement until it approves a focused child; #967 owns reconciliation/splitting of broader Day 1 OCR correction semantics. Docs-only audit #971 owns reconciliation and creation/adoption of focused Local Mode/offline OCR reachability and sync implementation children; those approved children, not #971 itself, own runtime closure. #357/#359/#380 remain open.
