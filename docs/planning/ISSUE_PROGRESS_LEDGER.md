...
- `codemagic.yaml` remained byte-identical. It defines no `triggering.events`; GitHub workflows invoke no Codemagic API/webhook. The manual `mobile-ios-testflight-internal` integration, App Store signing, internal-only export option, IPA/archive artifacts, integration-auth upload, `submit_to_testflight: false`, and `submit_to_app_store: false` remain intact under current official Codemagic semantics. No Codemagic cloud build, signing, upload, TestFlight/App Store action, or production deployment ran.
- External limitation: repository webhook metadata is readable and shows an active Codemagic event webhook, but Codemagic dashboard/account state was not independently observable. The repository YAML and GitHub workflow evidence prove no automatic build route; external-account confirmation remains manual.
