...
- Overall readiness is **partial**. Seventeen Day 1 structural findings are recorded: mobile 6, user web 6, admin web 1, server/cross-boundary 4; severity high 13 / medium 4; readiness `missing` 3 / `partial` 13 / `blocked/reference` 1. English-only Day 1 remains allowed. Actual Traditional Chinese resources, language preference and translated notifications/errors remain Day 2.
- High-priority Day 1 owner recommendations are a focused `mobile-build-config` English resource/config prerequisite followed by separate `mobile-application` catalog/key-retention adoption, ordinary-formatting and manually gated money-display tasks; separate `web-user-ui` catalog/bootstrap, ordinary-formatting, money-display, notification-key, expansion and manual-auth-reviewed password-reset-copy tasks after #963/#973/#965 reconciliation; admin catalog bootstrap through the child split by #964; focused `api-domain-runtime` push and generic SMTP notification-email template locale tasks; a non-runnable `docs-planning` `ProblemDetails.code` adoption parent that stays open through bounded API-domain children; and a manual-gated `auth-session-security` invitation/reset email locale seam. Mobile plural/collation and expansion/direction remain separate focused tasks. #409 remains the durable audit/registry reference after closure but owns no runtime mutation; none of its unnumbered recommendations is runnable until separately reconciled into a numbered focused issue.
