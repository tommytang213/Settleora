...
| **logical removal** | A state or relationship change that stops defined ordinary use without claiming physical erasure. It may be reversible, conditionally reversible, or irreversible under its source policy: revoked session authority is not restored, and a later sign-in creates new authority. Archive, revoke, disable, unlink, and link removal remain distinct. |
...
| --- | --- | --- | --- |
| `SYN-FIN-01` bill roots; `FIN-CHOICE-001..010`, `025..026`, `029`, `062..072`, `076..080` | Financial/API domain; `RC-A`, possible source-approved `RC-B`, `RC-E`, or `RC-X`. A saved financial draft is not `RC-D` merely because dependency checks find no references. | Bill/revision/settlement/participant/file/sync/audit references; stable identity and accepted financial basis; import/local/export copies; bounded audit without payment details or file content. | Accepted/shared history is not ordinarily disposable. Draft eligibility needs `FIN-CHOICE-002`, positive dependency/copy proof, and the financial retention/destructive gates. `FIN:G-RET/G-DEST/G-PRIV/G-MONEY`; PKT-01/17 owners. |
...
| `SYN-AUTH-04` reset/invitation/abuse/policy/audit; `AUTH-LC-CHOICE-007..009`, `011..012` | Auth/security/event owner; `RC-A/B/C/D/E/X`; process-memory abuse buckets may be `RC-D`, while durable security/audit evidence remains source-classified. | One-time result truth, delivery/save ordering, policy version, actor/subject/correlation, abuse and audit evidence; exclude raw links/tokens. | Current implementation clocks are not universal retention approval. Same auth gates; PKT-16/19. |
...
| PKT-02 group/membership lifecycle authorization | Reuse #784/#785 where invitation/owner-role work is exact; #722 must search current group, membership, invitation, temporary-participant, and historical-access owners and create one focused group/authz child only for a materially unowned transition family. | After applicable membership choices and this policy; authz/product/identity/privacy/retention/destructive gates remain pending, and Day 2 default-exclusion/leave/rejoin work stays later-day. |
