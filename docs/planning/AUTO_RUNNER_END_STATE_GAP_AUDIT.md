...
#800 remains open pending #894 merge and post-merge proof. The foundational
...
Manual gates remain real gates. Sensitive work is not categorically prohibited
from autonomous implementation when a reviewed lane policy permits it, but
manual-gated work and manual actions still block. Production deploy, mobile
store release, destructive data/migration execution, secret/auth config,
public/admin exposure, architecture replacement, force-like history, branch
deletion/cleanup, Day 1 cuts, unresolved product/policy/financial authority,
auth/session/security-critical runtime, storage/file privacy/authz authority,
money/settlement/payment authority, schema migrations, OpenAPI/generated
clients, sync/import/export/restore authority, and Docker/CI/deployment
infrastructure remain manual or split-required unless a later reviewed policy
...
| F. Exact-head approved-domain merge | #889/PR #898 added approved-lane auto-merge policy with exact issue contract, path, validation, review, CI/security, scanner, thread, issue-state, base/head, and `--match-head-commit` protection. Tests in `auto-runner.test.mjs` and `completion-hygiene.test.mjs`. | Pass for foundation behavior. | #894 remains manual-merge-required and must not auto-merge. |
...
authority changes, money/settlement calculation authority changes, schema
migrations, OpenAPI/generated-client changes, sync/restore authority changes,
CI/deployment infrastructure changes, and tasks explicitly marked PR-only or
human-merge-only.
