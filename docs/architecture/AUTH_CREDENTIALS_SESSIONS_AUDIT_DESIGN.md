...
- Internal refresh credential generation, rotation, replay classification, and linked family revocation behavior exists behind the refresh runtime service boundary. `POST /api/v1/auth/sign-in` creates refresh-capable session families and initial refresh-like credentials through that boundary, and `POST /api/v1/auth/refresh` rotates submitted refresh-like credentials without adding raw refresh-token storage or new schema. `GET /api/v1/auth/current-user`, `POST /api/v1/auth/sign-out`, `POST /api/v1/auth/sign-out-all`, `GET /api/v1/auth/sessions`, and `DELETE /api/v1/auth/sessions/{sessionId}` exist behind the `SettleoraSession` bearer scheme where applicable. Generated web and Dart client foundations exist from the OpenAPI contract. General registration, arbitrary/admin session management beyond the current implementation, and UI behavior do not exist yet.
...
- Public registration or invite onboarding only after credential issuance, abuse policy, and audit behavior are explicit.
- Password reset/change flows, OIDC runtime, passkeys, and MFA only as separate reviewed slices.
- UI integration over the generated client foundations only in separate reviewed slices.
- Admin session management beyond the current account-owned session list/revocation, retention cleanup, and distributed hardening in separate reviewed slices.
