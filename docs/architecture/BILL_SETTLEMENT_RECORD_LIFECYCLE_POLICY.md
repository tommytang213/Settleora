...
| `FIN-REC-011` | Group forecast occurrence — **Generate draft bill** | Any current active group member admitted by the group visibility/authorization path may generate the occurrence; the generated bill records the template owner as bill owner and the actor as creator. | **Implemented current fact with database uniqueness and broad group-member authority.** `VisibleTemplates`; `AuthorizeTemplateAccessAsync`; `GenerateDraftAsync`; `RecurringBillDraftBuilder`; tests. Intended actor policy remains `FIN-CHOICE-011`. |
