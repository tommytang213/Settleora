...
- `ExpenseBillWorkflowEndpoints` implements draft submit, pending participant
  ...
  creator or bill owner cancel a draft future bill: it sets status `cancelled`,
  sets `ArchivedAtUtc`, writes the purpose-specific audit event, and removes the
  bill from ordinary future-bill lists. No current general bill-workflow
...
- **User-visible action surface:** `FIN-BILL-*`, `FIN-COMP-*`, and `FIN-PART-*` use the bill
...
| `FIN-COMP-008` | Pending active bill payer — proposed **Reject payer contribution** | No active-payer rejection transition is accepted today. Availability, actor, bill-root/selection effect, and correction remain blocked. | **Persistence/status only; runtime unimplemented.** `ExpenseBillPayerConfirmationStatuses.Rejected` and timestamp; `FIN-CHOICE-022/023`; `G-PRODUCT/G-MONEY`. |
...
| `FIN-BILL-003..009` | Submit/accept/reject changes future selection and settlement eligibility; future-draft cancellation removes that retained cancelled bill from ordinary future lists. Finalized and status-archived entry remain unavailable. Retained acknowledgements/status/archive/audit evidence explains current bill state. | Submit, participant decisions, and future-draft cancel are not “restore.” No cancel re-entry exists. Finalized/status-archived are not reachable. Correction/re-entry uses an explicit revision/new root chosen by policy, never status overwrite. | Revisions, payer confirmation, settlement candidates, membership/history, sync, audit, notifications, policy/hash. Missing general cancel/finalize/status-archive/reopen and rejection/cancel-recovery policy. |
| `FIN-COMP-001..008` | Draft removal may exclude an input from future calculation. Accepted source meaning must remain readable through authorized revision snapshots/component evidence; current active participant/payer rows are not immutable history. Active payer creation may start pending, but no later acceptance transition exists. | Draft item restore is unresolved. Confirmed component change uses a new revision. Active payer confirm/reject is unavailable. Pre-apply meaning relies on retained snapshot/revision evidence because active participants are overwritten and active payers may be deleted/replaced. | Root state; active payer creator/status/actor and bill effect; complete baseline/proposed snapshot and revision component/approval coverage; OCR/file #719; participant #720; calculation hash/version; settlement lines; reports/exports/import; audit/copies. Missing active-payer transitions, item tombstone actor/reason/version, snapshot coverage, active-row disposition audit, and full graph. |
...
| `FIN-BILL-003..009`, `FIN-COMP-*`, `FIN-PART-*` | Status/ack/payer timestamps exist; no entity concurrency token/idempotency key. Future-bill cancellation sets status and archive timestamp in one save. Revision apply mutates active participant rows and can physically remove/replace active payer rows. Future accepted mutation must bind calculation hash/policy and expected version. | Accepted bill meaning is retained through the root plus complete revision snapshots/revision component evidence; retention of every former active component row is not implemented. Draft, active-payer, participant-settlement, and active-row disposition classifications remain unresolved. | Current workflow/future-bill-cancel/revision successes are audited, but payer-row removal has no separate lifecycle outcome. Future edit/remove/apply/cancel/finalize/status-archive/payer/participant attempts require inventory, history proof, denial/conflict outcomes, and redacted audit. |
...
| `FIN-BILL-003..009` | `FIN-CHOICE-003/004/025/026/010` | Implemented/partial/unimplemented by row | Existing bill/revision owners, then #722/#724 split | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA`; transition/history, finalized/status-archive distinction, retention, copy-disposition, and dependency tests. |
| `FIN-COMP-001..008` | `FIN-CHOICE-002/005/020..023/010` | Partial/unimplemented by row | #722 money runtime plus schema if tombstone/version fields; #724 retention | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; active-payer authority/effect, positive dependency, component inventory, removed-payer and snapshot coverage, redacted audit, deterministic rebuild tests. |
...
| `FIN-REV-001..010` | `FIN-CHOICE-004/006/015..019/010` | Implemented/partial/unimplemented by row | Existing revision lane; #722/#724 follow-ups | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA`; any new cancellation, payer-rejection, or settled-impact behavior remains pending; concurrency/snapshot/hash/payer/history tests. |
...
| `FIN-REC-003/004/007/008/011` | `FIN-CHOICE-011/012/014/010` | Implemented/unimplemented by row | Recurring owner through #722 split | `G-MONEY/G-PRODUCT/G-RET/G-CLIENT/G-PRIV/G-DEST`; intended group-generation actor and generated-bill owner plus skip/cancel/re-entry remain pending; uniqueness/replay/generated-bill independence tests. |
...
| `FutureBillEndpoints`, `FutureBillEndpointTests` | `CancelFutureBillAsync`; `CancelFutureBillArchivesDraftAndHidesItFromDefaultList` | Implemented creator/owner future-draft cancellation, archive timestamp, ordinary-list exclusion, and audit |
...
| revision domain/endpoints/policies | revision statuses, approvals, payer confirmation, proposal/apply services | Implemented revision and dependent payer-confirmation lifecycle plus settlement apply block |
