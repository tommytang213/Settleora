...
- Revision rejection has a current server mismatch: the capability policy
  advertises `canReject` only to bill participants, but the mutation endpoint
  loads any creator, owner, participant, or payer who passes current visibility
  and `RejectProposal` does not require an approval row. The broader mutation
  acceptance is current runtime fact, not accepted authority policy. No source
  implements the separate `cancelled_by_authorized_editor` transition.
...
- **User-visible action surface:** `FIN-BILL-*` and `FIN-COMP-*` use the bill
  ...
  `FIN-SET-*`, `FIN-LINE-*`, `FIN-PAY-*`, `FIN-ALLOC-*`, and `FIN-RES-*` use
  settlement request/payment detail and history; `FIN-REC-*` uses recurring
  template/occurrence management; `FIN-DER-*` uses balance, report, or forecast
...
| `FIN-COMP-002` | Item splits, participants, payers, adjustments — not independently user-removable after acceptance | Changes are draft edits or revision snapshot/application inputs; no independent archive/restore. On apply, current participant rows are mutated and active payer rows may be overwritten, inserted, or physically removed. The retained revision snapshots plus revision participant/payer/approval rows, rather than every prior active row, must explain pre-apply money meaning. | **Implemented persistence and apply mutation; lifecycle/history safety partial.** `ExpenseBillRevisionProposalService.ApplyParticipantState`; `ExpenseBillRevisionEndpoints.SynchronizeAppliedPayers`; `BillRevisionSnapshotFoundation`; revision apply tests. Exact historical coverage, active-row disposition, denial/audit, and dependency proof remain `FIN-CHOICE-005`. |
...
| `FIN-REV-005` | Pending revision — **Reject proposal** / proposed **Cancel proposal** | Current rejection accepts any authenticated actor who passes profile access and the broad `LoadVisibleBillAsync` predicate (creator, owner, participant, or payer visibility), then moves `submitted_for_review → rejected`; it does not require an approval row. The capability policy advertises rejection only to bill participants, so presentation and mutation authorization disagree. `cancelled_by_authorized_editor` is a schema/status possibility only: no current endpoint or domain transition accepts it. Either terminal outcome must remain retained history with no restore; a later correction is a new revision. | **Reject implemented with an authorization/capability mismatch; cancellation unimplemented.** `ExpenseBillRevisionEndpoints.RejectBillRevisionAsync` and `LoadVisibleBillAsync`; `ExpenseBillRevisionProposalService.RejectProposal`; `ExpenseBillRevisionActionCapabilityPolicy.CanReject`; revision tests; `ExpenseBillRevisionStatuses`. The intended rejection actor and exact cancellation actor/preconditions remain unresolved under `FIN-CHOICE-004` and `G-PRODUCT/G-MONEY`; future acceptance must align the server mutation guard, capability readout, tests, and audit without relying on hidden controls. |
...
| `FIN-COMP-001/002` | Draft removal may exclude an input from future calculation. Accepted source meaning must remain readable through authorized revision snapshots/component evidence; current active participant/payer rows are not immutable history. | Draft item restore is unresolved. Confirmed component change uses a new revision. Pre-apply meaning relies on retained snapshot/revision evidence because active participants are overwritten and active payers may be deleted/replaced. | Root state; complete baseline/proposed snapshot and revision participant/payer/approval coverage; OCR/file #719; participant #720; calculation hash/version; settlement lines; reports/exports/import; audit/copies. Missing item tombstone actor/reason/version, positive proof that snapshots cover every deleted/mutated active fact, active-row disposition audit, and full dependency inventory. |
...
| `FIN-BILL-003/004/005` | `FIN-CHOICE-003/004/010` | Partial | Existing bill/revision owners, then #722/#724 split | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA`; transition/history, retention, copy-disposition, and dependency tests. |
| `FIN-COMP-001/002` | `FIN-CHOICE-002/005/010` | Partial | #722 money runtime plus schema if tombstone/version fields; #724 retention | `G-MONEY/G-RET/G-PRIV/G-DEST`, conditional `G-SCHEMA`; positive dependency, pre/post-apply component inventory, removed-payer snapshot coverage, redacted audit, and deterministic history rebuild tests. |
| `FIN-REV-001..005` | `FIN-CHOICE-004/006/010` | Implemented/partial/unimplemented by row | Existing revision lane; #722/#724 follow-ups | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`; any new cancellation or settled-impact behavior remains pending; concurrency/snapshot/hash/history tests. |
| `FIN-SET-001..004`, `FIN-LINE-001` | `FIN-CHOICE-007/008/009/010` | Implemented/partial/unimplemented by row | #722 money runtime; #724 retention; #723 UI | `G-MONEY/G-PRODUCT/G-RET/G-CLIENT/G-PRIV/G-DEST`, conditional `G-SCHEMA/G-CONTRACT`; exact line/source, cancellation/dispute/reopen/archive, concurrency, rebuild tests. |
| `FIN-PAY-001..004`, `FIN-ALLOC-001`, `FIN-RES-001/002` | `FIN-CHOICE-007/010` | Implemented/partial | Existing settlement owners; #722/#724 | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`; rejection/reopen/reversal/refund choices remain pending and outside this task; hostile replay/atomicity/projection tests. |
...
| `FIN-CHOICE-004` | Server bill participant acknowledgements, rejected bill roots, revision rejection authority, and revision cancellation | Is bill/participant rejection reversible by resubmitting the same root, which acknowledgements reset, who is intended to reject a revision, and who may accept revision cancellation? Submit source can reset certain rejected participant rows only from a draft root; the revision capability says participant while the mutation accepts any actor with broad bill visibility; and no rejected-root or revision-cancel transition exists. | Preserve current runtime as current fact, but do not present broad visibility as intended rejection authority. Rejected roots remain rejected, revision cancellation remains unavailable, and future mutation expansion is blocked until server guard/capability/audit semantics agree. | Bill product/money owner via #722; `G-PRODUCT/G-MONEY` pending. |
