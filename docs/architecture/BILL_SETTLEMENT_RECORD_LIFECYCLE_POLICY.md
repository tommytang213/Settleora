...
| `LOCAL-BILL-001` | Local-only bill root — **Archive** only after a local domain exists | A proven local store would own a local archive transition; later server acceptance is a separate sync/import request with full revalidation and conflict handling. | **Unimplemented.** Current mobile queue models a server archive request, not a locally authoritative bill transition. `FIN-CHOICE-013/054..056`. |
| `LOCAL-BILL-002` | Local-only archived bill root — **Restore** only after a local domain exists | A proven local store would own local restore to a named local state; it cannot restore or overwrite the server record by implication. | **Unimplemented.** Current mobile queue models a server restore request, not locally authoritative re-entry. `FIN-CHOICE-013/054..056`. |
| `LOCAL-RECON-001` | Local-only bill reconciliation state | No local mutation may establish or overwrite server `unreconciled|reconciled|ignored` truth. A future local store would require its own state/version, audit, conflict, and later server-acceptance contract rather than inheriting `FIN-RECON-001/002` authority. | **Unimplemented.** Current source proves server reconciliation authority only; no locally authoritative reconciliation store or sync acceptance exists. `FIN-CHOICE-048/054..056`. |
| `LOCAL-REV-001` | Local-only bill revision root | Local proposal/review/apply authority and later server conflict handling are blocked; no local acceptance may rewrite a server bill. | **Unimplemented.** No source proves a locally authoritative revision workflow. `FIN-CHOICE-049/054..056`. |
| `LOCAL-ITEM-001` | Local-only bill item | Local item lifecycle, calculation-version identity, disposition, and server acceptance are blocked; no item is disposable by assumption. | **Unimplemented.** No source proves this local financial family. `FIN-CHOICE-049/054..056`. |
| `LOCAL-SPLIT-001` | Local-only bill item split / assignment | A local split cannot become accepted server calculation input without conflict-safe server revalidation. | **Unimplemented.** No local split authority is proven. `FIN-CHOICE-049/054..056`. |
| `LOCAL-PART-001` | Local-only bill participant/share relationship | Local participant/share state cannot establish server participation, acknowledgement, or historical identity by implication. | **Unimplemented.** No local participant authority is proven; #720 remains the cross-domain dependency. `FIN-CHOICE-049/054..056`. |
| `LOCAL-PAYER-001` | Local-only bill payer/contribution relationship | Local payer state cannot establish or rewrite accepted server contribution history. | **Unimplemented.** No local payer authority is proven. `FIN-CHOICE-049/054..056`. |
| `LOCAL-ADJ-001` | Local-only bill adjustment/allocation | Local adjustment state cannot change an accepted total, split, payer contribution, or server calculation result. | **Unimplemented.** No local adjustment authority is proven. `FIN-CHOICE-049/054..056`. |
| `LOCAL-FX-001` | Local-only bill FX snapshot | A local rate/display conversion cannot become the accepted immutable bill-level FX snapshot without new conflict-safe server validation, explicit calculation/policy identity, and server acceptance. | **Unimplemented.** No local FX-snapshot authority or sync contract is proven. `FIN-CHOICE-049/054..056`; the server accepted requirement is `FIN-FX-001`. |
| `LOCAL-SET-001` | Local-only settlement request root | Money-moving local acceptance and later server merge are blocked; no client may claim server settlement finality. | **Unimplemented.** No safe local request authority/idempotency/conflict contract is proven. `FIN-CHOICE-050/054..056`. |
| `LOCAL-LINE-001` | Local-only settlement request line | A local line may not become accepted source selection or clearing evidence without explicit server acceptance. | **Unimplemented.** No local authoritative line lifecycle is proven. `FIN-CHOICE-050/054..056`. |
| `LOCAL-PAY-001` | Local-only payment claim | A local claim is pending presentation only until new server acceptance; it cannot establish paid or settled state. | **Unimplemented.** No local payment acceptance/replay contract is proven. `FIN-CHOICE-051/054..056`. |
| `LOCAL-PROOF-001` | Local-only payment-proof relationship | A local proof link is not an accepted payment claim or server file relationship; bytes remain #719 and later upload/linking requires separate server acceptance. | **Unimplemented.** No local proof-link authority is proven. `FIN-CHOICE-051/054..056`. |
| `LOCAL-ALLOC-001` | Local-only payment allocation | A client allocation cannot clear a server balance or become immutable accepted evidence by sync implication. | **Unimplemented.** No local allocation authority is proven. `FIN-CHOICE-051/054..056`. |
| `LOCAL-RES-001` | Local-only settlement residual | A client residual choice cannot waive, credit, or carry forward server debt without new server acceptance. | **Unimplemented.** No local residual authority is proven. `FIN-CHOICE-051/054..056`. |
| `LOCAL-REC-001` | Local-only recurring template | Local schedule state and later server acceptance are blocked pending an explicit local authority/sync contract. | **Unimplemented.** Mobile current state is server-mode reads. `FIN-CHOICE-052/054..056`. |
| `LOCAL-OCC-001` | Local-only recurring occurrence | Local generation identity cannot authorize or duplicate a server bill occurrence without conflict-safe server acceptance. | **Unimplemented.** No local occurrence authority is proven. `FIN-CHOICE-052/054..056`. |
...
| `FIN-BILL-003..019` | Creation rows establish a new draft plus calculated dependents under their exact personal, group, future, import, or recurring authority; they do not restore an old root. General submit and creator-only future-bill post independently change future selection and settlement eligibility; participant decisions do likewise. Future-draft update changes only mutable merchant/due date, while cancellation removes the retained cancelled bill from ordinary future lists. Finalized and status-archived entry remain unavailable. | Creation and import/generation are new accepted roots, not restore or replay. Draft future-bill update is a new audited same-state write. General submit, creator-only future post, participant decisions, and cancel are not restore; no cancel re-entry exists. Finalized/status-archived are unreachable. Correction/re-entry uses an explicit revision/new root chosen by policy, never status overwrite. | Creation input/payload visibility; future-post creator and group reauthorization; calculation and payer-confirmation basis; import session/replay; occurrence/template identity; revisions; settlement candidates; membership/history; sync; audit/notifications; policy/hash. Missing creation/post idempotency/version, import/occurrence concurrent retry proof, general cancel/finalize/status-archive/reopen, and rejected/cancelled recovery policy. |
...
| `FIN-BILL-003..019` | `FIN-CHOICE-003/004/011/013/014/025/026/029/010` as applicable | Implemented/partial/unimplemented by row | Existing bill, import, recurring, and revision owners, then #722/#724 split | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA/G-SYNC`; per-path creation/post authority, payload visibility, calculation/payer basis, retry/duplicate behavior, import/occurrence identity, transition/history, cancelled re-entry, finalized/status-archive distinction, retention, copy-disposition, and dependency tests. |
| `FIN-COMP-001..009` | `FIN-CHOICE-002/005/020..023/030..032/042..044/057/010` | Partial/unimplemented by row | #722 money runtime plus OCR/file owner #719 and schema if tombstone/version fields; #724 retention | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; OCR reviewed-source/version, no-settlement, transaction/retry, and calculation-history tests; active-payer availability/actor/basis/effect/correction; positive dependency; component inventory; independently decided tombstone restore, revision threshold, participant history, removed-payer history, and revision component application/history; redacted audit; deterministic rebuild. |
...
| `LOCAL-*` | `FIN-CHOICE-013/048..056` as applicable | Unimplemented | sync-import-export-restore plus local/mobile and money lanes, after #961/#722 split | `G-MONEY/G-SYNC/G-PRIV/G-PRODUCT/G-RET/G-DEST`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; per-family local authority, operation identity, offline conflict, replay, retention/disposition, encryption/backup, and server acceptance tests. |
...
| `FIN-CHOICE-013` | Local-only bill-root lifecycle authority | May a local-only bill root become authoritative for local archive/restore and financial history, and which exact local states/transitions apply? Current mobile source proves only queued server requests, not a local bill domain. | Treat local bill UI/queue state as pending presentation; no locally authoritative archive/restore. | Local/mobile bill and money owners after #961/#722; blocks local bill lifecycle; `G-MONEY/G-PRIV/G-PRODUCT` and conditional `G-SCHEMA/G-CLIENT` pending. |
...
| `FIN-CHOICE-048` | Local-only reconciliation authority | May local-only reconciliation status be authoritative before server acceptance, and which local states/actions exist? Source proves only server reconciliation writes. | No locally authoritative reconciliation state; local presentation cannot overwrite server truth. | Local/mobile reconciliation owner after #961/#722; blocks local reconciliation lifecycle; `G-MONEY/G-PRIV/G-PRODUCT` pending. |
| `FIN-CHOICE-049` | Local-only bill revision/component authority | May local revisions, items, splits, participants, payers, adjustments, or FX snapshots become authoritative local financial inputs before server acceptance? No current local domain/calculation store proves that authority. | Treat them as pending presentation only; they cannot rewrite accepted server bill history or calculation truth. | Local/mobile bill-money owner after #961/#722; blocks local revision/component lifecycle; `G-MONEY/G-PRIV/G-PRODUCT` and conditional `G-SCHEMA/G-CLIENT` pending. |
| `FIN-CHOICE-050` | Local-only settlement request/line authority | May a local settlement request and its selected lines become authoritative before server acceptance? No current local money-moving request domain exists. | No local settlement finality, source selection, or clearing evidence. | Local/mobile settlement owner after #961/#722; blocks local request/line lifecycle; `G-MONEY/G-PRIV/G-PRODUCT` pending. |
| `FIN-CHOICE-051` | Local-only payment/allocation/residual/proof authority | May a local payment claim, allocation, residual choice, or proof relationship become authoritative before server acceptance? Current clients prove no such local financial authority. | Keep all such state pending presentation; it cannot mark paid, clear, waive, credit, or establish a server proof link. | Local/mobile payment-settlement owner plus #719 for file/link boundaries after #961/#722; `G-MONEY/G-PRIV/G-PRODUCT` pending. |
| `FIN-CHOICE-052` | Local-only recurring template/occurrence authority | May a local recurring template or generated occurrence become authoritative and reserve generation identity before server acceptance? Current source proves server-mode reads/commands only. | No local schedule or generation authority; never infer a server occurrence/bill identity. | Local/mobile recurring-money owner after #961/#722; `G-MONEY/G-PRIV/G-PRODUCT` pending. |
| `FIN-CHOICE-053` | Local derived-output persistence authority | Is any local balance/report/forecast output an authoritative retained snapshot rather than rebuildable cache? Current source proves no accepted local snapshot family. | Treat local derived outputs as staleable/rebuildable presentation only. | Report/mobile/privacy owner after #961/#722/#724; blocks snapshot retention/disposition claims; `G-MONEY/G-PRIV/G-RET` pending. |
...
| `FIN-CHOICE-056` | Server acceptance of local financial state | Which validation, authorization, dependency, policy/calculation-version, and audit contract makes a later local operation newly accepted server truth? No current endpoint defines that boundary. | Local state remains unaccepted; server history is unchanged. | sync/import/API/money/privacy owners after #961/#722; `G-SYNC/G-MONEY/G-PRIV` and conditional `G-SCHEMA/G-CONTRACT/G-CLIENT` pending. |
