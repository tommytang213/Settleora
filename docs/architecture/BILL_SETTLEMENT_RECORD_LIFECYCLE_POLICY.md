...
- **Retention:** confirmed, accepted, applied, allocated, settled, disputed,
  ...
  their exact local persistence/lifecycle and retention model exists. Exact durations,
...
| `FIN-CLAIM-001` | Eligible draft bill item — **Open item for claiming** | Creator, responsible owner/editor, or an approved context role moves the item’s planning state `unassigned → open_to_claim` only while current bill/item/participant policy allows; this creates claimable review state, not a calculated share. | **Documented accepted requirement; runtime/schema/API unimplemented.** `QUANTITY_CLAIM_STATE_MONEY_POLICY.md` authority, vocabulary, and open behavior; #350. Exact persistence/contract/UI remains gated. |
...
| `FIN-CLAIM-001..008` | Open/partial/conflict/review claim states are draft/review facts and block authoritative submit/finalize/settlement/export/report truth. Only validated full coverage may enter share calculation; accepted bases later remain historical. | Mutable own claims may reduce/cancel only before accepted downstream dependency. Creator review is a distinct resolution. Locked claim evidence is not reopened or recalculated in place; correction uses reviewed revision/successor policy. | Bill/item state; item and accepted-claim quantities; participant/placeholder eligibility #720; tax/fee/discount/refund components; calculation/hash/policy version; payer reconfirmation; revision and complete settlement graph; sync/import identity; reports/exports/audit. Runtime/schema/API/UI and golden history proof remain #350/focused-owner work. |
...
| `FIN-CLAIM-*` | No runtime metadata exists. Future claim acceptance requires stable claim/basis IDs, item and accepted-quantity versions, actor/participant identity, policy/calculation hash, idempotency/correlation identity, expected version, and atomic claim/readiness/audit outcome. | Accepted/locked claim facts are `FINANCIAL_HISTORY_RETAINED`; unresolved draft/review claims are `FINANCIAL_DRAFT_UNRESOLVED` and not disposable without positive dependency/copy proof. Hard delete/purge remain unresolved or ineligible by class. | Audit bounded open/create/reduce/cancel/conflict/review/readiness/lock/denial outcomes and quantity counts; redact raw OCR/file/payment/storage material and unrelated identities/money. |
...
| `LOCAL-*` | The local-only store is the accepted authority inside its boundary, but exact metadata/idempotency/concurrency runtime is unimplemented; a distinct operation/import identity is required before any server submission. | `LOCAL_AUTHORITY_UNIMPLEMENTED`; authoritative local history is not disposable as cache, while cache classification requires positive proof. Device cleanup cannot delete server history or local authoritative records by assumption. | Local audit/privacy/backup rules remain unresolved; never upload logs/material as server acceptance evidence. |
...
| `FIN-BILL-003..021` | `FIN-CHOICE-003/004/013/014/025/026/029/061..063/010` as applicable | Implemented/partial/unimplemented by row | Existing bill, import, recurring, #345/#967, then #722/#724 split | `G-MONEY/G-PRODUCT/G-RET/G-PRIV/G-DEST`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA/G-SYNC`; per-path creation/edit/post authority, delegated owner, payload visibility, calculation/payer basis, retry/duplicate behavior, import/occurrence identity, transition/history, cancelled re-entry, finalized/status-archive distinction, retention, copy-disposition, and dependency tests. |
...
| `FIN-CLAIM-001..008`, `LOCAL-CLAIM-001` | `FIN-CHOICE-049/054..056/010` as applicable to local/import/retention only | Documented accepted requirement; runtime/schema/API/client unimplemented | Existing #350 after #961, split through #722/#723/#724 and #720 participant dependency | `G-MONEY/G-PRODUCT/G-RET/G-PRIV`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT/G-SYNC`; exact quantity/state/basis, authorization, claim mutation, conflict/review/readiness, calculation refusal, revision/settlement block, local/import boundary, retention, redaction, and deterministic history tests. |
...
| `LOCAL-*` | `FIN-CHOICE-013/048..056` as applicable | Documented accepted local authority; exact lifecycle/runtime unimplemented | sync-import-export-restore plus local/mobile and money lanes, after #961/#722 split | `G-MONEY/G-SYNC/G-PRIV/G-PRODUCT/G-RET/G-DEST`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; per-family local lifecycle, operation identity, offline conflict, replay, retention/disposition, encryption/backup, and server acceptance tests. |
...
| `FIN-CHOICE-062` | Personal/group ordinary draft-bill edit contract | Which exact metadata and component fields may a responsible editor change in each draft state, and what recalculation, participant/payer acknowledgement, history, version, retry, and conflict effects apply? Day 1 requires editing but current general edit runtime and a complete accepted mutation contract do not exist. | Do not infer a general edit endpoint from future-bill metadata update, OCR apply, or revision support; keep ordinary draft edits unavailable until a focused contract is approved. | #345/#967 then #722 money/API/schema split and #723 UI; blocks general draft edit runtime; `G-PRODUCT/G-MONEY` and conditional `G-SCHEMA/G-CONTRACT/G-CLIENT` pending. |
| `FIN-CHOICE-063` | Group on-behalf responsible bill-owner selection | Which eligible group relationship may be selected as responsible owner/editor, who may select/change it, and what confirmation, audit, retry, and conflict contract binds it while keeping creator and paid-by identities distinct? Day 1 requires the distinction, but current group create has no owner field and current authority does not choose the exact workflow. | Preserve current actor-owned runtime only as an implementation gap; do not call the conflation intended policy or accept a client-supplied owner without focused authorization. | Existing #967 owns domain/mobile reconciliation with #720 membership and #722 contract/schema work if needed; `G-PRODUCT/G-MONEY` and conditional `G-SCHEMA/G-CONTRACT/G-CLIENT` pending. |
...
not serialize their complete state. `FIN-CHOICE-057` therefore blocks any claim
that current retained facts deterministically reconstruct the old and new
component-to-total relationship. The chain above is the required acceptance
shape, not a statement that this source-proven gap is already closed.
...
- Existing #350 owns quantity-claim runtime/schema/API/client decomposition and
  ...
  delegated responsible-owner create reconciliation. Neither is started here.
