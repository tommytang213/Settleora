...
- `ExpenseBillWorkflowEndpoints` implements draft submit, pending participant
  ...
  participant acknowledgements, accepts the creator, moves the root to
  `pending_confirmation` or `confirmed`, and stages audit plus notifications.
...
| `FIN-BILL-019` | Unarchived future-bill draft — **Post future bill** | Only the visible creator posts `draft → pending_confirmation|confirmed`; current profile authorization and any group authorization are rechecked, eligible participant acknowledgements reset, the creator becomes accepted, and audit plus participant notifications are staged. | **Implemented current fact.** `FutureBillEndpoints.PostFutureBillAsync`; `FutureBillEndpointTests`. It is distinct from general bill submit: bill-owner visibility alone is insufficient, the request must have no body, and there is no idempotency key/version token. |
