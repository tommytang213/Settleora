...
| `FIN-COMP-001/002` | `FIN-CHOICE-002/005/010` | Partial | #722 money runtime plus schema if tombstone/version fields; #724 retention | `G-MONEY/G-RET/G-PRIV/G-DEST`, conditional `G-SCHEMA`; positive dependency and deterministic history rebuild tests. |
...
| `FIN-CHOICE-005` | Server bill-item tombstone and revision/history authority | Can a draft item tombstone be restored, and when must removal instead be a revision? `DeletedAtUtc` exists without a lifecycle service, actor/reason/version, or complete policy. | Retain tombstoned row; block restore/delete expansion; accepted bills use revision only. | Bill runtime/schema owners via #722, retention via #724; `G-SCHEMA/G-MONEY/G-RET` pending as applicable. |
