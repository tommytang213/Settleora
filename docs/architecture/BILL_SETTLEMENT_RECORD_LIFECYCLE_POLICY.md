...
| `FIN-REV-005` | Pending revision — **Reject proposal** / proposed **Cancel proposal** | An affected reviewer can accept `submitted_for_review → rejected`. `cancelled_by_authorized_editor` is a schema/status possibility only: no current endpoint or domain transition accepts it. Either terminal outcome must remain retained history with no restore; a later correction is a new revision. | **Reject implemented; cancellation unimplemented.** `ExpenseBillRevisionEndpoints.RejectBillRevisionAsync`, revision tests, `ExpenseBillRevisionStatuses`; no cancel route/service operation. The exact cancellation actor/preconditions remain a product choice under `FIN-CHOICE-004` and `G-PRODUCT`. |
...
| `FIN-BILL-003/004/005` | `FIN-CHOICE-003/004` | Partial | Existing bill/revision owners, then #722 split | `G-MONEY`, `G-PRODUCT`, conditional `G-CONTRACT/G-CLIENT/G-SCHEMA`; transition/history tests. |
...
| `FIN-REV-001..005` | `FIN-CHOICE-004/006/010` | Implemented/partial/unimplemented by row | Existing revision lane; #722/#724 follow-ups | `G-MONEY`, `G-PRODUCT`; any new cancellation or settled-impact behavior remains pending; concurrency/snapshot/hash/history tests. |
| `FIN-SET-001..004`, `FIN-LINE-001` | `FIN-CHOICE-007/008/009/010` | Implemented/partial/unimplemented by row | #722 money runtime; #724 retention; #723 UI | `G-MONEY`, `G-RET`, `G-PRODUCT`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; exact line/source, cancellation/dispute/reopen/archive, concurrency, rebuild tests. |
| `FIN-PAY-001..004`, `FIN-ALLOC-001`, `FIN-RES-001/002` | `FIN-CHOICE-007/010` | Implemented/partial | Existing settlement owners; #722/#724 | `G-MONEY`, `G-RET`, `G-PRODUCT`; rejection/reopen/reversal/refund choices remain pending and outside this task; hostile replay/atomicity/projection tests. |
| `FIN-REC-001/002` | `FIN-CHOICE-011/010` | Partial | #722 money runtime; #724 retention; #723 UI | `G-MONEY`, `G-PRODUCT`, conditional `G-SCHEMA/G-CONTRACT/G-CLIENT`; generated-history and schedule-policy tests. |
| `FIN-REC-003/004` | `FIN-CHOICE-012/010` | Partial/unimplemented | Recurring owner through #722 split | `G-MONEY`, `G-PRODUCT`; skip/cancel/re-entry remains pending; uniqueness/replay/generated-bill independence tests. |
| `FIN-DER-001/002` | `FIN-CHOICE-009/010` | Implemented read/partial policy | Report/balance owner after #961 synthesis; UI separately | `G-MONEY` and `G-PRODUCT` if accepted semantics change; conditional `G-CONTRACT/G-CLIENT`. Golden rebuild, archive presentation, inconsistent-source fail-closed tests. |
...
| `FIN-CHOICE-004` | Server bill participant acknowledgements, rejected bill roots, and revision cancellation | Is bill/participant rejection reversible by resubmitting the same root, which acknowledgements reset, and who may accept revision cancellation? Submit source can reset certain rejected participant rows only from a draft root, while no rejected-root or revision-cancel transition exists. | Rejected root remains rejected and revision cancellation remains unavailable; no client-side resubmit/cancel inference. | Bill product/money owner via #722; `G-PRODUCT/G-MONEY` pending. |
