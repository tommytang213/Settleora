...
is the narrower #369 source-policy gate for pending receiver-confirmation
settlement residual handoffs after PR #654. It records that existing
`settlement.payment_partially_paid` remains a payment-claim/request-status
notice, defines the dedicated `settlement.residual_review_needed` event for
successful debtor-created pending residual creation, and keeps broader
mismatch/review events blocked until broader settlement review states exist.

[Sync notification source policy](SYNC_NOTIFICATION_SOURCE_POLICY.md) is the
narrower #369 source-policy gate for remaining sync queued/retry/resolved and
conflict-resolution notifications after `sync.conflict_detected` and
`sync.operation_failed`. It records that the current server runtime has no
queued, retrying, retry-failed, resolved, reopened, or resolution-applied
source states, defers noisy automatic queue/retry notifications, and requires
...
| Settlements, payment requests, proof/review/dispute/mismatch | Settlement request/payment/proof services. Required for request created, marked paid, partial paid, residual review needed, confirmed, disputed, cancelled, and proof attached. Money-impactful and storage/privacy-impactful for proof. | Safe IDs: settlement request/payment, group, already-visible bill, stable proof file ID only through authorized proof APIs. Exclude raw payment handles, QR contents, account numbers, proof contents, bank screenshots, storage object keys, hidden bill lines, residual amounts/reasons, and unbounded amount/payment details in external snippets. | Implemented settlement constants, writer helpers, endpoint integrations, and endpoint tests now cover `settlement.request_created`, `settlement.payment_marked_paid`, `settlement.payment_partially_paid`, `settlement.residual_review_needed`, `settlement.payment_confirmed`, `settlement.request_disputed`, `settlement.payment_disputed`, `settlement.request_cancelled`, `settlement.payment_cancelled`, and `settlement.proof_attached` writes after successful authorized transitions. OpenAPI enum includes settlement request/payment/proof/residual-review events. Current residual runtime persists pending receiver-confirmation residual rows during debtor-created payment claims, writes `settlement.residual_review_needed` to the receiver/creditor only for those pending residual handoffs, blocks payment confirmation until pending residuals are resolved, and exposes a receiver confirmation route. | Existing `settlement.payment_partially_paid` remains a payment-claim/request-status notice. Broader mismatch/review events remain blocked until broader settlement review states exist. Tests must continue to prove counterparty derivation, no unauthorized recipient/event leaks, proof authorization on open, residual-review notification privacy, and no settlement/payment/proof/residual mutation from notification read/archive. |
...
| Sync/offline conflict and failure handoffs | Sync acceptance and offline queue services. Required for conflicts and failures needing user action. Security-impactful where sync touches auth, storage, money, or server/local boundaries. UI/Figma-gated for conflict/failure states. | Safe IDs: sync operation ID, bounded correlation/hash, target type, safe target ID only when already visible. Exclude raw queued request body, local mutation payload, local paths/cache data, hidden server current data, storage internals, and unrelated user data. | #571 implements the first narrow sync runtime slice for existing persisted `SyncOperation` conflict rows only. Newly persisted stale-base-version and resource-state conflicts write one unread `sync.conflict_detected` in-app notification to the owning/current actor, with attention priority, `sync_operation` subject type, first-class `syncOperationId`, safe `expenseBillId` where the operation already targets an expense bill, and a route-like `/api/v1/sync/operations/{syncOperationId}` action URL. The current repo also implements the narrow `sync.operation_failed` runtime slice for newly persisted current-actor `SyncOperationStatuses.Rejected` rows only. Rejected rows write one unread attention notification to the owning/current actor with the same first-class `syncOperationId`, route-like current-actor sync operation action URL, and bounded safe metadata. The read endpoint is current-actor-only and returns the bounded sync operation response without idempotency keys, request payload hashes, raw payloads, local paths/cache data, hidden server-current data, storage internals, OCR text, tokens, or unrelated user data. Accepted operations do not write failure notifications, replayed/idempotent reads of existing operations do not duplicate notifications, idempotency-key mismatch responses that do not persist a new row do not notify, invalid requests that do not persist sync operation rows do not notify, and conflict rows remain covered only by `sync.conflict_detected`. `SYNC_NOTIFICATION_SOURCE_POLICY.md` now records that current runtime has no queued/retrying/retry-failed/resolved/reopened/resolution-applied source states. | Broader `sync.operation_queued`, `sync.operation_retrying`, `sync.operation_retry_failed`, `sync.conflict_resolved`, `sync.conflict_reopened`, `sync.resolution_applied`, conflict resolution behavior, mobile deep links/UI, and broad offline-sync expansion remain future work. Ordinary queued/retrying churn is deferred; future runtime may add only exact user-actionable source transitions such as retry exhaustion or explicit conflict resolution, with event enum/OpenAPI/generated-client/EF constraint changes only in that later runtime slice. |
...
3. Settlement/payment/proof/residual or mismatch event gap closure for exact
   implemented source states. The pending residual handoff now follows
   [settlement residual review notification source policy](SETTLEMENT_RESIDUAL_REVIEW_NOTIFICATION_SOURCE_POLICY.md)
   with `settlement.residual_review_needed`; keep existing
   `settlement.payment_partially_paid` as payment-claim coverage, and block
   broader mismatch/review events until source states exist.
...
5. Remaining sync notification events after exact persisted source states exist:
   queued/readout events, retry exhaustion, conflict resolution/reopen,
   resolution-applied readouts, and broader conflict/failure UX remain
   unimplemented after the persisted `sync.conflict_detected` and
   `sync.operation_failed` slices. The
