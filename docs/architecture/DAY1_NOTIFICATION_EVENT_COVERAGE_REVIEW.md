...
5. Remaining sync notification events after exact persisted source states exist:
   queued/readout events, conflict resolution, retry, and broader
   conflict/failure UX remain unimplemented after the persisted
   `sync.conflict_detected` and `sync.operation_failed` slices.
