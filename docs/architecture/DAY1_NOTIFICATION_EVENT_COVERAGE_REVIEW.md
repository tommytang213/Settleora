...
notice, recommends a future dedicated event such as
`settlement.residual_review_needed` only for successful debtor-created pending
residual creation, and keeps broader mismatch/review events blocked until
broader settlement review states exist.
...
| Settlements, payment requests, proof/review/dispute/mismatch | Settlement request/payment/proof services. Required for request created, marked paid, partial paid, confirmed, disputed, cancelled, and proof attached. Money-impactful and storage/privacy-impactful for proof. | Safe IDs: settlement request/payment, group, already-visible bill, stable proof file ID only through authorized proof APIs. Exclude raw payment handles, QR contents, account numbers, proof contents, bank screenshots, storage object keys, hidden bill lines, and unbounded amount/payment details in external snippets. | Implemented settlement constants, writer helpers, endpoint integrations, and endpoint tests now cover `settlement.request_created`, `settlement.payment_marked_paid`, `settlement.payment_partially_paid`, `settlement.payment_confirmed`, `settlement.request_disputed`, `settlement.payment_disputed`, `settlement.request_cancelled`, `settlement.payment_cancelled`, and `settlement.proof_attached` writes after successful authorized transitions. OpenAPI enum includes settlement request/payment/proof events. Current residual runtime persists pending receiver-confirmation residual rows during debtor-created payment claims, blocks payment confirmation until pending residuals are resolved, and exposes a receiver confirmation route; [settlement residual review notification source policy](SETTLEMENT_RESIDUAL_REVIEW_NOTIFICATION_SOURCE_POLICY.md) records that this is not yet residual-review notification runtime. | Future pending residual notification work should use a dedicated event such as `settlement.residual_review_needed` only after a narrow runtime task, targeting the receiver/creditor through existing settlement request/payment IDs where sufficient. Existing `settlement.payment_partially_paid` remains a payment-claim/request-status notice. Broader mismatch/review events remain blocked until broader settlement review states exist. Tests must continue to prove counterparty derivation, no unauthorized recipient/event leaks, proof authorization on open, and no settlement/payment/proof/residual mutation from notification read/archive. |
...
3. Settlement/payment/proof/residual or mismatch event gap closure for exact
   implemented source states. Pending residual handoff should follow
   [settlement residual review notification source policy](SETTLEMENT_RESIDUAL_REVIEW_NOTIFICATION_SOURCE_POLICY.md):
   use a future dedicated residual-review event if implemented, keep existing
