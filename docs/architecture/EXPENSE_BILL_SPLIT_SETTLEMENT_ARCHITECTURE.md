...
- No business migrations for expenses, bills, splits, settlements, balances, recurring bills, or reconciliation exist yet.
...
This document does not implement migrations. Future schema branches should introduce small table groups with explicit constraints.
...
- `expense_bills`: root bill/expense record with creator, optional group, merchant/date/category metadata, status, total amount/currency, version/timestamps, archive fields, and same-currency Day 1 constraint.
- `expense_bill_items`: item rows for a bill with description, quantity where needed, amount/currency, ordering, optional OCR candidate linkage, and soft-delete/archive direction.
...
- `expense_bill_payers`: original payer contribution rows with user profile, amount/currency, optional payment method hint, and non-negative amount constraint.
- `expense_bill_adjustments`: tax, service charge, discount, credit, or manual adjustment rows with explicit type, direction, amount/currency, allocation method, and non-hidden sign policy.
- `expense_bill_attachments`: receipt/supporting file references using stable `file_objects.id`, purpose/lifecycle constraints, subject ownership, and no provider path fields.
...
1. Expense/bill schema foundation only.
