...
- No public bill edit, generic balance, recurring, reconciliation, standalone receipt/OCR upload/download outside bill attachments, OCR engine/worker, or bill-related notification endpoints exist yet.
- Personal and group bill create/read, bill workflow, bill attachment, receipt OCR review intake/apply-preview/draft-apply, settlement candidate preview, settlement request creation, settlement request list/get, settlement payment list/get, settlement payment claim, settlement payment confirmation, settlement dispute, settlement cancellation, settlement payment proof, settlement basket, settlement payment residual confirmation, and settlement balance projection OpenAPI paths and generated clients exist. No generic balance, recurring, reconciliation, standalone receipt/OCR upload/download outside bill attachments, OCR engine/worker, or bill-related notification OpenAPI paths exist yet.
- Settlement persistence schema exists for request roots, payment claims, proof attachment file references, basket request lines, payment allocations, and residual tracking. Candidate preview, first settlement request creation runtime, read-only current-actor settlement request list/get endpoints, read-only settlement payment list/get endpoints, debtor-authored same-currency full/partial payment claim endpoints, receiver-authored payment confirmation endpoints, bounded request/payment dispute endpoints, guarded request/payment cancellation endpoints, settlement-scoped counterparty payment-details/QR reads, purpose-specific settlement payment proof attach/list/content/remove endpoints, scoped settlement basket preview/create endpoints, payment-claim-time residual proposal persistence, receiver residual confirmation/finalization, and read-only current-actor balance projection runtime exist for confirmed personal/group bill candidates. Recurring bill runtime, reconciliation runtime, broad credit ledgers, refund workflows, settlement simplification, settlement reopen/adjustment policy, UI, worker behavior, and balance projection writes/caches do not exist yet.
...
- `settlement_request_lines`: exact selected basket line rows tied to settlement requests and source expense bills, with optional bounded candidate key, positive amount/currency, allocation order, constrained line status, timestamps, indexes, and restrictive foreign keys. Current single-bill request creation writes one server-derived request line for the selected candidate, and current pay-all basket creation writes concrete request lines for eligible same-currency current-actor/counterparty candidates.
- `settlement_payment_allocations`: payment-to-request-line allocation rows with positive cleared amount/currency, allocation order, timestamp, indexes, and restrictive foreign keys. Current same-currency payment claim runtime writes allocation rows against selected request lines and exposes bounded allocation summaries on payment responses.
- `settlement_residuals`: underpayment/overpayment, waiver, carried-forward, and credit tracking rows tied to a settlement payment and/or request, debtor/creditor profiles, constrained direction/policy/status values, positive amount/currency, optional bounded reason, lifecycle timestamps, counterparty and payment-or-request presence checks, indexes, and restrictive foreign keys. Current payment-claim-time residual runtime writes pending residual proposals for explicit same-currency underpayment/overpayment policies, receiver residual confirmation resolves one pending residual to its policy-derived status, and the current read-only balance projection exposes bounded receiver-confirmed remaining, waived, and credit residual effects without broad credit ledger or refund behavior.
...
Settlement candidate preview, first settlement request creation, read-only current-actor settlement request list/get, settlement-scoped counterparty payment-details/QR reads, read-only settlement payment list/get, settlement payment claim, settlement payment confirmation, settlement payment residual confirmation, settlement dispute, settlement cancellation, settlement payment proof paths, settlement balance projection, settlement basket preview/create, and bill attachment paths now exist in OpenAPI and generated clients. Recurring, reconciliation, standalone receipt/OCR upload/download outside bill attachments, OCR, and notification paths do not exist yet.
...
7. Settlement basket/residual schema foundation. This is now landed as persistence, and later scoped runtime slices now use it for server-derived request lines, payment allocations, residual proposals, residual confirmation/finalization, read-only balance projection, and pay-all basket preview/create. UI, worker, broad credit ledger, refund, simplification, and reopen/adjustment behavior remain out of scope.
...
14. Read-only current-actor settlement balance projection. This is now landed for request-line/allocation runtime plus bounded receiver-confirmed residual effects.
15. Settlement basket preview/create. This is now landed for same-currency `pay_all_outstanding_for_counterparty` selection.
16. Bounded residual finalization. This is now landed for receiver-confirmed remaining, waived, and credit residual effects without broad credit ledgers or refunds.
