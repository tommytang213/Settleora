...
| `AUTH-LC-CHOICE-005` | What anti-lockout, policy-dependent step-up, allowed source-state, concurrency, session/challenge response, and terminal material-erasure rules apply to passkey/TOTP/recovery disable, re-enable, revoke, replace, use, expiry, and depletion? (`PKY-*`, `TOTP-*`, `RCV-*`) | Current sequential revoke/replace/use works, but concurrent TOTP starts can create multiple pending factors, TOTP cancel/revoke lacks atomic transition guards, generic factor revoke accepts all non-revoked states, concurrent recovery generation can create multiple active batches, concurrent recovery use lacks atomic consume, and last-factor/session/terminal-secret policy is unproved. | Keep precise current behavior and classify races partial; do not add disable/re-enable/admin reset/expiry runtime or claim secret disposition. #394/#775/#776/#465; manual auth/recovery/secret/schema gate. |
