...
| `AUTH-LC-INV-001` | Template + `E-INVITE`; current source/tested | Accepted invitation excluded; created account independently readable/usable | Relational transaction and sequential single-use/compensating rollback are tested; concurrent acceptance remains a required case | Invite class §5.3; HD=no for action; P=unresolved | Accept/denial safe IDs/categories, no contact/material; generic public result | `CHOICE-008`; implemented/default-off; #784; `G-AUTH`,`G-PROV`,`G-EXPOSE`,`G-RET`,`G-PRIV`,`G-DEST`; concurrent single-use/rollback tests required |
...
| `AUTH-LC-POL-002` | Template + `E-INVITE`; current source/tested sequentially | New active read governs; retired version retained; later change allowed | Conditional mutation and current-version retry are source-defined; concurrent version race coverage remains required | Policy class §5.3; HD=no; P=unresolved | Actor/version/prior-new/outcome, no unsafe config; refresh readout | `CHOICE-011`; implemented sequentially; #784/#785; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`; concurrent race/grace/lockout tests required |
