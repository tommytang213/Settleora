...
| `AUTH-LC-TOTP-002` | Pending TOTP factor — start replacement setup | Starting a new enrollment marks an earlier pending row `→ revoked`, then creates fresh pending material. | Earlier setup material stops working; enrolled factors/sessions remain. | Irreversible replacement of pending material. Implemented; `AUTH-LC-CHOICE-005`. Owner #394/#776. |
| `AUTH-LC-TOTP-003` | Pending TOTP factor — **Setup expired** | Server verification observes the deadline and changes `pending → expired`. | Expired setup cannot enroll/authenticate; other authority remains. | Terminal for that setup. Implemented lazily; `AUTH-LC-CHOICE-005`. Owner #394/#776. |
...
| `AUTH-LC-TOTP-006` | Enrolled TOTP factor — **Revoke authenticator** | Current owner, after required fresh step-up, changes retained factor `enrolled → revoked`. | Factor stops future TOTP authentication; current code does not revoke existing sessions, challenges, recovery batches, or other factors. | Irreversible for that factor; re-enroll creates new protected material. Implemented, but protected-secret disposition and anti-lockout/session response are unresolved under `AUTH-LC-CHOICE-005`. |
...
| `AUTH-LC-CHL-002` | Passkey challenge — not user-visible: **Consumed** | Successful passkey ceremony changes `pending → consumed`. | Challenge cannot be reused; only its scoped enrollment/sign-in/step-up result may proceed. | Irreversible. Implemented; sign-in credential handoff remains `REC-AUTH-PASSKEY-SESSION-001`. |
| `AUTH-LC-CHL-003` | MFA/passkey challenge — **Expired** | Server time observed during verification changes `pending → expired`. | No auth result; existing sessions/factors remain. | Terminal. Implemented lazily; retention is `AUTH-LC-CHOICE-006`. |
...
| `AUTH-LC-PKY-001` | Template + `E-FACTOR`; current source/tested | Revoked credential excluded; metadata owner-read may remain; new enrollment only | Already-revoked idempotent; ownership/step-up checked; concurrent revoke tested | Passkey class §5.3; HD=no; P=unresolved | Revoke/denial safe credential ID, no assertion/private data; refresh factor list | `CHOICE-005`; implemented; #394/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; post-revoke/session/disposition tests |
...
| `AUTH-LC-TOTP-001` | Template + `E-FACTOR`; current source/tested | Pending setup excluded; new enrollment allowed | Already-unavailable generic; ownership checked; concurrent outcome current/tested | TOTP class §5.3; HD=no; P=unresolved | Cancel/denial safe ID, never protected secret/OTP; refresh setup state | `CHOICE-005`; implemented; #394/#776; `G-AUTH`,`G-RET`,`G-PRIV`,`G-DEST`; cancel/secret-erasure tests |
| `AUTH-LC-TOTP-002` | Template + `E-FACTOR`; current source/tested | Prior pending excluded; new pending setup eligible | New begin deterministically supersedes pending rows; transaction/race evidence required | TOTP class §5.3; HD=no; P=unresolved | Replacement lifecycle, no setup secret in audit; show only current setup | `CHOICE-005/012`; implemented; #394/#776; `G-AUTH`,`G-RET`,`G-PRIV`; concurrent begin/no-secret tests |
| `AUTH-LC-TOTP-003` | Template + `E-FACTOR`; current source/tested | Expired setup excluded; restart allowed | Repeated verify denies; expiry materialized lazily; race tested/required | TOTP class §5.3; HD=no; P=unresolved | Expiry/failure, no OTP/secret; show setup expired | `CHOICE-005`; implemented; #394/#776; `G-RET`,`G-PRIV`,`G-DEST`; clock/race/disposition tests |
...
| `AUTH-LC-TOTP-006` | Template + `E-FACTOR`; current source/tested | Revoked factor excluded; new enrollment only | Already-revoked idempotent; ownership/step-up checked; race current/tested | TOTP class §5.3; HD=no; P=unresolved | Revoke/denial safe ID, no secret/OTP; refresh factor list | `CHOICE-005`; implemented; #394/#465/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; last-factor/session/secret-erasure tests |
| `AUTH-LC-RCV-001` | Template + `E-FACTOR`; current source/tested | Old batch/verifiers excluded; new batch active | Explicit replacement required; transaction guards one active batch; retry redisplay forbidden | Recovery class §5.3; HD=no; P=unresolved | Replace/generate counts, no codes/verifiers; display once only | `CHOICE-005/012`; implemented; #775/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; race/no-redisplay tests |
| `AUTH-LC-RCV-002` | Template + `E-FACTOR`; current source/tested | Consumed verifier excluded; batch remains as count permits | Conditional exact-one consume; replay denies; concurrent consume tested/required | Recovery class §5.3; HD=no; P=unresolved | Consumption safe IDs/counts, no code/verifier; refresh batch count | `CHOICE-005`; implemented; #775; `G-AUTH`,`G-RET`,`G-PRIV`,`G-DEST`; exact-one/depletion tests |
| `AUTH-LC-RCV-003` | Template + `E-FACTOR`; current source/tested | Revoked batch/verifiers excluded; new batch distinct | Already-revoked idempotent; transaction covers unused verifiers | Recovery class §5.3; HD=no; P=unresolved | Revoke/counts, no codes/verifiers; refresh list | `CHOICE-005`; implemented; #775/#776; same gates as RCV-002; revoke/race/disposition tests |
...
| `AUTH-LC-CHL-001` | Template + `E-FACTOR`; current MFA source/tested | Verified challenge terminal; scoped continuation only | Single completion; terminal replay denies; concurrent completion guarded | Challenge class §5.3; HD=no; P=unresolved | Purpose/factor/outcome/attempts, no OTP/verifier; continue scoped flow | `CHOICE-006`; implemented mapped flows; #394/`REC-AUTH-MFA-SIGNIN-001`; `G-AUTH`,`G-RET`,`G-PRIV`,`G-DEST`; binding/replay tests |
| `AUTH-LC-CHL-002` | Template + `E-FACTOR`; current passkey source/tested | Consumed challenge terminal; scoped continuation only | Single completion; terminal replay denies; concurrent completion guarded | Challenge class §5.3; HD=no; P=unresolved | Purpose/credential/outcome, no assertion/challenge; continue scoped flow | `CHOICE-006`; implemented mapped flows; #394/`REC-AUTH-PASSKEY-SESSION-001`; same gates; binding/handoff tests |
| `AUTH-LC-CHL-003` | Template + `E-FACTOR`; current source/tested | Expired challenge excluded; new ceremony required | Repeated completion denies; lazy materialization race current/required | Challenge class §5.3; HD=no; P=unresolved | Expiry/purpose safe IDs, no material; show expired/generic public failure | `CHOICE-006`; implemented mapped flows; #394; `G-RET`,`G-PRIV`,`G-DEST`; clock/race tests |
...
| `AUTH-LC-RST-001` | Template + `E-RESET`; current source/tested | Replaced request excluded; newest pending may proceed | New issue revokes pending set transactionally; old retry generic | Reset class §5.3; HD=no; P=unresolved | Replacement/requested events, no email/material/hash; generic delivery response | `CHOICE-007`; implemented; #339/#724; `G-AUTH`,`G-PROV`,`G-RET`,`G-PRIV`,`G-DEST`; replacement/race/redaction tests |
| `AUTH-LC-RST-002` | Template + `E-RESET`; current source/tested | Consumed request excluded; later request distinct | Single consume; replay becomes suspicious; credential/session transaction guarded | Reset class §5.3; HD=no; P=unresolved | Consume/change/revoke events, no password/material; prompt sign-in | `CHOICE-007`; implemented; #339; same gates as RST-001; concurrent completion/session tests |
| `AUTH-LC-RST-003` | Template + `E-RESET`; current source/tested | Expired request excluded; later request distinct | Repeated completion generic; lazy expiry race current/required | Reset class §5.3; HD=no; P=unresolved | Expiry/denial safe metadata, no identifier/material; generic failure | `CHOICE-007`; implemented; #339/#724; `G-RET`,`G-PRIV`,`G-DEST`; clock/redaction tests |
...
| `AUTH-LC-CHOICE-005` | What anti-lockout, session/challenge response and terminal material-erasure rules apply to passkey/TOTP/recovery disable, re-enable, revoke, replace, expiry, and depletion? (`PKY-*`, `TOTP-*`, `RCV-*`) | Current revoke/replace works, but does not prove last-factor recovery or session policy; revoked TOTP protected payloads and terminal recovery verifiers remain stored. | Keep precise current revoke/replace behavior; do not add disable/re-enable/admin reset/expiry runtime or claim secret disposition. #394/#775/#776/#465; manual auth/recovery/secret/schema gate. |
