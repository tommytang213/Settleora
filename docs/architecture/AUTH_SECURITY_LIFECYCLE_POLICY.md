...
| `AUTH-LC-PKY-001` | Template + `E-FACTOR`; current source/tested | Revoked credential excluded; metadata owner-read may remain; new enrollment only | Already-revoked idempotent; ownership and policy-dependent step-up checked; concurrent revoke result tested | Passkey class §5.3; HD=no; P=unresolved | Revoke/denial safe credential ID, no assertion/private data; refresh factor list | `CHOICE-005`; implemented; #394/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; post-revoke/session/disposition tests |
...
| `AUTH-LC-TOTP-006` | Template + `E-FACTOR`; current source/tested | Revoked factor excluded; new enrollment only | Already-revoked idempotent; ownership/policy-dependent step-up checked; race current/tested | TOTP class §5.3; HD=no; P=unresolved | Revoke/denial safe ID, no secret/OTP; refresh factor list | `CHOICE-005`; implemented; #394/#465/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; last-factor/session/secret-erasure tests |
| `AUTH-LC-TOTP-007` | Template + `E-FACTOR`; current source/tested | Any owned non-revoked factor row becomes revoked; no re-entry | Already-revoked idempotent; broad source-state acceptance and policy-dependent step-up are current; desired guard unresolved | TOTP class §5.3; HD=no; P=unresolved | Revoke/source-state/reason safe metadata, no secret/OTP; refresh factor/setup state | `CHOICE-005`; implemented broad behavior; #394/#776; same gates as TOTP-006; pending/expired/disabled endpoint and audit tests |
...
| `AUTH-LC-CHL-002` | Template + `E-FACTOR`; current passkey source/tested sequentially | Consumed challenge terminal after commit; scoped continuation only | No conditional status update/concurrency token; concurrent completions can both succeed and passkey sign-in can create two sessions | Challenge class §5.3; HD=no; P=unresolved | Purpose/credential/outcome, no assertion/challenge; continue scoped flow | `CHOICE-006`; partial concurrency; #394/`REC-AUTH-PASSKEY-SESSION-001`; same gates; atomic binding/handoff tests |
...
| `AUTH-LC-CHOICE-005` | What anti-lockout, policy-dependent step-up, allowed source-state, concurrency, session/challenge response, and terminal material-erasure rules apply to passkey/TOTP/recovery disable, re-enable, revoke, replace, use, expiry, and depletion? (`PKY-*`, `TOTP-*`, `RCV-*`) | Current sequential revoke/replace/use works, but generic factor revoke accepts all non-revoked states; concurrent recovery generation can create multiple active batches; concurrent recovery use lacks atomic consume; last-factor/session and terminal-secret policy are unproved. | Keep precise current behavior and classify races partial; do not add disable/re-enable/admin reset/expiry runtime or claim secret disposition. #394/#775/#776/#465; manual auth/recovery/secret/schema gate. |
...
| `AUTH-LC-CHOICE-007` | How is password-reset consumption made atomic, and what bounded retention/disposition applies to terminal rows and hashes, including replay evidence and `CleanupEligibleAtUtc`? (`RST-*`) | Concurrent completion can read the same pending row without a status-qualified consume/concurrency token; the cleanup field exists but current service neither sets a cleanup time nor deletes/compacts rows. | Treat completion concurrency as partial; retain current rows; do not expose hashes or add cleanup. #339/#724; security/privacy/destructive gate. |
| `AUTH-LC-CHOICE-008` | What idempotency/delivery rule governs resend, and should terminal cleanup hard-delete rows after 90 days or retain/compact row evidence with holds/dependency/copy proof? (`INV-*`) | Each ready resend retry rotates again before delivery, so timeout/send failure can make link delivery ambiguous; cleanup deletes and writes aggregate audit without proving aggregate evidence/holds/copies are sufficient. | Do not promise retry idempotency or broaden/treat the timer as general purge authority. Focused #784/#724 decision and later separate runtime task; manual provider/destructive/security/privacy gate. |
...
This document must retain exactly 52 unique `AUTH-LC-*` transition rows and 13
