...
| `AUTH-LC-CHL-002` | Passkey challenge — not user-visible: **Consumed** | Successful passkey ceremony changes `pending → consumed`. For sign-in, policy support is checked when options are created but is not revalidated at completion. The service creates an `AuthSession` but omits its raw session token from the response, and the endpoint sets no credential cookie/header. | Challenge cannot be reused after a successful commit. Enrollment/step-up may produce their scoped outcome; passkey sign-in persists a session record but gives the caller no bearer credential for authenticating a subsequent request. A policy change after options issuance also does not prevent completion/session creation. | Irreversible. Sequential ceremony behavior is implemented; usable sign-in credential handoff is absent, while completion concurrency and policy revalidation remain partial under `REC-AUTH-PASSKEY-SESSION-001`. |
...
| `AUTH-LC-CHL-002` | Template + `E-FACTOR`; current passkey source/tested sequentially | Consumed challenge terminal after commit; sign-in completion does not revalidate passkey policy and persists a session whose raw credential is discarded instead of returned/set | No conditional status update/concurrency token; concurrent completions can both succeed and create two unusable-to-caller sessions; intervening policy disable does not currently block completion | Challenge class §5.3; HD=no; P=unresolved | Purpose/credential/outcome, no assertion/challenge/session token; do not claim the caller is signed in | `CHOICE-005/006`; missing credential handoff/partial concurrency/policy revalidation; #394/`REC-AUTH-PASSKEY-SESSION-001`; same gates; atomic binding/usable-handoff/policy-change tests |
...
| `AUTH-LC-CHOICE-005` | What anti-lockout, policy-dependent step-up, completion-time policy revalidation, allowed source-state, factor eligibility, concurrency/idempotency, session/challenge response, and terminal material-erasure rules apply to passkey/TOTP/recovery enroll, disable, re-enable, revoke, replace, use, expiry, and depletion? (`PKY-*`, `TOTP-*`, `RCV-*`, passkey `CHL-002`) | Current sequential lifecycle paths work, but passkey sign-in completion does not recheck policy and discards the new session credential, passkey/factor revoke accepts all non-revoked states, recovery generation requires enrolled TOTP rather than any factor, recovery revoke rewrites every owned batch state, concurrent TOTP begin/enroll/cancel and recovery generation/use lack atomic guards, and last-factor/session/terminal-secret policy is unproved. | Keep precise current behavior and classify handoff, eligibility, broad-state, repeat, and race outcomes partial; do not add disable/re-enable/admin reset/expiry runtime or claim secret disposition. #394/#775/#776/#465 and `REC-AUTH-PASSKEY-SESSION-001`; manual auth/recovery/secret/schema gate. |
| `AUTH-LC-CHOICE-006` | How are challenge attempt counting and terminal transitions made atomic, how long is safe metadata retained, and how is cleanup audited? (`CHL-*`) | Validity/attempt limits exist, but concurrent invalid attempts can overwrite the same count/increment and defer blocking; challenge completion also uses ordinary tracked updates without a concurrency token/status-qualified consume. No retention clock, hold, cleanup, or copy-disposition rule exists. | Treat attempt limits and completion as sequentially implemented but concurrency-partial; terminal challenges stay unusable after commit; no purge authorization. #394 plus auth-retention follow-up; security/privacy/destructive gate. |
...
| Challenge retention/abuse | `REC-AUTH-MFA-SIGNIN-001` and `REC-AUTH-ABUSE-001`; docs decision then split runtime/config lanes | Resolve binding, atomic attempt increments/blocking, expiry/replay/completion and cleanup choices. Manual security/proxy gates. |
| Passkey sign-in handoff | `REC-AUTH-PASSKEY-SESSION-001`; docs decision then split auth runtime and contract lanes | Keep completion-time policy revalidation, challenge atomicity, and the currently discarded session credential/usable handoff with the #965 owner; #721 adds lifecycle wording only. |
...
This document must retain exactly 55 unique `AUTH-LC-*` transition rows and 13
