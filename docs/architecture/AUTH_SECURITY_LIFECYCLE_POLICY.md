...
| `AUTH-LC-CHL-001` | MFA challenge — not user-visible: **Verified** | Successful bounded MFA ceremony changes `pending → verified` and records consumption time. For TOTP, a successful proof records only factor `LastUsedAtUtc`; it does not persist/reject the accepted timestep, so the same current code can satisfy a separate pending challenge within the allowed drift window. | Only the scoped outcome may proceed; success never silently extends refresh lifetime. Cross-challenge TOTP-proof replay can currently authorize multiple scoped outcomes. | Irreversible per challenge. Implemented sequentially per challenge, but cross-challenge proof replay and completion concurrency are partial under `AUTH-LC-CHOICE-006`; password-primary binding remains `REC-AUTH-MFA-SIGNIN-001`. |
...
| `AUTH-LC-PKY-005` | Template + `E-FACTOR`; current source/tested sequentially | Enrolled credential eligible after commit; revoke is distinct; completion does not revalidate policy | Account/challenge binding and observed collision check exist, but concurrent completion/credential insertion and intervening policy change lack atomic guards | Passkey class §5.3; HD=no; P=unresolved | Enrollment/outcome safe credential/challenge IDs only, no attestation/assertion/private material; refresh list | `CHOICE-005/006`; partial concurrency/policy; #394/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; duplicate/race/policy-change/redaction tests |
...
| `AUTH-LC-TOTP-008` | Template + `E-FACTOR`; current source/tested sequentially | Enrolled factor becomes eligible; no re-entry to pending; cancel/revoke are separate | OTP verification and enrollment audit exist, but ordinary tracked update leaves verify-versus-cancel/revoke concurrency unguarded | TOTP class §5.3; HD=no; P=unresolved | Enrollment/failure safe factor ID/status only, no OTP/secret/provisioning URI; show enrolled | `CHOICE-005`; partial concurrency; #394/#776; `G-AUTH`,`G-LOCK`,`G-RET`,`G-PRIV`,`G-DEST`; atomic source-state/race/secret-redaction tests |
...
| `AUTH-LC-CHL-001` | Template + `E-FACTOR`; current MFA source/tested sequentially | Verified challenge terminal after commit; scoped continuation only; same current TOTP can satisfy another pending challenge | No conditional challenge update/concurrency token, and no accepted TOTP timestep/proof identifier is persisted to reject cross-challenge reuse | Challenge class §5.3; HD=no; P=unresolved | Purpose/factor/outcome/attempts, no OTP/verifier; continue scoped flow without claiming proof uniqueness | `CHOICE-006`; partial completion/proof replay; #394/`REC-AUTH-MFA-SIGNIN-001`; `G-AUTH`,`G-RET`,`G-PRIV`,`G-DEST`; atomic completion/cross-challenge TOTP replay tests |
...
| --- | --- | --- | --- | --- |
| `AUTH-LC-SES-001`–`AUTH-LC-SES-004` | Source-defined session validity; post-expiry/revocation metadata retention is operationally required but duration unresolved. | Session/account IDs, status, issued/expires/last-seen/revoked time, bounded reason/device label; never raw access material, token hash in client/audit, full IP, or unbounded agent. | Session creation/validation/revocation/family events exist; no retention cleanup/readout proof. | `AUTH-LC-CHOICE-010`; #338/#1059, `auth-session-security`; security/privacy/destructive gates. Accept expiry/revoke/family isolation, relational rotation-versus-revoke, family-aware validation, redaction, and retained-copy disposition tests. |
...
| `AUTH-LC-ACC-001`–`AUTH-LC-ACC-003` | Account lifecycle evidence operationally required; duration/disposal unresolved. Deletion-marker presence is not physical-purge authority. | Account/profile link, bounded status/time/actor/reason; keep account deletion distinct from disablement, identity unlinking, and profile/domain-data lifecycle. | Architecture requires disable/re-enable/delete/denied audit; mutation runtime absent. | `AUTH-LC-CHOICE-001`; #788/#785/#724. Owner-lockout/privacy/destructive manual gates. Accept last-owner concurrency, alternate-auth recovery, session/material cascade, dependencies/holds/copies, re-entry/restore posture and denial audit. |
...
| `AUTH-LC-PKY-001`–`AUTH-LC-PKY-005` | Enrolled public-key metadata is operational credential state; revoked/disabled metadata is operationally required for collision/replay/security review; duration unresolved. | Server credential ID, credential-ID hash, public key only while justified, counter/backup/transport categories, status/times/reason; no private material, raw assertion, full attestation payload. | Enrollment/assertion/revoke/failure audit exists; enrollment completion does not revalidate policy, and generic revoke accepts every owned non-revoked source state; disable/re-enable and terminal cleanup do not. | `AUTH-LC-CHOICE-005`; #394/#776 plus `REC-AUTH-PASSKEY-SESSION-001`. Accept enrollment uniqueness/concurrency/policy, source-state, last-factor/step-up, post-transition sign-in, session decision, replay and disposition tests. |
| `AUTH-LC-TOTP-001`–`AUTH-LC-TOTP-008` | Pending setup validity is source-defined. Enrolled protected secret is operational authentication material. Terminal metadata duration unresolved; reusable protected secret retention after terminal state is forbidden merely for history. | Factor ID/type/status/times/policy/reason and bounded label; lifecycle evidence must exclude decryptable secret, provisioning URI/key, submitted OTP. | Enrollment/cancel/revoke/failure audit exists; generic revoke accepts every owned non-revoked source state; disable/re-enable, terminal secret erasure, and cleanup absent. | `AUTH-LC-CHOICE-005`; #394/#465/#776. Secret-management/schema/security gates. Accept enrollment/cancel races, source-state guards, atomic secret erasure/rotation, last-factor/recovery, sessions, retry and audit-redaction. |
...
| --- | --- | --- | --- |
| `AUTH-LC-CHOICE-001` | What exactly do account disable, re-enable, and deletion do to sessions/families, outstanding MFA/passkey challenges, reset/invitation material, identities, password/factors, roles, profile/domain data, evidence/copies, notifications, owner recovery, and re-entry? (`ACC-001/002/003`) | Disable/delete markers and unavailable-account consumers exist, but no authorized transition service, atomic cascade, re-enable/delete recovery posture, or last-owner policy exists; deletion remains distinct from profile deletion and physical purge. Anonymous TOTP/recovery challenge verification also does not reload account availability after issuance. | Do not expose mutation or authorize physical deletion. #788/#785/#394/#724; manual owner-lockout/auth/privacy/schema/destructive gates. Blocks server/contract/admin UI and #975 acceptance; acceptance must cover disable/delete-between-challenge-and-verification, dependencies/holds/copies, and restore denial. |
| `AUTH-LC-CHOICE-002` | How is password replacement made atomic under concurrent change/reset, when may a credential be disabled, re-enabled, or revoked, what happens to sessions, and when is verifier material erased? (`PWD-001/003/004`) | Concurrent changes can both verify the same predecessor and report success while last-writer verifier state wins; lifecycle runtime for disable/re-enable/revoke and terminal-material retention are absent. | Treat replacement concurrency as partial; do not expose lifecycle mutation or retain verifier solely as history. #339/#788; auth/security, schema/privacy gates. |
...
| `AUTH-LC-CHOICE-005` | What anti-lockout, policy-dependent step-up, completion-time policy revalidation, allowed source-state, factor eligibility, concurrency/idempotency, session/challenge response, and terminal material-erasure rules apply to passkey/TOTP/recovery enroll, disable, re-enable, revoke, replace, use, expiry, and depletion? (`PKY-*`, `TOTP-*`, `RCV-*`, passkey `CHL-002`) | Current sequential lifecycle paths work, but enrollment/sign-in/step-up completion does not recheck passkey policy, sign-in discards the new session credential, passkey enrollment collision checking is not an atomic uniqueness transition, passkey/factor revoke accepts all non-revoked states, recovery generation requires enrolled TOTP rather than any factor, recovery revoke rewrites every owned batch state, concurrent TOTP begin/enroll/cancel and recovery generation/use lack atomic guards, and last-factor/session/terminal-secret policy is unproved. | Keep precise current behavior and classify handoff, policy, uniqueness, eligibility, broad-state, repeat, and race outcomes partial; do not add disable/re-enable/admin reset/expiry runtime or claim secret disposition. #394/#775/#776/#465 and `REC-AUTH-PASSKEY-SESSION-001`; manual auth/recovery/secret/schema gate. |
| `AUTH-LC-CHOICE-006` | How are challenge proof uniqueness, attempt counting, and terminal transitions made atomic, how long is safe metadata retained, and how is cleanup audited? (`CHL-*`) | The same current TOTP can satisfy separate pending challenges because accepted timestep/proof identity is not persisted; concurrent invalid attempts can overwrite the same increment and defer blocking; challenge completion uses ordinary tracked updates without a concurrency token/status-qualified consume. No retention clock, hold, cleanup, or copy-disposition rule exists. | Treat proof replay, attempt limits, and completion as sequentially implemented but security/concurrency-partial; terminal challenges stay unusable after commit; no purge authorization. #394 plus auth-retention follow-up; security/privacy/destructive gate. |
...
| `AUTH-LC-CHOICE-010` | How are refresh rotation and session/family revocation serialized, how must session validation respond to family status, how long are terminal lineage records retained, and when may sensitive lookup hashes be compacted? (`SES-*`, `REF-*`) | Revocation selects existing credentials/sessions before save; overlapping rotation can insert a replacement session outside those sets, and access-session validation does not consult family status. Post-terminal retention and copy disposition are also undefined. | Describe authority revocation as concurrency-partial; keep terminal rows unusable and authorize no cleanup. #338/#1059/#724; auth/security/privacy/destructive gate with relational rotation-versus-revoke tests. |
...
| Account/credential/identity/role lifecycle decisions | #788/#785/#787/#339/#724; `auth-session-security`, later split schema/contract/admin UI/retention lanes | Resolve choices 001–004 with last-owner/recovery safety, account deletion distinct from profile/domain-data deletion, and no implied physical purge. Manual auth/owner-lockout/privacy/destructive gates. |
| Factor/passkey/recovery lifecycle and terminal material | #394/#775/#776/#465; `auth-session-security`, split schema/contract/client lanes | Resolve enrollment/cancel concurrency, recovery's TOTP-only prerequisite, broad revoke states, and terminal material under choice 005 without recreating merged runtime. Manual recovery/secret/security gates. |
| Challenge retention/abuse | `REC-AUTH-MFA-SIGNIN-001` and `REC-AUTH-ABUSE-001`; docs decision then split runtime/config lanes | Resolve binding, cross-challenge TOTP-proof replay, atomic attempt increments/blocking, expiry/replay/completion and cleanup choices. Manual security/proxy gates. |
...
This document must retain exactly 57 unique `AUTH-LC-*` transition rows and 13
