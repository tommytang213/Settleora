...
- Settlement request/payment/proof attachment plus request-line/allocation/residual schema exists as a persistence foundation. [Settlement runtime architecture](SETTLEMENT_RUNTIME_ARCHITECTURE.md) defines the design gate for request creation, payment claims, receiver confirmation, dispute, cancellation, proof linkage, audit, and rebuildable balance projections. Candidate preview, first settlement request creation, read-only current-actor settlement request list/get, debtor-authored payment claim, receiver-authored payment confirmation, bounded request/payment dispute endpoints, guarded request/payment cancellation endpoints, and purpose-specific proof endpoints now exist, but basket selection runtime, residual creation/confirmation behavior, balance projection endpoints, OCR, broader business audit implementation, sync, passkeys, MFA, reset tokens, and recovery codes do not exist yet.
...
- `settlement_request_lines`: schema-only basket line rows linked to settlement requests and source expense bills, with optional bounded candidate key, positive exact amount, currency, allocation order, constrained line status, timestamps, indexes, and restrictive foreign keys.
- `settlement_payment_allocations`: schema-only payment allocation rows linked to settlement payments and request lines, with positive cleared amount, currency, allocation order, timestamp, lookup indexes, and restrictive foreign keys.
...
The schema foundation by itself does not authorize public runtime behavior; the existing auth/session runtime, business authorization service, self-profile, self payment-details, self payment QR, group, group-member, admin local-user, internal local file-object storage foundation, internal bill calculation/split service, settlement candidate preview, settlement request creation, settlement request list/get, settlement payment claim, settlement payment confirmation, settlement request/payment dispute, and settlement request/payment cancellation endpoints are separate API/domain layers. Settlement schema tables do not by themselves add basket selection runtime, residual handling runtime, balance projection endpoints, proof upload/download bytes beyond the purpose-specific proof endpoints, receipt upload/download runtime, OCR behavior, notifications, or UI behavior. The settlement runtime architecture document is a policy gate for further settlement implementation. Invitations, friends, broader business endpoints, counterparty QR reads, generic file APIs, item-split workflows beyond the internal calculation service, settlement proof workflows beyond cancellation, OCR, sync, and reporting are not implemented yet.
...
- Settlement basket/residual runtime, balance projections, approvals, status history, and audit records beyond the current schema foundation.
...
- Settlement basket/residual runtime workflows, balance projection endpoints, settlement proof upload/download bytes beyond the purpose-specific proof endpoints, expense/bill runtime beyond implemented slices, item-split workflows beyond the internal calculation service, public file workflows, OCR, audit, sync, or additional identity/session persistence beyond the listed schema foundations.
