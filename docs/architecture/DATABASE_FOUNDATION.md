...
- Settlement request/payment/proof attachment schema exists as a persistence foundation. [Settlement runtime architecture](SETTLEMENT_RUNTIME_ARCHITECTURE.md) defines the design gate for request creation, payment claims, receiver confirmation, dispute, cancellation, proof linkage, audit, and rebuildable balance projections. Candidate preview, first settlement request creation, read-only current-actor settlement request list/get, debtor-authored payment claim, receiver-authored payment confirmation, bounded request/payment dispute endpoints, and guarded request/payment cancellation endpoints now exist, but proof upload/download bytes, OCR, broader business audit implementation, sync, passkeys, MFA, reset tokens, and recovery codes do not exist yet.
