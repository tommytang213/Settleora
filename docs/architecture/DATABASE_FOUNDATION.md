...
- `settlement_payment_allocations`: payment allocation rows linked to settlement payments and request lines, with positive cleared amount, currency, allocation order, timestamp, lookup indexes, and restrictive foreign keys. Current runtime writes these rows for the same-currency single selected request-line payment claim flow.
- `settlement_residuals`: schema-only underpayment/overpayment rows linked to a settlement payment and/or request, debtor and creditor profiles, constrained direction/policy/status values, positive amount, currency, optional bounded reason, lifecycle timestamps, counterparty distinction and payment-or-request presence checks, projection-oriented indexes, and restrictive foreign keys.

The schema foundation by itself does not authorize public runtime behavior; the existing auth/session runtime, business authorization service, self-profile, self payment-details, self payment QR, group, group-member, admin local-user, internal local file-object storage foundation, internal bill calculation/split service, settlement candidate preview, settlement request creation with one selected request line, settlement request list/get, settlement payment claim with allocation persistence, settlement payment confirmation, settlement request/payment dispute, settlement request/payment cancellation endpoints, read-only settlement balance projection endpoint, and read-only settlement basket preview endpoint are separate API/domain layers. Settlement schema tables do not by themselves add basket write runtime, residual handling runtime, balance projection writes/caches beyond the current read-only endpoint, proof upload/download bytes beyond the purpose-specific proof endpoints, receipt upload/download runtime, OCR behavior, notifications, or UI behavior. The settlement runtime architecture document is a policy gate for further settlement implementation. Invitations, friends, broader business endpoints, generic file APIs, item-split workflows beyond the internal calculation service, basket write/residual workflows beyond the current projection and preview reads, OCR, sync, and reporting are not implemented yet.
...
- Settlement basket write/residual runtime beyond the current read-only preview, balance projection caches or writes beyond the current read-only endpoint, approvals, status history, and audit records beyond the current schema foundation.
...
- Settlement basket write/residual runtime workflows beyond the current read-only preview, balance projection writes/caches beyond the current read-only endpoint, settlement proof upload/download bytes beyond the purpose-specific proof endpoints, expense/bill runtime beyond implemented slices, item-split workflows beyond the internal calculation service, public file workflows, OCR, audit, sync, or additional identity/session persistence beyond the listed schema foundations.
