...
- Settlement request/payment/proof attachment schema exists as a persistence foundation. [Settlement runtime architecture](SETTLEMENT_RUNTIME_ARCHITECTURE.md) defines the design gate for request creation, payment claims, receiver confirmation, dispute, cancellation, proof linkage, audit, and rebuildable balance projections. Candidate preview, first settlement request creation, and read-only current-actor settlement request list/get endpoints now exist, but settlement payment claim, partial payment, confirmation, dispute, cancellation, proof upload/download bytes, OCR, broader business audit implementation, sync, passkeys, MFA, reset tokens, and recovery codes do not exist yet.
...
The schema foundation by itself does not authorize public runtime behavior; the existing auth/session runtime, business authorization service, self-profile, self payment-details, self payment QR, group, group-member, admin local-user, internal local file-object storage foundation, internal bill calculation/split service, settlement candidate preview, settlement request creation, and settlement request list/get endpoints are separate API/domain layers. Settlement schema tables do not by themselves add mark-paid, confirm, dispute, cancel, proof upload/download bytes, receipt upload/download runtime, OCR behavior, notifications, or UI behavior. The settlement runtime architecture document is a policy gate for further settlement implementation. Invitations, friends, broader business endpoints, counterparty QR reads, generic file APIs, item-split workflows beyond the internal calculation service, settlement payment/confirmation/dispute/cancel/proof workflows beyond first request creation/list/get, OCR, sync, and reporting are not implemented yet.
