...
- Guarded self-profile, self payment-details, self payment QR, group foundation, group member management, bill attachment, bill-scoped receipt OCR review intake/apply-preview/draft-apply, recurring bill template/forecast/draft-generation, and admin local-user endpoints exist as narrow API layers on the current user/group/payment/file metadata foundations.
...
- `recurring_bill_templates`: API-owned recurring bill template rows linked to the owner and creator `user_profiles`, optional `user_groups`, bounded merchant/name/description fields, date-based schedule fields, constrained status, next occurrence date, forecast amount/currency snapshot, payload version, and a bounded canonical template payload JSON value. The payload is generation configuration only and is revalidated before draft generation; generated `expense_bills` remain financial truth.
- `recurring_bill_occurrences`: API-owned occurrence tracking rows linked to `recurring_bill_templates`, with occurrence date, optional due date, constrained occurrence status, optional generated draft `expense_bills` reference, generated actor/timestamps, and a unique template/date constraint for idempotent explicit draft generation.
- `notification_global_policies`: admin/global notification policy root rows with policy version, status, bounded channel caps, bounded provider-readiness categories, required in-app posture, external content safety class, quiet-hours/digest default readout modes, timestamps, and optional creator/updater auth-account references. This table stores no provider secrets, provider config values, SMTP hostnames/usernames, APNs/FCM credentials, raw device tokens, provider payloads, raw OCR text, storage internals, payment details, private notes, hidden bill data, or auth/session token material.
- `notification_event_policy_overrides`: bounded event-family override rows linked to `notification_global_policies`, with event-family category, channel caps, external content class, required in-app posture, digest eligibility, quiet-hours eligibility, and timestamps. Overrides are category/readout state only and do not create notification writers, provider attempts, source events, or delivery success.
...
The schema foundation by itself does not authorize public runtime behavior; the existing auth/session runtime, business authorization service, self-profile, self payment-details, self payment QR, group, group-member, bill attachment, receipt OCR review intake/apply-preview/draft-apply, recurring bill template/forecast/draft-generation, admin local-user, internal local file-object storage foundation, internal bill calculation/split service, settlement candidate preview, settlement request creation with selected request lines, settlement request list/get, settlement payment claim with allocation and pending residual persistence, settlement payment confirmation, settlement payment residual confirmation, bounded receiver-confirmed residual projection/finalization, settlement request/payment dispute, settlement request/payment cancellation endpoints, read-only settlement balance projection endpoint, read-only settlement basket preview endpoint, pay-all settlement basket creation endpoint, and internal residual policy decision foundation are separate API/domain layers. Settlement schema tables do not by themselves add broad credit ledgers, refund workflows, balance projection writes/caches beyond the current read-only endpoint, proof upload/download bytes beyond the purpose-specific proof endpoints, standalone receipt/OCR upload/download runtime outside bill attachments, OCR engine/worker behavior, automatic OCR-to-bill finalization beyond explicit draft-only receipt OCR review apply, recurring auto-generation workers, notifications, reminders, or UI behavior. The settlement runtime architecture document is a policy gate for further settlement implementation. Invitations, friends, broader business endpoints, generic file APIs, item-split workflows beyond the internal calculation service, residual workflows beyond payment-claim-time proposal persistence, receiver residual confirmation, and bounded confirmed residual projection/finalization, OCR engine/worker behavior, advanced recurrence exceptions, sync, and reporting are not implemented yet.
...
- Reimbursements and reconciliation records.
- Advanced recurring exception calendars, recurring reminder queues, and background recurring generation jobs.
...
- OCR-derived data remains provisional until explicit user action and API validation; current receipt OCR review intake rows are not authoritative bill, split, settlement, balance, or payment records until the draft-only apply operation creates API-validated OCR-sourced draft item candidates.
...
- Settlement residual workflows beyond payment-claim-time pending residual proposal persistence, receiver residual confirmation, and bounded confirmed residual projection/finalization, balance projection writes/caches beyond the current read-only endpoint, settlement proof upload/download bytes beyond the purpose-specific proof endpoints, expense/bill runtime beyond implemented slices, recurring bill background generation/reminder workflows beyond explicit draft generation, item-split workflows beyond the internal calculation service, public file workflows, OCR engine/worker behavior or automatic OCR-to-bill finalization beyond explicit draft-only receipt OCR review apply, broader audit, sync, or additional identity/session persistence beyond the listed schema foundations.
