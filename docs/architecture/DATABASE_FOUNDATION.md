...
- `settlement_residuals`: underpayment/overpayment rows linked to a settlement payment and request, debtor and creditor profiles, constrained direction/policy/status values, positive amount, currency, optional bounded reason, lifecycle timestamps, counterparty distinction and payment-or-request presence checks, projection-oriented indexes, and restrictive foreign keys. Current runtime can create pending residual rows from explicit payment-claim residual proposals and can neutralize pending rows to disputed or cancelled during dispute/cancellation.

The schema foundation by itself does not authorize public runtime behavior; the existing auth/session runtime, business authorization service, self-profile, self payment-details, self payment QR, group, group-member, admin local-user, internal local file-object storage foundation, internal bill calculation/split service, settlement candidate preview, settlement request creation with selected request lines, settlement request list/get, settlement payment claim with allocation and pending residual persistence, settlement payment confirmation, settlement request/payment dispute, settlement request/payment cancellation endpoints, read-only settlement balance projection endpoint, read-only settlement basket preview endpoint, pay-all settlement basket creation endpoint, and internal residual policy decision foundation are separate API/domain layers. Settlement schema tables do not by themselves add receiver residual confirmation, confirmed credit/waiver finalization, balance projection writes/caches beyond the current read-only endpoint, proof upload/download bytes beyond the purpose-specific proof endpoints, receipt upload/download runtime, OCR behavior, notifications, or UI behavior. The settlement runtime architecture document is a policy gate for further settlement implementation. Invitations, friends, broader business endpoints, generic file APIs, item-split workflows beyond the internal calculation service, residual workflows beyond payment-claim-time proposal persistence, OCR, sync, and reporting are not implemented yet.
...
- Receiver residual confirmation, confirmed residual credit/waiver finalization, balance projection caches or writes beyond the current read-only endpoint, approvals, status history, and audit records beyond the current schema foundation.
