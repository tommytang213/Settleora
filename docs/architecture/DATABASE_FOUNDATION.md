...
- Guarded self-profile, self payment-details, self payment QR, group foundation, group member management, bill attachment, bill-scoped receipt OCR review intake/apply-preview/draft-apply, and admin local-user endpoints exist as narrow API layers on the current user/group/payment/file metadata foundations.
...
- Settlement residual workflows beyond payment-claim-time pending residual proposal persistence, receiver residual confirmation, and bounded confirmed residual projection/finalization, balance projection writes/caches beyond the current read-only endpoint, settlement proof upload/download bytes beyond the purpose-specific proof endpoints, expense/bill runtime beyond implemented slices, item-split workflows beyond the internal calculation service, public file workflows, OCR engine/worker behavior or automatic OCR-to-bill finalization beyond explicit draft-only receipt OCR review apply, broader audit, sync, or additional identity/session persistence beyond the listed schema foundations.
