...
- Settlement request/payment/proof attachment schema exists only as a persistence foundation. [Settlement runtime architecture](SETTLEMENT_RUNTIME_ARCHITECTURE.md) now defines the design gate for future request creation, payment claims, receiver confirmation, dispute, cancellation, proof linkage, audit, and rebuildable balance projections, but settlement runtime endpoints, OpenAPI paths, generated clients, proof upload/download bytes, OCR, business audit implementation, sync, passkeys, MFA, reset tokens, and recovery codes do not exist yet.
