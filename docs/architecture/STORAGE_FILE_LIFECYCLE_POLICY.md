...
| `FILE-LC-PKG-019` | Confirm persisted CSV import session | Current API sequentially accepts `ready_for_confirmation → confirmed` after challenge and current authorization/group membership, then deserializes the stored candidate plan and creates draft bills plus bill audit rows in the same save boundary. It does not rebuild/revalidate candidates against current participants or calculation policy. | Neither file object nor subject link; import-session state and authoritative bill-domain rows change. | Sequential further confirmation stops. Without a concurrency token/status predicate, racing confirm versus discard/lazy expiry can nevertheless create bills after another request observed a terminal transition, or a later terminal save can leave a misleading discarded/expired status after bill creation. `ReviewJson` and `CandidateJson` remain stored. |
...
| --- | --- | --- | --- | --- |
| `FILE-LC-OBJ-001` | Active actor/account; nonempty owner ID; supported purpose/provider category; bounded metadata. Current endpoints pass the actor's own profile, but the service does not require owner = actor or independently authorize a different supplied owner. Duplicate/replay semantics absent. | Creation is not reactivation; failure routes to `OBJ-003`. | Subject intent and self ownership are checked by current endpoint flows before the service call, but reservation has no persisted subject/idempotency link and future callers must not rely on the service as the ownership boundary. | Partial: `EfFileObjectLifecycleService.CreatePendingAsync`; endpoint/service tests. |
...
| `FILE-LC-PKG-019` | Same actor/auth session; exact unexpired `ready_for_confirmation`; matching scope/group/digest/version/challenge; current authorization/membership; nonempty stored candidates. Stored candidates are deserialized, not rebuilt against current participant/calculation policy. | Terminal session status plus newly authoritative draft bills; it is not file-object activation or restore. | Stored candidate bill graph, current actor/group authorization, challenge fields, bill validation/audit and transaction outcome. JSON payloads remain after success. | Implemented in `ConfirmBillImportSessionAsync`; no concurrency token/status-predicate write or request idempotency. Racing confirm/discard/expiry can mutate bills after a terminal observation or leave a terminal status after bills were created, depending on commit order. #406/#971/#724. `FILE-LC-CHOICE-001/006`. |
...
| `FILE-LC-PKG-019` | Authority/current evidence: `ConfirmBillImportSessionAsync`. | **Confirm import** and report created draft-bill summaries; never claim retained review/candidate data was erased or freshly recalculated. | Persisted/current: `confirmed`, confirmed/update times and unchanged review/candidate JSON; authoritative draft bills and per-bill import audit are added in one save boundary. No session lifecycle audit/version. Stored candidates are deserialized after current actor/group authorization, not rebuilt against changed participant/calculation policy. | Sequential replay conflicts as not confirmable. Concurrent confirm/discard/expiry requests can all act on a previously loaded ready row; without a concurrency token/status predicate, commit ordering can create bills after a terminal observation or leave terminal status after creation. `CHOICE-001/006`. |
