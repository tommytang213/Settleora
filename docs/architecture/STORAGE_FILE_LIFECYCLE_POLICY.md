...
- A non-cancellation storage-write failure triggers a best-effort attempt to
  ...
  a deterministic release time, controls its disposal. Association-save failure
...
| `FILE-LC-LINK-001` | **Attach receipt image** | Current bill API creates an active `ExpenseBillAttachment` after object activation, bill mutation authorization/status checks, `receipt_image` purpose/image-media validation, and duplicate-pair checks. It does not enforce an attachment-count capacity. | Receipt bill link only; object remains `active`. | Current authorized bill viewers may list/read under bill policy; the link may later own restrictive OCR review/assignment dependencies. Link creation never broadens object ownership. |
| `FILE-LC-LINK-011` | **Attach supporting file** | Current bill API creates an active `ExpenseBillAttachment` after object activation, bill mutation authorization/status checks, `supporting_attachment` purpose/image-or-PDF validation, and duplicate-pair checks. It does not enforce an attachment-count capacity. | Supporting bill link only; object remains `active`. | Current authorized bill viewers may list/read under bill policy. Supporting links do not enter receipt OCR review/assignment flows, and link creation never broadens object ownership. |
...
| `FILE-LC-PKG-020` | Discard persisted CSV import session | Current API changes eligible `ready_for_confirmation|needs_correction → discarded` after applying lazy expiry and records `DiscardedAtUtc`. | Neither file object nor subject link; persisted import-session state only. | Confirmation stops. The row and both JSON payload fields remain stored, but the same-actor/session GET exposes only a response reconstructed from `ReviewJson`; `CandidateJson` is not an API-readable terminal payload. Discard is not erasure. |
| `FILE-LC-PKG-021` | Lazy persisted CSV import-session expiry | A later same-actor/session get, discard, or confirm changes eligible `ready_for_confirmation|needs_correction → expired` when the stored deadline has passed. | Neither file object nor subject link; persisted import-session state only. | Confirmation/discard transition stops. Both JSON fields remain stored; same-actor/session GET returns review-derived response data, not `CandidateJson`. No background sweep or disposal exists. |
...
| `FILE-LC-PKG-023` | Direct CSV import | Current personal/group direct import reads and parses a request-scoped CSV and, after current authorization/validation, creates draft bills plus per-bill import audits in one save boundary without a session or confirmation step. | Neither file object nor subject link; authoritative bill-domain/audit rows change while request-memory CSV/plan copies remain transient. | Created drafts follow bill lifecycle. No idempotency key or retained source-file relationship exists; replay can create another set of drafts, and GC/zeroization timing for request copies is unproved. |
...
| `FILE-LC-LINK-001` | Active compatible `receipt_image` object; uploader and current bill mutation authorization; mutable bill status, image media, and no duplicate bill/file pair; same transaction or recoverable compensation. Current runtime has no attachment-count capacity check. | Link may later be removed; receipt attachment creation is not object creation. | Bill ownership/visibility/status, receipt purpose, existing bill links, restrictive OCR review/assignment and notification dependencies. | Implemented for receipt attachments, with association-after-activation compensation and attachment-count quota gaps. #341/#966. |
| `FILE-LC-LINK-011` | Active compatible `supporting_attachment` object; uploader and current bill mutation authorization; mutable bill status, accepted image/PDF media, and no duplicate bill/file pair; same transaction or recoverable compensation. Current runtime has no attachment-count capacity check. | Link may later be removed; supporting attachment creation is not object creation. | Bill ownership/visibility/status, supporting purpose and existing bill/history links; receipt OCR dependencies are inapplicable. | Implemented for supporting attachments, with association-after-activation compensation and attachment-count quota gaps. #341/#966. |
...
| `FILE-LC-PKG-023` | Authenticated actor; personal-profile or current group authorization/membership; bounded valid CSV; import plan succeeds; database save accepts draft bills/audits. | Created drafts follow bill lifecycle; the transient source cannot be restored through file lifecycle. | Current actor/group, request CSV/plan, draft bill graph, bill audit, transaction outcome and replay/duplicate risk. | Implemented in personal/group `import.csv`; no source file/session, confirmation, idempotency key, or request-buffer disposal proof. #406/#971/#724 and bill-domain owners. `FILE-LC-CHOICE-006`. |
...
| `PKG-018..023` | Persisted session review/candidate JSON and row remain after confirmation, discard, and lazy expiry; same-actor/session responses expose review-derived data but not raw `CandidateJson`. Current 30-minute expiry is a confirmation-availability clock, not payload retention or disposal. Direct preflight/import request copies are transient with GC-controlled release; direct import creates authoritative drafts/audits immediately and has no source session or confirmation. | Session creation/status changes have no dedicated lifecycle audit. Session confirmation and direct import write bounded per-bill audit, but never log raw CSV, review/candidate JSON, private rows, digest/challenge values, or imported contents. | #406/#971 import lane; #724 retention; relevant bill-domain owners for confirmation/direct import. API/privacy, persistence, audit/retention and user data-ingress gates remain pending for changed behavior or disposal. |
| `CLN-003..005`, `LOCAL-001..002` | Generation/buffering is non-destructive. Transient hard delete is eligible only after positive non-authoritative proof; exact clock unresolved. Managed request buffers have no deterministic release/zeroization. Never cleanup authoritative/history content. | Local and request-memory audit applicability and zeroization proof are unresolved; server cleanup needs bounded result without local paths, object keys, content or private queue data. | #358 receipt intake, #971 local/offline/import, #1062 derivatives/security, #724 retention, #966 storage; provider/destructive/local-data gates pending as applicable. |
...
| `FILE-LC-CLN-005` | `FILE-LC-GATE-001`, `003`, `006`, `008`, `010` |
...
| `FILE-LC-PKG-022..023` | `FILE-LC-GATE-001`, `003`, `006`, `008` |
...
| `FILE-LC-PKG-020` | Authority/current evidence: `DiscardBillImportSessionAsync` and session GET mapping. | **Discard import**; never claim stored review/candidate data or bills were deleted. | Persisted/current: `discarded`, discarded/update times and unchanged review/candidate JSON. GET returns review-derived response data, not stored `CandidateJson`. No lifecycle audit or disposal outcome. | Repeat/terminal call returns current review mapping; candidate use is blocked by terminal status; confirm/discard/expiry race has no explicit expected-version result. `CHOICE-001/006`. |
| `FILE-LC-PKG-021` | Authority/current evidence: `ExpireSessionIfNeeded` on get/discard/confirm and session GET mapping. | **Import session expired** only after later access detects the deadline. | Persisted/current: `expired` and update time; row plus review/candidate JSON remain. GET returns review-derived response data, not stored `CandidateJson`. No sweep, payload clearing, expiry event time or lifecycle audit. | Later GET returns the terminal review mapping and confirm conflicts; candidate use is blocked by terminal status; confirm/discard/expiry ordering has no explicit concurrency token. `CHOICE-001/006`. |
...
| `FILE-LC-PKG-023` | Authority/current evidence: personal/group `Import*BillsCsvAsync`. | **Import draft bills** on the authorized bill surface; never imply a staged session or confirm step occurred. | Current: draft bills and per-bill audit commit together; raw CSV/plan stay request-scoped and are not persisted as a source. Missing source relationship, lifecycle audit, idempotency and request-buffer disposal proof. | No key; replay can create another set of draft bills; database write failure returns bounded failure but no durable prior-result contract exists. `CHOICE-006`. |
