...
- Current OpenAPI and generated clients expose subject-specific bill
  ...
  package artifacts in process memory, permits an authenticated one-use content
  download while its on-access validity checks pass, and parses submitted
...
| Generated backup package | `export_file` is the existing architectural category for a persisted backup/export bundle; package manifests must distinguish backup from ordinary export without inventing a second file purpose. | Current API creates process-local data-only package artifacts, serves authenticated one-use content downloads while lazy validity checks pass, and parses restore previews; it does not create a `FileObject`, write through storage, include file bytes, or apply restore mutations. An unaccessed artifact is not swept and can remain in memory until process restart. | Current source defines access-validity TTLs, not guaranteed memory disposal; missing eager cleanup is a retention gap. Any future persisted package is dependency-gated and requires a destructive/manual disposal decision. Persisted duration, encryption, copy inventory, and recovery hold remain unresolved. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-LINK-001` | Active compatible object; uploader and current subject mutation authorization; subject status/capacity; no duplicate; same transaction or recoverable compensation. | Link may later be removed; attachment creation is not object creation. | Bill/payment and purpose-specific ownership; existing links and OCR constraints. | Implemented for bill/proof, with association-after-activation compensation gaps. #341/#966. |
...
| `LINK-001`, `LINK-005` | Retain with active subject/history. No hard delete/purge from attach. | Current attach audit is implemented for bill/proof/payment detail with bounded metadata; public DTO privacy tests exclude storage internals. | #341/#966 and subject-domain owners. Runtime storage/privacy gate remains for changes. |
...
| --- | --- |
| `FILE-LC-OBJ-001..003`, `FILE-LC-LINK-001`, `FILE-LC-LINK-005`, `FILE-LC-CLN-001` | `FILE-LC-GATE-001`, `002`, `003`, `006`, `007` |
...
| `FILE-LC-LINK-001` | Authority: subject-domain and metadata architecture. Current evidence: bill/proof handlers, link entities/config/tests. | **Attach receipt**, **Add supporting file**, or **Add proof** on its exact subject surface. | Applicable/current: bill/proof link creator/time plus subject audit. Missing: common link ID/version/idempotency and atomic provider/link step record. | No key; retry can create a new file/link; composite keys prevent only the same file/subject pair; timeout result is unknown; concurrent capacity is re-queried but has no general version contract. `CHOICE-004`. |
...
| `FILE-LC-LINK-002` | Authority: bill/settlement domain plus shared unlink rule. Current evidence: bill/proof removal handlers/tests. | **Remove attachment** or **Remove proof** on the exact bill/payment surface; must not say bytes are deleted. | Applicable/current: link `RemovedAtUtc`, subject removal audit, and best-coupled object deletion metadata. Missing: link actor/reason/version and complete other-reference proof. | No key; replay is normally unavailable rather than prior success; retry after timeout cannot prove which step saved; concurrent OCR/link/object change has no explicit version conflict. `CHOICE-002/004`. |
...
| `FILE-LC-LINK-004` | Authority: shared restore/unlink taxonomy plus owning bill/settlement/payment policy; no runtime evidence. | **Restore attachment**, **Restore proof**, or **Reattach QR** on future subject/Trash surface after file restore. | Applicable/required: exact subject/file/purpose, requester/reason, restored time, source/target link state, expected version and separate audit. Absent. | Required key/version; identical replay returns same link; changed subject/file conflicts; replacement/object/subject-state races block. `CHOICE-002`. |
...
| `FILE-LC-CHOICE-006` — How are persisted export versus backup packages distinguished within `export_file`, encrypted, expired, downloaded, and copy-disposed? | Sync/import/export/restore, storage, privacy/vault and backup recovery. | Current exports stream. Backup package generation/download and restore-preview parsing use process-local data-only artifacts and metadata; TTL checks are lazy and no background sweep/removal guarantees memory disposal. There is no `FileObject`/storage relationship, file-byte inclusion, persisted-copy inventory, encryption/disposition contract, or restore apply. | Owner-only, no persisted-package claim, no automatic disposal. Current lazy availability expiry is neither guaranteed in-memory cleanup nor a retention decision for a future stored package. | #406/#971/#724/#966; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `008`. | Current in-memory cleanup acceptance plus stored export/backup generation, persisted download/copy inventory, expiry, recovery and copy-disposition acceptance. |
