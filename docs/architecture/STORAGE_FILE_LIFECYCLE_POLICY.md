...
| `FILE-LC-LINK-001` | **Attach receipt/supporting file** | Current bill API creates an active `ExpenseBillAttachment` after object activation, bill mutation/capacity checks, and receipt/supporting purpose validation. | Bill link only; object remains `active`. | Current authorized bill viewers may list/read under bill policy. Link creation never broadens object ownership. |
...
| `FILE-LC-LINK-001` | Active compatible receipt/supporting object; uploader and current bill mutation authorization; mutable bill status, attachment capacity, no duplicate; same transaction or recoverable compensation. | Link may later be removed; bill attachment creation is not object creation. | Bill ownership/visibility/status, receipt/supporting purpose, existing bill links, OCR and notification constraints. | Implemented for bill attachments, with association-after-activation compensation gaps. #341/#966. |
...
| `FILE-LC-PKG-002` | Same actor/session; current `created|ready_to_download`; current profile access; bounded package generation. Current reprepare can replace the artifact while old unexpired actions remain. | Cancel or lazy expiry clears the server reference; downloaded copies are outside server authority. | Current database snapshot, artifact format/version, actor/session, existing actions, cancellation/expiry race and private-data bounds. | Implemented in `PreparePackageSessionAsync`; no durable step/idempotency/audit or eager cleanup. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-010` | Same actor/session and `created|ready_to_download`; current authorization or package-generation validation denies preparation. The exact branch controls whether prior artifact/actions remain. | `blocked` has no re-entry transition; create a new session. A preserved artifact may age out only when later artifact/action/content access invokes lazy artifact expiry. | Authorization/validation result, source state, prior artifact/actions and concurrent reprepare/cancel/download. | Implemented branches in `PreparePackageSessionAsync`; blocked is excluded from cancel/discard/session-expiry transitions, and no durable audit exists. #406/#971/#724. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-012` | Authenticated actor; bounded request/package sizes; valid JSON, package format/version/manifest/hash, bounded section count/state and expiry. Current parsing does not enforce a section-name allowlist or prove that the declared section inventory matches `data`. | Preview may be discarded or lazily expire; recreation parses a new request. | Submitted package validity, actor/session, package expiry, bounded summary and concurrent duplicate requests. Raw submitted content is not retained in the preview object. Missing section-name/inventory validation remains a blocker for treating preview acceptance as complete package validation. | Partial in `CreateRestorePreviewAsync`; process-local, no durable audit/idempotency, section allowlist, or inventory-to-data proof. #406/#971/#724. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-002` | Authority/current evidence: `PreparePackageSessionAsync` and data-only artifact tests. | **Prepare backup** / **Ready to download** on that session only. | Process-local/current: status, artifact bytes, format/version, safe filename, generation/expiry and row counts. No durable audit; reprepare does not bind existing actions to an immutable artifact version. | No key; replay while ready rebuilds/replaces current bytes; old unexpired actions remain and can resolve the new current artifact; cancel/expiry races lack serialization. `CHOICE-006`. |
| `FILE-LC-PKG-010` | Authority/current evidence: authorization and generation-validation branches in `PreparePackageSessionAsync`. | **Backup preparation blocked** with a bounded safe failure code; no content action. | Process-local/current: `blocked` and safe failure code. Authorization failure leaves a prior artifact/actions intact; validation failure nulls/clears them. No durable audit or branch-neutral disposition result. | Repeated prepare returns blocked state; no retry transition exists. Concurrent reprepare/download/cancel ordering and preserved-artifact identity are not versioned; only later lazy artifact expiry can clear a preserved artifact. `CHOICE-006`. |
...
| `FILE-LC-CHOICE-006` — How are persisted export versus backup packages distinguished within `export_file`, encrypted, expired, downloaded, and copy-disposed? | Sync/import/export/restore, storage, privacy/vault and backup recovery. | Current direct exports stream with no server artifact or client-copy inventory. Backup generation/download uses process-local data-only artifacts; multiple action IDs can download one artifact, and one action blocks sequential reuse after consumption but can serve concurrent racing requests because check/update is not atomic. TTL checks are lazy, with no background sweep or package-session entry removal. Restore preview/confirmation entries retain package summaries after discard/expiry. Later access can clear artifact references and prune actions, but no path proves all memory/client copies disposed. There is no `FileObject`/storage relationship, file-byte inclusion, encryption/disposition contract, or restore apply. | Owner-only, no persisted-package claim, no automatic disposal. Current request/availability expiry is neither guaranteed in-memory cleanup nor a retention decision for a future stored or downloaded package copy. | #406/#971/#724/#966; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `008`. | Current response/in-memory/downloaded-copy inventory and cleanup acceptance plus stored export/backup generation, expiry, recovery and copy-disposition acceptance. |
