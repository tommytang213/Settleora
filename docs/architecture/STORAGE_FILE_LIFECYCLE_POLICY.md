...
- Current OpenAPI and generated clients expose subject-specific bill
  ...
  backup package operations. The backup runtime holds short-lived sessions and
  data-only package artifacts in process memory, permits an authenticated
  one-use content download, and parses submitted package content into a bounded
  restore preview and metadata-only confirmation session. It does not persist a
...
| Generated backup package | `export_file` is the existing architectural category for a persisted backup/export bundle; package manifests must distinguish backup from ordinary export without inventing a second file purpose. | Current API creates short-lived process-local data-only package artifacts, serves authenticated one-use content downloads, and parses restore previews; it does not create a `FileObject`, write the artifact through storage, include file bytes, or apply restore mutations. | The current process-local artifact has source-defined short lifetimes; any future persisted package is dependency-gated and requires a destructive/manual decision for disposal. Persisted duration, encryption, copy inventory, and recovery hold remain unresolved. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-LINK-001` | Authority: subject-domain and metadata architecture. Current evidence: bill/proof/QR handlers, link entities/config/tests. | **Attach receipt**, **Add supporting file**, **Add proof**, or **Add QR image** on its exact subject surface. | Applicable/current: bill/proof link creator/time or profile link/update time plus subject audit. Missing: common link ID/version/idempotency and atomic provider/link step record. | No key; retry can create a new file/link; composite keys prevent only the same file/subject pair; timeout result is unknown; concurrent capacity/replace is re-queried but has no general version contract. `CHOICE-004`. |
...
| `FILE-LC-CHOICE-006` — How are persisted export versus backup packages distinguished within `export_file`, encrypted, expired, downloaded, and copy-disposed? | Sync/import/export/restore, storage, privacy/vault and backup recovery. | Current exports stream. Backup package generation/download and restore-preview parsing exist only as short-lived process-local data-only artifacts and metadata; there is no `FileObject`/storage relationship, file-byte inclusion, persisted-copy inventory, encryption/disposition contract, or restore apply. | Owner-only, no persisted-package claim, no automatic disposal. The current in-memory expiry is not a retention decision for a future stored package. | #406/#971/#724/#966; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `008`. | Stored export/backup generation, persisted download/copy inventory, expiry, recovery and copy-disposition acceptance. |
