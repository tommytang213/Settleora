...
- Storage-write failure triggers a best-effort attempt to mark the reserved
  object `upload_failed`, but each upload handler ignores that lifecycle result;
  metadata can remain `pending`. The local provider may already have created
  partial bytes and the endpoint does not remove them. Association-save failure
  triggers a best-effort attempt to
...
- Current OpenAPI and generated clients expose subject-specific bill
  ...
  not a server artifact, and has no downloaded-copy inventory. Persisted CSV
  import sessions retain their review/candidate JSON after confirmation,
  discard, or lazy expiry; no cleanup removes those fields or rows. The backup
  ...
  session. Its advertised TTLs are enforced lazily on later access; there is no
  ...
  discard or lazy expiry; their dictionaries have no removal sweep. It does not
...
| Imported-source file | `statement_upload` for a file-backed statement/import source. Package-local blobs remain untrusted package input until API acceptance; current CSV request bodies are not `file_objects`. | Enum reserved; CSV import sessions persist `ReviewJson` and `CandidateJson`, but no file-object statement/package link. Confirm, discard, and lazy expiry change status/timestamps without clearing those payload fields or deleting the row. | Required but duration unresolved. Current candidate/review data remains retained after confirmation, discard, or expiry until a separate authorized retention operation exists; those statuses are not disposal proof. |
...
| `FILE-LC-LINK-001` | **Attach receipt/supporting file** | Current bill API creates an active `ExpenseBillAttachment` after object activation, bill mutation authorization/status checks, and receipt/supporting purpose validation. It does not enforce an attachment-count capacity. | Bill link only; object remains `active`. | Current authorized bill viewers may list/read under bill policy. Link creation never broadens object ownership. |
...
| `FILE-LC-LINK-002` | **Remove bill attachment** | Current bill API sets the bill link `RemovedAtUtc`; it also calls object `active → deleted` in the save boundary. | Bill link and object currently change, but unlink and object removal remain conceptually distinct. | This bill link and all ordinary object reads stop. Other schema-possible references prevent any inference of physical disposability. |
...
| `FILE-LC-LINK-010` | **Replace QR image** | Current POST reserves, writes, and activates a new object, changes `QrFileObjectId` from old ID → new ID, saves, then attempts old object `active → deleted`; that later result is ignored. | New object plus QR link change first; old-object transition is separate and may not occur. | New QR becomes reachable under current policies; old QR loses profile reachability. Failed link save can strand the new object, and failed old-object transition can leave an active unlinked old object. |
| `FILE-LC-LINK-004` | **Restore bill attachment** | Proposed explicit removed → active bill relationship after object readiness and current bill authorization/status/capacity/purpose checks. | Bill link only; object restore is separately accepted first. | Bill access resumes only for authorized current bill viewers. OCR/notification dependencies and other links are unchanged. |
...
| `FILE-LC-PKG-012` | Create backup restore preview | Current API validates submitted package content and creates actor/session-bound `ready` preview metadata plus a bounded package summary in process memory. | Neither; process-local preview/summary entry only. No file object, link, provider write, or restore mutation. | Same authenticated profile/auth session may read it until discard/expiry; accepted preview never grants file/content restore authority. |
...
| `FILE-LC-PKG-015` | Create metadata-only restore confirmation session | From an actor/session-bound ready preview and exact confirmation contract, current API creates `metadata_only` confirmation metadata referencing the same package summary. It applies no restore. | Neither; process-local confirmation/summary entry only. | Same authenticated profile/auth session may read it. It cannot mutate business/file/link state and `CanApplyRestore` remains false. |
...
| `FILE-LC-PKG-019` | Confirm persisted CSV import session | Current API accepts exact `ready_for_confirmation → confirmed` after challenge, current authorization/group membership, and candidate revalidation, and creates draft bills plus bill audit rows in the same save boundary. | Neither file object nor subject link; import-session state and authoritative bill-domain rows change. | Further confirmation stops. `ReviewJson` and `CandidateJson` remain stored; confirmation is not payload disposal. |
| `FILE-LC-PKG-020` | Discard persisted CSV import session | Current API changes eligible `ready_for_confirmation|needs_correction → discarded` after applying lazy expiry and records `DiscardedAtUtc`. | Neither file object nor subject link; persisted import-session state only. | Confirmation stops, but the row and both JSON payload fields remain readable to the same actor/session. Discard is not erasure. |
| `FILE-LC-PKG-021` | Lazy persisted CSV import-session expiry | A later same-actor/session get, discard, or confirm changes eligible `ready_for_confirmation|needs_correction → expired` when the stored deadline has passed. | Neither file object nor subject link; persisted import-session state only. | Confirmation/discard transition stops. The row and both JSON payload fields remain; no background sweep or disposal exists. |
| `FILE-LC-PKG-003` | Cancel package generation/artifact | Current API accepts `created|ready_to_download → cancelled`, clears the artifact reference and all download actions. | Neither; process-local session/artifact/action state only. | Further ordinary artifact/download access stops. Clearing references is not guaranteed memory zeroization or downloaded-copy disposal. |
| `FILE-LC-PKG-004` | Discard unprepared package session | Current API accepts only `created → discarded`; no artifact bytes exist for this accepted path. | Neither; process-local session metadata only. | Further session generation is unavailable. The response must not imply deletion of bytes that were never created. |
| `FILE-LC-PKG-005` | Lazy package-session expiry | On a later session access, current API changes expired `created|ready_to_download → expired`, clears artifact reference and download actions. | Neither; process-local session/artifact/action state only. | Subsequent API access is unavailable; no background sweep or prompt memory-disposal proof exists. |
...
| `FILE-LC-PKG-007` | Issue package download action | For a current ready artifact, the API creates a new actor/session-scoped, expiring, unconsumed action; multiple actions can exist for one artifact. | Neither; process-local action metadata only. | Sequential use makes the action unavailable after consumption, but current non-atomic consumption does not prove single-use under concurrent requests. |
| `FILE-LC-PKG-008` | Consume package download action | Current API checks and then marks one valid action consumed before returning current artifact bytes, without an atomic compare-and-set or lock across that check/update. | Neither; process-local action flag changes and bytes egress to the client. | Sequential replay cannot read again, but concurrent requests can both pass the unconsumed check and return bytes; another action may also read. Server lifecycle cannot prove disposition of downloaded copies. |
| `FILE-LC-PKG-009` | Lazy download-action pruning | On later artifact/action/content access, current API removes consumed or expired action entries. | Neither; process-local action metadata only. | The removed action cannot authorize content. Pruning does not affect the artifact or any downloaded copy. |
...
| `FILE-LC-LINK-001` | Active compatible receipt/supporting object; uploader and current bill mutation authorization; mutable bill status and no duplicate of the same bill/file pair; same transaction or recoverable compensation. Current runtime has no attachment-count capacity check. | Link may later be removed; bill attachment creation is not object creation. | Bill ownership/visibility/status, receipt/supporting purpose, existing bill links, OCR and notification constraints. | Implemented for bill attachments, with association-after-activation compensation and attachment-count quota gaps. #341/#966. |
...
| `FILE-LC-LINK-002` | Current active readable bill link; uploader/owner and bill mutation rights; mutable bill state. Current object transition lacks multi-reference proof. | Bill link re-entry conditional under `LINK-004`; object needs independent `OBJ-007`. | Other bill/file links, OCR review/assignment, notifications and bill/history dependencies. | Implemented for bill attachment removal; object coupling is partial/unsafe for shared references. #966/#724. |
...
| `FILE-LC-LINK-010` | Current self profile authorization and existing QR; replacement upload/object activation complete before swap; current profile/link still eligible. No expected-version guard protects the loaded old ID. | Replacement is explicit but does not restore the old link; the new link can later be removed under `LINK-003`. | Profile/visibility, new/old object ownership and purpose, settlement-scoped reads, other references, concurrent replace/remove, both compensation outcomes. | Implemented replacement; association and old-object compensation are separate and partial. #966. |
| `FILE-LC-LINK-004` | Authorized bill requester; exact removed bill link; object independently active; receipt/supporting purpose compatible; bill mutable/current; privacy/integrity/capacity/duplicate checks; idempotency/version. | Conditional explicit bill re-link. Cached association is irrelevant. | Historical bill link, current bill, replacement links, OCR/notification dependencies, copies/holds. | Unimplemented. #722/#723 after #961 plus bill owner. `FILE-LC-CHOICE-002`. |
...
| `FILE-LC-PKG-013` | Same actor/session; exact `ready` preview; lazy expiry check runs first. | Terminal status for that preview; create another preview to retry. | Existing confirmation sessions and retained package summary; discard does not remove either dictionary entry or summary reference. | Implemented in `DiscardRestorePreviewAsync`; no eager cleanup. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-014` | Later actor-authorized preview read/discard/confirmation access at/after preview expiry. | Terminal availability state for that preview; restart loses process state. | Preview and any confirmation summary references; no background sweep. | Implemented lazily in `ExpireRestorePreviewIfNeeded`; status-only, summary retained. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-015` | Same actor/session; exact ready preview; supported scope and confirmation label; expected preview/code/hash/digest; optional idempotency key. | Confirmation may be discarded or lazily expire; it never applies restore. | Preview/summary validity, idempotency match, actor/session, expiry and concurrent discard/expiry/create. | Implemented in `CreateRestoreConfirmationSessionAsync`; process-local idempotency scan, no business mutation or durable audit. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-016` | Same actor/session; exact confirmation; lazy expiry first; only `metadata_only` changes to discarded. | Terminal status; create a new confirmation only from a still-ready preview. | Preview/package summary and concurrent idempotent create/read/expiry; discard retains entry/summary. | Implemented in `DiscardRestoreConfirmationSessionAsync`; no eager cleanup. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-017` | Later actor-authorized confirmation read/discard or matching idempotency scan at/after expiry. | Terminal availability state; restart loses process state. | Referenced preview/package summary, idempotency records and concurrent read/discard/create; no background sweep. | Implemented lazily in `ExpireRestoreConfirmationSessionIfNeeded`; status-only, entry/summary retained. #406/#971/#724. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-019` | Same actor/auth session; exact unexpired `ready_for_confirmation`; matching scope/group/digest/version/challenge; current authorization/membership; nonempty candidates. | Terminal session status plus newly authoritative draft bills; it is not file-object activation or restore. | Candidate bill graph, current actor/group, challenge fields, bill validation/audit and transaction outcome. JSON payloads remain after success. | Implemented in `ConfirmBillImportSessionAsync`; sequential replay conflicts, but no explicit session concurrency token/idempotency key makes racing-confirm outcome a database-dependent write conflict rather than a durable prior result. #406/#971/#724. `FILE-LC-CHOICE-001/006`. |
| `FILE-LC-PKG-020` | Same actor/auth session; lazy expiry first; only `ready_for_confirmation|needs_correction` changes to `discarded`. | Terminal session status; create a new session to retry. | Retained review/candidate JSON and any concurrent get/confirm/expiry. No domain bills are deleted. | Implemented in `DiscardBillImportSessionAsync`; repeat/terminal requests return current state, payloads remain, and no lifecycle audit exists. #406/#971/#724. `FILE-LC-CHOICE-001/006`. |
| `FILE-LC-PKG-021` | Later same-actor/session get/discard/confirm at or after expiry; only `ready_for_confirmation|needs_correction` transitions. | Terminal session availability state; row/payload disposal is separate and absent. | Stored deadline/status, review/candidate JSON and concurrent confirm/discard; no background sweep. | Implemented lazily in `ExpireSessionIfNeeded`; persisted status only, with no payload clearing, row cleanup, or lifecycle audit. #406/#971/#724. `FILE-LC-CHOICE-001/006`. |
| `FILE-LC-PKG-003` | Same actor/session; current `created|ready_to_download`; later access first applies lazy session expiry. | Terminal for this process-local session; create a new session to retry. | Current artifact/actions and concurrent prepare/download. Reference clearing does not prove zeroization or client-copy disposal. | Implemented in `CancelPackageGenerationAsync`; in-memory only. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-004` | Same actor/session; current `created`; later access first applies lazy expiry. | Terminal for this process-local session; create a new session to retry. | Prove status is still unprepared; this accepted path has no artifact. | Implemented in `DiscardPackageSessionAsync`; ready sessions are not discarded by this operation. #406/#971. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-005` | Later authorized session access at/after session expiry; only `created|ready_to_download` transitions. | Terminal availability state; restart also loses all process-local state. | Current clock/session status and concurrent prepare/cancel/download; no background scheduler. | Implemented lazily by `ExpirePackageSessionIfNeeded`; no guaranteed wall-clock cleanup. #406/#971/#724. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-007` | Same actor/session; current `ready_to_download`; artifact not expired. | Action remains usable until observed consumed, expiry, cancellation/session expiry, or process loss; concurrent consumption is not serialized. | Current artifact identity/expiry and concurrent reprepare/cancel/expiry/content requests; multiple actions are permitted. | Implemented in `CreatePackageDownloadActionAsync`; no durable audit/idempotency or atomic single-use guarantee. #406/#971. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-008` | Same actor/session; exact action observed unconsumed/unexpired; ready unexpired current artifact. The observation and consumed update are not atomic. | The stored consumed flag is one-way for that action; issuing another action is separate, while racing requests can both return content. | Action, current artifact, expiry and concurrent cancel/reprepare/content; downloaded copy becomes client-controlled. | Implemented in `DownloadPackageContentAsync`; consumed flag is process-local and set before response transfer outcome is known, without a lock or compare-and-set. #406/#971/#724. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-009` | Later authorized artifact/action/content access; action consumed or expired. | Pruned action cannot reactivate; a new action may be issued only for a ready artifact. | Current action/session/artifact clocks and concurrent issue/content. | Implemented lazily in `ExpireArtifactAndDownloadActionsIfNeeded`; not a byte cleanup. #406/#971. `FILE-LC-CHOICE-006`. |
...
| `LINK-001`, `LINK-005`, `LINK-006` | Retain with active subject/history. No hard delete/purge from attach. | Current attach audit is implemented separately for bill, proof, and payment details with bounded metadata; public DTO privacy tests exclude storage internals. | #341/#966 and separate bill/settlement/payment-details owners. Runtime storage/privacy gate remains for changes. |
...
| `PKG-018..021` | Persisted CSV import review/candidate JSON and the session row remain after confirmation, discard, and lazy expiry. Current 30-minute expiry is a confirmation-availability clock, not payload retention or disposal; no row/payload cleanup exists. | Creation/status changes have no dedicated lifecycle audit. Confirmation writes bounded bill-import audit for each created draft, but never log raw CSV, review/candidate JSON, private rows, digest/challenge values, or imported contents. | #406/#971 import lane; #724 retention; relevant bill-domain owners for confirmation. API/privacy, persistence, audit/retention and user data-ingress gates remain pending for changed behavior or disposal. |
| `CLN-003..004`, `LOCAL-001..002` | Generation is non-destructive. Transient hard delete is eligible only after positive non-authoritative proof; exact clock unresolved. Never cleanup authoritative/history content. | Local audit applicability and memory-zeroization proof are unresolved; server cleanup needs bounded result without local paths, object keys, content or private queue data. | #358 receipt intake, #971 local/offline/import, #1062 derivatives/security, #724 retention, #966 storage; provider/destructive/local-data gates pending as applicable. |
...
| --- | --- |
| `FILE-LC-OBJ-001..003`, `FILE-LC-LINK-001`, `FILE-LC-LINK-005..006` | `FILE-LC-GATE-001`, `002`, `003`, `006`, `007` |
...
| `FILE-LC-OBJ-006`, `FILE-LC-LINK-002..004`, `FILE-LC-LINK-007..010` | `FILE-LC-GATE-001`, `002`, `003`, `004`, `006`, `008` |
...
| `FILE-LC-PKG-003`, `FILE-LC-PKG-005..006`, `FILE-LC-PKG-009` | `FILE-LC-GATE-001`, `003`, `005`, `006`, `008` |
| `FILE-LC-PKG-011..017` | `FILE-LC-GATE-001`, `003`, `004`, `006`, `008` |
| `FILE-LC-PKG-018..021` | `FILE-LC-GATE-001`, `002`, `003`, `004`, `006`, `008` |
...
| `FILE-LC-LINK-001` | Authority: bill-domain and metadata architecture. Current evidence: bill attachment handler, entity/config and endpoint tests. | **Attach receipt** or **Add supporting file** on the bill surface. | Applicable/current: bill-link creator/time and bill-attachment audit. Missing: link version/idempotency, attachment-count quota and atomic provider/link step record. | No key; retry can create a new object/link; the bill/file composite key prevents only the same pair; timeout result is unknown; concurrent uploads are not bounded by an attachment-count capacity. `CHOICE-004`. |
...
| `FILE-LC-LINK-002` | Authority: bill domain plus shared unlink rule. Current evidence: bill attachment removal handler/tests. | **Remove attachment** on the exact bill surface; must not say bytes are deleted. | Applicable/current: bill-link `RemovedAtUtc`, removal audit, and coupled object deletion metadata. Missing: link actor/reason/version and complete other-reference proof. | No key; replay is unavailable rather than prior success; timeout cannot prove which step saved; concurrent OCR/link/object change has no explicit version conflict. `CHOICE-002/004`. |
...
| `FILE-LC-LINK-004` | Authority: shared restore/unlink taxonomy plus bill policy; no runtime evidence. | **Restore attachment** on a future bill/Trash surface after file restore. | Applicable/required: bill/file/purpose, requester/reason, restored time, source/target bill-link state, expected version and separate audit. Absent. | Required key/version; identical replay returns same bill link; changed bill/file conflicts; OCR/replacement/object/bill-state races block. `CHOICE-002`. |
...
| `FILE-LC-PKG-013` | Authority/current evidence: `DiscardRestorePreviewAsync`. | **Discard restore preview**; never claim package-summary data was erased. | Process-local/current: `discarded`, safe code and discarded time. Preview dictionary entry and summary remain; no durable audit. | Replay after discard is unavailable rather than prior success; confirmation/discard/expiry ordering uses mutable process state. `CHOICE-006`. |
| `FILE-LC-PKG-014` | Authority/current evidence: `ExpireRestorePreviewIfNeeded`. | **Restore preview expired** only after later access detects the deadline. | Process-local/current: status/code change; entry and package summary remain. No expiry time event, sweep, removal or durable audit. | Later accesses return expired/unavailable state; confirmation creation and expiry race without a persisted version. `CHOICE-006`. |
| `FILE-LC-PKG-015` | Authority/current evidence: `CreateRestoreConfirmationSessionAsync`. | **Confirm restore intent** / metadata-only; never **Restore completed**. | Process-local/current: confirmation/preview/actor/session IDs, scope, label, optional key, request digest, `metadata_only`, times and shared bounded package summary. No applied mutation or durable audit. | A sequential retry with the optional actor/session-scoped key returns the matching existing session, including its lazily expired state; changed digest/preview/scope conflicts. The lookup and insert are not atomic, so concurrent requests with the same new key can create distinct sessions. Unkeyed replay also creates another session. `CHOICE-006`. |
| `FILE-LC-PKG-016` | Authority/current evidence: `DiscardRestoreConfirmationSessionAsync`. | **Discard restore confirmation**; never claim summary/package copies were erased. | Process-local/current: `discarded`, safe code and time. Dictionary entry and package summary remain; no durable audit. | Repeat returns current terminal mapping; concurrent read/idempotent create/expiry uses mutable process state. `CHOICE-006`. |
| `FILE-LC-PKG-017` | Authority/current evidence: `ExpireRestoreConfirmationSessionIfNeeded`. | **Restore confirmation expired** only after later access/idempotency scan detects the deadline. | Process-local/current: status/code change; entry, key/digest and package summary remain. No expiry event time, sweep, removal or durable audit. | Later access returns expired state; matching idempotent create returns that existing state; discard/create/expiry ordering is not persisted/versioned. `CHOICE-006`. |
...
| `FILE-LC-PKG-019` | Authority/current evidence: `ConfirmBillImportSessionAsync`. | **Confirm import** and report created draft-bill summaries; never claim retained review/candidate data was erased. | Persisted/current: `confirmed`, confirmed/update times and unchanged review/candidate JSON; authoritative draft bills and per-bill import audit are added in one save boundary. No session lifecycle audit/version. | Sequential replay conflicts as not confirmable. Concurrent confirms can both observe ready; database constraints may make one write fail, but there is no durable idempotent prior-result or explicit concurrency contract. `CHOICE-001/006`. |
| `FILE-LC-PKG-020` | Authority/current evidence: `DiscardBillImportSessionAsync`. | **Discard import**; never claim review/candidate data or bills were deleted. | Persisted/current: `discarded`, discarded/update times and unchanged review/candidate JSON. No lifecycle audit or disposal outcome. | Repeat/terminal call returns current state; confirm/discard/expiry race has no explicit expected-version result. `CHOICE-001/006`. |
| `FILE-LC-PKG-021` | Authority/current evidence: `ExpireSessionIfNeeded` on get/discard/confirm. | **Import session expired** only after later access detects the deadline. | Persisted/current: `expired` and update time; row plus review/candidate JSON remain. No background sweep, payload clearing, expiry event time or lifecycle audit. | Later access returns the terminal session or confirm conflict; confirm/discard/expiry ordering has no explicit concurrency token. `CHOICE-001/006`. |
| `FILE-LC-PKG-003` | Authority/current evidence: `CancelPackageGenerationAsync`. | **Cancel backup generation**; never claim downloaded copies were removed. | Process-local/current: `cancelled`, cancel time, null artifact and cleared actions. No durable audit or zeroization proof. | Repeated request returns current terminal state; prepare/download races use mutable in-memory ordering, not an explicit version. `CHOICE-006`. |
| `FILE-LC-PKG-004` | Authority/current evidence: `DiscardPackageSessionAsync`. | **Discard unprepared backup session**; never claim package bytes were deleted. | Process-local/current: `discarded` and discard time only when source is `created`; no artifact exists on the accepted path. | Repeated request returns current state; ready sessions do not transition; prepare/discard ordering is not versioned. `CHOICE-006`. |
| `FILE-LC-PKG-005` | Authority/current evidence: `ExpirePackageSessionIfNeeded`. | **Backup session expired** after later access detects the deadline. | Process-local/current: `expired`, null artifact, cleared actions. No expiry event time, durable audit, background sweep, or memory-zeroization proof. | Access repeats terminal result; expiry competes with prepare/cancel/download through mutable process state. `CHOICE-006`. |
...
| `FILE-LC-PKG-007` | Authority/current evidence: `CreatePackageDownloadActionAsync`. | **Create download action** for this ready artifact. | Process-local/current: opaque action ID, created/expiry times, unconsumed flag. Missing immutable artifact binding, durable audit and idempotency. | No key; replay issues another action; multiple actions coexist; reprepare/cancel/expiry races are not versioned. `CHOICE-006`. |
| `FILE-LC-PKG-008` | Authority/current evidence: `DownloadPackageContentAsync`. | **Download backup** for the exact action; never promise atomic single-use or imply the artifact itself has only one download. | Process-local/current: action observed and then marked consumed before response; bytes egress in the response. Missing atomic compare-and-set/lock, durable outcome and downloaded-copy inventory. | Sequential replay of that action is unavailable, but racing requests can both pass the check and return bytes; network failure after consumption cannot be distinguished; another action can download again; current-artifact replacement race is not versioned. `CHOICE-006`. |
| `FILE-LC-PKG-009` | Authority/current evidence: lazy action loop in `ExpireArtifactAndDownloadActionsIfNeeded`. | No separate success surface; an expired/used action becomes unavailable. | Process-local/current: consumed/expired action entry removed on later access. No durable audit; artifact/client copies unchanged. | Repeated lookup is unavailable; concurrent issue/content/prune ordering is process-local and not explicitly versioned. `CHOICE-006`. |
| `FILE-LC-CLN-001` | Authority: metadata architecture compensating-cleanup direction and #724; no cleanup service/runner. | Not user-visible; owner may receive bounded **Upload could not be completed** while maintenance gets safe state categories. | Applicable/required: upload intent, exact step, byte-state category, link state, attempts/lease, policy/proof version, outcome. Absent. | Required upload/cleanup key; replay resumes exact step; retry returns prior/updated bounded outcome; concurrent completion/link/cleanup acquires one authoritative lease or conflicts. `CHOICE-001/004`. |
...
| Statement/import source | Required but duration unresolved. Current persisted CSV import `ReviewJson`/`CandidateJson` survive confirmation, discard and lazy expiry; those statuses do not remove the row or payload fields. A separate retention/disposal operation is absent. |
...
| `FILE-LC-CHOICE-004` — What reference registry, copy inventory, leases, consistent-snapshot proof, and idempotency record establish orphan/failed-upload cleanup eligibility? | File objects, every domain/history link, workers/jobs, packages, backups/copies and provider execution. | Current source has several restrictive references but no complete registry, proof transaction, cleanup record, lease, copy inventory, or runner. | Unknown/incomplete evidence blocks cleanup; failed or unreadable is not disposable. | #724/#966 must adopt or recommend a focused owner; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `007`, `010`. | Orphan classification, failed-upload reconciliation, cleanup executor and physical outcome. |
...
| `FILE-LC-CHOICE-006` — How are import sessions and persisted export versus backup packages distinguished, integrity-validated, encrypted, expired, downloaded, and copy-disposed? | Sync/import/export/restore, storage, privacy/vault, bill-domain import and backup recovery. | Current direct exports stream with no server artifact or client-copy inventory. Backup generation uses separate non-transactional database queries, so its sections are not a proved consistent snapshot. Download uses process-local data-only artifacts; multiple action IDs can download one artifact, and one action blocks sequential reuse after consumption but can serve concurrent racing requests because check/update is not atomic. Blocked-session action hints contradict handlers. Restore preview makes whole-package hash comparison optional and does not verify declared section hashes/names/inventory against `data`; preview/confirmation entries retain summaries after discard/expiry. Persisted CSV import sessions retain private review/candidate JSON and their rows after confirmation, discard, and lazy expiry. No path proves all memory/client/persisted copies disposed, and no current package has a `FileObject`/storage relationship, file-byte inclusion, encryption/disposition contract, or restore apply. | Actor/session-bound, no persisted-file-package claim, no automatic disposal. Availability/confirmation expiry is neither guaranteed memory/payload cleanup nor a retention decision for a stored or downloaded copy. | #406/#971/#724/#966 plus bill-domain owners for import confirmation; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `008`. | Consistent export/backup generation, import-session payload retention/disposal, package integrity, response/in-memory/downloaded-copy inventory, stored package expiry/recovery and copy-disposition acceptance. |
