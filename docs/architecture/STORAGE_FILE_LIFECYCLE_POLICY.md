...
- Current OpenAPI and generated clients expose subject-specific bill
  ...
  background sweep or dictionary removal, so an unaccessed artifact can remain
  in process memory until process restart. It does not persist a backup as a
  `FileObject` or through the storage provider, include file bytes, or apply a
  restore. No file-object restore, quarantine, retention, cleanup, or
...
- **Persisted lifecycle metadata:** stable file ID, purpose, bounded prior/new
  ...
  `FILE-LC-LOCAL-001`, server file-object/link fields are not applicable;
...
| `FILE-LC-LINK-005` | Current self actor is authorized for the profile; the loaded profile has no QR link; newly activated object is owned by that user and has `payment_qr`. Current code has no expected-version/null-at-commit guard. | Initial link can later be removed or replaced under `LINK-003`; it is not object creation or restore. | Profile ownership/status, object ownership/purpose/readiness, observed null link, settlement visibility implications, and concurrent attach. | Implemented for initial self QR attachment; association occurs after object activation and compensation can leave an active unlinked object. #341/#966. |
...
| `FILE-LC-CLN-001` | `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `007` |
| `FILE-LC-OBJ-004..005` | `FILE-LC-GATE-001`, `002`, `003`, `006`, `007`, `008` |
| `FILE-LC-OBJ-006..008`, `FILE-LC-LINK-002..004`, `FILE-LC-LINK-007..009` | `FILE-LC-GATE-001`, `002`, `003`, `004`, `005`, `006`, `008` |
...
| `FILE-LC-OBJ-009` | Authority: shared taxonomy, metadata/file policy and #724; `DeleteAsync` exists but has no production caller. | Future **Permanently delete eligible file** on an approved Trash/maintenance surface with consequence warning and explicit confirmation when user-initiated. | Applicable/required: durable operation/lease, proof-set and policy versions, actor/approval, reason, provider outcome category, attempts, disposed time, tombstone. Absent. | Required key/expected target version; exact replay resumes/returns prior outcome; changed target/proof conflicts; link/hold/restore races revalidate immediately before provider work. `CHOICE-001/004/008`. |
...
| `FILE-LC-LINK-003` | Authority: payment-details/visibility policy. Current evidence: self QR replace/remove handlers/tests. | **Remove QR image** or **Replace QR image** on self payment details. | Applicable/current: nullable link and profile update time plus payment audit; old-object transition is best-effort. Missing: link lifecycle timestamp/version and old-object reconciliation outcome. | Replace has no key and replay uploads/replaces again; remove replay returns no-content at link surface; object retry result is ignored; concurrent replace/remove is last accepted save without explicit version conflict. `CHOICE-002/004`. |
| `FILE-LC-LINK-007` | Authority: settlement/payment domain plus shared unlink rule. Current evidence: proof removal handler/tests. | **Remove proof** on the exact settlement-payment surface; must not say bytes are deleted. | Applicable/current: proof-link `RemovedAtUtc`, proof audit/notification effects, and coupled object deletion metadata. Missing: link actor/reason/version and complete other-reference proof. | No key; replay is unavailable rather than prior success; timeout cannot prove which step saved; concurrent payment/proof/object change has no explicit version conflict. `CHOICE-002/004`. |
