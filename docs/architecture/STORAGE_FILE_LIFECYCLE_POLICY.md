...
- Current OpenAPI and generated clients expose subject-specific bill
  ...
  backup package operations. The backup runtime holds sessions and data-only
  ...
  and actions, while generation-validation failure clears them. It does not
...
| `PKG-001..010` | Current TTLs are source-defined availability checks; cancel/session expiry clear server references, action consumption/pruning changes only action state, discard accepts only an unprepared session, and blocked preparation may preserve or clear prior bytes by failure branch. None proves eager memory zeroization or downloaded-copy disposal. | Current process-local operations have actor/session checks and safe response metadata but no durable lifecycle audit. Never log package bytes, contents, filenames, or private rows. | #406/#971 package lane; #724 retention/copy disposition; #966 storage boundary. API/privacy, retention, user data-egress and destructive gates remain pending for changed or persistent behavior. |
...
| `FILE-LC-PKG-001..002`, `FILE-LC-PKG-004`, `FILE-LC-PKG-007..008` | `FILE-LC-GATE-001`, `003`, `004`, `006`, `008`, `011` |
...
| Backup package | Current process-local artifact and action availability TTLs are source-defined and lazily enforced, but eager memory disposal and downloaded-copy inventory are unproved. Any future persisted package is dependency-gated; recovery, encryption, manifest, copy/replica and hold obligations remain unresolved. Disposal is destructive/manual. |
...
| `FILE-LC-CHOICE-006` — How are persisted export versus backup packages distinguished within `export_file`, encrypted, expired, downloaded, and copy-disposed? | Sync/import/export/restore, storage, privacy/vault and backup recovery. | Current exports stream. Backup package generation/download and restore-preview parsing use process-local data-only artifacts and metadata; multiple action IDs can download one artifact, each action is one-use, TTL checks are lazy, and no background sweep or package-session entry removal guarantees memory disposal. Later access can clear artifact references and prune actions. There is no `FileObject`/storage relationship, file-byte inclusion, downloaded-copy inventory, encryption/disposition contract, or restore apply. | Owner-only, no persisted-package claim, no automatic disposal. Current lazy availability expiry is neither guaranteed in-memory cleanup nor a retention decision for a future stored or downloaded package copy. | #406/#971/#724/#966; `FILE-LC-GATE-001`, `002`, `003`, `005`, `006`, `008`. | Current in-memory/downloaded-copy inventory and cleanup acceptance plus stored export/backup generation, expiry, recovery and copy-disposition acceptance. |
