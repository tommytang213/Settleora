...
| `FILE-LC-PKG-001` | Create process-local backup session | Current API creates actor/session-bound `created` metadata in process memory, with no artifact bytes at this step. Its OpenAPI description nevertheless says generation/download remain unsupported although mapped follow-up operations implement them. | Neither file object nor subject link; process-local session metadata only. | Only the same authenticated user profile and auth session may read or act on it. Contract prose is not capability authority while it contradicts runtime and generated methods. |
...
| `FILE-LC-PKG-001` | Authenticated actor; new opaque session ID; process-local insert. OpenAPI accurately describes no bytes at creation but falsely says later generation/download remain unsupported. | Creation is reversible only through current discard/cancel/expiry paths; restart loses the session. | Actor user-profile ID and auth-session ID; no `FileObject`, provider, or package bytes at this transition; mapped later operations and generated methods remain relevant. | Implemented in `CreatePackageSessionAsync`; process-local and non-durable. API/OpenAPI/client owners must reconcile capability prose. #406/#971. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-001` | Authority/current evidence: API local-backup package contract and `CreatePackageSessionAsync`. | **Create backup session** on the authenticated backup surface; do not repeat the OpenAPI claim that generation/download are unsupported. | Process-local/current: opaque session ID, actor profile/session IDs, `created`, created/expiry times. No durable audit or `FileObject`. Contract prose omits implemented follow-up capability. | No key; replay creates another session; restart loses it; concurrent sessions coexist. API/OpenAPI/client owners must reconcile descriptive capability. `CHOICE-006`. |
