...
| `FILE-LC-PKG-007` | Issue package download action | For a current ready artifact, the API creates a new actor/session-scoped, expiring, unconsumed action; multiple actions can exist for one artifact. | Neither; process-local action metadata only. | The action can authorize exactly one later authenticated content response while both action and artifact remain valid. |
| `FILE-LC-PKG-008` | Consume package download action | Current API marks one valid action consumed before returning current artifact bytes. | Neither; process-local action flag changes and bytes egress to the client. | That action cannot read again, but another action may; server lifecycle cannot prove disposition of downloaded copies. |
...
| `FILE-LC-PKG-007` | Same actor/session; current `ready_to_download`; artifact not expired. | Action remains usable until one consumption, expiry, cancellation/session expiry, or process loss. | Current artifact identity/expiry and concurrent reprepare/cancel/expiry; multiple actions are permitted. | Implemented in `CreatePackageDownloadActionAsync`; no durable audit/idempotency. #406/#971. `FILE-LC-CHOICE-006`. |
| `FILE-LC-PKG-008` | Same actor/session; exact unconsumed/unexpired action; ready unexpired current artifact. | Consumption is one-way for that action; issuing another action is separate. | Action, current artifact, expiry and concurrent cancel/reprepare/content; downloaded copy becomes client-controlled. | Implemented in `DownloadPackageContentAsync`; consumed flag is process-local and set before response transfer outcome is known. #406/#971/#724. `FILE-LC-CHOICE-006`. |
...
| `FILE-LC-PKG-001`, `FILE-LC-PKG-004`, `FILE-LC-PKG-007..008` | `FILE-LC-GATE-001`, `003`, `004`, `006`, `008`, `011` |
| `FILE-LC-PKG-002` | `FILE-LC-GATE-001`, `003`, `004`, `005`, `006`, `008`, `011` |
...
| `FILE-LC-PKG-011..017` | `FILE-LC-GATE-001`, `003`, `004`, `006`, `008`, `011` |
...
| `FILE-LC-PKG-015` | Authority/current evidence: `CreateRestoreConfirmationSessionAsync`. | **Confirm restore intent** / metadata-only; never **Restore completed**. | Process-local/current: confirmation/preview/actor/session IDs, scope, label, optional key, request digest, `metadata_only`, times and shared bounded package summary. No applied mutation or durable audit. | Optional actor/session-scoped key returns the matching existing session, including its lazily expired state; changed digest/preview/scope conflicts. Unkeyed replay creates another session. `CHOICE-006`. |
...
| `FILE-LC-PKG-008` | Authority/current evidence: `DownloadPackageContentAsync`. | **Download backup** once per exact action; never imply the artifact itself has only one download. | Process-local/current: action consumed before response; bytes egress in the response. Missing durable outcome and downloaded-copy inventory. | Replay of that action is unavailable; network failure after consumption cannot be distinguished; another action can download again; current-artifact replacement race is not versioned. `CHOICE-006`. |
