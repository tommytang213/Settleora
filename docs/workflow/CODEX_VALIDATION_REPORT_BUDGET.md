...
User-web validation is selected for `apps/web-user/**` and `packages/client-web/**`, plus the scaffold workflow, classifier, web manifest helper, and their policy/regression tests. It uses the Node 22 baseline and the web lockfile to clean-install, lint/typecheck, test, build, scan, checksum, and upload exact-source static package evidence. Missing or invalid classification fails closed into this lane; ordinary docs-only changes intentionally skip it.
