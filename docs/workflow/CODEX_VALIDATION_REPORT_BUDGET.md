...
The required `Scaffold Validation` workflow must keep the final `Validate scaffold` job name stable for branch protection. Its single changed-file classifier emits docs-only, full, mobile, iOS, and user-web routing decisions. The final `if: always()` aggregate accepts optional jobs as skipped only when trustworthy classifier output proves they were unnecessary; a failed classifier, missing/invalid output, or failed/cancelled/incorrectly skipped required lane fails the aggregate.

The classifier treats only `README.md` and Markdown/static documentation assets under `docs/` as docs-only. Docs-only PRs install the Node repository tooling, run `npm run validate:scaffold`, and execute the classifier/workflow-policy tests; full API/generated-artifact, Linux Flutter, macOS iOS, and user-web jobs are intentionally skipped, while `Validate scaffold` still resolves successfully. A first push with a missing/zero before SHA on a non-default branch may qualify only after a successful fixed `origin/main` fetch, matching checked-out event SHA, complete history, one validated merge-base, consistent ancestry, and a successful non-empty docs-only diff. Default-main first pushes, any other changed path, unavailable or untrustworthy proof, and unsupported events default to full validation. Proof failures conservatively emit full, mobile, iOS, and user-web requirements; on pull requests all four required lanes run, while push events run full validation and intentionally skip the three PR-only mobile/iOS/user-web lanes.

Workflow, CI, package metadata, tooling, scripts, services, apps, contracts, generated clients, source, tests, Docker/compose, infrastructure, deployment, runtime config, OpenAPI, schema, migration, security, storage/privacy, money, settlement, payment, and bill-calculation changes keep the broader GitHub Actions validation path enabled. Mobile/iOS lanes are additionally selected for `apps/mobile/**`, `packages/client-dart/**`, `.github/workflows/scaffold-validation.yml`, `.github/workflows/mobile-ios-validation.yml`, `tools/ci/scaffold-validation-changes.mjs`, both `tools/ci/test/scaffold-validation-changes.test.mjs` and `tools/ci/test/ci-workflow-policy.test.mjs`, and `codemagic.yaml`; `package.json`, `tools/doctor-validation.mjs`, and every path under `tools/ocr-models/**` select Linux mobile validation at minimum.

User-web validation is selected for `apps/web-user/**` and `packages/client-web/**`, plus the scaffold workflow, classifier, web manifest helper, release identity collectors that build or validate web evidence, and their policy/regression tests. Release identity collectors that build or validate Android evidence also select Linux mobile and hosted macOS iOS validation. It uses the Node 22 baseline and the web lockfile to clean-install, lint/typecheck, test, build, scan, checksum, and upload exact-source static package evidence. Missing or invalid classification fails closed into this lane; ordinary docs-only changes intentionally skip it.
