...
- Before extraction, verifies the IPA central directory has bounded size/expansion, unique safe relative UTF-8 names, only regular files/directories under `Payload` and optional `SwiftSupport`, matching local-header identities, and no ZIP64, encryption, symlinks, special files, path traversal, case collisions, overlapping entries, or unsupported compression. It then verifies archive integrity and the unchanged IPA digest across extraction.
