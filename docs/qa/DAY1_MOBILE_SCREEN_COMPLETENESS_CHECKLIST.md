...
- Owner: [#1092](https://github.com/tommytang213/Settleora/issues/1092), completed by implementation PR #1148 plus same-task ledger hygiene, remains the behavior owner. #1176 / catalog PR #1177 and application PR #1178 separately complete `L10N-D1-021` for the five current English payload strings. The explicit `1.0.0+1` bundled release key must be updated with `pubspec.yaml` for a future release that should present new notes. Future release content and actual translation remain separate; server announcements remain #1094 and contextual-help extraction remains `L10N-D1-022`.
