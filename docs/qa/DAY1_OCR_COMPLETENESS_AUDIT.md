...
| 11 | Offline behavior/model availability | `unavailable` | App bootstrap/local-mode/server-shell routing; ML Kit provider uses no Settleora server call; artifact cache reports `secure_receipt_cache_deferred`; #437 plan | The provider itself is device-side, but the real provider is injected only after authenticated server bootstrap. Local Mode has no bill/OCR route, and an offline/server-unavailable cold path cannot reach the authenticated OCR shell. Existing failure/manual editing helps only after that shell is reachable. | #971 must reconcile and split local-mode/offline OCR reachability, persistence, queue and conflict behavior. R01/#1209 remains a prerequisite for packaged model/build facts; #975 owns final device proof. | P09/#971 |
...
| P09 | #971, with #1060 only if its cross-domain idempotency framework is adopted | `sync-offline-local-mode`; split local-mode/offline OCR reachability and mobile persistence/queue from API acceptance/contracts | Audit-defined cold-start/local-mode/offline mobile plus API/sync tests; `strong_independent` | R01 supplies package/model facts first. Local-security, conflict, restore, schema/contract and destructive decisions retain manual gates. Close the OCR slice only with reachable Local Mode and offline/server-unavailable capture/review, durable queued/synced/conflict/failed states, preserved edits, replay/idempotency and authoritative server acceptance evidence. |
