...
| 11 | Offline behavior/model availability | `partial` | App bootstrap/local-mode/server-shell routing; historical ML Kit provider evidence; artifact cache reports `secure_receipt_cache_deferred`; closed #437 planning; completed #1209 Latin-package proof | A user already inside the authenticated shell retains the injected device-side provider after connectivity loss, so local extraction remains reachable in that warm session. Local Mode has no bill/OCR route, and an offline/server-unavailable cold path cannot bootstrap into the authenticated OCR shell; durable local review/queue behavior is absent. #1209 proves only the superseded Latin ML Kit package, not the approved PaddleOCR Global Core or current real-provider acceptance. | #1247 owns PaddleOCR Global Core packaging and real Android/iOS offline provider acceptance. Docs-only audit #971 must reconcile cold-start/local-mode reachability, persistence, queue, and conflict behavior without implementing those gaps; #975 and its children own final device/offline evidence. | #1247 provider evidence plus P09/#971 reconciliation and approved children |
