...
This audit inventories **25 capabilities exactly once**: 6 `implemented`, 11
`partial`, 4 `blocked`, 2 `externally-gated`, 1 `documentation-only`, and 1
...
| 2 | Image preprocessing/safety/normalization | `partial` | `receipt_image_artifact_processor.dart`, `receipt_image_normalization_policy.dart`, `receipt_intake_safety.dart`; `_processedReceiptAttachmentArtifact`/`_runReceiptOcrPreview` and later upload loops in `bill_list_screen.dart`; artifact tests; normalization/artifact PRs #212/#213 | JPEG/PNG/WebP bytes are decoded and re-encoded as normalized JPEG plus thumbnail in memory; that derivative becomes draft attachment/upload bytes. The derivative retains the original `localPath`, so ML Kit reads the original selected file. When normalization is not produced for PDF/HEIC/unknown/decode failure, the original selection remains in the draft and the later attachment loop can upload its original bytes; guidance does not prevent that bypass. | OCR can bypass the derivative, and failed/unsupported normalization can bypass normalized upload/storage policy. There is no orientation/crop/document-boundary/perspective/enhancement proof, explicit metadata-strip proof, encrypted cache, offline/replacement/web policy convergence, or API-policy acceptance. | P01/#358 |
...
| 11 | Offline behavior/model availability | `unavailable` | App bootstrap/local-mode/server-shell routing; ML Kit provider uses no Settleora server call; artifact cache reports `secure_receipt_cache_deferred`; #437 plan | The provider itself is device-side, but the real provider is injected only after authenticated server bootstrap. Local Mode has no bill/OCR route, and an offline/server-unavailable cold path cannot reach the authenticated OCR shell. Existing failure/manual editing helps only after that shell is reachable. | Docs-only audit #971 must reconcile and split local-mode/offline OCR reachability, persistence, queue and conflict behavior into approved focused implementation owners; it must not implement those gaps itself. R01/#1209 remains a prerequisite for packaged model/build facts; #975 owns final device proof. | P09/#971 reconciliation and its approved children |
...
| 25 | Final Day 1 platform/UI/privacy acceptance | `externally-gated` | Docs-only #975 readiness-audit contract, native plan, current automated widget/API/build evidence | Current automated evidence supplies inputs but not final sign-off; #975 itself neither runs product acceptance nor grants human approval. | Before actual acceptance, requires R01, #358/#401/#438/#740/#959/#442/#966/#967, #971 and every focused runtime child it creates/adopts, #357's server-complement child/disposition, and every focused acceptance owner identified by #975. Those owners must bind exact final artifacts to physical-device, accessibility, visual, privacy and human-gate evidence. | P12/#975 reconciliation and its approved acceptance owners |
...
| P12 | Docs-only readiness audit #975 owns reconciliation and creation/adoption of focused acceptance owners | #975 remains `qa-acceptance` documentation only with no product mutation; resulting owners receive separately approved artifact/device/platform/UI/privacy lanes | #975 closes on a source-bound evidence and ownership map. Resulting owners bind exact final artifacts to Android/iOS physical-device, offline/permission/accessibility/visual/privacy/manual matrices and independent acceptance review. | Depends on every non-complete Day 1 owner, including every child produced by #971 and #357's server-complement child/disposition. Human/device/provider gates remain. #975 does not grant final sign-off itself; its resulting owners and explicitly authorized humans close their own evidence/acceptance gates. |
...
6. Docs-only #975 reconciles exact final artifacts and completed domain maps,
   ...
   for every runtime child produced by #971 and every acceptance owner produced
   by #975; neither audit performs runtime acceptance or grants sign-off itself.
