...
- Explicit draft generation: forecast rows with `status == forecasted` and `draftGenerated == false` enable a `Generate Draft` action. The UI shows a confirmation dialog, calls `generateDraft(templateId, occurrenceDate)` only after confirmation, blocks duplicate generation while the operation is in progress, surfaces server-returned generated draft context, updates the matching forecast row only from the server response, and refreshes server state after success. If the refresh fails after generation, the generated context remains visible and retrying refresh does not repeat draft generation.
...
- Not implemented in mobile yet: recurring lifecycle offline queueing, reminder scheduling, notification delivery, background auto-generation, advanced exception handling, and generated-draft navigation to bill detail. The recurring screen does not invent a generated-bill route because the current shell does not provide a safe bill-detail route dependency in this context.
...
- User confirmation path for generated recurring bill instances: current mobile requires a confirmation dialog before `generateDraft`, blocks duplicate in-flight taps, shows neutral `Draft ready` copy for generated or idempotent existing-draft responses, preserves generated context when refresh fails, and lets users retry server-state refresh without repeating draft generation.
...
- Mobile tests now cover create/edit forms, generated repository request mapping, pause/resume/archive actions, duplicate mutation prevention for create/lifecycle actions, post-lifecycle detail refresh, generated-draft context display, idempotent existing-draft copy, and refresh-after-generate failure handoff.
- No mobile tests cover generated-draft bill navigation because M5 did not add a route dependency or invent a new route. The screen shows bounded open-from-Bills guidance instead.
...
- `M5-003` hardens forecast, detail, and explicit draft-generation handoff behavior for stale state, inactive templates, generated-draft refresh failures, and safe generated context without local recurrence or financial authority. Status: complete.
...
## M5-003 Implementation Coverage

- Added generated-draft context on the recurring screen that uses the server-returned draft result without showing raw generated bill IDs or recalculating money/recurrence locally.
- Updated successful generation copy to neutral `Draft ready` language so idempotent existing-draft responses are not presented as a newly created bill.
- Reconciled the matching forecast row from the generated draft response while awaiting a server refresh, then clears the local generated context only when refreshed forecast state no longer matches the returned generated draft.
- Preserved generated context when refresh after generation fails and added a refresh retry that reloads server state without repeating draft generation.
- Kept non-generatable forecast rows read-only with bounded guidance for generated, skipped, cancelled, and unknown server states.
- Did not add generated bill navigation because the recurring screen does not currently receive a safe bill repository/route dependency. Users get bounded open-from-Bills guidance instead.
