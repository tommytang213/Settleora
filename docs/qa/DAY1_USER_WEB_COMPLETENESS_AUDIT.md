...
| P03 | 004-007 | Separate bill-list, detail, create and lifecycle/edit children under [#344](https://github.com/tommytang213/Settleora/issues/344) | `web-user-ui`; contract child first where needed | `web-ui`; strong; money gate | P01; #344, #346, #349-#352 and #967 before UI where their contracts are incomplete; list close requires free-text, status, reconciliation, date-range, currency, merchant, archive/context filters plus receipt/OCR and pending-review indicators; detail close requires server-authoritative item rows, resolved split/allocation and adjustment detail, activity and empty/unavailable states; create/edit/lifecycle close requires category, optional bill payment-method field, bill/shared notes/comments, multi-payer/on-behalf confirmation, split modes/exclusions, quantity/open self-claim, multi-tax/fee/discount/refund/mismatch controls, manual FX, operations, safe retry/no-duplicate, refresh and visual proof | #346/#349-#352/#967 own the accepted money/domain and missing discovery/detail/metadata/discussion boundaries; #350/#967 specifically own persisted self-claim/unresolved depth, and #582 summary/filter runtime is credited. |
...
| P05 | 009 | [#966](https://github.com/tommytang213/Settleora/issues/966) splits metadata/content/upload/remove from file authority | storage/privacy then `web-user-ui`; contract/UI separate | `api-storage` then `web-ui`; strong; **privacy/file gate** | P01/#966; close with authorized content retrieval/viewing, normalized upload before OCR/storage, temporary raw retention, preview/manual fallback, removal, OCR submission, API enforcement, retry/denied/stale-refresh/no-duplicate action evidence, and #401-authorized duplicate candidates with review-existing/save-anyway/cancel states | OCR extraction #959 stays untouched; #401 owns duplicate policy and P25 post-upload review. Generated content/upload/remove/OCR methods are currently unused. |
...
| P20 | 037 | Existing [#1094](https://github.com/tommytang213/Settleora/issues/1094) must extend its post-authority handoff split to user web without bundling provider delivery or What’s New | `docs-planning` → API/OpenAPI/client → `web-user-ui`, each separate | matching contract/security then `web-ui`; strong independent; **admin exposure/auth manual gate** | #1094 server authority first and P01 for protected targeting; domain close requires redacted audit for publish/policy transitions with actor/action/subject/timestamp/correlation IDs plus server-authoritative read/dismiss mutations and policy-driven non-dismissible states; web close requires safe content, window/target/severity/read state, read/dismiss persistence, stale/expired/offline/denied handling, idempotent retry and authz | #1094 is the existing announcement authority owner but currently names mobile handoff; notification #973 and bundled notes P19 are distinct. |
...
| P25 | 042 | Existing [#970](https://github.com/tommytang213/Settleora/issues/970) with #967 splits OCR review/correction; #959 stays untouched | OCR/revision/file domain before `web-user-ui` | OCR/contract then `web-ui`; strong; **file/privacy/money gates** | P01/#970/#967/#966 plus #397/#398/#429 boundaries; close with queue/detail, field/item/tax/discount/refund/classification correction, OCR-line merge/split, derived-item-to-source-line lineage, deterministic merchant cleanup, assignment, apply preview and draft-versus-revision acceptance | Generated methods unused; #970 owns workflow and coordinates #397/#398/#429, while #959 remains independent. |
