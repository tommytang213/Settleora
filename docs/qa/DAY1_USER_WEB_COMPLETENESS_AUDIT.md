...
| P03 | 004-007 | Separate bill-readout, create and lifecycle/edit children under [#344](https://github.com/tommytang213/Settleora/issues/344) | `web-user-ui`; contract child first where needed | `web-ui`; strong; money gate | P01; #344 plus #350/#967 for quantity assignment and open/self-claim; close with unresolved/self-claim states, operations, safe retry/no-duplicate, refresh and visual proof | #350/#967 own persisted self-claim/unresolved depth; #582 runtime is credited. |
...
| P08 | 013-014 | [#969](https://github.com/tommytang213/Settleora/issues/969)/#353 split readout from settlement actions | money domain then `web-user-ui` | money/`web-ui`; strong; **money gate** | P01/#969; #966/P05 for settlement-scoped counterparty QR content/retry/denied authz; close with API totals/status, payment instructions, no duplicates, residual/dispute and visuals | #585/#587/#590 and current metadata credited. |
...
| P10 | 017-018 | [#973](https://github.com/tommytang213/Settleora/issues/973) reconciles mutations, typed opens and preferences before focused web children | planning → `web-user-ui`; contracts separate | docs then `web-ui`; strong; privacy/auth gates | P01/#973; preference close includes admin caps, channels, quiet hours, digest/immediate, per-event categories, group mute, disabled/unconfigured/save failure; open/mutation close includes authz revalidation, safe fallback, refresh/deep links | #368/#369/#403/#973 found; #409 is localization evidence only. |
| P11 | 019, 030 | [#977](https://github.com/tommytang213/Settleora/issues/977), #405/#404 reconcile reports, reconciliation and cross-record search | domain then `web-user-ui`; contract/UI split | domain then `web-ui`; strong; money gate if totals change | P01/#977; close with totals plus authorized global result families, bounded queries/filters, empty/denied, result navigation/actions and narrow/a11y evidence | #404/#405/#977 found; #591 bill search credited. |
...
| P15 | 025-026 | #971/#361 owns queue/offline/local/conflict reconciliation | sync domain before UI | sync profile; strong; **local-security gate** | P01/#971 plus security design before persistence; close with encryption/key handling, retention/device loss/sign-out, app PIN where feasible, no-collaboration warning, explicit local-to-server movement, authority, queue/retry/conflict tests | #361/#364/#406/#971 found; browser storage absent. |
...
| P19 | 036 | Recommend focused **user-web bundled What’s New** child | `web-user-ui`; bundled notes/component/tests and local seen state | `web-ui`; cheap; no server contract | After copy/version source; close with version-keyed seen persistence, automatic unseen-version display, skip/dismiss, settings/help/about reopen, offline notes and narrow/a11y evidence | No focused owner; #1094 excludes What’s New. |
