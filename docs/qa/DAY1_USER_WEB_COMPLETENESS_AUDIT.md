...
| --- | --- | --- | --- |
| Auth/bootstrap/onboarding/password/session/device/MFA/passkey/recovery | Empty-deployment first owner; policy-safe invitation create/accept/resend/expiry/revoke, admin-created user, default-off public registration and OIDC/local-account availability without owner lockout; sign-in/refresh/logout; reset and authenticated password change; current-user bootstrap; current/other session display, revoke and sign-out-all; factor/passkey enroll, challenge/step-up, inspect, revoke; recovery batch generate/revoke; successful and denied security audit. Raw MFA secrets, passkey private material, recovery codes, reusable challenges and provider/session/reset tokens must be absent from logs, API responses, generated clients and audit metadata. | Reset completion is the only complete web operation. Session/auth methods exist, but the shell supplies no token and has no credential lifecycle. | P01/#965 reuses the named auth owners; API/OpenAPI/client, security runtime and web UI remain separate manual-gated children. |
...
| Personal/group bills and revisions | Search/filter/list/detail plus amount, currency, merchant, date, category, participants, payer and payment method; personal/group create and draft-safe edit/correction; notes/discussion; acknowledgement; multi-payer/on-behalf confirmation; bill/item equal/exact/percentage-share/exclusion/quantity/open-self-claim allocation. Money coverage includes mixed tax-included/excluded and tax-group-constrained allocation; before/after-tax discounts; linked returns/refunds/tax corrections; editable coupon/points/gift-card/store-credit contribution treatment; tender/change/void/free/zero-price/unknown-negative exclusion or review; generic fee tax/allocation/source-line properties; manual adjustments, mismatch review, deterministic rounding residuals and manual FX. Lifecycle includes submit, participant accept/reject, archive/restore and one-active revision. Revision approval binds amount, currency and calculation hash; only affected participants reapprove and payer-impact changes reconfirm. Server review supplies baseline or full review, changed markers with accessible labels, category summary and viewer-specific impact before revise/resubmit/withdraw/approve/reject/payer-confirm/apply. Forms require validation, policy-denied/unsupported states and safe cancel/dirty-state handling. Conditional bill tags remain a #967-owned Day 1 scope decision and are neither silently admitted nor omitted. | Lists and partial detail are unreachable but source-backed; creation and lifecycle controls are disabled; revision metadata loads but full review/actions do not. Transport is partial and does not prove mapped server behavior. | P03/P04 through #344/#346/#349-#352/#967/#402; contracts/domain precede separately gated web UI. |
| Attachments and OCR | Multiple personal/group attachments by purpose; receipt images default to normalized JPEG with raw retention off unless policy says otherwise, while supporting images/documents use their own normalization/format rules; metadata stripping, type/source/output-size/count/quota/retention policy, authorized content lifecycle and duplicate-candidate review. OCR requires complementary server-job handoff/status where used, queue/detail, merchant/field/item correction, tax rate/category and tax-included/excluded interpretation, receipt tax summaries, discount/refund/tax-correction handling, and explicit item/tax/fee/discount/coupon/points/tender/change/void/free/zero-price/cash/unknown-negative/manual-review classifications. Merge/split preserves source lineage; assignment lifecycle, preview, explicit apply and draft-versus-revision routing stay provisional and never finalize financial truth automatically. | Attachment metadata only is rendered. Content/upload/remove and OCR methods exist but are unused; no web review surface exists, and complementary server OCR remains separately incomplete. | P05/P25 through #966/#401/#970/#967/#397/#398/#429/#357. #959 remains isolated. |
...
| Privacy vault | Standard Secure is the default unless deployment policy requires otherwise; policy readout covers allowed/required/disabled modes; explicit Recoverable Private Vault selection/change only after encryption, key wrapping and recovery runtime; unavailable-key, protected field/file, recovery warning, audit, backup-compatibility and server-revalidation states. Preserve a future Recoverable-to-Strict migration path with key rotation/re-wrapping, recovery-envelope removal/disablement, audit, recovery-key-loss warnings and older-backup retention caveats without admitting Strict runtime into Day 1. | No contract or UI; closed #421 is reference only. | P22 through #966 and P01, with privacy/auth/schema manual gates. |
| Cross-record lifecycle | Archive/Trash/restore across applicable records with explicit status, timestamps, actor, reason, restoration, purge and concurrency metadata plus dependency restrictions, retention, idempotency, authorization and audit. Dependency-free drafts may hard-delete only when no settlement/audit/sync/storage/file/shared-user/accepted-bill/notification/import/export/financial-history dependency exists; policy may clean temporary failed-upload blobs, scratch/cache and true orphans without losing authority. Sessions/factors revoke or disable through ordinary actions. Eligible purge is a separate confirmed policy/admin action and must block financial/shared/audit/sync/storage/notification/import/export dependencies. | Only bounded bill and notification archive operations exist. | P24/#716; destructive/privacy/money gates precede runtime/UI. |
...
| P24 | 041 | Existing [#716](https://github.com/tommytang213/Settleora/issues/716) splits cross-record lifecycle domain and user-web Trash children | destructive domain before `web-user-ui` | domain then `web-ui`; strong; **destructive/privacy/money gates; physical purge separately manual-gated** | P01/#716; close archive/restore with record coverage; explicit status/timestamp/actor/reason/restoration/purge/concurrency metadata; dependency restrictions; retention; Trash list/detail; idempotency; authz and audit. Dependency-free draft hard-delete is allowed only with no settlement/audit/sync/storage/file/shared-user/accepted-bill/notification/import/export/financial-history dependency; temporary failed-upload blobs, scratch/cache and true orphans may be policy-cleaned without authoritative loss; sessions/factors use revoke/disable. Any eligible physical purge needs a distinct retention/admin/Trash action, consequence warning, explicit confirmation, blocked-attempt and concurrency evidence, and must reject financial/shared/audit/sync/storage/notification/import/export dependencies | Bill/notification archive credit does not replace #716 policy, and policy existence alone cannot authorize purge. |
| P25 | 042 | Existing [#970](https://github.com/tommytang213/Settleora/issues/970) with #967 splits OCR review/correction; #959 stays untouched | OCR/revision/file domain before `web-user-ui` | OCR/contract then `web-ui`; strong; **file/privacy/money gates** | P01/#970/#967/#966 plus #397/#398/#429; close personal/group queue/detail with review get/upsert/remove; merchant/field/item correction; tax rate/category and tax-included/excluded interpretation; receipt-level tax summaries; discount/refund/tax-correction handling; explicit item/tax/fee/discount/coupon/points/tender/change/void/free/zero-price/cash/unknown-negative/manual-review classifications; OCR-line merge/split with source lineage; assignment get/upsert/complete/cancel; apply preview plus explicit confirmed apply; draft-versus-revision routing; authoritative refresh and stale-source/blocked/conflict/retry/no-duplicate outcomes | Generated methods unused; #970 coordinates #397/#398/#429, while #959 remains independent. |
