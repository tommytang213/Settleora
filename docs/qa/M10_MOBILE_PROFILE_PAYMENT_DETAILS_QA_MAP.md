...
- Visibility values are currently `private`, `settlement_counterparties_only`, and `group_members_when_shared`, with labels `Private`, `Settlement counterparties`, and `Group members when shared`, plus bounded descriptions that explain self-only private readout, server-authorized settlement/payment relationships, and concrete shared group/bill/settlement/payment context. Unknown visibility values fall back to a title-cased label helper for display only.
...
- Visibility copy states that visibility is a server-returned profile fact, not a client-side authorization decision; payment details are not globally visible; and the API decides who may see details, including settlement-scoped counterparty access.
- Empty/unconfigured payment readout says blank or cleared payment fields mean the API currently has no payment text to show, while the visibility value remains a server-returned default/readout when present.
- QR readout is metadata-only: `QR not linked` points to a separate file-handling slice, while `QR available` shows only content type, byte size, and updated timestamp and says QR bytes, file IDs, storage paths, and download URLs are not shown. There is no QR upload, remove, content-read, file picker, image normalization, camera/gallery permission, or QR byte rendering UX.
...
- Visibility readout covers `private`, `settlement_counterparties_only`, and `group_members_when_shared` labels/descriptions, including non-global/API-authority copy and no client-side authorization decisions from cached profile/payment rows.
- Empty payment details show `No payment details yet`, `Not set` rows, default settlement-counterparties visibility, unconfigured/cleared guidance, and QR-not-linked file-handling-slice copy.
- QR metadata-only readout covers content type, byte size, updated timestamp, absence of upload/remove/content-read UX, and suppression of QR byte rendering.
- Unsafe raw text suppression covers profile/payment/QR IDs, API paths, storage paths, provider internals, vault internals, QR byte markers, request-body markers, tokens, stack traces, and unrelated user data.
...
M10-003 changed only the existing mobile self profile/payment-details readout helpers, screen copy, focused profile tests, M10 QA docs, and `.ai` control state, preserving the generated-client repository seam and app-shell injection boundaries.
...
| Visibility setting | API supports constrained values; mobile model, generated mapping, labels, dropdown edit, summary readout, and coverage exist. | M10-003 completed label/description and server-authority copy hardening without changing visibility policy or making client authorization decisions. |
| Optional QR/payment image attachment | API and generated clients include self QR endpoints; mobile self payment-details response displays safe QR metadata only. | M10-003 completed QR metadata-only readout/copy hardening. QR upload/remove/content UX, file picker dependencies, image normalization, permissions, content-read UX, and QR byte rendering remain explicit non-goals for M10. |
| Payment details not globally visible | Architecture requires API authorization and no global lookup; current mobile copy says server authorization controls readership. | M10-003 strengthened non-global visibility copy without changing policy or counterparty authorization. |
...
- Added bounded visibility descriptions for `private`, `settlement_counterparties_only`, and `group_members_when_shared` that explain server-authorized access boundaries without changing visibility policy.
- Hardened profile payment readout copy so blank/cleared fields, unconfigured/default visibility, non-global payment-detail exposure, API authority, and no client-side authorization decisions are explicit.
