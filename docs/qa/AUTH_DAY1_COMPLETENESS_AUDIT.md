...
once: `implemented` 9, `partial` 13, `disabled` 2, `unavailable` 5, `blocked` 2,
`superseded` 0, and `manual-evidence` 1. There are **27 remaining gap IDs** and
...
| `AUTH-D1-013` Password-reset product surfaces | Request and completion reachable on supported Day 1 clients | `E-MOBILE` provides the accepted reset-request entry and `E-WEB` provides hardened fragment capture, scrub, and completion. The current Day 1 mobile checklist explicitly excludes an in-app completion/deep link from its completeness count. | `implemented` | —; optional future mobile deep-link work under #772 does not reopen this Day 1 row. | #339; #772 remains optional/outside the Day 1 count; #1204 owns password-manager semantics | Current Day 1 surface split is accepted. Row 12 retains provider delivery; close remains contingent on the existing token scrub, expiry/replay, and cancellation tests, without claiming optional native completion. |
...
| `REC-MOBILE-AUTH-001` Mobile account-security composition outside #776 | Mobile UI/adapters/tests only, adopting #774/#784/#1204 APIs and preserving generated clients. #776 exclusively retains factor/passkey/recovery/policy presentation; this recommendation covers only the remaining required cross-entry composition. Optional #772 reset deep linking is outside the Day 1 count. No auth/API/policy behavior. | Stable owning APIs/reference first. Accept password change, invitation, session/security-center composition, error/offline/revoked states, a11y and real-device proof without absorbing #776. | `mobile`; strong auth + visual/platform manual review. Close only when the unowned required composition is merged and every owner-specific gate, including #776, remains visible. |
...
- 32-row status arithmetic (`9 + 13 + 2 + 5 + 2 + 0 + 1 = 32`);
