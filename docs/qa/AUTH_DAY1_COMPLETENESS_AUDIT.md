...
first-owner bootstrap, local sign-in, passkey ceremonies, current-actor resolution,
...
perform an MFA challenge; passkey sign-in creates server session state but
...
sign-in/session boundary; and public exposure remains separately blocked.
...
`superseded` 1, and `manual-evidence` 1. There are **24 remaining gap IDs** and
**7 focused recommendation IDs**. Existing owners are reused wherever one
...
sign-in session/credential handoff and any required-MFA continuation. It must
...
not exist are not treated as current truth.
...
| `E-RESET` | `Auth/PasswordReset/LocalPasswordResetEndpoints.cs`, `LocalPasswordResetService.cs`, `PasswordResetMaterialService.cs`, delivery/readiness/orchestrator/template/SMTP classes; reset service, route, abuse, audit-redaction, delivery-readiness, orchestrator, and template tests | OpenAPI/generated `requestLocalPasswordReset`, `completeLocalPasswordReset`; runtime PR #739, contract PR #736, public route PR #763, user-web completion PR #767, mobile request PR #765. `PasswordResetEmailDeliveryOptions.Enabled` defaults false. |
| `E-INVITE` | `Auth/Invitations/InvitationPolicyService.cs`, `InvitationManagementService.cs`, `InvitationAcceptanceService.cs`, lifecycle, abuse, link, delivery/readiness/template/sender classes; invitation policy, management, acceptance, lifecycle, abuse, delivery, and redaction tests | OpenAPI/generated capability, admin lifecycle, resend/revoke, and public `acceptInvitation` methods; contract PR #792; runtime PRs #793-#799. Default policy and provider posture are disabled/off; no M/W/A invitation UI. |
| `E-MFA` | `Auth/Passkeys/PasskeyRuntimeService.cs`, `PasskeyEndpoints.cs`, `Fido2PasskeyWebAuthnProvider.cs`, `Auth/Mfa/MfaRuntimeService.cs`, `ITotpSecretProtector.cs`, `RecoveryCodeHasher.cs`, `Auth/Policy/AuthSecurityPolicyService.cs`; passkey, TOTP, policy, schema, and security regression tests | OpenAPI/generated enrollment/sign-in/step-up/factor/recovery/policy methods; schema/contract/runtime/policy/QA work #501-#506 via PRs #508-#513. `PasskeySignInCompleteResponse` deliberately has only `status`, `currentUser`, and `mfaChallenge`; the anonymous completion endpoint sets no cookie/header and the created raw access credential is discarded. No M/W/A factor flow. Closed #417 supplies reference only; #465 remains admin UI owner. |
...
| `AUTH-D1-019` Sign-in abuse controls | Layered identifier/source/global throttles without enumeration | `E-SIGNIN` has generic outcomes and layered process-local counters, but the endpoint uses a fixed source bucket and no trusted proxy/client-source boundary; restart/multi-node bypass remains. | `partial` | `AUTH-GAP-011`: define and implement trusted-source, distributed-safe sign-in abuse enforcement. | No focused issue; `REC-AUTH-ABUSE-001`; exposure #777 | Docs prerequisite then API/config in separate tasks; strong/manual security and proxy gates. Close on spoof/restart/multi-node/concurrency/privacy tests and bounded operations. |
...
| `AUTH-D1-022` MFA, passkeys, recovery codes, and step-up | Day 1 passkey + TOTP + one-time recovery, server policy | `E-MFA`: server schema/contract/runtime/policy/regression foundation is real; TOTP secrets are protected, recovery verifiers hashed, passkey private material absent. Password sign-in does not challenge for MFA. Passkey completion creates a session then discards its raw access credential and returns no credential/cookie, so `signed_in` cannot authorize a later client request. No client factor flow exists. | `partial` | `AUTH-GAP-014`: complete required-factor sign-in/enforcement, anti-lockout recovery, and client surfaces. `AUTH-GAP-025`: repair the passkey sign-in session/credential handoff without overexposure. | #394/#465; recovery #775; no narrow handoff owner, so `REC-AUTH-PASSKEY-SESSION-001`; UI recommendations remain separate | Passkey handoff design/API/contract first, then enforcement and separate clients; strong/manual auth, contract, recovery, platform, visual gates. Close only after an exact credential handoff, no orphan session, MFA continuation, replay/revoke, and full client acceptance. |
...
The 25 unique gap IDs are `AUTH-GAP-001` through `AUTH-GAP-025`; no ID is
reused for a second capability. Existing issue families participate in 21
gaps. Four execution boundaries lack a current narrow owner and therefore have
...
| #339/#772/#773/#775 | 004, 005, 014 | Delivery, app-link, provider, recovery and factor boundaries remain separate. |
...
| `REC-AUTH-ABUSE-001` with #777 | 011-012 | One shared design prerequisite; workflow semantics remain separate. |
| #721/#369/#973 | 013, 015 | Audit lifecycle and notification-source work stay separate. |
| #394/#465/#775 + `REC-AUTH-PASSKEY-SESSION-001` | 014, 025 | Merged factor runtime credited; passkey credential handoff, enforcement, recovery, and client completion remain. |
...
- No live focused owner was found for passkey sign-in credential handoff,
  distributed auth-abuse design, the mobile account-security composition, or
...
the durable credential path and how `mfa_required` continues. Any later
...
- 25 unique gap IDs and 7 unique recommendation IDs;
