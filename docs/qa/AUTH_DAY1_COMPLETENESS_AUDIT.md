...
once: `implemented` 8, `partial` 14, `disabled` 2, `unavailable` 5, `blocked` 2,
`superseded` 0, and `manual-evidence` 1. There are **28 remaining gap IDs** and
...
The 28 unique gap IDs are `AUTH-GAP-001` through `AUTH-GAP-028`; no ID is
...
| #772/#774/#784/#1204, #776 + client recommendations | 016 | Mobile composition adopts current owners; #776 retains factor/policy presentation. |
...
| #776 | 028 | Reconcile the unmapped standalone MFA-policy contract and mobile factor/policy presentation without recreating merged runtime. |
...
| `REC-MOBILE-AUTH-001` Mobile account-security composition outside #776 | Mobile UI/adapters/tests only, adopting #772/#774/#784/#1204 APIs and preserving generated clients. #776 exclusively retains factor/passkey/recovery/policy presentation; this recommendation covers only the remaining cross-entry composition. No auth/API/policy behavior. | Stable owning APIs/reference first. Accept password change, invitation, session/security-center composition, error/offline/revoked states, a11y and real-device proof without absorbing #776. | `mobile`; strong auth + visual/platform manual review. Close only when the unowned composition is merged and every owner-specific gate, including #776, remains visible. |
...
- 32-row status arithmetic (`8 + 14 + 2 + 5 + 2 + 0 + 1 = 32`);
- 28 unique gap IDs and 9 unique recommendation IDs;
