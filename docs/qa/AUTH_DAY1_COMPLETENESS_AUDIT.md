...
| `AUTH-D1-033` Canonical auth source-of-truth alignment | Current PRD, architecture, and technical guidance distinguish live behavior from future work and agree on credential-issuance policy | `docs/architecture/AUTH_IDENTITY_FOUNDATION.md` still says invitations and passkey/MFA/reset/recovery tables or flows do not exist, despite merged source above. `docs/features/auth-session/TECHNICAL_SPEC.md` still presents mapped auth endpoints as future and prohibits raw tokens in all responses. `docs/prd/MVP_DAY1_SCOPE.md` and `docs/architecture/AUTH_MFA_PASSKEY_ARCHITECTURE.md` likewise prohibit session tokens in API responses/generated clients, while the reviewed refresh/current-user designs and current sign-in/refresh contracts intentionally return one-time access/refresh credentials. This is both stale current-state text and a normative security-policy conflict; current runtime evidence does not resolve the policy decision by itself. | `partial` | `AUTH-GAP-030`: manually reconcile canonical PRD/architecture/technical current-state and credential-issuance policy, then align route, persistence, response, redaction, and future-work wording without silently changing behavior. | No focused live owner found; `REC-AUTH-DOC-DRIFT-001` | `docs-only` first; strong independent plus **manual auth/security policy gate**. Close only after the owner explicitly retains reviewed one-time response issuance or separately gates a runtime/contract replacement, and all canonical guidance agrees while logs, audit, metadata reads, and persistence remain credential-free. |
...
| `REC-AUTH-DOC-DRIFT-001` | 030 | Resolve stale canonical current-state text and the PRD/architecture/technical credential-issuance policy conflict behind a manual security gate. |
...
| `REC-AUTH-DOC-DRIFT-001` Canonical auth current-state and credential-issuance policy reconciliation | Docs/control decision across `MVP_DAY1_SCOPE.md`, `AUTH_IDENTITY_FOUNDATION.md`, `AUTH_MFA_PASSKEY_ARCHITECTURE.md`, and the auth/session technical spec in a separately allowed task. Credit merged invitation, reset, MFA/passkey/recovery, session-family and refresh behavior and name actual mapped routes. Do not assume the outcome of the conflicting credential-return rules: the owner must explicitly retain reviewed immediate successful access/refresh issuance or require a separately scoped runtime/OpenAPI/generated-client replacement. Either outcome must continue to prohibit raw credentials from persistence, logs, audit, later metadata, examples, and unsafe diagnostics. | Row 33 / gap 030. Accept exact source/route/schema parity, explicit current-versus-future labeling, an owner-recorded issuance decision, and no weakening of hashing, authorization, redaction, provider, public-exposure, or client-authority rules. | `docs-only` decision; strong independent plus **manual auth/security policy gate**. Close only after the policy decision and canonical text merge on one exact reviewed head. Any runtime/OpenAPI/generated-client/config effect requires a separate authorized task and its own gates. |
