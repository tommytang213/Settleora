...
import '../future_bills/generated_future_bill_repository.dart';
import '../groups/generated_group_repository.dart';
import '../groups/group_repository.dart';
import '../manual_finance/generated_manual_finance_repository.dart';
...
import '../reports/generated_report_repository.dart';
...
typedef SettleoraBillAttachmentRepositoryFactory =
    SettleoraBillAttachmentRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );

typedef SettleoraBillRevisionRepositoryFactory =
    SettleoraBillRevisionRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );
...
typedef SettleoraRecurringBillRepositoryFactory =
    SettleoraRecurringBillRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );

typedef SettleoraFutureBillRepositoryFactory =
    SettleoraFutureBillRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );

typedef SettleoraGroupRepositoryFactory =
    SettleoraGroupRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );

typedef SettleoraManualFinanceRepositoryFactory =
    SettleoraManualFinanceRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );
...
typedef SettleoraMonthlyReportRepositoryFactory =
    SettleoraMonthlyReportRepository Function(
      SettleoraApiConfiguration configuration,
      SettleoraAccessTokenProvider accessTokenProvider,
    );
...
class SettleoraAppBootstrap extends StatefulWidget {
  const SettleoraAppBootstrap({
    ...
    this.futureBillRepositoryFactory,
    this.groupRepositoryFactory,
    this.manualFinanceRepositoryFactory,
  ...
  final SettleoraFutureBillRepositoryFactory? futureBillRepositoryFactory;
  final SettleoraGroupRepositoryFactory? groupRepositoryFactory;
  final SettleoraManualFinanceRepositoryFactory? manualFinanceRepositoryFactory;
...
class _SettleoraAppBootstrapState extends State<SettleoraAppBootstrap> {
  ...
  Widget build(BuildContext context) {
    ...
    final billAttachmentRepository = _billAttachmentRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    final billRevisionRepository = _billRevisionRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    ...
    final recurringBillRepository = _recurringBillRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    final futureBillRepository = _futureBillRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    final groupRepository = _groupRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    final manualFinanceRepository = _manualFinanceRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
    ...
    final reportRepository = _reportRepositoryFactory(
      apiConfiguration,
      tokenProvider,
    );
  ...
  SettleoraBillAttachmentRepositoryFactory
  get _billAttachmentRepositoryFactory =>
      widget.billAttachmentRepositoryFactory ??
      _defaultBillAttachmentRepositoryFactory;

  SettleoraBillRevisionRepositoryFactory get _billRevisionRepositoryFactory =>
      widget.billRevisionRepositoryFactory ??
      _defaultBillRevisionRepositoryFactory;
  ...
  SettleoraRecurringBillRepositoryFactory get _recurringBillRepositoryFactory =>
      widget.recurringBillRepositoryFactory ??
      _defaultRecurringBillRepositoryFactory;

  SettleoraFutureBillRepositoryFactory get _futureBillRepositoryFactory =>
      widget.futureBillRepositoryFactory ?? _defaultFutureBillRepositoryFactory;

  SettleoraGroupRepositoryFactory get _groupRepositoryFactory =>
      widget.groupRepositoryFactory ?? _defaultGroupRepositoryFactory;

  SettleoraManualFinanceRepositoryFactory get _manualFinanceRepositoryFactory =>
      widget.manualFinanceRepositoryFactory ??
      _defaultManualFinanceRepositoryFactory;
  ...
  SettleoraMonthlyReportRepositoryFactory get _reportRepositoryFactory =>
      widget.reportRepositoryFactory ?? _defaultReportRepositoryFactory;
...
SettleoraBillAttachmentRepository _defaultBillAttachmentRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraBillAttachmentRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}

SettleoraBillRevisionRepository _defaultBillRevisionRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraBillRevisionRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}
...
SettleoraRecurringBillRepository _defaultRecurringBillRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraRecurringBillRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}

SettleoraFutureBillRepository _defaultFutureBillRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraFutureBillRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}

SettleoraGroupRepository _defaultGroupRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
...
SettleoraManualFinanceRepository _defaultManualFinanceRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraManualFinanceRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}
...
SettleoraMonthlyReportRepository _defaultReportRepositoryFactory(
  SettleoraApiConfiguration configuration,
  SettleoraAccessTokenProvider accessTokenProvider,
) {
  return GeneratedSettleoraMonthlyReportRepository.fromConfiguration(
    configuration: configuration,
    accessTokenProvider: accessTokenProvider,
  );
}
