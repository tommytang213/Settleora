...
This directory contains the ASP.NET Core Web API project. It is not a complete Day 1 API yet, but it now has guarded backend slices for auth/session, profile, payment details, groups, bills, settlements, payment proof, file metadata, and schema foundations.
...
- `GET /api/v1/users/me/payment-details` and `PATCH /api/v1/users/me/payment-details` expose the guarded self payment-details foundation. They require `Settleora.AuthenticatedUser`, derive profile identity from the server-side current actor, call `IBusinessAuthorizationService`, and return only the actor's own payment details plus safe QR metadata when linked. PATCH creates or updates one active default payment profile, defaults visibility to `settlement_counterparties_only` on create, normalizes blank nullable text to null, rejects unsupported fields without echoing property names or values, and writes bounded safe audit events for successful create/update/visibility-change actions.
- `POST /api/v1/users/me/payment-details/qr`, `DELETE /api/v1/users/me/payment-details/qr`, and `GET /api/v1/users/me/payment-details/qr/content` expose the self-only payment QR linkage foundation. They require `Settleora.AuthenticatedUser`, derive the actor from the current session, call `IBusinessAuthorizationService`, use only the actor's active payment profile, and never accept client-submitted profile or file IDs. QR upload accepts a single PNG/JPEG/WEBP file up to 2 MB, creates or reuses the self payment profile, writes bytes through the internal file lifecycle/storage foundation, links only active `payment_qr` file metadata owned and created by the actor, marks failed uploads `upload_failed`, marks replaced/removed QR files deleted, and returns no object key, storage path, provider URL, vault reference, original filename, file bytes, auth/session data, or counterparty data.
...
- `GET /api/v1/groups/{groupId}/members`, `POST /api/v1/groups/{groupId}/members`, `PATCH /api/v1/groups/{groupId}/members/{userProfileId}`, and `DELETE /api/v1/groups/{groupId}/members/{userProfileId}` expose the guarded group member management foundation for existing registered users. Active group members can list active members; active group owners can add existing active users with auth accounts, update `owner`/`member` group roles, and mark active memberships `removed` with no hard delete. Successful add, role update, and removal writes `auth_audit_events` actions `group_member.added`, `group_member.role_updated`, and `group_member.removed` with bounded metadata containing only workflow name, group/member IDs, and role/status transition categories. Last active owner demotion/removal returns `409 Conflict`. This is not invitations, guest placeholders, default-excluded/left runtime behavior, notification behavior, billing participation defaults, audit UI, admin audit viewing, audit export, audit retention cleanup, failure-audit coverage for denied or invalid membership requests, or UI integration.
...
- `POST /api/v1/bills`, `GET /api/v1/bills`, and `GET /api/v1/bills/{billId}` expose guarded same-currency personal bill create/list/get behavior backed by the bill schema and internal calculation/split service.
- `POST /api/v1/groups/{groupId}/bills`, `GET /api/v1/groups/{groupId}/bills`, and `GET /api/v1/groups/{groupId}/bills/{billId}` expose guarded group bill create/list/get behavior through active group authorization.
- `POST /api/v1/bills/{billId}/submit`, `POST /api/v1/bills/{billId}/participants/{userProfileId}/accept`, `POST /api/v1/bills/{billId}/participants/{userProfileId}/reject`, and the matching group-scoped routes expose the first bill submit/participant acknowledgement workflow.
- `GET /api/v1/bills/{billId}/settlement-candidates` and `GET /api/v1/groups/{groupId}/bills/{billId}/settlement-candidates` expose guarded settlement candidate previews for confirmed personal/group bills.
- `POST /api/v1/bills/{billId}/settlement-requests` and `POST /api/v1/groups/{groupId}/bills/{billId}/settlement-requests` create one settlement request from a server-derived candidate key; clients do not submit debtor, creditor, amount, or currency as final truth.
- `GET /api/v1/settlements` and `GET /api/v1/settlements/{settlementId}` expose read-only current-actor settlement request visibility.
- `GET /api/v1/settlements/{settlementId}/counterparties/{userProfileId}/payment-details` and `GET /api/v1/settlements/{settlementId}/counterparties/{userProfileId}/payment-details/qr/content` expose settlement-scoped counterparty payment-details and QR content reads. They require an authenticated actor, a visible settlement relationship between actor and target profile, payment-profile visibility that allows the relationship, and safe file metadata/lifecycle state; they are not global user/profile lookup APIs.
- `GET /api/v1/settlements/{settlementId}/payments` and `GET /api/v1/settlement-payments/{paymentId}` expose read-only settlement payment visibility through visible settlement requests.
- `GET /api/v1/settlement-balances` exposes read-only current-actor settlement balance projections grouped by counterparty, direction, optional group, and currency. It derives only from settlement requests, selected request lines, settlement payments, and payment allocations; separates marked-paid pending coverage from confirmed cleared coverage; and does not write EF state, create residuals, expose bill merchant/item details, expose payment details/proof/file/storage internals, or implement basket/pay-all behavior.
- `POST /api/v1/settlements/{settlementId}/payments` exposes debtor-authored same-currency payment claims with full and partial payment support, server-derived request-line allocations, line clearing status updates, and overpayment rejection.
- `POST /api/v1/settlement-payments/{paymentId}/confirm` exposes receiver-authored payment confirmation for eligible marked-paid claims.
- `POST /api/v1/settlements/{settlementId}/dispute`, `POST /api/v1/settlement-payments/{paymentId}/dispute`, `POST /api/v1/settlements/{settlementId}/cancel`, and `POST /api/v1/settlement-payments/{paymentId}/cancel` expose bounded no-body dispute and cancellation transitions where current policy allows them.
- `POST /api/v1/settlement-payments/{paymentId}/proof`, `GET /api/v1/settlement-payments/{paymentId}/proof`, `GET /api/v1/settlement-payments/{paymentId}/proof/{fileId}/content`, and `DELETE /api/v1/settlement-payments/{paymentId}/proof/{fileId}` expose purpose-specific settlement payment proof attach/list/content/remove behavior through the file lifecycle/storage foundation. They return safe metadata or authorized content streams and do not expose storage paths, object keys, provider URLs, original filenames, or provider internals.

The current EF Core model includes schema foundation entities for user profiles, user payment profiles, user groups, group memberships, file object metadata, auth accounts, auth identities, system role assignments, local password credentials, auth sessions, auth session families, auth refresh credential history, auth audit events, expense/bill roots/items/item splits/participants/payers/adjustments/attachments, settlement requests, settlement payments, settlement proof attachments, settlement request lines, settlement payment allocations, and settlement residuals. Settlement request creation now writes one server-derived request line for the selected single-bill candidate, request list/get responses expose bounded line summaries, payment claim/read/confirmation/cancellation/dispute responses expose bounded payment allocation summaries, the first balance projection read endpoint derives bounded current-actor rows from those request-line/allocation records, and the first basket preview endpoint expands eligible same-currency current-actor/counterparty candidate lines without writes. Public basket creation/pay-all writes, residual confirmation, settlement simplification, and settlement reopen/adjustment policy remain future slices. The user payment profile table includes nullable `qr_file_object_id` linkage to `file_objects`; it does not store storage paths, object keys, provider URLs, or vault references. The refresh/session-family tables store refresh credential hashes and bounded lineage/status/expiry metadata only; no raw refresh tokens are stored. Public local sign-in now issues refresh-capable access sessions plus initial refresh-like credentials and does not return `authAccountId` or `userProfileId`; clients should call `GET /api/v1/auth/current-user` after sign-in to initialize actor/profile/session/role state. First-owner local bootstrap is setup-only and becomes unavailable as soon as any auth account exists; after that, account creation is limited to the guarded owner/admin local-user foundation until invitations or self-registration policy are implemented separately. Generated web and Dart clients come from the OpenAPI contract; run `npm run generate:clients` after OpenAPI changes and review the generated diff.

Still absent: generic public file upload/download APIs, receipt upload/download runtime, OCR endpoints or worker runtime, recurring bill runtime, forecasting, reconciliation, basket write/residual runtime beyond the read-only projection and basket preview, broader business audit taxonomy/storage beyond current bounded foundations, general registration, invitations, broader admin user management, owner/admin role assignment, group delete/archive/restore, guest/default-excluded/left membership runtime behavior, group presets, admin payment-details/QR reads, arbitrary/admin session revocation endpoints, raw token storage, mobile/web/admin product UI behavior, and notification behavior.
...
File metadata now lives in PostgreSQL through the `file_objects` foundation. Internal file bytes can go through the local file-object storage provider, and the internal lifecycle service can reserve pending metadata rows, mark upload completion/failure, mark deleted, and mark purged while writing bounded file lifecycle audit events. API responses must not expose direct storage object keys or filesystem paths. Public file-byte access is limited to purpose-specific authorized flows: self payment QR content, settlement-scoped counterparty payment QR content, and settlement payment proof content. No generic public file metadata/content API, receipt/statement upload, OCR file processing, or broader file subject association workflow exists yet.
