...
cleanup() {
  docker rm -f "$ingress_container" "$mock_container" >/dev/null 2>&1 || true
...
for rendered in "$tmp_dir/source.json" "$tmp_dir/image.json"; do
  jq -e '
    ...
    (.services.ingress.cap_add == ["NET_BIND_SERVICE"]) and
...
docker run -d --name "$mock_container" --network "$network" --network-alias api \
  busybox:1.36.1 sh -c \
  "mkdir -p /www/health/ready; printf '%s' ready >/www/health/ready/index.html; exec httpd -f -p 8080 -h /www" >/dev/null
docker run -d --name "$ingress_container" --network "$network" \
  --entrypoint /bin/sh \
  --cap-drop ALL --cap-add NET_BIND_SERVICE --read-only --security-opt no-new-privileges \
